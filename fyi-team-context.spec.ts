...
describe("buildTeamContextDataset", () => {
    it("joins categories, support memories and character roles into an app-facing team context", () => {
        ...
        equal(dataset.characterCount, 2);
        equal(dataset.supportMemoryCount, 2);
        equal(dataset.leaderCount, 1);
        equal(dataset.supportUnitCount, 1);
        ...
        deepEqual(androids?.leaders.map(character => character.id), ["c1"]);
        deepEqual(androids?.supportUnits.map(character => character.id), ["c2"]);
        deepEqual(androids?.supportMemories.map(memory => memory.id), ["m1"]);
        deepEqual(android13?.leaderOfCategories.map(category => category.id), ["1"]);
        deepEqual(android13?.applicableSupportMemories.map(memory => memory.id), ["m1", "m2"]);
        deepEqual(memory1?.categories.map(category => category.id), ["1", "2"]);
