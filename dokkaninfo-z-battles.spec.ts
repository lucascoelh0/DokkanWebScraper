...
import {
    buildDokkanInfoZBattleDataset,
    mapDokkanInfoZBattleEvent,
    mapDokkanInfoZBattleIndex,
    mapDokkanInfoZBattleStats,
} from "./dokkaninfo-z-battles";
...
describe("DokkanInfo Z-Battle dataset", () => {
    it("discovers and normalizes Z-Battles from the index payload", () => {
        ...
        const mapped = mapDokkanInfoZBattleIndex(dom.window.document);
        deepEqual(mapped.map(value => value.id), ["1", "210"]);
        equal(mapped[1].sourcePath, "https://dokkaninfo.com/events/zbattle/210");
        equal(mapped[1].announcementId, "300909");
        equal(mapped[1].enableBattleAuto, true);
        equal(mapped[1].bannerPath, "zbattle_list_banner_210.png");
    });
    ...
    it("maps weaknesses, conditions, ranges, enemies, skills and reward columns", () => {
        ...
        `);
        const shell = mapDokkanInfoZBattleEvent(dom.window.document, {
            id: "210",
            type: "ZBattleStage::Normal",
        ...
        equal(shell.displayName, "All-Out Ultimate Battle");
        equal(shell.statsPath, "https://dokkaninfo.com/events/zbattle/210/stats");
        deepEqual(shell.weaknesses, [{ kind: "category", id: "43", name: "Target: Goku", startsAtLevel: 1 }]);
        deepEqual(shell.conditions, [{ text: "SSR or lower can only cause limited damage", startsAtLevel: 11 }]);
        equal(shell.ranges.length, 1);
        equal(shell.ranges[0].startLevel, 11);
        equal(shell.ranges[0].endLevel, null);
        equal(shell.ranges[0].enemy?.id, "9028061");
        equal(shell.ranges[0].skillIcons[0].id, "1005");
        equal(shell.ranges[0].damageReductionPercent, 90);
        equal(shell.ranges[0].medalRewards[0].name, "Rainbow Medal");
        equal(shell.ranges[0].cardRewards[0].quantity, 2);
        equal(shell.ranges[0].stoneRewards[0].rewardType, "Dragon Stone");
    });
    ...
    it("maps every rendered stats row without retaining the repeated card payload", () => {
        ...
        deepEqual(mapDokkanInfoZBattleStats(dom.window.document), [
    ...
    it("reports event and stats failures separately", () => {
        const dataset = buildDokkanInfoZBattleDataset([{
            id: "210",
            type: "ZBattleStage::Normal",
            displayName: "Battle",
            sourcePath: "https://dokkaninfo.com/events/zbattle/210",
        ...
        deepEqual(dataset.failedEventIds, ["209"]);
        deepEqual(dataset.failedStatsIds, ["210"]);
    });
});
