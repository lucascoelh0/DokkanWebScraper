...
- `unlockAcquisition.sources[].stageReferences`: stage IDs, level titles, difficulty, DokkanInfo paths, and direct `dokkan.fyi/stages/{id}` paths when a mission exposes `sugoroku_map_ids`; historical stages are filtered out using the mission's explicit stage range/list and the current event id
