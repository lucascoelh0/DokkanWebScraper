...
(0, mocha_1.describe)("team-analysis first-party Entrance Animation conditions", function () {
    ...
    (0, mocha_1.it)("shares one trailing percent and cap across coordinated chance and reduction effects", () => {
        const passive = (0, team_analysis_1.parsePassive)("coordinated-category-scaling-cap:initial", "Menacing Legion", [
        ...
        (0, assert_1.equal)(passive.rules.length, 1);
        (0, assert_1.equal)(passive.rules[0].parseStatus, "supported");
        (0, assert_1.deepEqual)(passive.rules[0].effects.map(effect => ({
            kind: effect.kind,
            value: effect.value,
        ...
        const oxfordList = (0, team_analysis_1.parsePassive)("coordinated-oxford-list:initial", undefined, "Basic effect(s)\n- Chance of performing a critical hit, chance of evading enemy's attack, and damage reduction rate 8% (up to 24%)");
        (0, assert_1.deepEqual)(oxfordList.rules[0].effects.map(effect => [effect.kind, effect.value, effect.stackCap]), [
            ["critical_chance", 8, 24],
            ["evade_chance", 8, 24],
            ["damage_reduction", 8, 24],
        ]);
    });
    (0, mocha_1.it)("keeps typed turn phases between ally scaling values and their caps", () => {
        const startOfTurn = (0, team_analysis_1.parsePassive)("category-scaling-start-cap:initial", "Category Potential", [
            "Per \"Giant Form\" Category ally on the team",
            "- Recovers 6% HP at the start of turn (up to 18%)",
        ].join("\n"));
        const beforeAttacking = (0, team_analysis_1.parsePassive)("category-scaling-before-cap:initial", "Category Potential", [
            "Per \"Ginyu Force\" Category ally on the team",
            "- Damage reduction rate 7% before attacking (up to 35%)",
        ].join("\n"));
        (0, assert_1.equal)(startOfTurn.rules[0].parseStatus, "supported");
        (0, assert_1.equal)(startOfTurn.rules[0].effects[0].stackCap, 18);
        (0, assert_1.equal)(startOfTurn.rules[0].effects[0].activationTiming?.moment, "start_of_turn");
        (0, assert_1.equal)(beforeAttacking.rules[0].parseStatus, "supported");
        (0, assert_1.equal)(beforeAttacking.rules[0].effects[0].stackCap, 35);
        (0, assert_1.equal)(beforeAttacking.rules[0].effects[0].activationTiming?.moment, "before_attacking");
    });
    (0, mocha_1.it)("types a flat ally scaling cap without treating a position clause as supported", () => {
        const flat = (0, team_analysis_1.parsePassive)("category-scaling-flat-cap:initial", "Category Potential", [
            "Per \"Giant Form\" Category ally on the team",
            "- All allies' DEF 2000 (up to 6000)",
        ].join("\n"));
        const positioned = (0, team_analysis_1.parsePassive)("category-scaling-position-cap:initial", "Category Potential", [
            "Per \"Peppy Gals\" Category ally on the team",
            "- Chance of evading enemy's attack 5% as the 1st attacker in a turn (up to 15%)",
        ].join("\n"));
        (0, assert_1.equal)(flat.rules[0].parseStatus, "supported");
        (0, assert_1.equal)(flat.rules[0].effects[0].unit, "flat");
        (0, assert_1.equal)(flat.rules[0].effects[0].stackCap, 6000);
        (0, assert_1.equal)(positioned.rules[0].parseStatus, "partial");
        (0, assert_1.equal)(positioned.rules[0].effects[0].stackCap, 15);
        (0, assert_1.equal)(positioned.rules[0].effects.some(effect => effect.kind === "unknown"), true);
    });
    ...
    (0, mocha_1.it)("preserves the attack style when an enemy launches a typed Super Attack at the character", () => {
        const cases = [
            ["Ki Blast", "ki_blast"],
            ["Unarmed", "unarmed"],
            ["Physical", "physical"],
        ...
        cases.forEach(([label, expectedStyle]) => {
            const passive = (0, team_analysis_1.parsePassive)(`incoming-${expectedStyle}-after:initial`, undefined, `After the enemy launches a ${label} Super Attack at the character\n- DEF 100%`);
            (0, assert_1.equal)(passive.rules[0].conditionStatus, "supported", label);
            const predicate = flattenPredicates(passive.rules[0].condition)[0];
            (0, assert_1.equal)(predicate.kind, "incoming_super_attack", label);
            (0, assert_1.equal)(predicate.combatEvent?.attackStyle, expectedStyle, label);
            (0, assert_1.equal)(predicate.combatEvent?.relativeTiming, "during_event", label);
        });
    });
...
(0, mocha_1.describe)("team-analysis validation and artifacts", function () {
    ...
    (0, mocha_1.it)("validates condition and structural provenance for additive passive modes", () => {
        const characters = JSON.parse(JSON.stringify(fixture.characters));
        const modeDetails = (0, fyi_scraper_1.passiveDetailsFromSkill)({
            id: 4124,
        ...
        (0, assert_1.ok)(modeDetails?.text && modeDetails.conditionEvidence?.[0] && modeDetails.structuralSource);
        ...
        const validDataset = (0, team_analysis_1.buildTeamAnalysisDataset)(characters, fixture.catalogEntries, options);
        (0, assert_1.deepEqual)((0, team_analysis_1.validateTeamAnalysisDataset)(validDataset, characters, fixture.catalogEntries), []);
        const invalidContract = JSON.parse(JSON.stringify(validDataset));
        invalidContract.states[0].passiveModes[0].availability = "dokkan_frontier";
        invalidContract.states[0].passiveModes.push(JSON.parse(JSON.stringify(invalidContract.states[0].passiveModes[0])));
        const contractCodes = (0, team_analysis_1.validateTeamAnalysisDataset)(invalidContract, characters, fixture.catalogEntries)
            .map(issue => issue.code);
        (0, assert_1.ok)(contractCodes.includes("passive-mode-contract"));
        const broken = JSON.parse(JSON.stringify(validDataset));
        ...
        const brokenCodes = (0, team_analysis_1.validateTeamAnalysisDataset)(broken, characters, fixture.catalogEntries)
            .map(issue => issue.code);
        (0, assert_1.ok)(brokenCodes.includes("condition-evidence-output"));
        (0, assert_1.ok)(brokenCodes.includes("structural-evidence-output"));
        const invalidSourceCharacters = JSON.parse(JSON.stringify(characters));
    ...
    (0, mocha_1.it)("produces byte-stable gzip output and an exact character compatibility manifest", () => {
        ...
        (0, assert_1.match)(first.manifest.datasetVersion, /characters-v1:parser-1\.10\.0/);
    });
});
