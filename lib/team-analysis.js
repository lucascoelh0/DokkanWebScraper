...
exports.TEAM_ANALYSIS_PARSER_VERSION = "1.9.8";
...
function buildTeamAnalysisDataset(characters, catalogEntries, options) {
    ...
    const states = characters
        .flatMap(character => buildCharacterStates(character, catalogById.get(character.id), options.nameIdentityContract))
...
function buildCharacterStates(character, catalogEntry, nameIdentityContract) {
    ...
    return forms.flatMap(form => {
        ...
        return releaseSources.map(releaseSource => {
            ...
            const activeSkillActivationCondition = analysisActiveSkillActivationCondition(form, releaseSource.releaseState);
            return {
                stateKey,
                characterId: identity.characterId,
    ...
    for (const block of blocks) {
        ...
        const headerConditionResult = currentCondition
            ? headerScaling || perTurnApplicationHeader
                ...
                : parseCondition(currentCondition.text, enrichedConditionText(currentCondition, conditionEvidence), context)
            : { condition: { op: "always" }, status: "supported" };
    ...
    if (condition.op === "predicate") {
        ...
        const structuralMatches = contract.bindings.filter(binding => binding.passiveSkillSetId === passiveSkillSetId
            ...
            && binding.count === expectedCount + (predicate.selfInclusion === "excluded"
                ...
                ? 1
                : 0));
        const exactMatches = structuralMatches.filter(binding => binding.canonicalNames.some(name => normalizeNameIdentityKey(name) === expectedName));
        const nameMatches = exactMatches.length > 0
            ? exactMatches
            : structuralMatches.filter(binding => binding.canonicalNames.some(name => normalizeNameIdentityKey(name).includes(expectedName)));
...
function parseExactConditionClause(text, sourceText) {
    ...
    if (/^receiving an? attack from an enemy who is hit by the character's Super Attack$/i.test(text)) {
        return predicateExpression({
            ...
            scope: "self",
            combatEvent: combatEventDescriptor(combatEventIdentity("receiving an attack", "targeted"), "current_event", "during_event", {
            ...
            for (const releaseSource of releaseSources) {
                expected.set(buildStateKey(character.id, form.id, releaseSource.releaseState), {
                    ...
                    ...(analysisActiveSkillActivationCondition(form, releaseSource.releaseState)
                        ? {
                            activeSkillActivationCondition: analysisActiveSkillActivationCondition(form, releaseSource.releaseState),
                        }
                        : {}),
