...
exports.TEAM_ANALYSIS_PARSER_VERSION = "1.9.14";
...
function buildTeamAnalysisDataset(characters, catalogEntries, options) {
    ...
    const states = characters
        .flatMap(character => buildCharacterStates(character, catalogById.get(character.id), options.nameIdentityContract, options.activeSkillActivationContract))
...
function buildCharacterStates(character, catalogEntry, nameIdentityContract, activeSkillActivationContract) {
...
function validateTeamAnalysisDataset(dataset, characters, catalogEntries, options = {}) {
    ...
    for (const state of dataset.states) {
        ...
        if (!expected) {
            ...
            validateStateSource(state, expectedSources.get(state.stateKey), issues, options.allowExternalActiveSkillConditions ?? false);
        }
...
function expectedStateSources(characters, activeSkillActivationContract) {
    const expected = new Map();
    for (const character of characters) {
        for (const form of [character, ...(character.transformations ?? [])]) {
            const releaseSources = analysisReleaseSources(form);
...
function validateStateSource(state, expected, issues, allowExternalActiveSkillConditions) {
    if (!expected) {
        return;
    }
