...
exports.TEAM_ANALYSIS_PARSER_VERSION = "1.10.0";
    ...
    return forms.flatMap(form => {
        ...
        return releaseSources.map(releaseSource => {
            ...
            const passiveModes = (releaseSource.passiveModes ?? []).map(mode => {
                ...
                const parsed = parsePassive(modeStateKey, mode.label, mode.text, modeDetails, {
                    characterId: identity.characterId,
                    formId: identity.formId,
                    canonicalId: identity.canonicalId,
                    releaseState: identity.releaseState,
...
function passiveModeDetailsForState(mode, modeStateKey) {
    return {
        ...mode,
...
function validateTeamAnalysisDataset(dataset, characters, catalogEntries, options = {}) {
    ...
    for (const state of dataset.states) {
        ...
        const passiveModeKinds = new Set();
        for (const mode of state.passiveModes ?? []) {
            if (!["standard", "survival"].includes(mode.mode)
            ...
            validatePassive(state, ruleIds, issues, mode.passive, `${state.stateKey}:mode:${mode.mode}`);
        }
    }
...
function validateStateSource(state, expected, issues, allowExternalActiveSkillConditions, enforceTransformationActivationCondition) {
    ...
    if (expected.passiveText) {
        ...
        if (expected.passiveDetails) {
            validatePassiveDetailsSource(state, state.passive, expected.passiveText, expected.passiveDetails.name, expected.passiveDetails, state.stateKey, issues);
        }
    }
    ...
    for (const expectedMode of expected.passiveModes ?? []) {
        ...
        validatePassiveDetailsSource(state, actualMode?.passive, expectedMode.text, expectedMode.label, modeDetails, modeStateKey, issues);
    }
    validateSuperAttacks(state, expected.superAttacks, issues);
}
function validatePassiveDetailsSource(state, passive, passiveText, passiveName, passiveDetails, sourceStateKey, issues) {
    ...
    const validEvidence = validConditionEvidence(sourceStateKey, passiveText, sourceMap.sourceFragments, sourceEvidence, {
    ...
    const validStructural = validStructuralEvidence(sourceStateKey, passiveText, passiveDetails.structuralSource, "passive", undefined, passiveDetails.sourceSkillId);
    ...
    const expectedStructuralOutput = parsePassive(sourceStateKey, passiveName, passiveText, passiveDetails, {
        characterId: state.characterId,
        formId: state.formId,
        releaseState: state.releaseState,
        ...(passiveDetails.sourceSkillId
...
function validatePassive(state, ruleIds, issues, passive = state.passive, ruleStateKey = state.stateKey) {
    if (!passive) {
        return;
    }
