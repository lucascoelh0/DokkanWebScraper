...
exports.buildS0S2Audit = exports.validatePinnedElf = exports.validatePinnedArtifactBytes = exports.S0_S2_NATIVE_STRING_PINS = exports.S0_S2_NATIVE_CODE_PINS = exports.S0_S2_ARTIFACTS = void 0;
const crypto_1 = require("crypto");
const native_runtime_elf_adapter_1 = require("../database-experiment/native-runtime-elf-adapter");
exports.S0_S2_ARTIFACTS = [
    {
        id: "global-apk-6-4-0", role: "apk", declared: true,
        sha256: "a51ba758e0555e0a756aa4f20278e6bec25ba6b0c7dcdcd0f4372e0fad159bc0", sizeBytes: 98799013,
    ...
    },
    {
        ...
        sha256: "7d6c2c1e095fc20a71ec4764e88a17b4d4b82f3f12952b9ba8c6eb0405a7215a", sizeBytes: 95662296,
        region: "global", gameBuild: "338", version: "6.4.0",
    ...
    },
    {
        ...
        sha256: "3654eb7db9e18dfe4c238abd02bcc06a688ffa6f30aa1ad93fd108dcfeb78265", sizeBytes: 95428608,
    ...
    },
    {
        ...
        sha256: "18012e59db4a7f7c8c5a1ffd56e676b1950dd94f0d1cc5ed18f4fe5266bf7f2c", sizeBytes: 1634304,
        region: "global", gameBuild: "338", version: "snapshot-2026-08-05",
        structuralIdentity: "SQLite format 3; pageSize=4096; pageCount=399; tables=137; integrity_check=ok; summon-named-tables=0",
    },
];
exports.S0_S2_NATIVE_CODE_PINS = [
...
exports.S0_S2_NATIVE_STRING_PINS = [
...
function hash(bytes) { return (0, crypto_1.createHash)("sha256").update(bytes).digest("hex"); }
function cstring(bytes) { const end = bytes.indexOf(0); return bytes.subarray(0, end < 0 ? bytes.length : end).toString("utf8"); }
function validatePinnedArtifactBytes(artifact, bytes) {
    const failures = [];
    if (bytes.length !== artifact.sizeBytes)
        failures.push(`${artifact.id}: size changed`);
    if (hash(bytes) !== artifact.sha256)
        failures.push(`${artifact.id}: SHA-256 changed`);
    if (artifact.role === "apk" && bytes.subarray(0, 4).toString("hex") !== "504b0304")
        failures.push(`${artifact.id}: APK ZIP header changed`);
    if ((artifact.role === "sqlite_current" || artifact.role === "sqlite_backup") && bytes.subarray(0, 16).toString("ascii") !== "SQLite format 3\0")
        failures.push(`${artifact.id}: SQLite header changed`);
    return failures;
}
exports.validatePinnedArtifactBytes = validatePinnedArtifactBytes;
function validatePinnedElf(bytes) {
    const failures = [];
    let elf;
    try {
        elf = (0, native_runtime_elf_adapter_1.parseNativeRuntimeElf)(bytes);
    }
    catch (error) {
        return [`ELF structure: ${error.message}`];
    }
    for (const pin of exports.S0_S2_NATIVE_CODE_PINS) {
        if (pin.symbol) {
            const symbol = elf.symbols.find(value => value.name === pin.symbol);
            if (!symbol || symbol.value !== pin.vma || symbol.size !== pin.sizeBytes)
                failures.push(`${pin.id}: symbol identity changed`);
        }
        try {
            if (hash(elf.readVirtualBytes(pin.vma, pin.sizeBytes)) !== pin.sha256)
                failures.push(`${pin.id}: code bytes changed`);
        }
        catch (error) {
            failures.push(`${pin.id}: ${error.message}`);
        }
    }
    for (const pin of exports.S0_S2_NATIVE_STRING_PINS) {
        try {
            if (cstring(elf.readVirtualBytes(pin.vma, Buffer.byteLength(pin.value) + 1)) !== pin.value)
                failures.push(`${pin.id}: string changed`);
        }
        catch (error) {
            failures.push(`${pin.id}: ${error.message}`);
        }
...
exports.validatePinnedElf = validatePinnedElf;
function buildS0S2Audit() {
    return {
        schemaVersion: 1,
        ...
        artifacts: exports.S0_S2_ARTIFACTS.map(value => ({ ...value })),
        ledger: [
...
exports.buildS0S2Audit = buildS0S2Audit;
//# sourceMappingURL=s0-s2-profile.js.map