...
const banner = (id, version, rules = "pinned") => ({ bannerId: id, bannerVersion: version, region: "global", validFromBucket: "2026-08-01T00:00Z", validUntilBucket: "2026-08-31T23:00Z", officialRulesStatus: rules, officialRulesEvidenceRef: rules === "pinned" ? rulesEvidenceId : null });
const row = (overrides) => ({ observationId: "obs_0000000000000001", anonymousSessionId: sessionId, temporalBucket: "2026-08-16T12:00Z", bannerId: "friend", bannerVersion: "2026_08", summonType: "friend_single", multiId: "multi_0000000000000001", slotIndex: 1, cardId: 1001, rarity: "R", featuredFlag: false, guaranteedSlotFlag: false, friendSummonSequenceId: sequenceId, friendAttemptIndex: 1, friendTriggerFlag: false, attemptsUntilTrigger: null, friendToNormalIntervalSeconds: null, origin: "manual_screen_recording", quality: "complete_verified", sourceEvidenceRef: sourceEvidenceId, ...overrides });
function valid() {
    const dataset = (0, s3_observation_contract_1.createEmptyS3ObservationDataset)();
...
(0, mocha_1.describe)("S3 summon observation contract", () => {
    (0, mocha_1.it)("accepts an empty pending template as infrastructure only", () => {
        const result = (0, s3_observation_contract_1.validateS3ObservationDataset)((0, s3_observation_contract_1.createEmptyS3ObservationDataset)());
        assert.equal(result.valid, true);
        assert.equal(result.analysisCandidateObservationCount, 0);
    });
    (0, mocha_1.it)("accepts complete negative Friend attempts, one trigger and a linked banner multi", () => {
        const result = (0, s3_observation_contract_1.validateS3ObservationDataset)(valid());
        assert.deepEqual(result.failures, []);
        assert.equal(result.analysisCandidateObservationCount, 4);
        assert.equal(result.friendSequenceCount, 1);
    });
    (0, mocha_1.it)("rejects real-looking session IDs and unknown sensitive fields", () => {
        const dataset = valid();
        dataset.sessions[0].anonymousSessionId = "real-player-123";
        dataset.sessions[0].accountId = "123";
        const result = (0, s3_observation_contract_1.validateS3ObservationDataset)(dataset);
    ...
    (0, mocha_1.it)("rejects raw authorization values even in an otherwise allowed text field", () => {
        const dataset = valid();
        dataset.triggerDefinition.observableRule = "Authorization: Bearer abcdefghijklmnop";
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(dataset).failures.some(value => value.includes("sensitive value")), true);
    });
    (0, mocha_1.it)("rejects a trigger association while the observable trigger is pending", () => {
        const dataset = valid();
        ...
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(dataset).failures.some(value => value.includes("unfixed trigger used")), true);
    });
    (0, mocha_1.it)("rejects missing negative attempts and attempts after a trigger", () => {
        const missing = valid();
        missing.observations = missing.observations.filter(value => value.friendAttemptIndex !== 1);
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(missing).failures.some(value => value.includes("missing negative Friend attempt")), true);
        const after = valid();
        after.observations.push(row({ observationId: "obs_0000000000000005", multiId: "multi_0000000000000005", friendAttemptIndex: 3 }));
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(after).failures.some(value => value.includes("Friend attempts after trigger")), true);
    });
    (0, mocha_1.it)("rejects duplicate Friend invocations for one attempt and reversed attempt time", () => {
        ...
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(duplicate).failures.some(value => value.includes("multiple summons for Friend attempt")), true);
        const reversed = valid();
        reversed.observations[0].temporalBucket = "2026-08-16T13:00Z";
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(reversed).failures.some(value => value.includes("Friend attempt time reversal")), true);
    });
    (0, mocha_1.it)("rejects non-contiguous or inconsistent multi rows", () => {
        const dataset = valid();
    ...
    (0, mocha_1.it)("rejects banner linkage without an observed trigger", () => {
        const dataset = valid();
        dataset.observations[1].friendTriggerFlag = false;
        dataset.observations[1].attemptsUntilTrigger = null;
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(dataset).failures.some(value => value.includes("linked banner lacks observed trigger")), true);
    });
    (0, mocha_1.it)("rejects a banner before its trigger and an interval inconsistent with hourly buckets", () => {
        const before = valid();
        before.observations[2].temporalBucket = "2026-08-16T11:00Z";
        before.observations[3].temporalBucket = "2026-08-16T11:00Z";
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(before).failures.some(value => value.includes("linked banner precedes trigger")), true);
        ...
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(inconsistent).failures.some(value => value.includes("interval inconsistent")), true);
    });
    (0, mocha_1.it)("requires role-correct source, rule and preregistration evidence descriptors", () => {
        const dataset = valid();
    ...
    (0, mocha_1.it)("keeps incomplete or unpinned observations exploratory", () => {
        const dataset = valid();
    ...
    (0, mocha_1.it)("rejects exact timestamp fields instead of silently retaining them", () => {
        const dataset = valid();
        dataset.observations[0].timestamp = "2026-08-16T12:34:56Z";
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(dataset).failures.some(value => value.includes("unknown field")), true);
    });
    (0, mocha_1.it)("rejects impossible civil dates instead of accepting Date normalization", () => {
        const february = valid();
        february.observations[0].temporalBucket = "2026-02-31T12:00Z";
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(february).failures.some(value => value.includes("observation contract")), true);
        const april = valid();
        april.banners[0].validFromBucket = "2026-04-31T00:00Z";
        assert.equal((0, s3_observation_contract_1.validateS3ObservationDataset)(april).failures.some(value => value.includes("banner contract")), true);
    });
    (0, mocha_1.it)("generates namespaced CSPRNG IDs and rejects malformed UTF-8", () => {
        assert.match((0, s3_observation_contract_1.generateS3Id)("anon"), /^anon_[a-f0-9]{16}$/);
        assert.throws(() => (0, s3_run_1.parseS3DatasetBytes)(Buffer.from([0x7b, 0x22, 0x78, 0x22, 0x3a, 0x22, 0xc3, 0x28, 0x22, 0x7d])), /UTF-8 JSON/);
    });
});
//# sourceMappingURL=s3-observation-contract.spec.js.map