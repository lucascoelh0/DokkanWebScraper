...
describe("FYI character release K21-K23", () => {
    it("builds a deterministic content-addressed release, local plan, and stopped receipt", () => {
        const input = fixture();
        const build = () => {
            const release = (0, fyi_character_release_1.buildFyiCharacterReleaseK21)({
        ...
        (0, assert_1.equal)(first.release.portraits.count, 2);
        (0, assert_1.equal)(first.release.releaseId.length, 129);
        (0, assert_1.equal)(first.plan.objects.length, 4);
        (0, assert_1.equal)(first.plan.stablePortraitKeys.requireRemoteHashProofBeforeUpload, true);
        (0, assert_1.equal)(first.plan.readiness.localPlan, "GO");
        (0, assert_1.equal)(first.plan.readiness.remoteInventory, "NO-GO");
        (0, assert_1.equal)(first.plan.budget.withinBucketLimit, null);
        (0, assert_1.equal)(first.receipt.readiness.localReleaseBundle, "GO");
        (0, assert_1.equal)(first.receipt.readiness.remoteDryRun, "NO-GO");
        (0, assert_1.equal)(first.receipt.readiness.publication, "NO-GO");
        (0, assert_1.equal)(first.receipt.checks.noNetworkCodeInvoked, true);
    });
    it("rejects a release without a clean K20 GO", () => {
        const input = fixture();
        input.k20.readiness.candidateGenerationValidation = "NO-GO";
        (0, assert_1.throws)(() => (0, fyi_character_release_1.buildFyiCharacterReleaseK21)({
            candidateGzip: input.artifact.gzipBuffer,
            candidateManifest: input.artifact.manifest,
            candidateReadyMarker: input.candidateReadyMarker,
            k20: input.k20,
    ...
    it("rejects traversal, missing portraits, and unexpected portrait inventory", () => {
        const input = fixture();
        const characters = [{ id: "1", portraitURL: "../secret.png" }];
        const artifact = (0, dataset_artifacts_1.buildCharacterDatasetArtifact)(characters, {
    ...
    it("fails closed when local budget limits are exceeded", () => {
        ...
        release.portraits.entries[0].sizeBytes = fyi_character_release_1.FYI_CHARACTER_RELEASE_MAX_BYTES;
        release.portraits.totalBytes = fyi_character_release_1.FYI_CHARACTER_RELEASE_MAX_BYTES + release.portraits.entries[1].sizeBytes;
        release.portraits.inventorySha256 = require("crypto").createHash("sha256")
            ...
            .digest("hex");
        (0, assert_1.throws)(() => (0, fyi_character_release_1.buildFyiCharacterReleaseK22)(release), /budget exceeded/);
        (0, assert_1.equal)(fyi_character_release_1.FYI_CHARACTER_BUCKET_MAX_BYTES, 10000000000);
    });
    it("rejects a mutated K22 plan at K23", () => {
        ...
        const plan = (0, fyi_character_release_1.buildFyiCharacterReleaseK22)(release);
        plan.readiness.publication = "GO";
        (0, assert_1.throws)(() => (0, fyi_character_release_1.buildFyiCharacterReleaseK23)(release, plan), /plan rejected/);
        const objectPlan = (0, fyi_character_release_1.buildFyiCharacterReleaseK22)(release);
        objectPlan.objects[0].sizeBytes += 1;
        (0, assert_1.throws)(() => (0, fyi_character_release_1.buildFyiCharacterReleaseK23)(release, objectPlan), /plan rejected/);
    });
    it("requires the exact explicit opt-in flag", () => {
        (0, assert_1.throws)(() => (0, fyi_character_release_1.parseFyiCharacterReleaseCli)([]), /exactly one/);
        (0, assert_1.throws)(() => (0, fyi_character_release_1.parseFyiCharacterReleaseCli)(["--opt-in-k21-k23", "--remote"]), /exactly one/);
        (0, assert_1.deepStrictEqual)((0, fyi_character_release_1.parseFyiCharacterReleaseCli)(["--opt-in-k21-k23"]), { optIn: true });
    });
});
//# sourceMappingURL=fyi-character-release.spec.js.map