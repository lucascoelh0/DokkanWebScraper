...
function preferredSuperAttacks(superAttacks, useExtremeState) {
    ...
    return Array.from(grouped.values())
        .map(group => selectPreferredAttack(group, useExtremeState, superAttackKind(group[0]) === "unit"))
        .filter((superAttack) => Boolean(superAttack));
}
exports.preferredSuperAttacks = preferredSuperAttacks;
function selectPreferredAttack(group, useExtremeState, allowBaseFallback) {
