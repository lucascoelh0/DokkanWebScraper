...
exports.exclusiveSkillOrbsFromFyi = exports.reversibleExchangeDetailsFromFyi = exports.obtainabilityDetailsFromFyi = exports.normalizeTransformationSource = exports.finishSkillsFromFyi = exports.standbyDetailsFromFyi = exports.preferredSuperAttacks = exports.selectCurrentState = exports.writeDokkanFyiContractReferenceSample = exports.buildDokkanFyiContractReferenceSample = exports.writeDokkanFyiExperiment = exports.runDokkanFyiExperiment = exports.getDokkanFyiData = exports.DEFAULT_DOKKAN_FYI_CONTRACT_SAMPLE_CHARACTER_IDS = exports.DEFAULT_DOKKAN_FYI_EXPERIMENT_CHARACTER_IDS = void 0;
...
async function ensureTransformation(transformations, targetCharacterId, baseCharacterId, fallbackEntry, client) {
    ...
    const transformation = mapDokkanFyiTransformation(baseCharacterId, targetPage.payload.props.character, fallbackEntry);
    transformations.push(transformation);
    return transformation;
}
function mapDokkanFyiTransformation(baseCharacterId, character, entry) {
    const currentState = selectCurrentState(character);
    const passive = passiveDetailsFromSkill(currentState.passiveSkill);
    const currentSuperAttacks = currentState.currentSuperAttacks;
    const normalSuperAttack = matchingFyiSuperAttack(currentSuperAttacks, "normal");
...
function mapSuperAttackDetails(superAttack) {
    if (!superAttack) {
        return undefined;
    }
    return {
...
function unitSuperAttacksFromFyi(superAttacks) {
    return superAttacks
...
function passiveDetailsFromSkill(skill) {
    if (!skill) {
        return undefined;
    }
