...
async function getDokkanFyiDataWithReport(characterIds) {
    ...
    const results = await mapWithConcurrency(ids, requestedCharacterConcurrency(), async (characterId) => {
        ...
        if (!client.shouldRefresh) {
            const cachedCharacter = await (0, fyi_mapped_character_cache_1.readFyiMappedCharacterCache)(mappedCachePath, client.cacheTtlMsValue);
            if (cachedCharacter) {
                return {
        ...
        try {
            ...
            await (0, fyi_mapped_character_cache_1.writeFyiMappedCharacterCache)(mappedCachePath, character);
            return {
                id: characterId.toString(),
                character,
            };
...
async function mapDokkanFyiCharacter(page, client) {
    ...
    return {
        ...
        superAttackDetails: mapSuperAttackDetails(normalSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), "initial", page.version, "normal"), initialState.maxSuperAttackLevel),
        ezaSuperAttackDetails: mapSuperAttackDetails(ezaNormalSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), releaseState, page.version, "normal"), awakenedState?.maxSuperAttackLevel),
        ultraSuperAttackDetails: mapSuperAttackDetails(ultraSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), "initial", page.version, "ultra"), initialState.maxSuperAttackLevel),
        ezaUltraSuperAttackDetails: mapSuperAttackDetails(ezaUltraSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), releaseState, page.version, "ultra"), awakenedState?.maxSuperAttackLevel),
        exSuperAttackDetails: mapSuperAttackDetails(extraSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), "initial", page.version, "extra"), initialState.maxSuperAttackLevel),
        ezaExSuperAttackDetails: mapSuperAttackDetails(ezaExtraSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), releaseState, page.version, "extra"), awakenedState?.maxSuperAttackLevel),
...
async function buildTransformations(rootCharacter, initialEntries, client) {
    ...
    while (queue.length > 0) {
        ...
        transformations.push(mapDokkanFyiTransformation(rootCharacter.id, targetPage.payload.props.character, current.entry, targetPage.version, inheritedAwakening));
...
async function ensureTransformation(transformations, targetCharacterId, baseCharacterId, fallbackEntry, client, inheritedAwakening) {
    ...
    const transformation = mapDokkanFyiTransformation(baseCharacterId, targetPage.payload.props.character, fallbackEntry, targetPage.version, inheritedAwakening);
    transformations.push(transformation);
    return transformation;
}
function mapDokkanFyiTransformation(baseCharacterId, character, entry, sourceVersion, inheritedAwakening) {
    const initialState = selectInitialState(character);
    ...
    return {
        ...
        exSuperAttack: formatSuperAttackEffect(extraSuperAttack),
        ezaExSuperAttack: formatSuperAttackEffect(ezaExtraSuperAttack) || undefined,
        superAttackDetails: mapSuperAttackDetails(normalSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), "initial", sourceVersion, "normal"), initialState.maxSuperAttackLevel),
        ezaSuperAttackDetails: mapSuperAttackDetails(ezaNormalSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), releaseState, sourceVersion, "normal"), awakenedState?.maxSuperAttackLevel),
        ultraSuperAttackDetails: mapSuperAttackDetails(ultraSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), "initial", sourceVersion, "ultra"), initialState.maxSuperAttackLevel),
        ezaUltraSuperAttackDetails: mapSuperAttackDetails(ezaUltraSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), releaseState, sourceVersion, "ultra"), awakenedState?.maxSuperAttackLevel),
        exSuperAttackDetails: mapSuperAttackDetails(extraSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), "initial", sourceVersion, "extra"), initialState.maxSuperAttackLevel),
        ezaExSuperAttackDetails: mapSuperAttackDetails(ezaExtraSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), releaseState, sourceVersion, "extra"), awakenedState?.maxSuperAttackLevel),
...
function selectAwakenedState(character, inheritedLatestType) {
...
function inheritedTransformationAwakening(rootCharacter) {
    ...
    if (!awakenedState || awakenedState.latestType !== "eza")
        return undefined;
    return {
        latestType: "eza",
...
function mapSuperAttackDetails(superAttack, evidenceContext, maxSuperAttackLevel) {
    if (!superAttack) {
        return undefined;
    }
...
function superAttackIncreaseFromFyi(superAttack, maxSuperAttackLevel) {
...
function superAttackEffectsFromFyi(effects, sourceVersion) {
    const mapped = (effects ?? []).reduce((result, effect) => {
        ...
        if (target === 1 && calculation === 2 && value1 !== undefined) {
            ...
            if (type === 3 && value2 !== undefined) {
                result.push({ ...base, id: `${id}:atk`, kind: "atk_raise", target: "self", value: value1, status: "supported" }, { ...base, id: `${id}:def`, kind: "def_raise", target: "self", value: value2, status: "supported" });
                return result;
            }
        }
        if (target === 3 && calculation === 3 && value1 !== undefined) {
            ...
            if (type === 3 && value2 !== undefined) {
                result.push({ ...base, id: `${id}:atk`, kind: "enemy_atk_lowering", target: "current_target", value: value1, status: "supported" }, { ...base, id: `${id}:def`, kind: "enemy_def_lowering", target: "current_target", value: value2, status: "supported" });
                return result;
            }
        }
...
function unitSuperAttacksFromFyi(superAttacks, evidenceContext, maxSuperAttackLevel) {
    return superAttacks
...
function activeSkillUltimateAttackFromFyi(ultimateAttack) {
