...
async function mapDokkanFyiCharacter(page, client) {
    ...
    return {
        ...
        leaderSkillBoost: (0, scraper_1.parseLeaderSkillDetails)(formatSkillDescription(currentState.leaderSkill))?.displayBoost,
        leaderSkillDetails: (0, scraper_1.parseLeaderSkillDetails)(formatSkillDescription(currentState.leaderSkill)),
        ...
        superAttackDetails: mapSuperAttackDetails(normalSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), releaseState, page.version, "normal")),
        ultraSuperAttackDetails: mapSuperAttackDetails(ultraSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), releaseState, page.version, "ultra")),
        exSuperAttackDetails: mapSuperAttackDetails(extraSuperAttack, superAttackEvidenceContext(character.id.toString(), character.id.toString(), releaseState, page.version, "extra")),
...
function mapDokkanFyiTransformation(baseCharacterId, character, entry, sourceVersion) {
    ...
    return {
        ...
        ultraSuperAttack: formatSuperAttackEffect(ultraSuperAttack),
        exSuperAttack: formatSuperAttackEffect(extraSuperAttack),
        superAttackDetails: mapSuperAttackDetails(normalSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), releaseState, sourceVersion, "normal")),
        ultraSuperAttackDetails: mapSuperAttackDetails(ultraSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), releaseState, sourceVersion, "ultra")),
        exSuperAttackDetails: mapSuperAttackDetails(extraSuperAttack, superAttackEvidenceContext(baseCharacterId.toString(), character.id.toString(), releaseState, sourceVersion, "extra")),
        passive: passive?.text ?? "",
        passiveDetails: passive,
        activeSkill: formatActiveSkill(activeSkill),
        activeSkillCondition: cleanMultilineText(activeSkill?.condition),
...
function effectStructuralSource(rawValue, normalizedText, context, sourceEntityId) {
    ...
    for (let lineOffset = 0; lineOffset < lines.length; lineOffset += 1) {
        ...
        const markerRuns = channel === "passive"
            ? [line.rawText.match(/^(\s*-\s*)((?:\{passiveImg:[^}]+\}\s*)+)/)].filter((match) => match !== null)
        ...
        for (const markerRun of markerRuns) {
            ...
            const markers = markerMatches.map((match, order) => {
                ...
                const markerKind = sourceToken === "once" || sourceToken === "forever"
                    ? sourceToken
                    : "unknown";
