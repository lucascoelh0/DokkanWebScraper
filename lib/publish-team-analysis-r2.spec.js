...
class FakeRunner {
    ...
    bucketSize = "100 MB";
    delays = [];
    getMissesAfterPut = new Map();
    pendingGetMisses = new Map();
    ...
    async run(args) {
        ...
        if (operation === "put") {
            ...
            if (getMisses > 0)
                this.pendingGetMisses.set(objectKey, getMisses);
            return success();
        }
        if (operation === "delete") {
...
(0, mocha_1.describe)("Team Analysis R2 delivery gate", function () {
    ...
    (0, mocha_1.it)("accepts the exact content-addressed fileName used by the public Characters manifest", async () => {
        ...
        fixture.options.manifestObjectKey = (0, publish_team_analysis_r2_1.buildTeamAnalysisManifestObjectKey)("production", "v2");
    ...
    (0, mocha_1.it)("refuses Team Analysis writes until the exact Character delivery is public", async () => {
        const characterManifestKey = (0, publish_r2_1.buildCharacterManifestObjectKey)(fixture.options.channel, fixture.options.contractLane);
        fixture.runner.objects.delete(characterManifestKey);
        await (0, assert_1.rejects)(() => (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock), /Required Character delivery is not public and identical/);
        (0, assert_1.equal)(fixture.runner.commands.some(args => args[2] === "put" || args[2] === "delete"), false);
    });
    (0, mocha_1.it)("refuses Team Analysis writes when the public Character payload is corrupt", async () => {
        const payloadKey = (0, publish_r2_1.buildRemoteDatasetObjectKey)(fixture.characterArtifact.manifest, fixture.options.channel, fixture.options.contractLane);
        fixture.runner.objects.set(payloadKey, Buffer.from("corrupt"));
        await (0, assert_1.rejects)(() => (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock), /Required Character payload is missing or corrupt/);
        (0, assert_1.equal)(fixture.runner.commands.some(args => args[2] === "put" || args[2] === "delete"), false);
    });
    (0, mocha_1.it)("waits for a newly uploaded payload to become readable before promoting the manifest", async () => {
        const payloadKey = (0, publish_team_analysis_r2_1.buildTeamAnalysisDatasetObjectKey)(fixture.artifact.manifest);
        fixture.runner.getMissesAfterPut.set(payloadKey, 2);
        await (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock);
        (0, assert_1.deepEqual)(fixture.runner.delays, [1000, 2000]);
        (0, assert_1.deepEqual)(putKeys(fixture.runner), [payloadKey, publish_team_analysis_r2_1.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY]);
    });
    (0, mocha_1.it)("waits for a newly uploaded manifest to become readable before completing publication", async () => {
        fixture.runner.getMissesAfterPut.set(publish_team_analysis_r2_1.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY, 1);
        await (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock);
        (0, assert_1.deepEqual)(fixture.runner.delays, [1000]);
        (0, assert_1.equal)((await (0, promises_1.readFile)(fixture.options.statePath, "utf8")).length > 0, true);
    });
    ...
    (0, mocha_1.it)("fails closed after bounded retries when a successful payload put never becomes readable", async () => {
        fixture.runner.ignorePutKey = (0, publish_team_analysis_r2_1.buildTeamAnalysisDatasetObjectKey)(fixture.artifact.manifest);
        await (0, assert_1.rejects)(() => (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock), /payload failed size\/SHA-256 verification/);
        (0, assert_1.equal)(fixture.runner.objects.has(publish_team_analysis_r2_1.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY), false);
        (0, assert_1.equal)(await fileExists(fixture.options.statePath), false);
        (0, assert_1.deepEqual)(fixture.runner.delays, [1000, 2000, 4000, 8000, 15000]);
    });
    ...
    (0, mocha_1.it)("retains every verified tracked release when cleanup is explicitly disabled", async () => {
        const previous = installPreviousRemote(fixture, "characters-v0:parser-1.7.1", "previous");
        const staleOne = makeRelease(fixture, "characters-v-1:parser-1.7.1", "stale-one");
        const staleTwo = makeRelease(fixture, "characters-v-2:parser-1.7.1", "stale-two");
        [staleOne, staleTwo].forEach(entry => fixture.runner.objects.set(entry.datasetObjectKey, entry.buffer));
        ...
        const plan = await (0, publish_team_analysis_r2_1.readTeamAnalysisR2PublishPlan)(fixture.options, fixture.runner);
        (0, assert_1.equal)(plan.retainedReleases.length, 4);
        (0, assert_1.equal)(plan.cleanupCandidates.length, 0);
        (0, assert_1.equal)(plan.bytesRemovable, 0);
        (0, assert_1.ok)(plan.warnings.some(warning => warning.includes("no release cleanup")));
        await (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock);
        (0, assert_1.equal)(fixture.runner.commands.some(args => args[2] === "delete"), false);
        (0, assert_1.equal)(fixture.runner.objects.has(previous.datasetObjectKey), true);
        (0, assert_1.equal)(fixture.runner.objects.has(staleOne.datasetObjectKey), true);
        (0, assert_1.equal)(fixture.runner.objects.has(staleTwo.datasetObjectKey), true);
    });
    ...
    (0, mocha_1.it)("publishes staging payload and manifest only under staging keys", async () => {
        fixture = await createFixture(["--channel", "staging"]);
        await (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, () => new Date("2026-08-23T22:00:00.000Z"));
        const stagingPayloadKey = (0, publish_team_analysis_r2_1.buildTeamAnalysisDatasetObjectKey)(fixture.artifact.manifest, "staging", "v1");
        const stagingManifestKey = (0, publish_team_analysis_r2_1.buildTeamAnalysisManifestObjectKey)("staging", "v1");
        (0, assert_1.equal)(fixture.runner.objects.has(stagingPayloadKey), true);
        (0, assert_1.equal)(fixture.runner.objects.has(stagingManifestKey), true);
        (0, assert_1.equal)(fixture.runner.objects.has(publish_team_analysis_r2_1.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY), false);
        (0, assert_1.equal)((0, publish_team_analysis_r2_1.buildTeamAnalysisManifestObjectKey)("production", "v2"), "v2/team-analysis-manifest.json");
        (0, assert_1.equal)((0, publish_team_analysis_r2_1.buildTeamAnalysisManifestObjectKey)("staging", "v2"), "staging/v2/team-analysis-manifest.json");
    });
    (0, mocha_1.it)("refuses a production write without explicit promotion", async () => {
        fixture.options.promoteProduction = false;
        await (0, assert_1.rejects)((0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner), /explicit --promote-production/);
        (0, assert_1.equal)(fixture.runner.commands.length, 0);
    });
    (0, mocha_1.it)("refuses remote writes when either fail-closed verification barrier is disabled", async () => {
        fixture.options.target = "remote";
        fixture.options.skipRemoteManifestCheck = true;
        await (0, assert_1.rejects)(() => (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock), /Remote publication requires the baseline manifest check and post-upload verification/);
        (0, assert_1.equal)(fixture.runner.commands.length, 0);
    ...
    (0, mocha_1.it)("refuses remote writes without an exact baseline pin", async () => {
        fixture.options.expectRemoteManifestAbsent = false;
        fixture.options.expectedRemoteBaselineSha256 = undefined;
        await (0, assert_1.rejects)(() => (0, publish_team_analysis_r2_1.publishTeamAnalysisR2)(fixture.options, fixture.runner, fixedClock), /requires exactly one baseline pin/);
        (0, assert_1.equal)(fixture.runner.commands.length, 0);
    ...
    (0, mocha_1.it)("uses conservative defaults and rejects conflicting targets or unsafe flags", () => {
        ...
        (0, assert_1.equal)(defaults.expectedRemoteBaselineSha256, undefined);
        (0, assert_1.equal)(defaults.expectRemoteManifestAbsent, false);
        (0, assert_1.equal)(defaults.retainAllReleases, false);
        (0, assert_1.equal)(defaults.allowUnknownBucketSize, false);
        (0, assert_1.equal)(defaults.channel, "production");
        (0, assert_1.equal)(defaults.contractLane, "v1");
        (0, assert_1.equal)(defaults.manifestObjectKey, publish_team_analysis_r2_1.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY);
        ...
        const preserveHistory = (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)([
            "--contract-lane", "v2",
            "--retain-all-releases",
        ]);
        (0, assert_1.equal)(preserveHistory.retainAllReleases, true);
        const firstPublication = (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)([
            "--contract-lane", "v2",
            "--expect-remote-manifest-absent",
        ]);
        (0, assert_1.equal)(firstPublication.expectRemoteManifestAbsent, true);
        ...
        const staging = (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)([
            "--channel", "staging",
            "--contract-lane", "v1",
            "--v1-projection-report", "projection-report.json",
        ]);
        (0, assert_1.equal)(staging.channel, "staging");
        (0, assert_1.equal)(staging.manifestObjectKey, "staging/v1/team-analysis-manifest.json");
        (0, assert_1.equal)(staging.statePath.endsWith("team-analysis-r2-publish-state-staging-v1.json"), true);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)([]), /Missing dataset contract lane/);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)(["--contract-lane", "v1"]), /requires --v1-projection-report/);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)(["--contract-lane", "v2", "--v1-projection-report", "proof.json"]), /only be used/);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)(["--contract-lane", "v2", "--remote", "--local"]), /Choose only one/);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)(["--contract-lane", "v2", "--unknown"]), /Unknown option/);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)(["--contract-lane", "v2", "--channel", "preview"]), /Invalid dataset publication channel/);
        (0, assert_1.rejects)(async () => (0, publish_team_analysis_r2_1.parseTeamAnalysisR2PublishArgs)([
            "--contract-lane", "v2",
...
async function createFixture(extraArgs = []) {
    ...
    const projectionReportPath = (0, path_1.resolve)(root, "android-v1-projection-report.json");
    ...
    const remoteCharacterManifest = {
        ...characterArtifact.manifest,
        fileName: (0, publish_r2_1.buildRemoteDatasetObjectKey)(characterArtifact.manifest, fixture.options.channel, fixture.options.contractLane),
    };
    fixture.runner.objects.set(remoteCharacterManifest.fileName, characterArtifact.gzipBuffer);
    fixture.runner.objects.set((0, publish_r2_1.buildCharacterManifestObjectKey)(fixture.options.channel, fixture.options.contractLane), serialize(remoteCharacterManifest));
    await writeLocalArtifact(fixture);
    return fixture;
}
async function writeProjectionReport(fixture) {
    const characterManifest = JSON.parse(await (0, promises_1.readFile)(fixture.options.characterManifestPath, "utf8"));
    const teamAnalysisManifest = JSON.parse(await (0, promises_1.readFile)(fixture.options.manifestPath, "utf8"));
    await (0, promises_1.writeFile)(fixture.projectionReportPath, serialize({
        schemaVersion: 1,
...
function pinRemoteTeamManifest(fixture, manifest) {
    fixture.options.expectRemoteManifestAbsent = false;
