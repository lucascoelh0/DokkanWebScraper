...
exports.parseWranglerBucketSize = exports.printTeamAnalysisR2PublishPlan = exports.publishTeamAnalysisR2 = exports.buildTeamAnalysisR2PublishPlan = exports.readTeamAnalysisR2PublishPlan = exports.buildTeamAnalysisDatasetObjectKey = exports.parseTeamAnalysisR2PublishArgs = exports.TEAM_ANALYSIS_MANIFEST_CACHE_CONTROL = exports.TEAM_ANALYSIS_PAYLOAD_CACHE_CONTROL = exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY = void 0;
...
function buildTeamAnalysisDatasetObjectKey(manifest) {
    const slug = datasetVersionSlug(manifest.datasetVersion);
    const key = `team-analysis/releases/${slug}/${manifest.sha256.toLowerCase()}/${team_analysis_delivery_1.TEAM_ANALYSIS_LOCAL_FILE_NAME}`;
...
function buildTeamAnalysisR2PublishPlan(options, validated, facts) {
    ...
    if (manifestUpdateNeeded) {
        plannedActions.push(`put ${exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY} last`);
        if (!options.skipUploadVerification) {
            plannedActions.push(`verify ${exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY}`);
        }
    }
...
async function publishTeamAnalysisR2(options, runner = createWranglerCommandRunner(), now = () => new Date()) {
    ...
    if (plan.manifestUpdateNeeded) {
        ...
        try {
            ...
            await putObject(runner, options, exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY, temporaryManifestPath, "application/json", exports.TEAM_ANALYSIS_MANIFEST_CACHE_CONTROL);
            if (!options.skipUploadVerification) {
...
async function inspectRemoteFacts(options, validated, stateRead, runner) {
    ...
    if (!options.skipRemoteManifestCheck) {
        const remote = await tryGetJsonObject(runner, options, exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY, "Team Analysis remote manifest");
        if (remote) {
...
async function assertRemoteManifestMatches(runner, options, expectedManifest, expectedBuffer) {
    const remote = await tryGetJsonObject(runner, options, exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY, "uploaded Team Analysis manifest");
