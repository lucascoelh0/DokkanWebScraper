...
function buildTeamAnalysisDatasetObjectKey(manifest, channel = "production", contractLane = "v1") {
    const slug = datasetVersionSlug(manifest.datasetVersion);
    const key = (0, dataset_publication_channel_1.contractLaneObjectKey)(channel, contractLane, `team-analysis/releases/${slug}/${manifest.sha256.toLowerCase()}/${team_analysis_delivery_1.TEAM_ANALYSIS_LOCAL_FILE_NAME}`);
...
function buildTeamAnalysisManifestObjectKey(channel = "production", contractLane = "v1") {
    return (0, dataset_publication_channel_1.contractLaneObjectKey)(channel, contractLane, exports.TEAM_ANALYSIS_MANIFEST_OBJECT_KEY);
}
...
async function readTeamAnalysisR2PublishPlan(options, runner = createWranglerCommandRunner()) {
    ...
    (0, publish_r2_1.assertExpectedRemoteManifestBaseline)(options.expectedRemoteBaselineSha256, options.expectRemoteManifestAbsent, remoteFacts.manifest);
...
function buildTeamAnalysisR2PublishPlan(options, validated, facts) {
    const datasetObjectKey = buildTeamAnalysisDatasetObjectKey(validated.manifest, options.channel, options.contractLane);
    ...
    plannedActions.push(`verify remote Character delivery ${(0, publish_r2_1.buildCharacterManifestObjectKey)(options.channel, options.contractLane)}`);
...
function assertRemoteWriteVerificationEnabled(options) {
    if (options.target !== "remote" || options.dryRun)
        return;
    if (options.skipRemoteManifestCheck || options.skipUploadVerification) {
        throw new Error("Remote publication requires the baseline manifest check and post-upload verification; "
    ...
    if (Boolean(options.expectedRemoteBaselineSha256) === options.expectRemoteManifestAbsent) {
        throw new Error("Remote Team Analysis publication requires exactly one baseline pin: "
...
async function inspectRemoteFacts(options, validated, stateRead, runner) {
    ...
    const plannedRelease = {
        ...
        datasetObjectKey: buildTeamAnalysisDatasetObjectKey(validated.manifest, options.channel, options.contractLane),
...
async function assertRemoteCharacterDeliveryMatches(runner, options, localManifest) {
    const manifestObjectKey = (0, publish_r2_1.buildCharacterManifestObjectKey)(options.channel, options.contractLane);
    const expectedPayloadKey = (0, publish_r2_1.buildRemoteDatasetObjectKey)(localManifest, options.channel, options.contractLane);
    const expectedManifest = {
        ...localManifest,
        fileName: expectedPayloadKey,
    };
    const remote = await tryGetJsonObject(runner, options, manifestObjectKey, "required Character remote manifest");
    if (!remote || canonicalJson(remote.value) !== canonicalJson(expectedManifest)) {
        throw new Error(`Required Character delivery is not public and identical in ${options.channel}/${options.contractLane}: ${manifestObjectKey}`);
    }
    ...
    if (!payloadMatches) {
        throw new Error(`Required Character payload is missing or corrupt in ${options.channel}/${options.contractLane}: ${expectedPayloadKey}`);
    }
}
async function assertRemoteManifestMatches(runner, options, expectedManifest, expectedBuffer) {
    const expectedSha256 = (0, team_analysis_artifacts_1.sha256)(expectedBuffer);
...
async function eventuallyMatches(runner, check) {
    if (await check())
        return true;
...
async function delayRunner(runner, milliseconds) {
    if (runner.delay) {
...
function validateRemoteManifest(manifest, channel, contractLane) {
    const stringFields = [
    ...
    const expectedKey = buildTeamAnalysisDatasetObjectKey({ ...manifest, sha256: manifest.sha256.toLowerCase() }, channel, contractLane);
...
function validateRelease(release, channel, contractLane) {
