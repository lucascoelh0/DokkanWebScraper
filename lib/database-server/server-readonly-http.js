...
exports.ServerReadonlyHttpClient = void 0;
...
class ServerReadonlyHttpClient {
    allowedHosts;
    minimumIntervalMs;
    ...
    constructor(options) {
        ...
        if (this.minimumIntervalMs < 0 || this.maximumResponseBytes <= 0 || this.maximumAggregateBytes <= 0 || this.maximumAttempts < 1 || this.maximumAttempts > 3)
            throw new Error("Invalid read-only HTTP limits.");
    }
    get totalBytes() { return this.aggregateBytes; }
    async get(urlValue) {
        let release;
    ...
    async getSerial(urlValue) {
        const url = new URL(urlValue);
        if (url.protocol !== "https:" || url.username || url.password || !this.allowedHosts.has(url.hostname.toLowerCase()))
            throw new Error(`URL is outside the read-only allowlist: ${url.origin}`);
        const blockedUntil = this.blockedUntilByHost.get(url.hostname.toLowerCase()) ?? 0;
        if (blockedUntil > this.now())
            throw new Error(`Endpoint family remains blocked by Retry-After until ${new Date(blockedUntil).toISOString()}`);
        for (const key of url.searchParams.keys())
            if (FORBIDDEN_QUERY_KEY.test(key))
                throw new Error(`Sensitive query key is prohibited: ${key}`);
        if (this.aggregateBytes + this.maximumResponseBytes > this.maximumAggregateBytes)
            throw new Error(`S1 projected response ceiling exceeds aggregate limit: ${this.aggregateBytes} + ${this.maximumResponseBytes} > ${this.maximumAggregateBytes}`);
        let lastStatus = 0;
        for (let attempt = 1; attempt <= this.maximumAttempts; attempt += 1) {
            ...
            if (response.status >= 300 && response.status < 400)
                throw new Error(`Redirects are prohibited for ${url.host}: ${response.status}`);
            ...
            if (!response.ok)
                throw new Error(`Read-only GET failed for ${url.host}: ${response.status}`);
            ...
            return {
                ...
                receipt: {
                    ...
                    sha256: (0, crypto_1.createHash)("sha256").update(body).digest("hex"),
                    responseHeaders,
    ...
    async readBoundedBody(response) {
        if (!response.body)
...
exports.ServerReadonlyHttpClient = ServerReadonlyHttpClient;
//# sourceMappingURL=server-readonly-http.js.map