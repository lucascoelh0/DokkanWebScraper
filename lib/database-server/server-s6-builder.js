...
exports.sourceLineageAggregateSha256 = exports.validateServerS6 = exports.buildServerS6 = void 0;
const crypto_1 = require("crypto");
const classifications = ["agreement", "representation_gain", "confirmed_conflict", "unknown", "unjoinable"];
const subjectKeys = ["active_banner_cache_delta", "featured_character_identity", "official_featured_relationship", "current_schedule_authority", "maintenance_authority", "banner_commercial_semantics", "sbr_root_and_stage_identity", "ultimate_clash_root_identity", "world_tournament_current_schedule", "burst_mode_root_identity", "pettan_series_map_relation", "reward_row_identity", "asset_reference_delivery", "current_asset_manifest_version"];
const sourceKeys = ["server_s1", "server_s2", "server_s3", "server_s4", "server_s5", "fyi_summons_implementation", "summons_index_cache", "summons_details_cache", "characters_manifest", "characters_payload"];
const allowedClassifications = new Map([
...
const emptyCounts = () => ({ agreement: 0, representationGain: 0, confirmedConflict: 0, unknown: 0, unjoinable: 0 });
const sortedUnique = (values) => [...new Set(values.map(String))].sort((a, b) => a.localeCompare(b, "en", { numeric: true }));
const sumCounts = (counts) => counts.agreement + counts.representationGain + counts.confirmedConflict + counts.unknown + counts.unjoinable;
const classificationCountKey = { agreement: "agreement", representation_gain: "representationGain", confirmed_conflict: "confirmedConflict", unknown: "unknown", unjoinable: "unjoinable" };
function completeSubject(value) {
    const counts = emptyCounts();
    for (const classification of classifications)
        counts[classificationCountKey[classification]] = value.identities[classification]?.length ?? 0;
    return { ...value, comparedCount: sumCounts(counts), counts, identityMaterialization: "complete" };
}
function countedSubject(value) {
    return { ...value, comparedCount: sumCounts(value.counts), identityMaterialization: "source_gate_only", identities: {} };
}
function gateLineage(input) {
    return { key: `server_${input.gate}`, path: input.path, contractVersion: input.contractVersion, sha256: input.sha256, sizeBytes: input.sizeBytes, authority: "validated_server_gate", generatedAt: input.generatedAt };
}
function buildServerS6(inputs) {
    const { s1, s2, s3, s4, s5, cache } = inputs;
    if (s1.dataset.authority?.role !== "community_shadow_only" || s1.dataset.authority?.officialDynamicAuthorityCount !== 0 || s1.dataset.schedules?.length !== 0)
        throw new Error("S6 S1 authority boundary");
    if (!s2.dataset.identityPolicy?.structuralIdsOnly || s2.dataset.identityPolicy?.titleJoinAllowed !== false)
        throw new Error("S6 S2 identity boundary");
    if (s3.dataset.authorityPolicy?.legacyRewardIdentityOrigin !== "unknown_payload_id_or_synthetic_index" || s3.dataset.authorityPolicy?.normalizedRowsLossless !== false)
        throw new Error("S6 S3 reward boundary");
    if (s4.dataset.e6Projection?.remoteManifestJoinedReferenceCount !== 0 || s4.dataset.e6Projection?.status !== "unknown")
        throw new Error("S6 S4 delivery boundary");
    if (s5.dataset.defaultEnabled !== false || s5.dataset.productionMutation !== false)
        throw new Error("S6 S5 activation boundary");
    const currentBannerIds = sortedUnique(s1.dataset.banners.map((value) => value.identity.id)), oldBannerIds = sortedUnique(cache.existingBannerIds);
    const oldBannerSet = new Set(oldBannerIds), currentBannerSet = new Set(currentBannerIds);
    const featuredEntries = s1.dataset.banners.flatMap((banner) => banner.featuredCharacters.entries.map((entry) => ({ bannerId: String(banner.identity.id), ordinal: String(entry.ordinal), cardId: String(entry.payloadCharacterId ?? entry.entryCharacterId) })));
    const featuredCardIds = sortedUnique(featuredEntries.map((value) => value.cardId)), characterSet = new Set(cache.characterIds);
    const sbr = s2.dataset.families.find((value) => value.key === "super_battle_road"), clash = s2.dataset.families.find((value) => value.key === "ultimate_clash"), tournament = s2.dataset.families.find((value) => value.key === "world_tournament"), burst = s2.dataset.families.find((value) => value.key === "burst_mode"), pettan = s2.dataset.families.find((value) => value.key === "pettan_battle");
    const identitySet = (family, key) => sortedUnique(family.identitySets.find((value) => value.key === key)?.ids ?? []);
    const join = (family, key) => family.joins.find((value) => value.key === key);
    const rewardCounts = emptyCounts();
    for (const assessment of s3.dataset.assessments)
        assessment.classification === "partial_candidate" ? rewardCounts.unknown++ : assessment.classification === "unjoinable" ? rewardCounts.unjoinable++ : assessment.classification === "agreement" ? rewardCounts.agreement++ : rewardCounts.confirmedConflict++;
    const subjects = [
...
exports.buildServerS6 = buildServerS6;
function validateServerS6(dataset) {
    const failures = [], exactSubjects = dataset.subjects.length === subjectKeys.length && new Set(dataset.subjects.map(value => value.key)).size === subjectKeys.length && subjectKeys.every(key => dataset.subjects.some(value => value.key === key));
    let exclusiveClassifications = exactSubjects, exactAccounting = exactSubjects;
    for (const subject of dataset.subjects) {
        if (Object.values(subject.counts).some(value => !Number.isSafeInteger(value) || value < 0) || sumCounts(subject.counts) !== subject.comparedCount)
        ...
        if (unique.size !== materialized.length)
            exclusiveClassifications = false;
        if (subject.identityMaterialization === "complete") {
            if (materialized.length !== subject.comparedCount)
                exactAccounting = false;
            for (const classification of classifications)
                if ((subject.identities[classification]?.length ?? 0) !== subject.counts[classificationCountKey[classification]])
                    exactAccounting = false;
        }
    ...
    for (const subject of dataset.subjects)
        for (const key of Object.keys(recomputed))
            recomputed[key] += subject.counts[key];
    if (JSON.stringify(recomputed) !== JSON.stringify({ agreement: dataset.totals.agreement, representationGain: dataset.totals.representationGain, confirmedConflict: dataset.totals.confirmedConflict, unknown: dataset.totals.unknown, unjoinable: dataset.totals.unjoinable }) || sumCounts(recomputed) !== dataset.totals.comparedCount || dataset.totals.subjectCount !== 14)
        exactAccounting = false;
    const expectedGateVersions = new Map([["server_s1", "0.2.0"], ["server_s2", "0.3.0"], ["server_s3", "0.4.0"], ["server_s4", "0.5.0"], ["server_s5", "0.6.0"]]);
    ...
    if (!exclusiveClassifications)
        failures.push("exclusive classifications");
    if (!exactAccounting)
        failures.push("exact accounting");
    if (!sourceLineageComplete)
        failures.push("source lineage");
    if (!classificationPolicyPreserved)
        failures.push("classification policy");
    if (!sqliteAuthorityPreserved)
        failures.push("SQLite authority");
    if (!zeroConflictNotCompleteness)
        failures.push("zero-conflict completeness boundary");
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-server-shadow-parity" || dataset.contractVersion !== "0.7.0" || dataset.generatedAtPolicy !== "latest_input_derivation_time" || dataset.collectionMode !== "validated_local_artifacts_no_network")
...
exports.validateServerS6 = validateServerS6;
function sourceLineageAggregateSha256(lineage) {
    return (0, crypto_1.createHash)("sha256").update(lineage.map(value => `${value.key}:${value.path}:${value.contractVersion ?? "none"}:${value.sha256}:${value.sizeBytes}:${value.authority}:${value.generatedAt ?? "none"}`).join("\n")).digest("hex");
}
exports.sourceLineageAggregateSha256 = sourceLineageAggregateSha256;
//# sourceMappingURL=server-s6-builder.js.map