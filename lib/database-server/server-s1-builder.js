...
exports.validateServerS1Dataset = exports.buildServerS1Coverage = exports.buildServerS1Dataset = exports.calculateAvailability = exports.timeValue = void 0;
function clean(value) {
    const normalized = value?.trim();
    return normalized ? normalized : undefined;
}
function timeValue(rawValue) {
    ...
    if (!offset)
        return { raw, timezone: { status: "unknown", missing: "explicit_timezone" } };
...
function calculateAvailability(value, asOf) {
    ...
    if (!Number.isFinite(now) || !Number.isFinite(start) || !Number.isFinite(end))
        return { state: "unknown", asOf, status: "unknown", basis: ["complete_explicit_period_unavailable"] };
    const state = now < start ? "future" : now > end ? "ended" : "active";
...
function schedule(raw) {
    ...
    return {
        ...
        status: "partial",
        ...((title || imagePath) ? { presentation: { ...(title ? { title } : {}), ...(imagePath ? { imagePath } : {}), locale: "source_embedded_unverified" } } : {}),
        period: value,
...
function banner(raw) {
    ...
    const entries = raw.featuredCharacters?.map((entry, ordinal) => ({ ordinal, ...entry, boundary: "community_structural_ids_not_official_featured_relation" }))
    ...
    return {
        ...
        ...((title || description || bannerUrl) ? { presentation: { ...(title ? { title } : {}), ...(description ? { description } : {}), ...(bannerUrl ? { bannerUrl } : {}), locale: "source_embedded_unverified" } } : {}),
        period: value,
...
function buildServerS1Dataset(options) {
    if (options.receipts.length === 0)
        throw new Error("S1 requires at least one successful public GET receipt.");
...
exports.buildServerS1Dataset = buildServerS1Dataset;
function buildServerS1Coverage(dataset) {
    const periods = [...dataset.schedules.map(value => value.period), ...dataset.banners.map(value => value.period)], times = periods.flatMap(value => [value.startsAt, value.endsAt]).filter((value) => Boolean(value));
    const states = ["active", "future", "ended", "unknown"], availability = [...dataset.schedules.map(value => value.calculatedAvailability.state), ...dataset.banners.map(value => value.calculatedAvailability.state)];
    return {
        schemaVersion: 1,
        ...
        availabilityCounts: Object.fromEntries(states.map(state => [state, availability.filter(value => value === state).length])),
        officialDynamicAuthorityCount: 0,
    };
}
exports.buildServerS1Coverage = buildServerS1Coverage;
function validateServerS1Dataset(dataset) {
    const failures = [], unique = (values) => new Set(values).size === values.length;
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-server-schedule-and-banners" || dataset.contractVersion !== "0.2.0")
        failures.push("contract identity");
    if (dataset.sourceSnapshotVersion !== "global-6.4.0-v338-2026-08-05" || dataset.sourceS0.contractVersion !== "0.1.0" || dataset.sourceS0.sha256 !== "199074a781dd94f64eb425f7965672edd84982041de82ccc8f7ac9211b7603a9" || JSON.stringify(dataset.sourceS0.endpointKeys) !== JSON.stringify(["fyi-active-summons", "fyi-summon-detail"]))
        failures.push("S0 lineage");
    const getOnly = dataset.collection.method === "GET" && dataset.collection.concurrency === 1 && dataset.collection.receipts.every(value => {
        try {
            ...
            return url.protocol === "https:" && url.hostname === "dokkan.fyi";
        }
        catch {
            return false;
        }
    ...
    if (!getOnly)
        failures.push("GET-only collection");
    if (!unique(dataset.schedules.map(value => `${value.identity.sourceFamily}:${value.identity.rootId}`)) || !unique(dataset.banners.map(value => value.identity.id)))
        failures.push("duplicate structural identity");
    if (dataset.schedules.length !== 0)
        failures.push("unauthorized schedule source");
    if (dataset.schedules.some(value => !/^\d+$/.test(value.identity.rootId)) || dataset.banners.some(value => !/^\d+$/.test(value.identity.id)))
        failures.push("non-numeric root identity");
    const allTimes = [...dataset.schedules.map(value => value.period), ...dataset.banners.map(value => value.period)].flatMap(value => [value.startsAt, value.endsAt]).filter((value) => Boolean(value));
    if (allTimes.some(value => value.timezone.status === "unknown" && value.normalizedInstant !== undefined))
        failures.push("timezone default");
    if (dataset.banners.some(value => value.currency.status !== "unknown" || value.stepsAndRates.status !== "unknown"))
        failures.push("commercial semantic promotion");
    ...
    if (!authorityBoundaryPreserved)
        failures.push("authority boundary");
    if (dataset.collection.fetchedBytes !== dataset.collection.receipts.reduce((sum, value) => sum + value.sizeBytes, 0) || dataset.collection.fetchedBytes > dataset.collection.maximumAggregateBytes)
        failures.push("byte accounting");
    const matchesReceipt = (value, kind) => {
        try {
            const url = new URL(value.url);
            return url.protocol === "https:" && url.hostname === "dokkan.fyi" && (kind === "summary" ? url.pathname === "/summons" && url.searchParams.get("active") === "true" && /^\d+$/.test(url.searchParams.get("category") ?? "") && /^\d+$/.test(url.searchParams.get("page") ?? "") : /^\/summons\/\d+$/.test(url.pathname) && url.search === "");
        }
        catch {
            return false;
        }
    ...
    const detailHashesByBannerId = new Map();
    for (const receipt of dataset.collection.receipts.filter(value => matchesReceipt(value, "detail"))) {
        ...
        const hashes = detailHashesByBannerId.get(bannerId) ?? new Set();
        hashes.add(receipt.sha256);
        detailHashesByBannerId.set(bannerId, hashes);
    }
    if (dataset.banners.some(value => value.provenance.summaryReceiptSha256s.length === 0
...
exports.validateServerS1Dataset = validateServerS1Dataset;
//# sourceMappingURL=server-s1-builder.js.map