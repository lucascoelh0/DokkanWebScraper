...
exports.validateServerS2Dataset = exports.buildServerS2Coverage = exports.buildServerS2Dataset = void 0;
function sortedIds(values) {
    return [...new Set(values)].sort((left, right) => BigInt(left) < BigInt(right) ? -1 : BigInt(left) > BigInt(right) ? 1 : 0);
}
function identitySet(key, kind, ids, source, authority) {
    return { key, kind, ids: sortedIds(ids), source, authority };
}
function buildServerS2Dataset(observation) {
...
exports.buildServerS2Dataset = buildServerS2Dataset;
function buildServerS2Coverage(dataset) {
...
exports.buildServerS2Coverage = buildServerS2Coverage;
function validateServerS2Dataset(dataset) {
    const failures = [];
    if (dataset.contract !== "dokkan-server-root-resolution" || dataset.contractVersion !== "0.3.0")
        failures.push("contract identity");
    if (dataset.collectionMode !== "local_static_and_cached_evidence_no_network")
        failures.push("network collection enabled");
    if (!dataset.identityPolicy.structuralIdsOnly || dataset.identityPolicy.titleJoinAllowed || dataset.identityPolicy.overlappingNumericNamespacesJoinAutomatically || dataset.identityPolicy.remotePresentationAuthority)
        failures.push("identity policy weakened");
    if (new Set(dataset.families.map(value => value.key)).size !== dataset.families.length)
        failures.push("duplicate family key");
    const expectedLineage = new Map([["events_e1", "sqlite_first_party"], ["events_e2", "sqlite_first_party"], ["events_e7", "community_shadow"], ["dokkaninfo_family_cache", "community_shadow"]]);
    const lineageKeys = dataset.sourceLineage.map(value => value.key);
    if (dataset.sourceLineage.length !== expectedLineage.size || new Set(lineageKeys).size !== expectedLineage.size || [...expectedLineage.keys()].some(key => !lineageKeys.includes(key)) || dataset.sourceLineage.some(value => expectedLineage.get(value.key) !== value.authority || !/^[a-f0-9]{64}$/.test(value.sha256) || value.sizeBytes <= 0 || !value.path.trim()))
        failures.push("invalid source lineage");
    for (const family of dataset.families) {
        const setKeys = new Set(family.identitySets.map(value => value.key));
        if (setKeys.size !== family.identitySets.length)
            failures.push(`duplicate identity set ${family.key}`);
        for (const set of family.identitySets) {
            if (new Set(set.ids).size !== set.ids.length || set.ids.some(id => !/^\d+$/.test(id)))
                failures.push(`non-structural identity ${set.key}`);
        }
        for (const join of family.joins) {
            if (join.leftSet !== "genkai_and_score_candidate_tables" && !setKeys.has(join.leftSet))
                failures.push(`unknown left set ${join.key}`);
            if (join.rightSet !== null && !setKeys.has(join.rightSet))
                failures.push(`unknown right set ${join.key}`);
            if (join.status === "supported" && (!join.joinKey || join.matchedCount === 0 || join.unmatchedCount !== 0))
                failures.push(`unsupported supported join ${join.key}`);
            if (join.status === "unjoinable" && join.matchedCount !== 0)
                failures.push(`unjoinable join has matches ${join.key}`);
            if (join.joinKey && /title|name/i.test(join.joinKey))
                failures.push(`text join ${join.key}`);
        }
    }
    ...
    if (sbr?.rootStatus !== "supported" || sbr.identitySets.find(value => value.key === "sbr_database_areas")?.ids.join(",") !== "710,720")
        failures.push("SBR database roots unresolved");
    const pettanJoin = pettan?.joins.find(value => value.key === "pettan_series_to_sd_map");
    if (pettanJoin?.status !== "unjoinable" || pettanJoin.joinKey !== null || pettanJoin.matchedCount !== 0)
        failures.push("Pettan namespace isolation lost");
    const expectedIdentityAuthorities = new Map([
    ...
    if (!sqliteAuthorityPreserved)
        failures.push("community identity promoted to database authority");
...
exports.validateServerS2Dataset = validateServerS2Dataset;
//# sourceMappingURL=server-s2-builder.js.map