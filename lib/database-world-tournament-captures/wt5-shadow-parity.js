...
exports.validateWt5 = exports.buildWt5 = exports.rewardDefinitionBoundary = exports.parentedStructuralMatches = void 0;
const crypto_1 = require("crypto");
const sha256 = (value) => (0, crypto_1.createHash)("sha256").update(value).digest("hex");
const totals = () => ({ agreement: 0, coverage_gap: 0, unknown: 0, unjoinable: 0 });
const numeric = (value) => typeof value === "number" && Number.isSafeInteger(value) ? value : typeof value === "string" && /^\d+$/.test(value) ? Number(value) : null;
const hasId = (values, id) => values.some(value => numeric(value) === id);
function parentedStructuralMatches(rows, idKey, ids, budokaiId) { return rows.filter((row) => ids.includes(numeric(row?.[idKey]) ?? -1) && numeric(row?.budokaiId) === budokaiId).length; }
exports.parentedStructuralMatches = parentedStructuralMatches;
function comparison(key, left, right, joinKey, structuralIds, classification, status, matchedCount, boundary) {
    return { key, left, right, joinKey, structuralIds, classification, status, matchedCount, boundary };
}
function rewardDefinitionBoundary(missionId, missionObserved, boxRankingId, boxObserved) {
...
exports.rewardDefinitionBoundary = rewardDefinitionBoundary;
function buildWt5(wt2, wt2Text, wt3, wt3Text, wt4, wt4Text, lockText, sources) {
    ...
    if (eventId !== 63 || JSON.stringify(mapIds) !== JSON.stringify([631, 632, 633, 634]) || JSON.stringify(boxIds) !== JSON.stringify([631]) || JSON.stringify(scheduleIds) !== JSON.stringify([5]) || JSON.stringify(missionIds) !== JSON.stringify([63001]))
        throw new Error("WT5 pinned structural coordinates changed");
    ...
    const e1Event = e1.catalog.some((value) => value.identity?.kind === "budokai" && numeric(value.identity.id) === eventId);
    const e1Lifecycle = e1.availabilityHints.some((value) => value.identity?.entityKind === "budokai" && numeric(value.identity.entityId) === eventId);
    const s2Family = s2.families.find((value) => value.key === "world_tournament"), s2Event = s2Family?.identitySets?.some((set) => set.kind === "budokai" && hasId(set.ids ?? [], eventId)) === true;
    const sqliteEvent = hasId(sources.sqlite.budokaiIds, eventId);
    const h3Event = h3.entities.some((value) => value.entityType === "budokai" && numeric(value.entityId) === eventId);
    ...
    const scheduleMatches = scheduleIds.filter(id => h3.entities.some((value) => value.entityType === "bonus_schedule" && numeric(value.entityId) === id)).length;
    const comparisons = [
    ...
    ].map(value => ({ key: value.key, sourceKind: "wt_gate", contractVersion: value.value.contractVersion, sizeBytes: Buffer.byteLength(value.text), sha256: sha256(value.text) }));
    const dataset = {
        schemaVersion: 1,
    ...
    if (!validation.valid)
        throw new Error(`WT5 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildWt5 = buildWt5;
function validateWt5(dataset) {
    const failures = [], counts = totals(), allowedKeys = ["bonus_schedules", "box_ranking", "briefing_supporters_advantageous", "budokai_maps", "event_identity", "event_lifecycle", "general_ranking", "global_event_status", "mission_relation", "opaque_start", "rank_definitions", "ranking_borders_my_ranking", "ranking_friends"];
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-world-tournament-shadow-parity" || dataset.contractVersion !== "0.6.0" || dataset.collectionMode !== "offline_pinned_structural_ids_only_no_requests" || dataset.productionMutation || dataset.defaultEnabled || dataset.identityPolicy !== "numeric_structural_ids_only_names_and_texts_never_identity" || !/^[a-f0-9]{64}$/.test(dataset.sourceLockSha256))
        failures.push("contract");
    if (JSON.stringify(dataset.sourceCoverage.map(value => value.series)) !== JSON.stringify(["E0-E9", "H0-H13", "S0-S7", "SQLite"]) || dataset.sourceCoverage.some(value => !value.boundary || (value.series === "SQLite" ? value.status !== "direct_and_pinned" : value.status !== "via_pinned_closure")))
        failures.push("source coverage");
    if (JSON.stringify(dataset.comparisons.map(value => value.key)) !== JSON.stringify(allowedKeys))
        failures.push("comparison keys");
    const fixed = new Map([["briefing_supporters_advantageous", "unjoinable"], ["general_ranking", "unjoinable"], ["global_event_status", "unknown"], ["opaque_start", "unjoinable"], ["rank_definitions", "unjoinable"], ["ranking_borders_my_ranking", "unjoinable"], ["ranking_friends", "unjoinable"]]);
    for (const value of dataset.comparisons) {
        ...
        if (value.status !== expected || !value.boundary || value.structuralIds.some(id => !Number.isSafeInteger(id) || id < 0) || new Set(value.structuralIds).size !== value.structuralIds.length || !Number.isSafeInteger(value.matchedCount) || value.matchedCount < 0)
            failures.push("comparison policy");
        if ((value.status === "agreement" && value.matchedCount === 0) || (value.status === "coverage_gap" && value.matchedCount !== 0 && value.key !== "bonus_schedules") || (value.status === "unjoinable" && value.matchedCount !== 0) || (fixed.get(value.key) === "unknown" && value.matchedCount !== 0))
            failures.push("comparison evidence");
        if (value.joinKey !== null && !/^[a-z_]+\.(?:id)$/.test(value.joinKey))
            failures.push("join key policy");
    }
    if (JSON.stringify(counts) !== JSON.stringify(dataset.totals))
    ...
    const missionObserved = dataset.comparisons.find(value => value.key === "mission_relation").matchedCount > 0, boxObserved = dataset.comparisons.find(value => value.key === "box_ranking").matchedCount > 0, expectedDefinitions = rewardDefinitionBoundary(63001, missionObserved, 631, boxObserved);
    if (JSON.stringify(dataset.rewards.definitions) !== JSON.stringify(expectedDefinitions) || dataset.rewards.granted.status !== "unknown" || dataset.rewards.granted.observed || dataset.rewards.granted.boundary !== "no_reward_grant_or_claim_observed")
        failures.push("reward boundary");
...
exports.validateWt5 = validateWt5;
//# sourceMappingURL=wt5-shadow-parity.js.map