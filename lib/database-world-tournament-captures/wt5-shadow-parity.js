...
function buildWt5(wt2, wt2Text, wt3, wt3Text, wt4, wt4Text, lockText, sources) {
    ...
    if (![eventId, ...mapIds, ...boxIds, ...scheduleIds, ...missionIds].every(Number.isSafeInteger) || mapIds.length === 0 || boxIds.length === 0 || scheduleIds.length === 0 || missionIds.length === 0 || [mapIds, boxIds, scheduleIds, missionIds].some(values => new Set(values).size !== values.length))
