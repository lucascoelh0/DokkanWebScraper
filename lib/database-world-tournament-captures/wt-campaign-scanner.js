...
function provenance(atom) { return { category: atom.category, structuralCategory: atom.structuralCategory, origin: atom.origin, method: atom.method, endpointHash: sha256(atom.endpoint), sourcePathHash: sha256(atom.path), kind: atom.kind }; }
function provenanceIdentity(value) { return `${value.category}\0${value.structuralCategory}\0${value.origin}\0${value.method}\0${value.endpointHash}\0${value.sourcePathHash}\0${value.kind}`; }
...
function distinctiveLiteral(atom, value) {
    if (Buffer.byteLength(value) < (atom.kind === "string" ? 16 : 8))
        return false;
    if (atom.kind !== "string")
        return true;
    if (/\s/.test(value))
        return false;
    if (/^[A-Za-z0-9]+$/.test(value) || /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(value) || /^[A-Z2-7]+={0,6}$/.test(value))
        return true;
    if (/^[A-Za-z0-9+/]+={0,2}$/.test(value) && value.length % 4 === 0 && /[A-Z0-9+=]/.test(value))
        return true;
    return /^[A-Za-z0-9_-]+={0,2}$/.test(value) && /[_-]/.test(value) && /[A-Z0-9]/.test(value);
}
function literalNeedsTokenBoundary(value) { return /^[A-Za-z0-9_+\/-]+={0,6}$/.test(value); }
function tokenByte(value) { return value !== undefined && ((value >= 48 && value <= 57) || (value >= 65 && value <= 90) || (value >= 97 && value <= 122) || value === 43 || value === 45 || value === 47 || value === 95); }
...
function buildMatchers(atoms) {
    ...
    const add = (pattern, atom, tokenBoundary) => {
        ...
        patterns.push({ bytes: pattern, anchorLength, tokenBoundary, ...source });
        let node = 0;
    ...
    for (const atom of atoms) {
        ...
        if (distinctiveLiteral(atom, literal)) {
            ...
            if (atom.kind === "string")
                add(Buffer.from(atom.canonicalValue, "utf8"), atom, false);
            if (atom.structuralCategory === "query_name" || atom.structuralCategory === "query_value" || atom.structuralCategory === "url_path_segment" || atom.structuralCategory === "url_username" || atom.structuralCategory === "url_password")
                add(Buffer.from(encodeURIComponent(literal), "utf8"), atom, false);
        }
    }
    const queue = [];
...
function matchLongValues(content, matcher, patterns) {
    ...
    for (let index = 0; index < content.length; index++) {
        ...
        for (const patternId of matcher[node].outputs) {
            ...
            if (start >= 0 && end <= content.length && content.subarray(start, end).equals(pattern.bytes) && (!pattern.tokenBoundary || (!tokenByte(content[start - 1]) && !tokenByte(content[end]))))
                matchedPatterns.add(patternId);
        }
    }
    return [...matchedPatterns].map(patternId => { const { bytes: _bytes, anchorLength: _anchorLength, tokenBoundary: _tokenBoundary, ...source } = patterns[patternId]; return { ...source, matchMode: "literal", foundStructuralCategory: "literal", foundPath: `$literal:${source.structuralCategory}` }; });
}
...
function scanContent(content, state) {
    ...
    for (const observation of deduplicateTargetObservations(observations)) {
        ...
        for (const source of matcher.get(key)?.values() ?? [])
            matches.push({ ...source, matchMode: "contextual", foundStructuralCategory: observation.structuralCategory, foundPath: observation.path });
    }
    const unique = new Map();
