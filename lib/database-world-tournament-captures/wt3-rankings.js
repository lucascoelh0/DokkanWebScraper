...
exports.validateWt3 = exports.buildWt3 = void 0;
...
function scope(route, path) {
    if (route.endsWith("/rankings"))
        return path.startsWith("$.rankers") ? "account_scoped" : "global";
    if (route.endsWith("/rankings/borders"))
        return path.startsWith("$.rankers") || path.startsWith("$.my_ranking") ? "account_scoped" : path === "$.point" ? "partial" : "global";
    if (route.endsWith("/rankings/friends"))
        return "account_scoped";
    if (route.includes("/box_rankings/"))
        return "partial";
    return "global";
}
function surface(wt1, path) {
    const route = wt1.routes.find(value => value.method === "GET" && value.route === path);
    if (!route)
        throw new Error(`WT3 missing ${path}`);
    const fields = route.responseSchema.map(value => ({ path: value.path, types: value.types, typeOccurrences: value.typeOccurrences, classification: scope(path, value.path), valueRetention: "omitted_schema_only" }));
    return { route: path, classification: scope(path, "$"), fields, sourceSchemaSha256: hash(JSON.stringify(route.responseSchema)) };
}
function buildWt3(wt0, wt0Text, wt1, wt1Text) {
    const lock = { ...wt0.source, fileName: "world_tournament_until_start_crash.har" };
    if (`${JSON.stringify(wt0, null, 2)}\n` !== wt0Text || `${JSON.stringify(wt1, null, 2)}\n` !== wt1Text || !(0, wt0_audit_1.validateWt0)(wt0, lock).valid || !(0, wt1_catalog_1.validateWt1)(wt1).valid || wt1.source.sha256 !== hash(wt0Text) || wt1.source.sizeBytes !== Buffer.byteLength(wt0Text))
        throw new Error("WT3 upstream mismatch");
    ...
    const ids = (kind) => [...new Set(wt0.structuralIds.filter(value => value.kind === kind).map(value => value.id))].sort((a, b) => a - b);
    ...
    const dataset = { schemaVersion: 1, contract: "dokkan-world-tournament-rankings-box-schedules", contractVersion: "0.4.0", lineage: [{ gate: "WT0", contractVersion: wt0.contractVersion, sizeBytes: Buffer.byteLength(wt0Text), sha256: hash(wt0Text) }, { gate: "WT1", contractVersion: wt1.contractVersion, sizeBytes: Buffer.byteLength(wt1Text), sha256: hash(wt1Text) }], generalRanking: { ...general, paginationPaths: ["$.current_page", "$.per_page", "$.total_page"], updatedAtPath: "$.updated_at" }, rankingBorders: { ...borders, myRankingPath: "$.my_ranking", updatedAtPath: "$.updated_at" }, rankingFriends: friends, boxRanking: { ...box, boxRankingIds: ids("box_ranking"), statusSemantics: "partial" }, bonusSchedules: { ...schedules, scheduleIds: ids("bonus_schedule") }, rewards: { observedSchemaPaths: rewardPaths, definitionsObserved: rewardPaths.length > 0, grantedOrClaimedObserved: false, boundary: "no_ranking_reward_definition_or_grant_response_observed" } };
    const validation = validateWt3(dataset);
    if (!validation.valid)
        throw new Error(`WT3 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildWt3 = buildWt3;
function validateWt3(dataset) {
    const failures = [], surfaces = [dataset.generalRanking, dataset.rankingBorders, dataset.rankingFriends, dataset.boxRanking, dataset.bonusSchedules], types = new Set(["array", "boolean", "null", "number", "object", "string"]), has = (surface, path) => surface.fields.some(value => value.path === path);
    ...
    if (invalidProjection)
        failures.push("lossless field projection");
    ...
    if (dataset.boxRanking.statusSemantics !== "partial" || !has(dataset.boxRanking, "$.ranking_status") || dataset.boxRanking.boxRankingIds.length === 0 || dataset.bonusSchedules.scheduleIds.length === 0 || [...dataset.boxRanking.boxRankingIds, ...dataset.bonusSchedules.scheduleIds].some(id => !Number.isSafeInteger(id) || id < 0))
        failures.push("structural identities");
    if (dataset.rewards.observedSchemaPaths.length !== 0 || dataset.rewards.definitionsObserved || dataset.rewards.grantedOrClaimedObserved || dataset.rewards.boundary !== "no_ranking_reward_definition_or_grant_response_observed")
...
exports.validateWt3 = validateWt3;
//# sourceMappingURL=wt3-rankings.js.map