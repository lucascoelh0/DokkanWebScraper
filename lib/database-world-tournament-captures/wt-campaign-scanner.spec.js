...
function focusedHar(options) {
    ...
    if (Object.prototype.hasOwnProperty.call(options, "requestBody") || options.requestText !== undefined)
        request.postData = { mimeType: options.requestMimeType ?? (Object.prototype.hasOwnProperty.call(options, "requestBody") ? "application/json" : "text/plain"), text: options.requestText ?? JSON.stringify(options.requestBody) };
    if (Object.prototype.hasOwnProperty.call(options, "responseBody") || options.responseText !== undefined)
        response.content = { mimeType: options.responseMimeType ?? (Object.prototype.hasOwnProperty.call(options, "responseBody") ? "application/json" : "text/plain"), text: options.responseText ?? JSON.stringify(options.responseBody) };
...
describe("World Tournament campaign scanner", () => {
    ...
    it("requires token boundaries for compact long literals", () => {
        ...
        const alphabetic = (0, wt_campaign_scanner_1.buildWtSensitiveCatalog)(focusedHar({ responseBody: { token: "abcdefghijklmnop" } }));
        assert.equal((0, wt_campaign_scanner_1.scanWtAuditTargets)(alphabetic, [target("larger-token.ts", "spec", "const unrelated = \"xabcdefghijklmnopy\";")]).valid, true);
    });
    ...
    it("fails closed for an expected malformed JSON body and an empty catalog", () => {
        assert.throws(() => (0, wt_campaign_scanner_1.buildWtSensitiveCatalog)(focusedHar({ url: "https://example.invalid/", requestText: "{broken", requestMimeType: "application/json" })), /JSON body is malformed/);
        assert.throws(() => (0, wt_campaign_scanner_1.buildWtSensitiveCatalog)(focusedHar({ url: "https://example.invalid/", requestText: "", requestMimeType: "application/json" })), /JSON body is malformed/);
        ...
        assert.throws(() => (0, wt_campaign_scanner_1.scanWtAuditTargets)(catalog, [target("invalid-json-body", "fixture", JSON.stringify({ postData: { mimeType: "application/json", text: "{broken" } }))]), /JSON body is malformed/);
