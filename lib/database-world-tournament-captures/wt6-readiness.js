...
function validateWt6(dataset) {
    const failures = [];
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-world-tournament-readiness" || dataset.contractVersion !== "0.7.0" || dataset.collectionMode !== "offline_double_generation_no_requests" || dataset.productionMutation || dataset.defaultEnabled)
    ...
    if (dataset.stop.gate !== "WT6" || dataset.stop.mergeMainAllowed || dataset.stop.deliveryPolicy !== "push_isolated_branch_only")
