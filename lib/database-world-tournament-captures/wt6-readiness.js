...
function buildWt6(source, lineage, aggregateSha256, implementationAggregateSha256, scan, memory) {
    ...
    const dataset = {
        schemaVersion: 1, contract: "dokkan-world-tournament-readiness", contractVersion: "0.11.0", collectionMode: "offline_double_generation_no_requests", productionMutation: false, defaultEnabled: false, source, lineage, determinism: { passCount: 2, upstreamArtifactCount: 18, wt6OutputFileCount: 3, totalArtifactCount: 21, upstreamAggregateSha256: aggregateSha256, implementationAggregateSha256, upstreamByteIdentical: true, wt6OutputByteIdentical: true }, security: scan, memory, decisions,
...
function validateWt6(dataset) {
    const failures = [];
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-world-tournament-readiness" || dataset.contractVersion !== "0.11.0" || dataset.collectionMode !== "offline_double_generation_no_requests" || dataset.productionMutation || dataset.defaultEnabled)
    ...
    const securityCategories = dataset.security.categoryTargetCounts, sourceCategories = dataset.security.sourceCategoryValueCounts, classes = dataset.security.matchClassCounts, evidence = dataset.security.firstPartyAppIdentityEvidence;
    const classTotal = Object.values(classes).reduce((sum, value) => sum + value, 0), permittedTotal = classes.permitted_protocol_structure + classes.permitted_public_game_structure + classes.permitted_synthetic_fixture;
    const evidenceValid = evidence?.schemaVersion === 1 && evidence.contract === "dokkan-wt-first-party-app-identity" && evidence.rule === wt_first_party_app_identity_1.WT_FIRST_PARTY_APP_IDENTITY_RULE && evidence.apk.sizeBytes === wt_first_party_app_identity_1.WT_FIRST_PARTY_APK_SIZE_BYTES && evidence.apk.sha256 === wt_first_party_app_identity_1.WT_FIRST_PARTY_APK_SHA256 && evidence.tool.name === "aapt" && evidence.tool.executableSha256 === wt_first_party_app_identity_1.WT_FIRST_PARTY_AAPT_SHA256 && evidence.tool.version === wt_first_party_app_identity_1.WT_FIRST_PARTY_AAPT_VERSION && evidence.tool.command === "aapt dump badging <pinned-apk>" && evidence.identity.jsonType === "string" && evidence.identity.sizeBytes === wt_first_party_app_identity_1.WT_FIRST_PARTY_APP_IDENTITY_SIZE_BYTES && evidence.identity.sha256 === wt_first_party_app_identity_1.WT_FIRST_PARTY_APP_IDENTITY_SHA256;
    if (!dataset.security.valid || dataset.security.sensitiveMatchCount <= 0 || classTotal !== dataset.security.sensitiveMatchCount || dataset.security.prohibitedSensitiveMatchCount !== 0 || classes.prohibited_sensitive !== 0 || dataset.security.unresolvedMatchCount !== 0 || classes.unresolved !== 0 || dataset.security.permittedStructuralMatchCount !== permittedTotal || permittedTotal !== dataset.security.sensitiveMatchCount || dataset.security.mutationBodyMatchCount <= 0 || dataset.security.publicFirstPartyIdentityMatchCount !== dataset.security.mutationBodyMatchCount || dataset.security.matchRuleCounts[wt_first_party_app_identity_1.WT_FIRST_PARTY_APP_IDENTITY_RULE] !== dataset.security.publicFirstPartyIdentityMatchCount || dataset.security.accountScopedPayloadMatchCount !== 0 || dataset.security.opaqueCredentialMatchCount !== 0 || !evidenceValid || dataset.security.rawHarTargetCount !== 0 || dataset.security.harStructureTargetCount !== 2 || dataset.security.tipHarStructureTargetCount !== 0 || dataset.security.historicalHarAllowlistCount !== 2 || !dataset.security.historicalHarAllowlistSatisfied || JSON.stringify([...dataset.security.historicalHarTargetFingerprints].sort()) !== JSON.stringify([...wt6_contract_1.WT6_ALLOWED_HISTORICAL_HAR_TARGET_FINGERPRINTS].sort()) || dataset.security.commitCount !== 11 || dataset.security.targetCount <= 0 || dataset.security.uniqueBlobCount <= 0 || dataset.security.uniqueBlobCount > dataset.security.targetCount || securityCategories.tip <= 0 || securityCategories.history_old <= 0 || securityCategories.history_new <= 0 || dataset.security.sensitiveValueCount <= 0 || Object.keys(sourceCategories).sort().join(",") !== "cookies,headers,query,request_body,response_body,url_credentials,url_path" || Object.values(sourceCategories).some(value => !Number.isSafeInteger(value) || value < 0) || !dataset.security.allTrackedTipBlobsExamined || !dataset.security.allChangedHistoricalBlobsExamined || !dataset.security.noPathOrDocumentTypeExclusions || dataset.security.requestReplayImplemented || dataset.security.credentialFlowImplemented)
