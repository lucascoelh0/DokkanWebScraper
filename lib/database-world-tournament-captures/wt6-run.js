...
async function main() {
    const sourceRootArgument = arg("--source-root"), sourceRelativePath = arg("--source-relative-path"), auditBase = arg("--audit-base"), auditTip = arg("--audit-tip"), firstPartyApk = arg("--first-party-apk"), aapt = arg("--aapt"), initialSource = (0, wt_source_boundary_1.readWtExternalSource)(root, sourceRootArgument, sourceRelativePath);
    ...
    const privateSnapshotRoot = (0, path_1.resolve)(root, ".agent-logs/world-tournament"), appIdentity = await (0, wt_first_party_app_identity_1.extractWtFirstPartyAppIdentity)(firstPartyApk, aapt, privateSnapshotRoot), catalog = (0, wt_campaign_scanner_1.buildWtSensitiveCatalog)(harText), gitTargets = (0, wt_campaign_scanner_1.collectWtGitAuditTargets)(root, auditBase, auditTip), auditScan = (0, wt_campaign_scanner_1.scanWtAuditTargets)(catalog, gitTargets, { allowedHistoricalHarTargetFingerprints: wt6_contract_1.WT6_ALLOWED_HISTORICAL_HAR_TARGET_FINGERPRINTS, firstPartyAppIdentity: appIdentity });
    if (!auditScan.valid)
        throw new Error(`WT6 historical scanner failed: prohibited=${auditScan.prohibitedSensitiveMatchCount}, unresolved=${auditScan.unresolvedMatchCount}, rawHar=${auditScan.rawHarTargetCount}, targets=${auditScan.targetCount}`);
    ...
    if (!auditScan.firstPartyAppIdentityEvidence || auditScan.prohibitedSensitiveMatchCount !== 0 || auditScan.unresolvedMatchCount !== 0 || auditScan.accountScopedPayloadMatchCount !== 0 || auditScan.opaqueCredentialMatchCount !== 0)
        throw new Error("WT6 classified scanner boundary is not green");
    const source = { sourceId: wt0.source.sourceId, sizeBytes: wt0.source.sizeBytes, sha256: wt0.source.sha256, entryCount: wt0.source.entryCount }, security = { commitCount: gitTargets.commitCount, targetCount: auditScan.targetCount, uniqueBlobCount: auditScan.uniqueBlobCount, categoryTargetCounts: { tip: auditScan.categoryTargetCounts.tip, history_old: auditScan.categoryTargetCounts.history_old, history_new: auditScan.categoryTargetCounts.history_new }, sensitiveValueCount: catalog.sensitiveValueCount, sourceCategoryValueCounts: catalog.categoryValueCounts, sensitiveMatchCount: auditScan.sensitiveMatchCount, matchClassCounts: auditScan.matchClassCounts, matchRuleCounts: auditScan.matchRuleCounts, prohibitedSensitiveMatchCount: auditScan.prohibitedSensitiveMatchCount, unresolvedMatchCount: auditScan.unresolvedMatchCount, permittedStructuralMatchCount: auditScan.permittedStructuralMatchCount, mutationBodyMatchCount: auditScan.mutationBodyMatchCount, publicFirstPartyIdentityMatchCount: auditScan.publicFirstPartyIdentityMatchCount, accountScopedPayloadMatchCount: auditScan.accountScopedPayloadMatchCount, opaqueCredentialMatchCount: auditScan.opaqueCredentialMatchCount, firstPartyAppIdentityEvidence: auditScan.firstPartyAppIdentityEvidence, harStructureTargetCount: 2, tipHarStructureTargetCount: 0, rawHarTargetCount: 0, historicalHarAllowlistCount: 2, historicalHarAllowlistSatisfied: true, historicalHarTargetFingerprints: [...auditScan.harStructureTargets.map(value => value.fingerprint).sort()], allTrackedTipBlobsExamined: true, allChangedHistoricalBlobsExamined: true, noPathOrDocumentTypeExclusions: true, valid: true, requestReplayImplemented: false, credentialFlowImplemented: false }, member = (fileName, text) => ({ fileName, sizeBytes: Buffer.byteLength(text), sha256: sha256(text) });
...
void main().catch(error => { process.stderr.write(`${error instanceof Error ? error.message : "WT6 failed"}\n`); process.exitCode = 1; });
