...
if ((0, fs_1.lstatSync)(sourceArgument).isSymbolicLink())
    throw new Error("WT6 source boundary");
const sourcePath = (0, fs_1.realpathSync)(sourceArgument);
...
function runGate(gate) {
    ...
    if (gate === 0) {
        ...
        args.push("--source", sourcePath, "--external-lock", (0, path_1.resolve)(lockDirectory, `wt6-double-${(0, crypto_1.randomUUID)()}.json`));
    }
...
const lineage = first, aggregateSha256 = sha256(lineage.map(value => `${value.gate}/${value.fileName}\0${value.sizeBytes}\0${value.sha256}\n`).join("")), before = (0, fs_1.statSync)(sourcePath), harText = (0, fs_1.readFileSync)(sourcePath, "utf8"), after = (0, fs_1.statSync)(sourcePath);
if (before.size !== after.size || before.mtimeMs !== after.mtimeMs)
    throw new Error("WT6 source changed during scan");
...
function scanFiles(directory, include) { const rows = []; for (const name of (0, fs_1.readdirSync)(directory).sort()) {
    const path = (0, path_1.resolve)(directory, name), info = (0, fs_1.statSync)(path);
    if (info.isDirectory())
        rows.push(...scanFiles(path, include));
...
const exactScanInputs = [
    ...scanFiles((0, path_1.resolve)(root, "database-world-tournament-captures/fixtures"), name => name.endsWith(".json") && !name.endsWith(".har.json")),
    ...scanFiles((0, path_1.resolve)(root, "data/database-world-tournament-captures"), name => name.endsWith(".json") && !name.startsWith("wt6-")),
], genericScanInputs = [
    ...scanFiles((0, path_1.resolve)(root, "database-world-tournament-captures"), name => /\.(?:ts|json|py)$/.test(name) && !name.endsWith(".spec.ts")),
    ...scanFiles((0, path_1.resolve)(root, "lib/database-world-tournament-captures"), name => name.endsWith(".js") && !name.endsWith(".spec.js")),
].filter(value => !value.name.includes("/fixtures/"));
if ([...exactScanInputs, ...genericScanInputs].some(value => value.name.toLowerCase().endsWith(".har")) || (0, child_process_1.execFileSync)("git", ["ls-files", "*.har"], { cwd: root, encoding: "utf8", timeout: 5000, windowsHide: true }).trim())
    throw new Error("WT6 raw HAR tracked");
const exactScan = (0, wt0_audit_1.scanWtCampaignTexts)(harText, wt0.structuralIds.map(value => value.id), exactScanInputs), genericScan = (0, wt0_audit_1.scanWtCampaignTexts)(harText, wt0.structuralIds.map(value => value.id), genericScanInputs), exactMatches = exactScan.exactCapturedValueMatches + genericScan.exactCapturedValueMatches, genericMatches = exactScan.genericSecretPatternMatches + genericScan.genericSecretPatternMatches, matchedFiles = [...exactScan.exactMatchedFiles, ...genericScan.exactMatchedFiles];
if (exactMatches !== 0 || genericMatches !== 0)
    throw new Error(`WT6 captured-value scan failed: exact=${exactMatches}, generic=${genericMatches}, files=${exactScan.scannedFileCount + genericScan.scannedFileCount}, matched=${matchedFiles.join(",")}`);
const implementationInputs = [...scanFiles((0, path_1.resolve)(root, "lib/database-world-tournament-captures"), name => name.endsWith(".js") && !name.endsWith(".spec.js")), ...scanFiles((0, path_1.resolve)(root, "database-world-tournament-captures"), name => name === "wt5-sqlite-readonly-bridge.py")].sort((left, right) => left.name.localeCompare(right.name)), implementationAggregateSha256 = sha256(implementationInputs.map(value => `${value.name}\0${Buffer.byteLength(value.text)}\0${sha256(value.text)}\n`).join(""));
...
const source = { sourceId: wt0.source.sourceId, sizeBytes: wt0.source.sizeBytes, sha256: wt0.source.sha256, entryCount: wt0.source.entryCount }, security = { scannedFileCount: exactScan.scannedFileCount + genericScan.scannedFileCount, capturedSensitiveValueCount: exactScan.capturedSensitiveValueCount, exactCapturedValueMatches: 0, genericSecretPatternMatches: 0, valid: true, rawHarTracked: false, requestReplayImplemented: false, credentialFlowImplemented: false }, member = (fileName, text) => ({ fileName, sizeBytes: Buffer.byteLength(text), sha256: sha256(text) });
