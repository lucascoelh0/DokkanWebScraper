...
describe("world tournament WT5", () => {
    ...
    it("validates an integrated observed definition branch", () => { const value = fixture(), mission = value.comparisons.find(item => item.key === "mission_relation"); mission.status = "unknown"; mission.matchedCount = 1; value.rewards.definitions = (0, wt5_shadow_parity_1.rewardDefinitionBoundary)(SYNTHETIC_MISSION, true, SYNTHETIC_BOX, false); value.totals.coverage_gap--; value.totals.unknown++; assert.equal((0, wt5_shadow_parity_1.validateWt5)(value).valid, true); });
    it("rejects an inter-Budokai structural ID collision", () => { assert.equal((0, wt5_shadow_parity_1.parentedStructuralMatches)([{ missionId: SYNTHETIC_MISSION, budokaiId: SYNTHETIC_EVENT - 1 }], "missionId", [SYNTHETIC_MISSION], SYNTHETIC_EVENT), 0); assert.equal((0, wt5_shadow_parity_1.parentedStructuralMatches)([{ missionId: SYNTHETIC_MISSION, budokaiId: SYNTHETIC_EVENT }], "missionId", [SYNTHETIC_MISSION], SYNTHETIC_EVENT), 1); });
