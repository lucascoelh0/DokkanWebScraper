...
async function downloadSupportMemoryAssets(entries) {
    await mapWithConcurrency(entries, SUPPORT_MEMORY_DOWNLOAD_CONCURRENCY, async (entry) => {
        ...
        if (entry.largeAsset) {
            setLocalAssetPath(entry.largeAsset, await downloadAsset(entry.largeAsset.remoteUrl, (0, path_1.resolve)(memoryDir, "large.png"), entry.detailUrl));
        }
        if (entry.completeAsset) {
            setLocalAssetPath(entry.completeAsset, await downloadAsset(entry.completeAsset.remoteUrl, (0, path_1.resolve)(memoryDir, "complete.png"), entry.detailUrl));
        }
        if (entry.requiredFilm) {
            setLocalAssetPath(entry.requiredFilm, await downloadAsset(entry.requiredFilm.remoteUrl, (0, path_1.resolve)(memoryDir, "film.png"), entry.detailUrl));
        }
        if (entry.enhancementItems.length > 0) {
            ...
            for (const item of entry.enhancementItems) {
                setLocalAssetPath(item.asset, await downloadAsset(item.asset.remoteUrl, (0, path_1.resolve)(enhancementDir, `${item.id}.png`), entry.detailUrl));
            }
        }
        if (entry.animation) {
            try {
...
async function downloadAnimationAssets(entry, animation) {
    ...
    setLocalAssetPath(animation.lwf, await downloadAsset(animation.lwf.remoteUrl, lwfAbsolutePath, entry.detailUrl, true));
    ...
    for (const textureFileName of textureFileNames) {
        ...
        try {
            ...
            texture.objectKey = (0, support_memory_dokkaninfo_enrichment_1.supportMemoryAssetObjectKey)(localPath);
...
function setLocalAssetPath(asset, localPath) {
    asset.localPath = localPath;
    asset.objectKey = (0, support_memory_dokkaninfo_enrichment_1.supportMemoryAssetObjectKey)(localPath);
}
