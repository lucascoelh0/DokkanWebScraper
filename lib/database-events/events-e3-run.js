...
async function runEventsE3(options = {}) {
    ...
    const baseline = JSON.parse(await (0, promises_1.readFile)(options.baselinePath ? (0, path_1.resolve)(options.baselinePath) : sourcePath("events-e0-baseline.json"), "utf8")), e3Baseline = JSON.parse(await (0, promises_1.readFile)(options.e3BaselinePath ? (0, path_1.resolve)(options.e3BaselinePath) : sourcePath("events-e3-baseline.json"), "utf8")), e2Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e2Dir, "events-e2-manifest.json", "events-e2-manifest.json"), "utf8"));
    const [e2Path, coveragePath, validationPath] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(e2Dir, e2Manifest.fileName, "events-e2-topology.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e2Dir, e2Manifest.coverage?.fileName, "events-e2-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e2Dir, e2Manifest.validation?.fileName, "events-e2-validation.json")]);
    const [e2Bytes, coverageBytes, validationBytes] = await Promise.all([(0, promises_1.readFile)(e2Path), (0, promises_1.readFile)(coveragePath), (0, promises_1.readFile)(validationPath)]);
    memory();
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, manifest.goldens.fileName, "events-e3-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get(manifest.goldens.fileName), goldensText), (0, promises_1.writeFile)(outputs.get("events-e3-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
