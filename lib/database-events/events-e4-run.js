...
async function runEventsE4(options = {}) {
    ...
    const baseline = JSON.parse(await (0, promises_1.readFile)(sourcePath("events-e0-baseline.json"), "utf8")), e3Baseline = JSON.parse(await (0, promises_1.readFile)(sourcePath("events-e3-baseline.json"), "utf8")), nativeEvidenceBytes = await (0, promises_1.readFile)(sourcePath("events-e4-native-enemy-efficacy-map.json")), nativeEvidence = JSON.parse(nativeEvidenceBytes.toString("utf8")), nativeEvidenceSha256 = sha(`${JSON.stringify(nativeEvidence, null, 2)}\n`);
