...
async function runEventsE4(options = {}) {
    ...
    const e3Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e3Dir, "events-e3-manifest.json", "events-e3-manifest.json"), "utf8")), e2Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e2Dir, "events-e2-manifest.json", "events-e2-manifest.json"), "utf8"));
    const [e3Path, e3CoveragePath, e3ValidationPath, e3GoldensPath, e2Path] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(e3Dir, e3Manifest.fileName, "events-e3-encounters.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e3Dir, e3Manifest.coverage?.fileName, "events-e3-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e3Dir, e3Manifest.validation?.fileName, "events-e3-validation.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e3Dir, e3Manifest.goldens?.fileName, "events-e3-goldens.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e2Dir, e2Manifest.fileName, "events-e2-topology.json")]);
    const [e3Bytes, e3CoverageBytes, e3ValidationBytes, e3GoldensBytes, e2Bytes] = await Promise.all([(0, promises_1.readFile)(e3Path), (0, promises_1.readFile)(e3CoveragePath), (0, promises_1.readFile)(e3ValidationPath), (0, promises_1.readFile)(e3GoldensPath), (0, promises_1.readFile)(e2Path)]);
    memory();
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e4-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get("events-e4-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
