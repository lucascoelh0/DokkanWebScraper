...
exports.validateEventsE5Dataset = void 0;
const events_e5_builder_1 = require("./events-e5-builder");
const str = (value) => String(value);
function validateEventsE5Dataset(dataset, observation, e2, e1, expected) {
    const failures = [];
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-events-database-first-rewards" || dataset.contractVersion !== "0.6.0" || dataset.generatedAtPolicy !== "pinned_to_source_snapshot_for_reproducible_bytes")
        failures.push("contract identity");
    const rebuilt = (0, events_e5_builder_1.buildEventsE5Dataset)({ observation, e2, e1, generatedAt: dataset.generatedAt, sourceSnapshotVersion: dataset.sourceSnapshotVersion, sourceDatabaseSha256: dataset.sourceDatabaseSha256, sourceE4Sha256: dataset.sourceE4.sha256, sourceE2Sha256: dataset.sourceE2.sha256, sourceE1Sha256: dataset.sourceE1.sha256 });
    const exactProjection = JSON.stringify(dataset) === JSON.stringify(rebuilt);
    if (!exactProjection)
        failures.push("exact source projection");
    if (expected && (dataset.generatedAt !== expected.generatedAt || dataset.sourceSnapshotVersion !== expected.sourceSnapshotVersion || dataset.sourceDatabaseSha256 !== expected.sourceDatabaseSha256 || dataset.sourceE4.sha256 !== expected.sourceE4Sha256 || dataset.sourceE2.sha256 !== expected.sourceE2Sha256 || dataset.sourceE1.sha256 !== expected.sourceE1Sha256))
    ...
    const coverage = (0, events_e5_builder_1.buildEventsE5Coverage)(dataset, e2, e1, observation);
    ...
    projectedRewardIds.sort();
    sourceRewardIds.sort();
    if (JSON.stringify(projectedRewardIds) !== JSON.stringify(sourceRewardIds))
        failures.push("lossless reward rows");
    if (new Set(projectedRewardIds).size !== projectedRewardIds.length)
        failures.push("duplicate reward source identity across families");
    ...
    if (previewProjectedCount !== previewSourceCount)
        failures.push("lossless preview items");
    for (const value of dataset.questDropPreviews)
        if (new Set(value.items.map(slot => slot.slot)).size !== value.items.length || value.items.some(slot => slot.slot < 1 || slot.slot > 6))
            failures.push(`quest preview slots ${value.identity.id}`);
    for (const value of dataset.missionCategoryPreviews)
        if (new Set(value.items.map(slot => slot.slot)).size !== value.items.length || value.items.some(slot => slot.slot < 1 || slot.slot > 4))
            failures.push(`mission preview slots ${value.sourceRowId}`);
    ...
    for (const value of (0, events_e5_builder_1.countEventsE5ItemReferences)(dataset))
        if (value.catalogJoin.status === "supported" && value.catalogJoin.targetId !== value.itemId)
            failures.push(`item target identity ${value.rawType}/${value.itemId}`);
    for (const value of dataset.questBossDrops)
        if (value.reward.quantity !== null || value.reward.chance.rawValue !== null || value.repeatability.status !== "unknown")
            failures.push(`boss drop inference ${value.identity.id}`);
    for (const value of [...dataset.questDropPreviews, ...dataset.missionCategoryPreviews])
        if (value.status !== "partial")
            failures.push("preview semantic promotion");
    if (dataset.zBattleFirstRewardAnchors.some(value => value.repeatability.status !== "partial") || dataset.zBattleNormalRewardGroups.some(value => value.repeatability.status !== "unknown") || dataset.linkedEventMissions.some(value => value.repeatability.status !== "unknown"))
        failures.push("repeatability semantic promotion");
    if (dataset.unboundRewardSurfaces.some(value => value.status !== "unknown"))
        failures.push("unbound reward semantic promotion");
    ...
    if (dataset.costAndRequirementReferences.questLevels.length !== e2QuestCosts || dataset.costAndRequirementReferences.zBattleCheckpoints.length !== e2ZCosts || dataset.costAndRequirementReferences.originBattles.length !== e2.originBattles.length || dataset.costAndRequirementReferences.areaRequirementSourceRowIds.length !== e2.areaRequirements.length)
        failures.push("E2 cost and requirement accounting");
...
exports.validateEventsE5Dataset = validateEventsE5Dataset;
//# sourceMappingURL=events-e5-validator.js.map