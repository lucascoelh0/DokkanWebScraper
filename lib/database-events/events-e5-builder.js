...
exports.countEventsE5ItemReferences = exports.buildEventsE5Coverage = exports.buildEventsE5Dataset = void 0;
const str = (value) => String(value);
const nullableId = (value) => value === null ? null : str(value);
const nullableString = (value) => value === null ? null : str(value);
const num = (value) => Number(value);
const group = (rows, column) => { const result = new Map(); for (const row of rows) {
    const key = str(row[column]);
    result.set(key, [...(result.get(key) ?? []), row]);
} return result; };
function itemResolver(observation) {
    ...
    return (rawTypeValue, itemIdValue) => {
...
function rewardValue(row, resolveItem) {
...
function previewItems(row, maximum, resolveItem) {
    const result = [];
    for (let index = 1; index <= maximum; index += 1)
        if (row[`item${index}_type`] !== null)
            result.push({ slot: index, item: resolveItem(row[`item${index}_type`], row[`item${index}_id`]) });
    return result;
}
...
function buildEventsE5Dataset(options) {
    ...
    const mapReward = (row) => ({ sourceRowId: str(row.id), value: rewardValue(row, resolveItem) });
    return {
...
exports.buildEventsE5Dataset = buildEventsE5Dataset;
function allItemReferences(dataset) {
    return [
...
function buildEventsE5Coverage(dataset, e2, e1, observation) {
    ...
    for (const value of dataset.questBossDrops)
        danglingIdCount += Number(!mapIds.has(value.mapId)) + Number(!questIds.has(value.questId)) + Number(mapIds.has(value.mapId) && questIds.has(value.questId) && !mapQuestPairs.has(`${value.mapId}:${value.questId}`));
    for (const value of dataset.questDropPreviews)
        danglingIdCount += Number(!questIds.has(value.questId));
    for (const value of dataset.zBattleFirstRewardAnchors)
        danglingIdCount += Number(!zIds.has(value.stageId));
    for (const value of dataset.zBattleNormalRewardGroups)
        danglingIdCount += Number(!zIds.has(value.stageId));
    for (const value of dataset.linkedEventMissions) {
        const targets = value.targets;
        danglingIdCount += Number(targets.areaId !== null && !areaIds.has(targets.areaId)) + Number(targets.zBattleStageId !== null && !zIds.has(targets.zBattleStageId)) + Number(targets.originEpisodeId !== null && !originEpisodeIds.has(targets.originEpisodeId)) + Number(targets.originBattleId !== null && !originBattleIds.has(targets.originBattleId));
    }
    for (const value of dataset.budokai.missions)
        danglingIdCount += Number(value.rootStatus === "supported" && !budokaiIds.has(value.budokaiId));
    for (const value of dataset.budokai.rankingGiftSets)
        danglingIdCount += Number(value.rootStatus === "supported" && !budokaiIds.has(value.budokaiId));
    for (const value of dataset.budokai.boxRankings)
        danglingIdCount += Number(!budokaiIds.has(value.budokaiId));
    for (const value of dataset.rmbattleMissions)
        danglingIdCount += Number(!rmbattleIds.has(value.rmbattleId));
    for (const value of observation.itemCatalogTargets)
        if (value.targetKind !== null) {
            const referenced = new Set(value.referencedIds.map(str)), joined = new Set(value.joinedIds.map(str));
            danglingIdCount += [...referenced].filter(id => !joined.has(id)).length + [...joined].filter(id => !referenced.has(id)).length;
        }
...
exports.buildEventsE5Coverage = buildEventsE5Coverage;
exports.countEventsE5ItemReferences = allItemReferences;
//# sourceMappingURL=events-e5-builder.js.map