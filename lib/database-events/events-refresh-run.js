...
async function runEventsRefresh(options = {}) {
    ...
    for (const baseline of profile.baselineFiles) {
        const baselinePath = await (0, events_artifact_path_1.resolveEventsInputFile)(profileDirectory, baseline.fileName, baselineNames[baseline.role]), bytes = await (0, promises_1.readFile)(baselinePath);
