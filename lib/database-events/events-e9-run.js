...
async function runEventsE9(options = {}) {
    ...
    const inputDir = (0, path_1.resolve)(options.inputDir ?? DEFAULT_DIRECTORY), outputDir = (0, path_1.resolve)(options.outputDir ?? inputDir), e7Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, "events-e7-manifest.json", "events-e7-manifest.json"), "utf8")), e8ManifestBytes = await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, "events-e8-manifest.json", "events-e8-manifest.json")), e8Manifest = JSON.parse(e8ManifestBytes.toString("utf8")), refreshEvidenceBytes = await (0, promises_1.readFile)(sourcePath("events-e9-refresh-evidence.json")), refreshEvidenceSha256 = sha(refreshEvidenceBytes), refreshEvidence = JSON.parse(refreshEvidenceBytes.toString("utf8"));
    const [e7Path, e7CoveragePath, e7ValidationPath, e8Path, e8CoveragePath, e8ValidationPath, e8ReceiptPath] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e7Manifest.fileName, "events-e7-shadow-parity.json"), (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e7Manifest.coverage?.fileName, "events-e7-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e7Manifest.validation?.fileName, "events-e7-validation.json"), (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e8Manifest.fileName, "events-sidecars.json"), (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e8Manifest.coverage?.fileName, "events-e8-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e8Manifest.validation?.fileName, "events-e8-validation.json"), (0, events_artifact_path_1.resolveEventsInputFile)(inputDir, e8Manifest.refreshReceipt?.fileName, "events-e8-refresh-receipt.json")]);
    const [e7Before, e7CoverageBytes, e7ValidationBytes, e8Before, e8CoverageBytes, e8ValidationBytes, e8ReceiptBytes] = await Promise.all([fingerprint(e7Path), (0, promises_1.readFile)(e7CoveragePath), (0, promises_1.readFile)(e7ValidationPath), fingerprint(e8Path), (0, promises_1.readFile)(e8CoveragePath), (0, promises_1.readFile)(e8ValidationPath), (0, promises_1.readFile)(e8ReceiptPath)]);
    memory();
    ...
    const e7 = JSON.parse(await (0, promises_1.readFile)(e7Path, "utf8")), e7Coverage = JSON.parse(e7CoverageBytes.toString("utf8")), e7Validation = JSON.parse(e7ValidationBytes.toString("utf8")), e8 = JSON.parse(await (0, promises_1.readFile)(e8Path, "utf8")), e8Coverage = JSON.parse(e8CoverageBytes.toString("utf8")), e8Validation = JSON.parse(e8ValidationBytes.toString("utf8"));
    ...
    const e7After = await fingerprint(e7Path), e8After = await fingerprint(e8Path);
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e9-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get("events-e9-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
