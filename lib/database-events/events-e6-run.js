...
async function runEventsE6(options = {}) {
    ...
    const baseline = JSON.parse(await (0, promises_1.readFile)(options.baselinePath ? (0, path_1.resolve)(options.baselinePath) : sourcePath("events-e0-baseline.json"), "utf8")), apkBaselineBytes = await (0, promises_1.readFile)(options.apkBaselinePath ? (0, path_1.resolve)(options.apkBaselinePath) : sourcePath("events-e6-apk-baseline.json")), apkBaseline = JSON.parse(apkBaselineBytes.toString("utf8")), apkBaselineSha256 = sha(apkBaselineBytes);
    const e5Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e5Dir, "events-e5-manifest.json", "events-e5-manifest.json"), "utf8"));
    const [e5Path, e5CoveragePath, e5ValidationPath] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(e5Dir, e5Manifest.fileName, "events-e5-rewards.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e5Dir, e5Manifest.coverage?.fileName, "events-e5-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e5Dir, e5Manifest.validation?.fileName, "events-e5-validation.json")]);
    const [e5Fingerprint, e5CoverageBytes, e5ValidationBytes] = await Promise.all([fingerprint(e5Path), (0, promises_1.readFile)(e5CoveragePath), (0, promises_1.readFile)(e5ValidationPath)]);
    memory();
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e6-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get("events-e6-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
