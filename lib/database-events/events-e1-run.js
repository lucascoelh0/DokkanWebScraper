...
async function runEventsE1(options = {}) {
    ...
    const baseline = JSON.parse(await (0, promises_1.readFile)(options.baselinePath ? (0, path_1.resolve)(options.baselinePath) : sourcePath("events-e0-baseline.json"), "utf8")), e0Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e0Dir, "events-e0-manifest.json", "events-e0-manifest.json"), "utf8"));
    const [e0Path, e0CoveragePath, e0ValidationPath] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(e0Dir, e0Manifest.fileName, "events-e0-inventory.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e0Dir, e0Manifest.coverage?.fileName, "events-e0-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e0Dir, e0Manifest.validation?.fileName, "events-e0-validation.json")]);
    const [e0Bytes, e0CoverageBytes, e0ValidationBytes] = await Promise.all([(0, promises_1.readFile)(e0Path), (0, promises_1.readFile)(e0CoveragePath), (0, promises_1.readFile)(e0ValidationPath)]);
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e1-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get("events-e1-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
