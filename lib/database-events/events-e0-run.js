...
async function runEventsE0(options = {}) {
    ...
    const baselinePath = options.baselinePath ?? (0, path_1.resolve)(__dirname, "events-e0-baseline.json");
    const sourceBaselinePath = (0, path_1.resolve)(__dirname, "..", "..", "database-events", "events-e0-baseline.json");
    const baseline = JSON.parse(await (0, promises_1.readFile)(require("fs").existsSync(baselinePath) ? baselinePath : sourceBaselinePath, "utf8"));
    ...
    await Promise.all([
        (0, promises_1.writeFile)((0, path_1.resolve)(outputDir, manifest.fileName), inventoryText),
        ...
        (0, promises_1.writeFile)((0, path_1.resolve)(outputDir, "events-e0-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`),
    ]);
    observeMemory();
