...
async function runEventsE2(options = {}) {
    ...
    const baseline = JSON.parse(await (0, promises_1.readFile)(options.baselinePath ? (0, path_1.resolve)(options.baselinePath) : sourcePath("events-e0-baseline.json"), "utf8")), e1Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e1Dir, "events-e1-manifest.json", "events-e1-manifest.json"), "utf8"));
    const [e1Path, coveragePath, validationPath] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(e1Dir, e1Manifest.fileName, "events-e1-catalog.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e1Dir, e1Manifest.coverage?.fileName, "events-e1-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e1Dir, e1Manifest.validation?.fileName, "events-e1-validation.json")]);
    const [e1Bytes, coverageBytes, validationBytes] = await Promise.all([(0, promises_1.readFile)(e1Path), (0, promises_1.readFile)(coveragePath), (0, promises_1.readFile)(validationPath)]), e1 = JSON.parse(e1Bytes.toString("utf8")), e1Coverage = JSON.parse(coverageBytes.toString("utf8")), e1Validation = JSON.parse(validationBytes.toString("utf8"));
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e2-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get("events-e2-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
