...
async function runEventsE7(options = {}) {
    ...
    const e6Manifest = JSON.parse(await (0, promises_1.readFile)(await (0, events_artifact_path_1.resolveEventsInputFile)(e6Dir, "events-e6-manifest.json", "events-e6-manifest.json"), "utf8"));
    const [e6Path, e6CoveragePath, e6ValidationPath] = await Promise.all([(0, events_artifact_path_1.resolveEventsInputFile)(e6Dir, e6Manifest.fileName, "events-e6-assets.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e6Dir, e6Manifest.coverage?.fileName, "events-e6-coverage.json"), (0, events_artifact_path_1.resolveEventsInputFile)(e6Dir, e6Manifest.validation?.fileName, "events-e6-validation.json")]);
    const [e6Fingerprint, e6CoverageBytes, e6ValidationBytes] = await Promise.all([fingerprint(e6Path), (0, promises_1.readFile)(e6CoveragePath), (0, promises_1.readFile)(e6ValidationPath)]);
    memory();
    ...
    const outputs = await (0, events_artifact_path_1.resolveEventsOutputFiles)(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e7-manifest.json"]);
    await Promise.all([(0, promises_1.writeFile)(outputs.get(manifest.fileName), payloadText), (0, promises_1.writeFile)(outputs.get(manifest.coverage.fileName), coverageText), (0, promises_1.writeFile)(outputs.get(manifest.validation.fileName), validationText), (0, promises_1.writeFile)(outputs.get("events-e7-manifest.json"), `${JSON.stringify(manifest, null, 2)}\n`)]);
    memory();
