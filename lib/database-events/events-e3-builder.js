...
exports.buildEventsE3Goldens = exports.buildEventsE3Coverage = exports.buildEventsE3Dataset = void 0;
const str = (value) => String(value);
const num = (row, key) => Number(row[key]);
const nullableId = (value) => value === null ? null : str(value);
function encounter(value, family) {
    return {
...
function buildEventsE3Dataset(options) {
    ...
    return {
        ...
        zBattleEnemyRanges: o.zBattleEnemies.map(row => {
            const type = (column) => str(row[column]);
            return {
...
exports.buildEventsE3Dataset = buildEventsE3Dataset;
const encounterCounts = (values) => ({ battles: values.reduce((n, value) => n + value.battles.length, 0), rounds: values.reduce((n, value) => n + value.battles.reduce((m, battle) => m + battle.rounds.length, 0), 0), enemies: values.reduce((n, value) => n + value.battles.reduce((m, battle) => m + battle.rounds.reduce((p, round) => p + round.enemies.length, 0), 0), 0) });
function buildEventsE3Coverage(dataset, e2) {
    ...
    for (const value of [...dataset.questEncounters, ...dataset.originEncounters])
        for (const battle of value.battles)
            for (const round of battle.rounds)
                for (const enemy of round.enemies)
                    danglingIdCount += Number(!cards.has(enemy.cardId)) + enemy.enemySkillIds.filter(id => !skills.has(id)).length + Number(enemy.enemyRoundSkillSetId !== null && !roundSets.has(enemy.enemyRoundSkillSetId));
    ...
    for (const value of dataset.enemyRoundSkillSets)
        danglingIdCount += value.enemyRoundSkillIds.filter(id => !roundSkills.has(id)).length;
    for (const value of dataset.zBattleEnemyRanges)
        danglingIdCount += value.cardEscalations.filter(row => !cards.has(str(row.card_id))).length + value.skillEscalations.filter(row => !skills.has(str(row.enemy_skill_id))).length;
...
exports.buildEventsE3Coverage = buildEventsE3Coverage;
function buildEventsE3Goldens(dataset) {
    const signature = (value) => (0, crypto_1.createHash)("sha256").update(JSON.stringify(value)).digest("hex");
    const representatives = [];
    const addEncounter = (kind, value) => { if (value)
        representatives.push({ kind, sourceId: value.identity.sourceId, structuralSha256: signature(value.battles) }); };
    ...
    const z = dataset.zBattleEnemyRanges.find(value => value.cardEscalations.length > 0 && value.skillEscalations.length > 0);
    if (z)
        representatives.push({ kind: "z_range_with_card_and_skill_curves", sourceId: z.identity.id, structuralSha256: signature({ stageId: z.stageId, levels: z.levels, rawEscalationTypeIds: z.rawEscalationTypeIds, cardEscalations: z.cardEscalations, skillEscalations: z.skillEscalations }) });
    const sd = dataset.sdEncounterBoundary[0];
    if (sd)
        representatives.push({ kind: "sd_opaque_enemy_reference", sourceId: sd.stageId, structuralSha256: signature(sd) });
    return { schemaVersion: 1, representatives };
}
exports.buildEventsE3Goldens = buildEventsE3Goldens;
const crypto_1 = require("crypto");
//# sourceMappingURL=events-e3-builder.js.map