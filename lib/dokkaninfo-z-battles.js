...
exports.mapDokkanInfoZBattleStats = exports.mapDokkanInfoZBattleEvent = exports.mapDokkanInfoZBattleIndex = exports.buildDokkanInfoZBattleDataset = exports.writeDokkanInfoZBattles = exports.getDokkanInfoZBattles = void 0;
...
async function getDokkanInfoZBattles() {
    const allSummaries = await fetchZBattleSummaries();
    const summaries = selectRequestedSummaries(allSummaries);
    const failedEventIds = [];
    const failedStatsIds = [];
    let completedEvents = 0;
    const shells = (await mapWithConcurrency(summaries, requestedConcurrency(), async (summary) => {
        try {
    ...
    const events = await mapWithConcurrency(shells, requestedConcurrency(), async (shell) => {
        ...
        let statsDataStatus = "not-provided";
        let levels = [];
        try {
...
exports.getDokkanInfoZBattles = getDokkanInfoZBattles;
async function writeDokkanInfoZBattles(dataset) {
    ...
    await (0, format_json_1.writeFormattedJson)(outputPath, dataset ?? await getDokkanInfoZBattles());
    return outputPath;
}
exports.writeDokkanInfoZBattles = writeDokkanInfoZBattles;
function buildDokkanInfoZBattleDataset(events, failedEventIds = [], failedStatsIds = []) {
...
exports.buildDokkanInfoZBattleDataset = buildDokkanInfoZBattleDataset;
function mapDokkanInfoZBattleIndex(document) {
    assertUsableDokkanInfoPage(document);
    const payload = parseJsonAttribute(document.querySelector("events")?.getAttribute("v-bind:eventjson")) ?? [];
...
exports.mapDokkanInfoZBattleIndex = mapDokkanInfoZBattleIndex;
function mapDokkanInfoZBattleEvent(document, summary) {
    assertUsableDokkanInfoPage(document);
    const displayName = cleanText(document.title).replace(/\s*\|\s*Dokkan Info!?$/i, "") || `Z-Battle ${summary.id}`;
    const statsPath = absoluteOptionalUrl(Array.from(document.querySelectorAll("a[href]"))
        .find(anchor => cleanText(anchor.textContent) === "Stats")
        ?.getAttribute("href")) ?? `${summary.sourcePath}/stats`;
...
exports.mapDokkanInfoZBattleEvent = mapDokkanInfoZBattleEvent;
function mapDokkanInfoZBattleStats(document) {
    assertUsableDokkanInfoPage(document);
    const tableHeader = Array.from(document.querySelectorAll(".row"))
...
exports.mapDokkanInfoZBattleStats = mapDokkanInfoZBattleStats;
function mapZBattleSummary(raw) {
...
function mapWeaknesses(document) {
    return Array.from(document.querySelectorAll('a[href*="/categories/"]'))
        ...
        return name ? withoutUndefined({ kind: "category", id, name, startsAtLevel }) : undefined;
    })
...
function mapSkillIcons(element) {
    if (!element) {
        return [];
    }
    return Array.from(element.querySelectorAll('img[src*="/condition/"]'))
        .map(image => {
        const path = absoluteUrl(image.getAttribute("src"));
        return withoutUndefined({ id: path.match(/\/st_(\d+)\.png$/)?.[1], path });
    });
}
function mapRewardColumn(element) {
    if (!element) {
        return [];
    }
    const componentRewards = Array.from(element.querySelectorAll("reward[v-bind\\:reward]"))
        .map(reward => parseJsonAttribute(reward.getAttribute("v-bind:reward")))
        .filter((reward) => Boolean(reward))
        .map((reward) => withoutUndefined({
...
function mapCardElement(element) {
    const raw = parseJsonAttribute(element?.getAttribute("v-bind:card"));
    const id = toOptionalString(raw?.id);
    if (!id) {
        return undefined;
    }
...
async function fetchZBattleSummaries() {
    const cached = await readCache("index.json");
    if (cached) {
        return cached;
    }
    const document = await (0, scraper_1.fetchFromWeb)(INDEX_URL);
    const summaries = mapDokkanInfoZBattleIndex(document);
    await writeCache("index.json", summaries);
    return summaries;
}
async function fetchZBattleEvent(summary) {
    const cached = await readCache(`events/${summary.id}.json`);
    if (cached) {
        return cached;
    }
    await delay(requestedDelayMs());
...
async function fetchZBattleStats(id, statsPath) {
    const cached = await readCache(`stats/${id}.json`);
    if (cached) {
        return cached;
    }
    const document = await (0, scraper_1.fetchFromWeb)(statsPath);
    const levels = mapDokkanInfoZBattleStats(document);
    await writeCache(`stats/${id}.json`, levels);
    return levels;
}
function selectRequestedSummaries(summaries) {
...
async function readCache(relativePath) {
    ...
    try {
        const raw = await (0, promises_1.readFile)((0, path_1.resolve)(__dirname, CACHE_DIR, relativePath), "utf8");
...
function requestedCacheTtlHours() {
...
//# sourceMappingURL=dokkaninfo-z-battles.js.map