...
exports.COMBAT_RULES_SCHEMA_VERSION = 1;
exports.COMBAT_RULES_VERSION = "1.0.0";
exports.COMBAT_RULES_GENERATED_AT = "2026-08-04T00:00:00.000Z";
exports.COMBAT_RULES_EVIDENCE_POLICY_VERSION = "1";
...
const RULE_UNITS = ["coefficient", "percent", "probability", "probability_per_level", "turns", "damage", "ordered_stages", "formula", "mapping", "reference", "none"];
const EVIDENCE_SOURCES = ["first_party_export", "first_party_structural_join", "reproducible_fixture", "community_guide", "community_workbook", "versioned_contract", "unresolved"];
...
function validateRuleValue(raw, path, issue) {
    ...
    if (raw.kind === "scalar")
        ...
        if (typeof raw.amountPerLevel === "number" && (raw.amountPerLevel < -1 || raw.amountPerLevel > 1))
            issue("percentage-range", `${path}.amountPerLevel`, "Per-level coefficient must be within -1..1.");
    }
...
function hasSufficientNormativeEvidence(rule) {
    ...
    if (numericKinds.includes(String(value.kind)) && !direct(["first_party_export", "reproducible_fixture"], "value"))
        return false;
    if (value.kind === "pipeline" && !direct(["first_party_export", "reproducible_fixture"], "order"))
        return false;
    if (value.kind === "reference" && !structuralContract())
        return false;
    if (value.kind === "structured") {
        if (!isRecord(value.fields) || Object.values(value.fields).some(field => typeof field === "number"))
            return false;
        if (!structuralContract())
            return false;
    }
    const order = isRecord(rule.applicationOrder) ? rule.applicationOrder : undefined;
    if (order?.status === "verified" && !direct(["first_party_export", "reproducible_fixture"], "order"))
        return false;
    const rounding = isRecord(rule.rounding) ? rule.rounding : undefined;
    if (rounding?.status === "verified" && !direct(["first_party_export", "reproducible_fixture"], "rounding"))
        return false;
    return true;
}
