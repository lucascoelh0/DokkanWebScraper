...
exports.validateTeamAnalysisDeliveryBuffers = exports.TEAM_ANALYSIS_LOCAL_FILE_NAME = void 0;
...
exports.TEAM_ANALYSIS_LOCAL_FILE_NAME = "team-analysis.json.gz";
function validateTeamAnalysisDeliveryBuffers(buffers, options = {}) {
    const manifest = parseJsonObject(buffers.manifestBuffer, "Team Analysis manifest");
    const characterManifest = parseJsonObject(buffers.characterManifestBuffer, "character manifest");
    ...
    const characters = parseJsonArray(charactersText, "character payload");
    if (characters.length !== characterManifest.characterCount) {
        throw new Error(`Character payload count ${characters.length} does not match manifest characterCount ${characterManifest.characterCount}.`);
    }
    validateTeamAnalysisManifest(manifest);
    const datasetText = gunzipUtf8(buffers.datasetBuffer, "Team Analysis payload");
    const dataset = parseJsonObject(datasetText, "Team Analysis payload");
    ...
    if (options.contract === "android-v1") {
        ...
        (0, android_v1_contract_projector_1.assertTeamAnalysisProjectedForAndroidV1)(dataset);
    }
    else {
        (0, team_analysis_1.assertValidTeamAnalysisDatasetForDelivery)(dataset, characters);
    }
    if (dataset.sourceCharacterDatasetVersion !== characterManifest.datasetVersion) {
        throw new Error(`Team Analysis source character version ${dataset.sourceCharacterDatasetVersion} does not match local character version ${characterManifest.datasetVersion}.`);
    }
    if (dataset.sourceCharacterPayloadSha256 !== characterManifest.sha256) {
        throw new Error(`Team Analysis source character SHA-256 ${dataset.sourceCharacterPayloadSha256} does not match local character SHA-256 ${characterManifest.sha256}.`);
    }
    return { dataset, manifest, characters, characterManifest };
}
exports.validateTeamAnalysisDeliveryBuffers = validateTeamAnalysisDeliveryBuffers;
function validateCharacterManifest(manifest, gzipBuffer) {
    ...
    if (manifest.sizeBytes !== gzipBuffer.byteLength) {
        throw new Error(`Character payload size ${gzipBuffer.byteLength} does not match manifest sizeBytes ${manifest.sizeBytes}.`);
    }
    if ((0, team_analysis_artifacts_1.sha256)(gzipBuffer) !== manifest.sha256.toLowerCase()) {
    ...
    if (manifest.uncompressedSizeBytes !== uncompressed.byteLength) {
        throw new Error(`Character payload uncompressed size ${uncompressed.byteLength} does not match manifest uncompressedSizeBytes ${manifest.uncompressedSizeBytes}.`);
    }
}
function assertExpectedCharacterManifestFileName(manifest) {
...
function validateTeamAnalysisManifest(manifest) {
    if (manifest.schemaVersion !== team_analysis_1.TEAM_ANALYSIS_SCHEMA_VERSION || manifest.compression !== "gzip") {
        throw new Error("Unsupported Team Analysis manifest schema or compression.");
    }
    if (manifest.rulesVersion !== team_analysis_1.TEAM_ANALYSIS_RULES_VERSION) {
        throw new Error(`Unsupported Team Analysis rulesVersion: ${manifest.rulesVersion}`);
    }
    if (manifest.parserVersion !== team_analysis_1.TEAM_ANALYSIS_PARSER_VERSION) {
    ...
    if (manifest.fileName !== exports.TEAM_ANALYSIS_LOCAL_FILE_NAME) {
...
function validateTeamAnalysisArtifact(manifest, dataset, gzipBuffer, jsonBuffer) {
    if (!Array.isArray(dataset.states)) {
        throw new Error("Team Analysis payload states must be an array.");
    }
    if (dataset.schemaVersion !== team_analysis_1.TEAM_ANALYSIS_SCHEMA_VERSION) {
        throw new Error(`Unsupported Team Analysis schemaVersion: ${dataset.schemaVersion}`);
    }
    if (dataset.rulesVersion !== team_analysis_1.TEAM_ANALYSIS_RULES_VERSION) {
        throw new Error(`Unsupported Team Analysis rulesVersion: ${dataset.rulesVersion}`);
    }
    if (dataset.parserVersion !== team_analysis_1.TEAM_ANALYSIS_PARSER_VERSION) {
    ...
    if (dataset.stateCount !== dataset.states.length || manifest.stateCount !== dataset.states.length) {
        throw new Error(`Team Analysis stateCount mismatch: manifest=${manifest.stateCount}, payload=${dataset.stateCount}, states=${dataset.states.length}.`);
    }
    if (manifest.sha256.toLowerCase() !== (0, team_analysis_artifacts_1.sha256)(gzipBuffer)) {
        throw new Error("Team Analysis payload SHA-256 does not match its manifest.");
    }
    if (manifest.sizeBytes !== gzipBuffer.byteLength) {
        throw new Error(`Team Analysis payload size ${gzipBuffer.byteLength} does not match manifest sizeBytes ${manifest.sizeBytes}.`);
    }
    if (manifest.uncompressedSizeBytes !== jsonBuffer.byteLength) {
        throw new Error(`Team Analysis payload uncompressed size ${jsonBuffer.byteLength} does not match manifest uncompressedSizeBytes ${manifest.uncompressedSizeBytes}.`);
    }
    ...
    for (const [field, manifestValue, datasetValue] of matchingFields) {
        if (manifestValue !== datasetValue) {
            throw new Error(`Team Analysis manifest ${field} ${String(manifestValue)} does not match payload value ${String(datasetValue)}.`);
        }
    }
}
function validateUniqueDeliveryIdentifiers(dataset) {
...
function visitIdentifiers(value, identifiers) {
...
function gunzipUtf8(value, label) {
    try {
        return (0, zlib_1.gunzipSync)(value).toString("utf8");
    }
    catch (error) {
        throw new Error(`${label} is not valid gzip: ${error instanceof Error ? error.message : String(error)}`);
    }
}
function assertSafeLocalFileName(value, label) {
...
function assertSafeRelativeObjectKey(value, label) {
    if (typeof value !== "string" || value.length === 0) {
        throw new Error(`${label} must be a non-empty string.`);
    }
...
//# sourceMappingURL=team-analysis-delivery.js.map