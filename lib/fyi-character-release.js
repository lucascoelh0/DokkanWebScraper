...
async function readValidatedFyiCharacterReleasePortrait(validated, fileName) {
    if (!/^portrait_\d+\.png$/.test(fileName))
        throw new Error("K28 portrait file name rejected");
    const entry = validated.release.portraits.entries.find(portrait => portrait.fileName === fileName);
    if (!entry || entry.objectKey !== `images/v2/${fileName}`)
        throw new Error("K28 portrait inventory entry rejected");
...
exports.readValidatedFyiCharacterReleasePortrait = readValidatedFyiCharacterReleasePortrait;
