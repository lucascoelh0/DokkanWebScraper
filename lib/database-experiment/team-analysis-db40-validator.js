...
exports.validateDatabaseTeamAnalysisDb40Dataset = void 0;
const TYPES = [3, 4];
...
function collect(expression) {
    ...
    if (expression.op === "all" || expression.op === "any")
        return [...new Map(expression.children.flatMap(collect).map(value => [`${value.type}|${value.id}`, value])).values()];
    return [];
}
function expectedPredicate(type, threshold) {
...
function validateDatabaseTeamAnalysisDb40Dataset(dataset, db11, tables, expected) {
    const failures = [];
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-team-analysis-battle-gauge-threshold-native-semantics-experiment" || dataset.contractVersion !== "0.39.0" || dataset.inheritedSemanticPromotionCount !== 45 || dataset.semanticPromotionCount !== 2)
        failures.push("contract identity");
    if (dataset.generatedAt !== db11.generatedAt || dataset.sourceSnapshotVersion !== db11.sourceSnapshotVersion || dataset.sourceDatabaseSha256 !== db11.sourceSha256)
        failures.push("snapshot lineage");
    if (dataset.sourceDb8.sha256 !== expected.db8Sha256 || dataset.sourceDb9.sha256 !== expected.db9Sha256 || dataset.sourceDb10.sha256 !== expected.db10Sha256 || dataset.sourceDb11.sha256 !== expected.db11Sha256 || JSON.stringify(dataset.nativeRuntime) !== JSON.stringify({ fileName: "libcocos2dcpp.so", sha256: expected.nativeSha256, sizeBytes: expected.nativeSizeBytes }) || JSON.stringify(dataset.nativeEvidence) !== JSON.stringify({ fileName: "native-battle-gauge-threshold-semantics.json", sha256: expected.evidenceSha256 }))
    ...
    for (const state of db11.states)
        for (const rule of state.passive?.rules ?? [])
            for (const found of collect(rule.condition))
                source.set(`${state.stateKey}|${rule.ruleKey}|${found.id}`, { passiveSkillId: rule.source.passiveSkillId, type: found.type });
    const handlers = new Map(expected.evidence.handlers.map(handler => [handler.causalityType, handler])), proofRoles = expected.evidence.codeRegions.map(region => region.role), seen = new Set();
    ...
    for (const resolution of dataset.resolutions) {
        ...
        seen.add(key);
        if (!association || !row || !handler || association.type !== resolution.causalityType || association.passiveSkillId !== resolution.passiveSkillId || integer(row.causality_type) !== resolution.causalityType) {
            failures.push(`association ${key}`);
            continue;
        }
        const threshold = integer(row.cau_val1);
        if (threshold === undefined || integer(row.cau_val2) !== 0 || integer(row.cau_val3) !== 0) {
            failures.push(`row ${key}`);
            continue;
        }
        losslessReconstructionCount++;
        if (JSON.stringify(resolution.raw) !== JSON.stringify({ cauVal1: row.cau_val1, cauVal2: row.cau_val2, cauVal3: row.cau_val3 }) || JSON.stringify(resolution.predicate) !== JSON.stringify(expectedPredicate(resolution.causalityType, threshold)) || resolution.semanticStatus !== "partial" || JSON.stringify(resolution.activation) !== JSON.stringify({ timing: "unknown", recurrence: "unknown", calculationBucket: "unknown", gaugePopulationAndReset: "unknown" }))
...
exports.validateDatabaseTeamAnalysisDb40Dataset = validateDatabaseTeamAnalysisDb40Dataset;
//# sourceMappingURL=team-analysis-db40-validator.js.map