...
function db3Rule(key, skillId, effect = {}) {
    return {
        ruleKey: key,
...
function db2Rule(key, skillId, compiled, causalities, overrides = {}) {
    return {
...
function dataset(db2Rules, db3Rules, tables = { card_categories: [] }) {
    return (0, team_analysis_db4_builder_1.buildDatabaseTeamAnalysisDb4Dataset)({
        db2: { states: [{ stateKey: "1:1:initial", passive: { rules: db2Rules } }] },
        db3: {
...
(0, mocha_1.describe)("database Team Analysis DB4 projector", function () {
    (0, mocha_1.it)("maps recursively nested AND and OR while retaining typed leaves", () => {
        const causalities = [
        ...
        const result = dataset([db2Rule("10:r1:1", "1", ["&", 21, ["|", 9, 141]], causalities)], [db3Rule("10:r1:1", "1")]);
        const condition = result.states[0].passive.rules[0].condition;
        (0, assert_1.equal)(condition.op, "all");
        if (condition.op === "all") {
            (0, assert_1.equal)(condition.children[1].op, "any");
            if (condition.children[1].op === "any")
                (0, assert_1.deepEqual)(condition.children[1].children.map(child => child.op), ["predicate", "predicate"]);
        }
        (0, assert_1.equal)(result.states[0].passive.rules[0].conditionStatus, "supported");
    });
    (0, mocha_1.it)("upgrades direct type 40 efficacy 98 to per-Super scaling", () => {
        ...
        rule.source.efficacyType = 98;
        rule.source.executionTimingType = 5;
        const result = dataset([db2Rule("10:r2:2", "2", 141, causality, { efficacyType: 98, executionTimingType: 5 })], [rule]);
        const upgraded = result.states[0].passive.rules[0];
        (0, assert_1.equal)(upgraded.condition.op, "always");
        (0, assert_1.deepEqual)(upgraded.effects[0].scaling, { kind: "per_combat_event", event: "super_attack_performed", eventsPerIncrement: 1 });
        (0, assert_1.equal)(upgraded.status, "supported");
    });
    (0, mocha_1.it)("aggregates only complete contiguous type 42 threshold siblings", () => {
        const make = (threshold, auxiliary = 0) => {
            const id = 200 + threshold;
            const key = `10:r${id}:${id}`;
            const causality = [{ id, causality_type: 42, cau_val1: 32, cau_val2: threshold, cau_val3: auxiliary }];
            const db3 = db3Rule(key, String(id));
            db3.condition = { op: "predicate", predicate: { kind: "ki_spheres_obtained", scope: "self", comparator: "gte", count: threshold, kiSphereTypes: ["rainbow"], sourceCausalityId: String(id), sourceCausalityType: 42, evidence: "first-party-row-join" } };
            db3.conditionStatus = "supported";
            db3.status = "supported";
        ...
        (0, assert_1.equal)(projected.states[0].passive.thresholdSeries.length, 1);
        (0, assert_1.deepEqual)(projected.states[0].passive.thresholdSeries[0].effect.scaling.thresholdValues, [1, 2, 3]);
        const gap = [make(1), make(3)];
        (0, assert_1.equal)(dataset(gap.map(value => value.db2), gap.map(value => value.db3)).states[0].passive.thresholdSeries.length, 0);
        const competingTuple = [make(1, 0), make(2, 1)];
        (0, assert_1.equal)(dataset(competingTuple.map(value => value.db2), competingTuple.map(value => value.db3)).states[0].passive.thresholdSeries.length, 0);
    });
    (0, mocha_1.it)("keeps a type 34 enemy-scope series partial even with a joined category", () => {
        const make = (threshold) => {
            const id = 300 + threshold;
            const key = `10:r${id}:${id}`;
        ...
        (0, assert_1.equal)(series.status, "partial");
        (0, assert_1.equal)(series.effect.scaling.qualifyingUnit?.scope, "enemy");
        (0, assert_1.deepEqual)(series.unknowns, ["enemy_or_unknown_category_scope_semantics_unknown"]);
    });
});
//# sourceMappingURL=team-analysis-db4-builder.spec.js.map