...
exports.renderDatabaseTeamAnalysisDb3Report = exports.compareDatabaseTeamAnalysisDb3 = exports.currentTeamAnalysisEffectSignature = exports.databaseEffectSignature = exports.currentTeamAnalysisPredicateSignature = exports.databasePredicateSignature = void 0;
...
function databasePredicateSignature(expression) {
...
exports.databasePredicateSignature = databasePredicateSignature;
function currentTeamAnalysisPredicateSignature(predicate) {
...
exports.currentTeamAnalysisPredicateSignature = currentTeamAnalysisPredicateSignature;
function walkDbCondition(expression, predicates) {
    if (expression.op === "predicate")
        predicates.push({ kind: expression.predicate.kind, signature: databasePredicateSignature(expression) });
    else if (expression.op === "all" || expression.op === "any")
        expression.children.forEach(child => walkDbCondition(child, predicates));
    else if (expression.op === "not")
        walkDbCondition(expression.child, predicates);
}
function walkCurrentCondition(value, predicates) {
    ...
    if (expression.op === "predicate") {
        ...
        if (predicate && typeof predicate.kind === "string")
            predicates.push({ kind: predicate.kind, signature: currentTeamAnalysisPredicateSignature(predicate) });
    }
        ...
        walkCurrentCondition(expression.child, predicates);
}
function databaseEffectSignature(effect) {
...
exports.databaseEffectSignature = databaseEffectSignature;
function currentTeamAnalysisEffectSignature(value) {
    const target = object(value.target);
    const scaling = object(value.scaling);
    const events = Array.isArray(scaling?.events) ? scaling.events.map(object).filter((item) => Boolean(item)) : [];
...
exports.currentTeamAnalysisEffectSignature = currentTeamAnalysisEffectSignature;
function difference(left, right) {
    const other = new Set(right);
    return [...new Set(left.filter(value => !other.has(value)))].sort();
}
function compareDatabaseTeamAnalysisDb3(database, current, siteAudit) {
    ...
    const dbAllPredicates = new Set();
    const currentAllPredicates = new Set();
    const dbAllEffects = new Set();
    const currentAllEffects = new Set();
    const examples = [];
    for (const { currentState, databaseState } of pairs) {
        ...
        dbEffects.forEach((effect, index) => {
            ...
            entry.database.add(currentState.stateKey);
            effectByKind.set(effect.kind, entry);
        });
    ...
    const parity = (values) => Object.fromEntries([...values.entries()].sort(([left], [right]) => left.localeCompare(right)).map(([kind, states]) => [kind, {
    ...
    return {
        ...
        examples: examples.sort((left, right) => (right.databaseOnlyPredicates.length + right.currentOnlyPredicates.length + right.databaseOnlyEffects.length + right.currentOnlyEffects.length)
...
exports.compareDatabaseTeamAnalysisDb3 = compareDatabaseTeamAnalysisDb3;
function percent(value, total) {
    return total === 0 ? "0.0%" : `${(value * 100 / total).toFixed(1)}%`;
}
function renderDatabaseTeamAnalysisDb3Report(parity, coverage, parserVersion) {
    ...
    const sample = (values) => values.slice(0, 2).map(value => `\`${value}\``).join("; ") || "none";
    const examples = parity.examples.slice(0, 8).map(example => `- \`${example.stateKey}\` — DB-only predicates (${example.databaseOnlyPredicates.length}): ${sample(example.databaseOnlyPredicates)}; current-only predicates (${example.currentOnlyPredicates.length}): ${sample(example.currentOnlyPredicates)}; DB-only effects (${example.databaseOnlyEffects.length}): ${sample(example.databaseOnlyEffects)}; current-only effects (${example.currentOnlyEffects.length}): ${sample(example.currentOnlyEffects)}.`).join("\n");
...
exports.renderDatabaseTeamAnalysisDb3Report = renderDatabaseTeamAnalysisDb3Report;
//# sourceMappingURL=team-analysis-db3-parity.js.map