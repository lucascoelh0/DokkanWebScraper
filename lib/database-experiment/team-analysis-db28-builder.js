...
exports.buildDatabaseTeamAnalysisDb28Coverage = exports.buildDatabaseTeamAnalysisDb28Dataset = void 0;
...
function validateCode(inspection, region, label) { const symbol = inspection.symbols.find(value => value.name === region.symbol); if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes || sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256)
    throw new Error(`DB28 ${label} native code mismatch`); }
...
function validateEvidence(inspection, evidence, nativeSha256) {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.auditScope !== "causality-types-47-54-revival-activation-counter" || JSON.stringify(evidence.conclusions) !== JSON.stringify(CONCLUSIONS) || JSON.stringify(evidence.unknowns) !== JSON.stringify(UNKNOWNS))
        throw new Error("DB28 native evidence identity mismatch");
    const payload = evidence.skillCausalityPayload;
    if (payload.containerOffset !== 8 || payload.elementSizeBytes !== 4 || JSON.stringify(payload.indexColumns) !== JSON.stringify(["cau_val1", "cau_val2", "cau_val3"]))
        throw new Error("DB28 SkillCausality payload layout mismatch");
    ...
    if (!abilitySymbol || abilitySymbol.value !== ability.vma || abilitySymbol.size !== ability.sizeBytes || ability.sizeBytes !== 360 || ability.deckIndexSlotOffset !== 88 || !relocation(inspection, ability.vma + ability.deckIndexSlotOffset, ability.deckIndexSymbol))
        throw new Error("DB28 AbilityStatus vtable evidence mismatch");
    ...
    if (!inGameSymbol || inGameSymbol.value !== inGame.vma || inGameSymbol.size !== inGame.sizeBytes || inGame.sizeBytes !== 472 || inGame.pureCurrentSlotOffset !== 40 || inGame.backCurrentSlotOffset !== 88 || !relocation(inspection, inGame.vma + inGame.pureCurrentSlotOffset, inGame.pureCurrentSymbol) || !relocation(inspection, inGame.vma + inGame.backCurrentSlotOffset, inGame.backCurrentSymbol))
        throw new Error("DB28 InGameData vtable evidence mismatch");
    const counter = evidence.counter;
    if (counter.inGameCharaDataOffset !== 616 || counter.widthBits !== 32 || JSON.stringify([counter.incrementWriter.observation, counter.availabilityReader.observation, counter.resetWriter.observation]) !== JSON.stringify(COUNTER_OBSERVATIONS))
        throw new Error("DB28 revival counter evidence mismatch");
    validateCode(inspection, counter.incrementWriter, "revival increment writer");
    validateCode(inspection, counter.availabilityReader, "revival availability reader");
    validateCode(inspection, counter.resetWriter, "revival reset writer");
    if (evidence.handlers.length !== 2 || new Set(evidence.handlers.map(value => value.causalityType)).size !== 2)
        throw new Error("DB28 handler cardinality mismatch");
    for (const handler of evidence.handlers) {
        ...
        if ([handler.scope, handler.predicate, handler.parameterReads.join(","), handler.ignoredParameters.join(","), handler.cauVal1Polarity ?? ""].join("|") !== expected.join("|"))
            throw new Error(`DB28 handler semantics mismatch ${handler.causalityType}`);
    }
}
function collect(expression) {
    const rawType = integer(expression.op === "unknown" ? expression.causalityType ?? (typeof expression.raw === "object" && expression.raw !== null ? expression.raw.causality_type : undefined) : undefined);
    if (expression.op === "unknown" && TYPES.includes(rawType))
        return [{ id: String(expression.causalityId), type: rawType }];
...
function buildDatabaseTeamAnalysisDb28Dataset(options) {
    if (options.db8.contractVersion !== "0.7.0" || options.db9.contractVersion !== "0.8.0" || options.db11.contractVersion !== "0.10.0" || options.db8.semanticPromotionCount !== 0 || options.db9.semanticPromotionCount !== 0 || options.db8.sourceSha256 !== options.db9.sourceDatabaseSha256 || options.db8.sourceSha256 !== options.db11.sourceSha256 || options.db8.sourceSnapshotVersion !== options.db9.sourceSnapshotVersion || options.db8.sourceSnapshotVersion !== options.db11.sourceSnapshotVersion || options.db9.sourceDb8.sha256 !== options.db8Sha256 || options.db9.nativeRuntime.sha256 !== options.nativeSha256 || options.db9.nativeRuntime.sizeBytes !== options.nativeSizeBytes)
        throw new Error("DB28 source lineage mismatch");
    ...
    for (const type of TYPES) {
        ...
        if (gaps.length !== 1 || runtimes.length !== 1 || dispatches.length !== 1)
            throw new Error(`DB28 evidence cardinality mismatch ${type}`);
        if (runtimes[0].identityStatus !== "runtime_identified" || runtimes[0].occurrenceCount !== gaps[0].occurrenceCount || runtimes[0].affectedStateCount !== gaps[0].affectedStateCount || runtimes[0].symbol !== handler.symbol || runtimes[0].symbolAddress !== handler.vma || dispatches[0].status !== "identified" || dispatches[0].symbol !== handler.symbol || dispatches[0].symbolAddress !== handler.vma)
            throw new Error(`DB28 runtime lineage mismatch ${type}`);
    }
    const rows = new Map();
    for (const row of options.tables.skill_causalities ?? []) {
        if (row.id === null || row.id === "")
            throw new Error("DB28 causality row has no id");
        const id = String(row.id);
        if (rows.has(id))
            throw new Error(`DB28 duplicate causality row ${id}`);
        rows.set(id, row);
    }
    ...
    for (const state of options.db11.states)
        for (const rule of state.passive?.rules ?? [])
            for (const found of collect(rule.condition)) {
                ...
                if (keys.has(key))
                    throw new Error(`DB28 duplicate occurrence ${key}`);
                keys.add(key);
                ...
                if (!row || !["causality_type", "cau_val1", "cau_val2", "cau_val3"].every(column => Object.prototype.hasOwnProperty.call(row, column)) || integer(row.causality_type) !== found.type)
                    throw new Error(`DB28 causality row missing or incomplete ${found.id}`);
                let predicate;
                if (found.type === 47)
                    predicate = { status: "supported", event: "revival_skill_activated", metric: "activation_count", scope: "ability_owner_pure_current_record", deckIndexSource: "ability_status", comparator: "gt", value: 0 };
                else {
                    const polarity = integer(row.cau_val1);
                    if (polarity === undefined)
                        throw new Error(`DB28 invalid polarity ${found.id}`);
                    predicate = { status: "supported", event: "revival_skill_activated", metric: "activation_count", scope: "party_pure_and_back_current_records", deckIndices: { from: 0, to: 6, inclusive: true }, aggregate: "any_gt_zero", expected: polarity === 0, rawPolarity: row.cau_val1 };
                }
    ...
    for (const type of TYPES) {
        ...
        if (values.length !== gap.occurrenceCount || new Set(values.map(value => value.stateKey)).size !== gap.affectedStateCount || new Set(values.map(value => value.causalityId)).size !== gap.uniqueCausalityCount)
            throw new Error(`DB28 source gap accounting mismatch ${type}`);
    }
...
exports.buildDatabaseTeamAnalysisDb28Dataset = buildDatabaseTeamAnalysisDb28Dataset;
function buildDatabaseTeamAnalysisDb28Coverage(dataset, sourceGapOccurrenceCount) {
    const counts = {}, states = {}, causalities = {};
    for (const type of TYPES) {
        ...
        causalities[String(type)] = new Set(values.map(value => value.causalityId)).size;
    }
...
exports.buildDatabaseTeamAnalysisDb28Coverage = buildDatabaseTeamAnalysisDb28Coverage;
//# sourceMappingURL=team-analysis-db28-builder.js.map