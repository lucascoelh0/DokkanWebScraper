...
exports.renderDatabaseTeamAnalysisDb5Report = exports.compareDatabaseTeamAnalysisDb5 = void 0;
...
function databaseBaseEffectSignature(projection) {
    const { scaling: _scaling, stackCap: _stackCap, ...effect } = projection.effect;
    return (0, team_analysis_db3_parity_1.databaseEffectSignature)(effect);
}
function currentBaseEffectSignature(effect) {
    const copy = { ...effect };
    delete copy.scaling;
    delete copy.stackCap;
    return (0, team_analysis_db3_parity_1.currentTeamAnalysisEffectSignature)(copy);
}
function sameStrings(left, right) {
    return JSON.stringify(left.slice().sort()) === JSON.stringify(right.slice().sort());
}
function currentPredicateMatches(subject, predicate) {
    if (subject.kind === "name_match_token")
        return false;
    if (subject.status !== "supported")
        return false;
    if (subject.kind === "ki_sphere")
        return false;
    ...
    if (!scopeMatches)
        return false;
    if (subject.classes.length > 0 && subject.types.length === 0)
        return kind === classKind && sameStrings(strings(predicate.classes), subject.classes);
    if (subject.types.length > 0 && subject.classes.length === 0)
        return kind === typeKind && sameStrings(strings(predicate.types), subject.types);
    return false;
}
function walkPredicates(value, visit) {
    ...
    if (Array.isArray(expression.children))
        expression.children.forEach(child => walkPredicates(child, visit));
    if (expression.child)
        walkPredicates(expression.child, visit);
}
function sphereSelectorMatches(subject, effect) {
...
function currentTargetSignature(effect) {
...
function currentCalculationBucketSignature(effect) {
...
function capComparison(projection, currentEffects, matchingEffects) {
    if (matchingEffects.length === 0)
        return { comparison: "no_base_effect_shape", caps: [] };
    const caps = [...new Set(matchingEffects.map(effect => effect.stackCap).filter((value) => typeof value === "number"))].sort((left, right) => left - right);
    if (caps.length === 0)
        return { comparison: "no_structured_cap", caps };
    const observed = projection.effect.scaling.observedMaximumContribution;
    if (observed === undefined)
        return { comparison: "mismatch", caps };
    if (caps.includes(observed))
        return { comparison: "exact_observed_contribution", caps };
    const hasStructuredBaseCandidate = matchingEffects.some(matchingEffect => {
        ...
        return typeof matchingEffect.stackCap === "number" && matchingBucket !== undefined && currentEffects.some(effect => effect !== matchingEffect
            ...
            && effect.value === matchingEffect.stackCap - observed);
    });
    return { comparison: hasStructuredBaseCandidate ? "candidate_observed_plus_structured_base" : "mismatch", caps };
}
function compareDatabaseTeamAnalysisDb5(database, current, siteAudit) {
    ...
    const selectorParityByKind = {};
    const examples = [];
    let matchedStateCount = 0;
    let projectionCount = 0;
    let currentBaseEffectShapeMatchCount = 0;
    let currentUnknownConditionShapeMatchCount = 0;
    let currentStructuredSelectorMatchCount = 0;
    for (const state of database.states) {
        ...
        const currentEffects = currentRules.flatMap(rule => rule.effects ?? []);
        for (const projection of state.passive?.countedScaling ?? []) {
            ...
            const matchingEffects = matchingRules.flatMap(rule => rule.effects ?? []).filter(effect => currentBaseEffectSignature(effect) === baseSignature);
            const baseEffectShapeMatched = matchingEffects.length > 0;
            if (baseEffectShapeMatched)
                currentBaseEffectShapeMatchCount += 1;
            const currentCondition = matchingRules.some(rule => {
                if (projection.effect.scaling.subject.kind === "ki_sphere") {
                    return (rule.effects ?? []).some(effect => sphereSelectorMatches(projection.effect.scaling.subject, effect));
                }
                let matched = false;
                walkPredicates(rule.condition, predicate => { if (currentPredicateMatches(projection.effect.scaling.subject, predicate))
                    matched = true; });
                return matched;
            }) ? "structured_selector_match"
                : matchingRules.some(rule => object(rule.condition)?.op === "unknown") ? "unknown" : "other_or_missing";
            if (currentCondition === "structured_selector_match")
                currentStructuredSelectorMatchCount += 1;
            if (currentCondition === "unknown")
                currentUnknownConditionShapeMatchCount += 1;
            ...
            if (baseEffectShapeMatched)
                bucket.baseEffectShapeMatched += 1;
            if (currentCondition === "structured_selector_match")
                bucket.structuredSelectorMatched += 1;
...
exports.compareDatabaseTeamAnalysisDb5 = compareDatabaseTeamAnalysisDb5;
function percent(value, total) {
    return total === 0 ? "0.0%" : `${(value * 100 / total).toFixed(1)}%`;
}
function renderDatabaseTeamAnalysisDb5Report(coverage, parity, parserVersion) {
    const selectorRows = Object.entries(parity.selectorParityByKind).map(([kind, value]) => `| ${kind} | ${value.projections} | ${value.baseEffectShapeMatched} | ${value.structuredSelectorMatched} |`).join("\n");
...
exports.renderDatabaseTeamAnalysisDb5Report = renderDatabaseTeamAnalysisDb5Report;
//# sourceMappingURL=team-analysis-db5-parity.js.map