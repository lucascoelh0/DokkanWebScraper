...
exports.buildDatabaseTeamAnalysisDb10Coverage = exports.buildDatabaseTeamAnalysisDb10Dataset = void 0;
...
function validateSymbol(inspection, symbolName, expectedVma, expectedSize, expectedHash) {
    const symbol = inspection.symbols.find(value => value.name === symbolName);
    if (!symbol)
        throw new Error(`DB10 native symbol missing: ${symbolName}`);
    if (symbol.value !== expectedVma || symbol.size !== expectedSize)
        throw new Error(`DB10 native symbol layout mismatch: ${symbolName}`);
    const actualHash = sha256(inspection.readVirtualBytes(symbol.value, symbol.size));
    if (actualHash !== expectedHash)
        throw new Error(`DB10 native symbol code hash mismatch: ${symbolName}`);
}
function resolution(handler, gap) {
    return {
...
function buildDatabaseTeamAnalysisDb10Dataset(options) {
    if (options.db8.contractVersion !== "0.7.0" || options.db9.contractVersion !== "0.8.0" || options.db9.sourceDb8ContractVersion !== "0.7.0" ||
        options.db8.sourceSnapshotVersion !== options.db9.sourceSnapshotVersion || options.db8.sourceSha256 !== options.db9.sourceDatabaseSha256 || options.db9.sourceDb8.sha256 !== options.db8Sha256)
        throw new Error("DB10 DB8/DB9 source lineage mismatch");
    if (options.layout.schemaVersion !== 1 || options.layout.sourceSha256 !== options.nativeSha256 || options.db9.nativeRuntime.sha256 !== options.nativeSha256)
        throw new Error("DB10 native source lineage mismatch");
    const constructor = options.layout.payloadLayout;
    if (constructor.containerOffset !== 8 || constructor.elementSizeBytes !== 4 || JSON.stringify(constructor.indexColumns) !== JSON.stringify(["cau_val1", "cau_val2", "cau_val3"]))
        throw new Error("DB10 SkillCausality payload layout mismatch");
    ...
    for (const handler of options.layout.handlers) {
        if (types.has(handler.causalityType))
            throw new Error(`DB10 duplicate causality semantic ${handler.causalityType}`);
        types.add(handler.causalityType);
        ...
        if (!slot || slot.status !== "identified" || slot.symbol !== handler.symbol || slot.symbolAddress !== handler.vma)
            throw new Error(`DB10 DB9 dispatch identity mismatch for causality ${handler.causalityType}`);
        const allParameters = [...handler.parameterReads, ...handler.ignoredParameters];
        if (allParameters.length !== 3 || new Set(allParameters).size !== 3 || !["cau_val1", "cau_val2", "cau_val3"].every(value => allParameters.includes(value)))
            throw new Error(`DB10 incomplete parameter audit for causality ${handler.causalityType}`);
        if (handler.status === "supported" && handler.unknowns.length > 0 || handler.status === "partial" && handler.unknowns.length === 0)
            throw new Error(`DB10 semantic status/unknown mismatch for causality ${handler.causalityType}`);
    }
    const byType = new Map();
    for (const gap of options.db8.causalityGaps) {
        const type = gapType(gap);
        if (type === undefined)
            continue;
        if (byType.has(type))
            throw new Error(`DB10 duplicate source gap type ${type}`);
        byType.set(type, gap);
    }
    const causalityResolutions = options.layout.handlers.map(handler => {
        const gap = byType.get(handler.causalityType);
        if (!gap)
            throw new Error(`DB10 source gap missing for causality ${handler.causalityType}`);
        return resolution(handler, gap);
    }).sort((left, right) => left.causalityType - right.causalityType);
    const supported = causalityResolutions.filter(value => value.status === "supported");
    const partial = causalityResolutions.filter(value => value.status === "partial");
    if (supported.length !== 3 || partial.length !== 1)
        throw new Error("DB10 audited promotion set must contain three supported and one partial causality types");
    return {
...
exports.buildDatabaseTeamAnalysisDb10Dataset = buildDatabaseTeamAnalysisDb10Dataset;
function buildDatabaseTeamAnalysisDb10Coverage(dataset, db8) {
    const supported = dataset.causalityResolutions.filter(value => value.status === "supported");
    const partial = dataset.causalityResolutions.filter(value => value.status === "partial");
...
exports.buildDatabaseTeamAnalysisDb10Coverage = buildDatabaseTeamAnalysisDb10Coverage;
//# sourceMappingURL=team-analysis-db10-builder.js.map