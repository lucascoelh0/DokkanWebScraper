...
exports.buildDatabaseTeamAnalysisDb5Coverage = exports.buildDatabaseTeamAnalysisDb5Dataset = exports.normalizeDb4ThresholdSeriesProjection = void 0;
...
function decodeClassTypeMask(rawMask, scope) {
    const mask = integer(rawMask);
    if (mask === null)
        return { kind: "class_type_mask", scope, rawMask, classes: [], types: [], unknownMask: null, status: "unknown", evidence: "unknown" };
    const classes = [];
    const types = [];
    if ((mask & 4) !== 0)
        types.push("INT");
    ...
    if ((mask & 32) !== 0)
        classes.push("Super");
    if ((mask & 64) !== 0)
        classes.push("Extreme");
    const knownMask = 4 | 8 | 16 | 32 | 64;
    const unknownMask = mask & ~knownMask;
    const hasKnownBits = classes.length > 0 || types.length > 0;
    const status = unknownMask === 0 && hasKnownBits && scope !== "unknown" ? "supported" : hasKnownBits ? "partial" : "unknown";
...
function normalizedSubject(projection) {
    ...
    if (projection.source.causalityType === 34)
        return {
            kind: "category",
            scope,
    ...
    if (projection.source.causalityType === 41)
        return {
...
function normalizeDb4ThresholdSeriesProjection(projection) {
    ...
    if (subject.kind === "name_match_token")
        unknowns.push("name_token_dictionary_unavailable");
    if (subject.kind === "class_type_mask" && subject.unknownMask)
        unknowns.push("class_type_mask_contains_unmapped_bits");
    if (subject.kind === "class_type_mask" && subject.status === "unknown")
        unknowns.push("class_type_mask_unknown");
    const status = subject.status === "supported" && projection.status !== "unknown" ? "supported"
...
exports.normalizeDb4ThresholdSeriesProjection = normalizeDb4ThresholdSeriesProjection;
function buildDatabaseTeamAnalysisDb5Dataset(db4) {
    return {
        schemaVersion: 1,
...
exports.buildDatabaseTeamAnalysisDb5Dataset = buildDatabaseTeamAnalysisDb5Dataset;
function buildDatabaseTeamAnalysisDb5Coverage(dataset) {
    ...
    const selectorKindCounts = {};
    const observedSeriesLengthCounts = {};
    let fullyDecoded = 0;
    ...
    for (const projection of projections) {
        ...
        if (subject.kind === "class_type_mask") {
            if (subject.status === "supported")
                fullyDecoded += 1;
            else if (subject.status === "partial")
                partiallyDecoded += 1;
            else
                unknown += 1;
        }
...
exports.buildDatabaseTeamAnalysisDb5Coverage = buildDatabaseTeamAnalysisDb5Coverage;
//# sourceMappingURL=team-analysis-db5-builder.js.map