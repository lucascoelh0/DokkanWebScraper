...
exports.renderDatabaseTeamAnalysisDb12Report = void 0;
const reasons = ["logical_context_mismatch", "polarity_mismatch", "exact_turn_encoding_candidate", "threshold_mismatch", "comparator_mismatch", "absent_in_current", "absent_in_database"];
function compactSignature(value) { const parsed = JSON.parse(value); return Object.entries(parsed).map(([key, item]) => `${key}=${item}`).join(", "); }
function list(values) { return values.length === 0 ? "None." : values.map(value => `- \`${value}\``).join("\n"); }
function renderDatabaseTeamAnalysisDb12Report(dataset, coverage) {
    ...
    const examples = reasons.flatMap(reason => {
        const database = dataset.databaseOnlyAttributions.find(value => value.reason === reason);
        const current = dataset.currentOnlyAttributions.find(value => value.reason === reason);
...
exports.renderDatabaseTeamAnalysisDb12Report = renderDatabaseTeamAnalysisDb12Report;
//# sourceMappingURL=team-analysis-db12-report.js.map