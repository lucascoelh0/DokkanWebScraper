...
function parseNativeRuntimeElf(buffer) {
    ...
    const readVirtualBytes = (vma, size) => {
        if (!Number.isSafeInteger(vma) || !Number.isSafeInteger(size) || size < 0)
            throw new Error("Invalid virtual byte range");
        const section = named.find(value => value.type !== 8 && vma >= value.address && vma + size <= value.address + value.size);
        if (!section)
            throw new Error(`Native VMA range 0x${vma.toString(16)}+${size} is not file-backed`);
        return buffer.subarray(section.offset + vma - section.address, section.offset + vma - section.address + size);
    };
    const readVirtualUint64 = (vma) => readVirtualBytes(vma, 8).readBigUInt64LE();
