...
function renderDatabaseTeamAnalysisDb8Report(dataset, coverage) {
    ...
    const examples = dataset.causalityGaps.filter(value => [3, 34, 43, 51, 55].includes(Number(value.causalityType))).flatMap(value => value.samples.slice(0, 1).map(sample => `- causality ${String(value.causalityType)}: \`${sample.stateKey}\`, rule \`${sample.ruleKey}\`, row \`${sample.sourceRowId}\`; raw domains ${JSON.stringify(value.rawValueDomains)}.`));
...
exports.renderDatabaseTeamAnalysisDb8Report = renderDatabaseTeamAnalysisDb8Report;
//# sourceMappingURL=team-analysis-db8-report.js.map