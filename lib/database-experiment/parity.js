...
exports.renderParityReport = exports.compareWithCurrentDataset = exports.readSiteAuditFixtures = exports.readCurrentCharacters = void 0;
...
function logicalAttacks(card) {
    ...
    for (const attack of attacks) {
        ...
        if (!byKey.has(key))
            byKey.set(key, attack);
    }
    return [...byKey.values()];
}
function sourcedText(row, column) {
    return normalizedText(row?.values[column]);
}
function databaseLinks(card) {
    return sortedStrings(card.links.map(link => link.skill?.values.name));
}
function databaseCategories(card) {
    return sortedStrings(card.categories.map(category => category.category?.values.name));
}
function currentLeaderPercentages(character) {
    const clauses = Array.isArray(character?.leaderSkillDetails?.clauses) ? character.leaderSkillDetails.clauses : [];
    const values = clauses.flatMap((clause) => [clause.hp, clause.atk, clause.def])
        .filter((value) => typeof value === "number" && Number.isFinite(value) && value !== 0);
    return Array.from(new Set(values)).sort((a, b) => a - b);
}
function databaseTransformationClosure(card, cardsById) {
    const visited = new Set();
    const pending = card.formRelations.flatMap(relation => relation.targetCardId ?? []);
    while (pending.length > 0) {
        const targetId = pending.shift();
        if (visited.has(targetId) || targetId === card.cardId)
            continue;
        visited.add(targetId);
        const target = cardsById.get(targetId);
        if (target)
            pending.push(...target.formRelations.flatMap(relation => relation.targetCardId ?? []));
    }
    return [...visited].sort((left, right) => Number(left) - Number(right));
}
async function readCurrentCharacters(gzipPath) {
    const compressed = await (0, promises_1.readFile)(gzipPath);
    const parsed = JSON.parse((0, zlib_1.gunzipSync)(compressed).toString("utf8"));
    if (!Array.isArray(parsed))
        throw new Error(`Current character payload is not an array: ${gzipPath}`);
    return parsed;
}
exports.readCurrentCharacters = readCurrentCharacters;
async function readSiteAuditFixtures(fixturePath = (0, fs_1.existsSync)((0, path_1.resolve)(__dirname, "site-audit-fixtures.json"))
    ? (0, path_1.resolve)(__dirname, "site-audit-fixtures.json")
    : (0, path_1.resolve)(__dirname, "..", "..", "database-experiment", "site-audit-fixtures.json")) {
...
exports.readSiteAuditFixtures = readSiteAuditFixtures;
function compareWithCurrentDataset(dataset, current, siteAudit) {
    ...
    const currentById = new Map(current.flatMap(character => {
        ...
        return characterId ? [[characterId, character]] : [];
    }));
    ...
    let exactRarityMatches = 0;
    let exactTypeMatches = 0;
    let exactClassMatches = 0;
    let classApplicableCount = 0;
    let leaderTextMatches = 0;
    let leaderPercentageMatches = 0;
    let linkSetMatches = 0;
    let categorySetMatches = 0;
    const conflicts = [];
    const mechanicParity = {};
    const recordMechanic = (name, databaseValue, currentValue) => {
        ...
        if (matches)
            entry.matches += 1;
        else if (present(databaseValue) && !present(currentValue))
            entry.databaseOnly += 1;
        else
            entry.currentOnly += 1;
    ...
    for (const [cardId, character] of currentById) {
        ...
        const fields = [];
        const details = {};
        const mismatch = (field, databaseValue, currentValue) => {
            fields.push(field);
            details[field] = { database: databaseValue, current: currentValue };
        };
        if (normalizedText(card.localizedText.name) === normalizedText(character.name))
            exactNameMatches += 1;
        else
            mismatch("name", card.localizedText.name, character.name);
        if (card.rarity.value === character.rarity)
            exactRarityMatches += 1;
        else
            mismatch("rarity", card.rarity, character.rarity);
        if (card.type.value === character.type)
            exactTypeMatches += 1;
        else
            mismatch("type", card.type, character.type);
        if (card.characterClass.value !== "unawakened") {
            classApplicableCount += 1;
            if (card.characterClass.value === character.characterClass)
                exactClassMatches += 1;
            else
                mismatch("characterClass", card.characterClass, character.characterClass);
        }
        ...
        if (dbLeaderText === currentLeaderText)
            leaderTextMatches += 1;
        else if (dbLeaderText || character.leaderSkill)
            mismatch("leaderSkillText", dbLeaderText, currentLeaderText);
        ...
        if (sameArray(dbLeaderPercentages, currentPercentages))
            leaderPercentageMatches += 1;
        else
            mismatch("leaderPercentages", dbLeaderPercentages, currentPercentages);
        const dbLinks = databaseLinks(card);
        const currentLinks = sortedStrings(Array.isArray(character.links) ? character.links : []);
        if (sameArray(dbLinks, currentLinks))
            linkSetMatches += 1;
        else
            mismatch("links", dbLinks, currentLinks);
        const dbCategories = databaseCategories(card);
        const currentCategories = sortedStrings(Array.isArray(character.categories) ? character.categories : []);
        if (sameArray(dbCategories, currentCategories))
            categorySetMatches += 1;
        else
            mismatch("categories", dbCategories, currentCategories);
        const currentTransformIds = sortedStrings((character.transformations ?? []).map((form) => currentId(form.id)));
        const databaseTransformIds = databaseTransformationClosure(card, databaseById);
        if (!recordMechanic("transformationTargetSet", databaseTransformIds, currentTransformIds))
            mismatch("transformationTargets", databaseTransformIds, currentTransformIds);
        ...
        if (!recordMechanic("active", activeValues[0], activeValues[1]))
            mismatch("activePresence", activeValues[0], activeValues[1]);
        const standbyValues = [card.standbySkills.length > 0, Boolean(character.standby)];
        if (!recordMechanic("standby", standbyValues[0], standbyValues[1]))
            mismatch("standbyPresence", standbyValues[0], standbyValues[1]);
        const finishValues = [card.finishSkills.length > 0, Array.isArray(character.finishSkills) && character.finishSkills.length > 0];
        if (!recordMechanic("finish", finishValues[0], finishValues[1]))
            mismatch("finishPresence", finishValues[0], finishValues[1]);
        const exchangeValues = [card.formRelations.some(relation => relation.kind.value === "reversible-exchange"), Boolean(character.reversibleExchange) || (character.transformations ?? []).some((form) => form.transformationSource === "reversible-exchange")];
        if (!recordMechanic("reversibleExchange", exchangeValues[0], exchangeValues[1]))
            mismatch("reversibleExchange", exchangeValues[0], exchangeValues[1]);
        ...
        if (!recordMechanic("unitAttackCount", unitValues[0], unitValues[1]))
            mismatch("unitAttackCount", unitValues[0], unitValues[1]);
        const exValues = [state.attacks.some(attack => attack.variant.value === "ex"), Boolean(character.exSuperAttack)];
        if (!recordMechanic("exAttack", exValues[0], exValues[1]))
            mismatch("exAttack", exValues[0], exValues[1]);
        if (fields.length > 0)
            conflicts.push({ cardId, name: card.localizedText.name, fields, details });
    }
    const currentMechanics = {
        ...
        reversibleExchange: current.filter(character => Boolean(character.reversibleExchange) || (character.transformations ?? []).some((form) => form.transformationSource === "reversible-exchange")).length,
    ...
    const currentIdsByAwakeningGroup = new Map();
    ...
    const fieldPriority = { transformationTargets: 11, leaderSkillText: 10, unitAttackCount: 10, standbyPresence: 10, finishPresence: 10, reversibleExchange: 10, exAttack: 10, links: 9, categories: 9, name: 8, rarity: 8, type: 8, leaderPercentages: 6, characterClass: 3 };
    return {
...
exports.compareWithCurrentDataset = compareWithCurrentDataset;
function pct(value, total) {
    return total === 0 ? "0.00%" : `${(100 * value / total).toFixed(2)}%`;
}
function idExamples(ids) {
    return ids.length === 0 ? "none" : ids.slice(0, 50).join(", ") + (ids.length > 50 ? ` … (+${ids.length - 50})` : "");
}
function renderParityReport(summary, coverage, siteAudit) {
    ...
    const compact = (value) => JSON.stringify(value).replaceAll("|", "\\|").slice(0, 180);
...
exports.renderParityReport = renderParityReport;
