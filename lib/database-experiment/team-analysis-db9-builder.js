...
exports.buildDatabaseTeamAnalysisDb9Coverage = exports.buildDatabaseTeamAnalysisDb9Dataset = void 0;
...
function decorate(values) { return values.map(value => ({ ...value, minimumOperationLabel: value.symbol ? LABELS[value.symbol] : undefined })); }
...
function gapEvidence(sourceEnumValue, count, states, slots, unresolvedFields) {
    const enumValue = parseEnumValue(sourceEnumValue);
    if (enumValue === undefined)
        return { sourceEnumValue, occurrenceCount: count, affectedStateCount: states, identityStatus: "invalid_value", unresolvedFields };
    const slot = slots[enumValue];
    if (!slot)
        return { sourceEnumValue, enumValue, occurrenceCount: count, affectedStateCount: states, identityStatus: "out_of_range", unresolvedFields };
    if (slot.status === "null")
        return { sourceEnumValue, enumValue, occurrenceCount: count, affectedStateCount: states, identityStatus: "unsupported_null", slotVma: slot.slotVma, unresolvedFields };
...
function buildDatabaseTeamAnalysisDb9Dataset(options) {
    const efficacySlots = decorate((0, native_runtime_elf_adapter_1.dispatchSlots)(options.inspection, options.layout.tables.efficacy.baseVma, options.layout.tables.efficacy.slotCount));
    const causalitySlots = decorate((0, native_runtime_elf_adapter_1.dispatchSlots)(options.inspection, options.layout.tables.causality.baseVma, options.layout.tables.causality.slotCount));
    ...
    return { schemaVersion: 1, contract: "dokkan-team-analysis-native-runtime-evidence-experiment", contractVersion: "0.8.0", generatedAt: options.db8.generatedAt,
        ...
        nativeRuntime: { fileName: (0, path_1.basename)(options.nativePath), sizeBytes: options.nativeSizeBytes, sha256: options.nativeSha256, elfClass: options.inspection.elfClass, endian: options.inspection.endian, machine: options.inspection.machine }, semanticPromotionCount: 0,
...
exports.buildDatabaseTeamAnalysisDb9Dataset = buildDatabaseTeamAnalysisDb9Dataset;
function buildDatabaseTeamAnalysisDb9Coverage(dataset) {
    const counts = (values) => ({ total: values.length, identified: values.filter(value => value.identityStatus === "runtime_identified").length, null: values.filter(value => value.identityStatus === "unsupported_null").length, outOfRange: values.filter(value => value.identityStatus === "out_of_range").length, invalid: values.filter(value => value.identityStatus === "invalid_value").length });
    const efficacy = counts(dataset.efficacyGapEvidence);
    const causality = counts(dataset.causalityGapEvidence);
    const efficacyTotal = dataset.efficacyGapEvidence.reduce((sum, value) => sum + value.occurrenceCount, 0);
    const efficacyIdentified = dataset.efficacyGapEvidence.filter(value => value.identityStatus === "runtime_identified").reduce((sum, value) => sum + value.occurrenceCount, 0);
    const causalityTotal = dataset.causalityGapEvidence.reduce((sum, value) => sum + value.occurrenceCount, 0);
    const causalityIdentified = dataset.causalityGapEvidence.filter(value => value.identityStatus === "runtime_identified").reduce((sum, value) => sum + value.occurrenceCount, 0);
...
exports.buildDatabaseTeamAnalysisDb9Coverage = buildDatabaseTeamAnalysisDb9Coverage;
//# sourceMappingURL=team-analysis-db9-builder.js.map