...
exports.buildDatabaseTeamAnalysisDb20Coverage = exports.buildDatabaseTeamAnalysisDb20Dataset = void 0;
const STATUSES = ["exact_numeric_match", "numeric_mismatch", "non_numeric_turn", "passive_skill_missing", "database_rule_missing"];
const COLUMNS = ["turn", "is_once", "exec_timing_type", "efficacy_type"];
...
function parseCandidate(value) {
    if (value.side !== "current" || value.reason !== "absent_in_other")
        return null;
    let atom;
    try {
        atom = JSON.parse(value.structuralSignature);
    }
    catch {
        throw new Error("DB20 invalid DB19 structural signature");
    }
    if (!atom || Array.isArray(atom) || atom.kind !== "turn_from_entry" || atom.comparator !== "lte" || atom.logicalContext !== "all" || atom.negated !== false || atom.scope !== "self")
        return null;
    const upperBound = numeric(atom.value);
    if (upperBound === null)
        throw new Error("DB20 candidate has a non-numeric upper bound");
    return { ...value, upperBound };
}
function key(stateKey, ruleKey) { return `${stateKey}|${ruleKey}`; }
function distribution(values, get) {
    const result = {};
    for (const value of values) {
        const raw = get(value);
        const label = raw === null || raw === undefined ? "null" : String(raw);
        result[label] = (result[label] ?? 0) + 1;
    }
    return Object.fromEntries(Object.entries(result).sort(([left], [right]) => left.localeCompare(right, "en", { numeric: true })));
}
function buildDatabaseTeamAnalysisDb20Dataset(options) {
    if (options.db11.contractVersion !== "0.10.0" || options.db19.contractVersion !== "0.18.0" || options.db11.sourceSha256 !== options.db19.sourceDatabaseSha256 || options.db11.sourceSnapshotVersion !== options.db19.sourceSnapshotVersion || options.db19.inheritedSemanticPromotionCount !== 3 || options.db19.semanticPromotionCount !== 0)
        throw new Error("DB20 source lineage mismatch");
    const rules = new Map();
    for (const state of options.db11.states)
        for (const rule of state.passive?.rules ?? []) {
            const ruleKey = key(state.stateKey, rule.ruleKey);
            if (rules.has(ruleKey))
                throw new Error("DB20 duplicate DB11 rule key");
    ...
    for (const row of options.tables.passive_skills ?? []) {
        ...
        if (!id)
            continue;
        if (passiveSkills.has(id))
            throw new Error("DB20 duplicate passive_skills id");
        passiveSkills.set(id, row);
    }
    const candidates = options.db19.residualAttributions.map(parseCandidate).filter((value) => value !== null);
    const correlations = candidates.map(candidate => {
        ...
        const correlationStatus = !rule ? "database_rule_missing" : !row ? "passive_skill_missing" : turn === null ? "non_numeric_turn" : turn === candidate.upperBound ? "exact_numeric_match" : "numeric_mismatch";
        return { stateKey: candidate.stateKey, databaseRuleKey: candidate.databaseRuleKey, currentRuleKey: candidate.currentRuleKey, currentStructuralSignature: candidate.structuralSignature, currentTurnFromEntryUpperBound: candidate.upperBound, passiveSkillId, raw: { turn: rawTurn, isOnce: row?.is_once ?? null, executionTimingType: row?.exec_timing_type ?? null, efficacyType: row?.efficacy_type ?? null }, correlationStatus, semanticStatus: "unknown", provenance: { table: "passive_skills", rowId: rowId(row), columns: [...COLUMNS] } };
...
exports.buildDatabaseTeamAnalysisDb20Dataset = buildDatabaseTeamAnalysisDb20Dataset;
function buildDatabaseTeamAnalysisDb20Coverage(dataset) {
    const counts = Object.fromEntries(STATUSES.map(status => [status, dataset.correlations.filter(value => value.correlationStatus === status).length]));
...
exports.buildDatabaseTeamAnalysisDb20Coverage = buildDatabaseTeamAnalysisDb20Coverage;
//# sourceMappingURL=team-analysis-db20-builder.js.map