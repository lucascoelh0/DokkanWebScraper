...
exports.buildDatabaseTeamAnalysisDb35Coverage = exports.buildDatabaseTeamAnalysisDb35Dataset = exports.projectDb35Target = void 0;
...
const rowId = (row) => row.id === null || row.id === undefined ? undefined : String(row.id);
...
function projectDb35Target(value) {
    const semantic = SUPPORTED_TARGETS.find(item => item.raw === integer(value));
    return semantic
        ? { status: "supported", value: { ...semantic, classPredicateRaw: [...semantic.classPredicateRaw], parametersRead: [...semantic.parametersRead], parametersIgnored: [...semantic.parametersIgnored] } }
        : { status: "unknown", value: "unknown" };
}
exports.projectDb35Target = projectDb35Target;
function validateEvidence(inspection, evidence, nativeSha256) {
    ...
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.auditScope !== "passive-target-type-native-dispatch-and-current-handler-behavior" || JSON.stringify(evidence.sqliteBinding) !== JSON.stringify(expectedBinding) || JSON.stringify(evidence.supportedTargets) !== JSON.stringify(SUPPORTED_TARGETS))
        throw new Error("DB35 evidence identity mismatch");
    if (!inspection.readVirtualBytes(expectedBinding.literalVma, 12).equals(Buffer.from("target_type\0")))
        throw new Error("DB35 SQLite literal mismatch");
    const dispatch = evidence.dispatch;
    if (dispatch.consumerSymbol !== EXPECTED_REGION_SYMBOLS.dispatch_consumer || dispatch.tableVma !== 92050896 || dispatch.entrySizeBytes !== 8 || dispatch.entryCount !== 17 || dispatch.zeroBytesSha256 !== "b707241545a346265aab1ffb32ff64b55bf8f8dc1b56a46ef33ce3d15db11d33" || sha256(inspection.readVirtualBytes(dispatch.tableVma, 136)) !== dispatch.zeroBytesSha256 || dispatch.targetGetterVtableOffsetFromObjectVptr !== 152 || dispatch.targetGetterSymbol !== EXPECTED_REGION_SYMBOLS.target_getter || JSON.stringify(dispatch.instructionBytes) !== JSON.stringify(EXPECTED_DISPATCH_INSTRUCTIONS) || JSON.stringify(dispatch.slots) !== JSON.stringify(EXPECTED_SLOTS.map(({ raw, handlerRole }) => ({ raw, handlerRole }))))
        throw new Error("DB35 dispatch metadata mismatch");
    for (const instruction of dispatch.instructionBytes)
        if (inspection.readVirtualBytes(instruction.vma, 4).toString("hex") !== instruction.hex)
            throw new Error(`DB35 dispatch instruction mismatch ${instruction.role}`);
    for (const slot of EXPECTED_SLOTS) {
        ...
        if (slot.symbol === null) {
            if (relocations.length !== 0 || inspection.readVirtualUint64(dispatch.tableVma + slot.raw * 8) !== 0n)
                throw new Error("DB35 null dispatch slot mismatch");
        }
        else if (relocations.length !== 1 || relocations[0].symbolName !== slot.symbol)
            throw new Error(`DB35 dispatch slot mismatch ${slot.raw}`);
    }
    if (evidence.codeRegions.length !== Object.keys(EXPECTED_REGION_SYMBOLS).length || new Set(evidence.codeRegions.map(item => item.role)).size !== evidence.codeRegions.length)
        throw new Error("DB35 code region cardinality mismatch");
    for (const region of evidence.codeRegions) {
        if (EXPECTED_REGION_SYMBOLS[region.role] !== region.symbol || sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256)
            throw new Error(`DB35 native code mismatch ${region.role}`);
        if (!region.symbol.startsWith("local@")) {
            ...
            if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes)
                throw new Error(`DB35 symbol mismatch ${region.role}`);
        }
    }
    const vtable = evidence.abilityStatusVtable;
    const symbol = inspection.symbols.find(item => item.name === vtable.symbol);
    if (!symbol || vtable.symbol !== "_ZTV22AbilityStatusCausality" || symbol.value !== vtable.vma || symbol.size !== vtable.sizeBytes || sha256(inspection.readVirtualBytes(vtable.vma, vtable.sizeBytes)) !== vtable.rawSha256 || vtable.targetGetterRelocationOffset !== 168 || vtable.targetGetterSymbol !== EXPECTED_REGION_SYMBOLS.target_getter || inspection.relocations.filter(item => item.offset === vtable.vma + 168 && item.symbolName === vtable.targetGetterSymbol).length !== 1)
        throw new Error("DB35 vtable mismatch");
    if (JSON.stringify(evidence.subTargetBoundary) !== JSON.stringify({ predicateRole: "contains_sub_target_type", appliedBeforeCallback: true, sqliteSetBinding: "unknown_in_db35", valueTypeSemantics: "unknown_in_db35" }))
        throw new Error("DB35 sub-target boundary mismatch");
    if (JSON.stringify(evidence.unknowns) !== JSON.stringify(EXPECTED_UNKNOWNS))
        throw new Error("DB35 unknown boundary mismatch");
}
function buildDatabaseTeamAnalysisDb35Dataset(options) {
    if (options.db11.contractVersion !== "0.10.0" || options.db33.contractVersion !== "0.32.0" || options.db33Sha256 !== EXPECTED_DB33_SHA256 || options.db33.sourceDb32.sha256 !== EXPECTED_DB32_SHA256 || options.db33.sourceDatabaseSha256 !== options.db11.sourceSha256 || options.db33.currentTeamAnalysis.sha256 !== options.currentSha256 || options.db33.nativeRuntime.sha256 !== options.nativeSha256 || options.db33.nativeRuntime.sizeBytes !== options.nativeSizeBytes)
        throw new Error("DB35 lineage mismatch");
    ...
    for (const state of options.db11.states)
        for (const rule of state.passive?.rules ?? []) {
            ...
            if (!row || !timing || timing.passiveSkillId !== rule.source.passiveSkillId || integer(row.target_type) !== integer(rule.source.targetType))
                throw new Error(`DB35 source mismatch ${key}`);
            ...
            ruleTargets.push({
                ...
                provenance: {
                    ...
                    ...(target.status === "supported" && handlerRole ? { runtime: { fileName: "libcocos2dcpp.so", sha256: options.nativeSha256, evidenceFile: "native-passive-target-dispatch-semantics.json", evidenceSha256: options.evidenceSha256, handlerRole, proofRoles } } : {}),
                },
            });
        }
    ...
    const currentAggregateTargetScopes = {};
...
exports.buildDatabaseTeamAnalysisDb35Dataset = buildDatabaseTeamAnalysisDb35Dataset;
const counts = (values, key) => {
    const result = {};
...
function buildDatabaseTeamAnalysisDb35Coverage(dataset) {
    const supported = dataset.ruleTargets.filter((rule) => rule.target.status === "supported");
    return {
...
exports.buildDatabaseTeamAnalysisDb35Coverage = buildDatabaseTeamAnalysisDb35Coverage;
//# sourceMappingURL=team-analysis-db35-builder.js.map