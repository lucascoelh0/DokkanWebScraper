...
exports.kiMultipliersFromCard = exports.validateKiMultipliers = void 0;
const FIELDS = ["eball_mod_min", "eball_mod_num100", "eball_mod_mid",
    "eball_mod_mid_num", "eball_mod_max", "eball_mod_max_num"];
function validateKiMultipliers(value) {
    if (value.schemaVersion !== 1 || !Array.isArray(value.points)
        || ![3, 4].includes(value.points.length))
        throw new Error("Invalid Ki anchors schema");
...
exports.validateKiMultipliers = validateKiMultipliers;
/** Missing old exports remain absent; partially populated/invalid rows fail loudly. */
function kiMultipliersFromCard(card) {
    ...
    if (midNum === 0 && mid !== 0)
        throw new Error(`Unpaired Ki middle anchor for card ${card.id}`);
    const result = {
        schemaVersion: 1,
        points: [
            { ki: 0, percent: min },
...
exports.kiMultipliersFromCard = kiMultipliersFromCard;
//# sourceMappingURL=game-db-ki-multipliers.js.map