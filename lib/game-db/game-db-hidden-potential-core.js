...
exports.lookup = exports.buildIndex = exports.calculateBoard = exports.calculateMax = exports.int = exports.check = exports.ALTERNATIVE = exports.PRESETS = exports.HIPO_VERSION = void 0;
/** Offline theoretical sheet stats, never account progress or combat stats. */
exports.HIPO_VERSION = "1.0.0";
exports.PRESETS = [
...
exports.ALTERNATIVE = { id: "three-routes-alternative", openedRouteIds: [3, 0, 2] };
function check(ok, message) { if (!ok)
    throw new Error(message); }
exports.check = check;
function int(v, label, min = 0, max = 2147483647) {
    check(Number.isSafeInteger(v) && v >= min && v <= max, `Invalid ${label}`);
    return v;
}
exports.int = int;
function unique(values, key, label) {
    ...
    for (const v of values) {
        const k = key(v);
        check(!m.has(k), `Duplicate ${label}`);
        m.set(k, v);
    }
    return m;
}
function calculateMax(card, level, coef) {
    int(level, "level", 1, 1000);
    int(card.lv_max, "base level", 1, 1000);
    if (coef === undefined)
        return null;
    check(Number.isFinite(coef) && coef >= 0, "Invalid curve coefficient");
    return ["hp", "atk", "def"].map(axis => {
...
function calculateBoard(board) {
    ...
    for (const n of board.nodes) {
        check(Array.isArray(n.bonus) && n.bonus.length === 3, "Invalid bonus");
        n.bonus.forEach(v => int(v, "bonus", 0, 100000));
        int(n.requiredSa, "required SA", 0, 10);
        int(n.choiceCount, "choice count", 0, 20);
        check(n.choiceCount !== 1, "Incomplete choice");
        if (n.choiceCount)
            check(n.bonus.every(v => v === 0), "Choice changes raw stats");
        if (n.route !== null) {
            int(n.route, "route", 0, 3);
            check(!gates.has(n.route), "Duplicate gate");
            gates.add(n.route);
        }
        adjacent.set(n.id, []);
    }
    ...
    for (const pair of board.edges) {
        ...
        const [a, b] = pair;
        check(nodes.has(a) && nodes.has(b) && a < b && !edges.has(`${a}:${b}`), "Invalid edge");
        edges.add(`${a}:${b}`);
        adjacent.get(a).push(b);
        adjacent.get(b).push(a);
    }
    const evaluate = (preset) => {
        const visited = new Set();
        const todo = preset.id === "none" ? [] : [...board.roots];
        while (todo.length) {
            const k = todo.pop();
            const node = nodes.get(k);
            if (visited.has(k) || (node.route !== null && !preset.openedRouteIds.includes(node.route)))
                continue;
            visited.add(k);
            todo.push(...adjacent.get(k));
        }
        const statBonus = [0, 0, 0];
        let requiredSa = 0;
        for (const k of visited) {
            const n = nodes.get(k);
            n.bonus.forEach((v, i) => statBonus[i] += v);
            requiredSa = Math.max(requiredSa, n.requiredSa);
        }
    ...
    const presets = exports.PRESETS.map(evaluate);
    ...
    for (let i = 2; i < presets.length; i++)
        check(presets[i].activatedNodeCount > presets[i - 1].activatedNodeCount, "Ineffective gate");
    return { boardId: board.id, totalActivatableNodeCount: board.nodes.length, presets, alternatives: [evaluate(exports.ALTERNATIVE)] };
}
exports.calculateBoard = calculateBoard;
function openTime(value) {
    ...
    check(Number.isFinite(time) && new Date(time).toISOString() === iso, "Invalid calendar date");
    return time;
}
function availability(dates, at) {
    const parsed = dates.map(openTime);
    return parsed.some(t => t !== null && t > at) ? "future" : parsed.includes(null) ? "unknown" : "released";
}
function buildIndex(source, scope, generatedAt) {
    const at = Date.parse(generatedAt);
    check(Number.isFinite(at) && new Date(at).toISOString() === generatedAt, "Invalid generatedAt");
    ...
    for (const g of curves.values())
        check(Number.isFinite(g.coef) && g.coef >= 0, "Invalid coefficient");
    ...
    for (const cardId of [...scope].sort((a, b) => a - b)) {
        ...
        let result;
        if (!c)
            result = { status: "unknown", reason: "missing-source-card", boardId: null, states: {} };
            ...
            if (c.potential_board_id !== null) {
                check(boardResults.has(c.potential_board_id), "Unknown board join");
                usedBoards.add(c.potential_board_id);
            ...
            for (const r of routes)
                check(r.awaked_card_id === cardId && [1, 2].includes(r.optimal_awakening_type), "Invalid optimal identity/state");
            if (routes.length)
                check(routes.every((r, i) => r.optimal_awakening_step === i + 1), "Incomplete optimal chain");
            const state = (releaseState, level, step, dependencies) => {
                ...
                const key = `${releaseState}:${status}:${av}`;
            ...
            const states = { base: state("base", c.lv_max, null, []) };
            for (const [kind, name] of [[1, "eza"], [2, "seza"]]) {
                ...
                if (!selected) {
                    ...
                    const key = `${name}:ineligible:not-configured`;
                    stateCounts[key] = (stateCounts[key] || 0) + 1;
                    continue;
                }
            ...
            if (states.seza.status === "supported")
                check(states.eza.status === "supported" && JSON.stringify(states.seza.maxStats) === JSON.stringify(states.eza.maxStats), "Unsupported SEZA stat change");
        ...
        byCardId[String(cardId)] = result;
        const statusKey = `${result.status}:${result.reason || "board-present"}`;
    ...
    const index = { schemaVersion: 1, contract: "dokkan-hidden-potential", contractVersion: exports.HIPO_VERSION, generatedAt,
...
function lookup(index, cardId, state, presetId) {
    const card = index.byCardId[cardId];
    if (!card || card.status !== "supported")
        return null;
    const release = card.states[state];
    if (release?.status !== "supported" || !release.eligibleForDisplay)
        return null;
    ...
    return { cardId, boardId: card.boardId, releaseState: state, level: release.level,
        displayPercent: preset.displayPercent, stats: release.maxStats.map((v, i) => v + preset.statBonus[i]) };
}
exports.lookup = lookup;
//# sourceMappingURL=game-db-hidden-potential-core.js.map