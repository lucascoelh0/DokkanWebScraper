...
async function buildFirstPartyPortraitCandidate(options) {
    ...
    const candidateEntries = await mapConcurrent([...referencesByCardId].sort(([left], [right]) => left.localeCompare(right, undefined, { numeric: true })), 8, async ([cardId, cardReferences]) => {
        ...
        const bytes = await (0, first_party_portrait_compositor_1.composeFirstPartyPortraitLayers)(verifiedPortraitLayers(spec, {
        ...
        if (reread.length !== bytes.length || sha256(reread) !== hash)
            throw new Error(`portrait candidate ${cardId} write rejected`);
