...
function samePromotedMemberIdentity(before, after) {
    ...
    async promoteArtifactDirectoryCreateOnly(snapshot, to, signal) {
        ...
        try {
            for (const name of ARTIFACT_MEMBER_NAMES) {
                ...
                if (!sameFileIdentity(member.identity, beforeLink))
                ...
                await (0, promises_1.link)(member.sourcePath, installedPath);
...
async function assertPromotedArtifactSnapshot(guard, directory, directoryIdentity, snapshot, signal) {
    ...
    for (const name of ARTIFACT_MEMBER_NAMES) {
        ...
        if (!samePromotedMemberIdentity(member.identity, opened) || !sameFileIdentity(opened, pathIdentity)
            || !samePath(canonical, target) || !samePath((0, path_1.dirname)(canonical), directory)) {
