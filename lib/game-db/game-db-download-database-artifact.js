...
class ArtifactStoreGuard {
    ...
    async installTrackedRegularFileAtomic(from, fromParent, to, toParent, label, signal) {
        const source = await this.assertParent(from, fromParent);
        const target = await this.assertParent(to, toParent);
        const expected = this.files.get(source);
        if (!expected)
            throw new Error(`${label} source identity is not controlled`);
        await assertPathMatchesFileIdentity(source, expected, `${label} source`);
        try {
            await (0, promises_1.lstat)(target);
            throw new Error(`${label} target was replaced before atomic installation`);
        }
        catch (error) {
            if (error?.code !== "ENOENT")
                throw error;
        ...
        this.files.delete(source);
        this.files.set(target, installed);
        if (!sameFileIdentity(expected, installed, false))
            throw new Error(`${label} atomic installation moved a replacement identity`);
...
async function validateLatestPointerAndCommits(guard, artifactsRoot, expectedCurrentIdentity, signal) {
    ...
    if (!firstIdentity)
        throw new Error("Latest pointer identity was not retained");
    const pointer = parseLatestPointer(firstText);
    if (expectedCurrentIdentity && pointer.currentIdentity !== expectedCurrentIdentity)
        throw new Error("Latest pointer does not reference the expected committed artifact");
    const metadata = await validateCommittedArtifact(guard, artifactsRoot, pointer.currentIdentity, undefined, signal);
    if (pointer.previousIdentity)
        await validateCommittedArtifact(guard, artifactsRoot, pointer.previousIdentity, undefined, signal);
    ...
    if (!secondIdentity || firstText !== secondText || !sameFileIdentity(firstIdentity, secondIdentity))
        throw new Error("Latest pointer changed while its commits were validated");
...
async function promoteLatest(guard, artifactsRoot, identity, signal) {
    ...
    const candidatePath = (0, path_1.resolve)(guard.root, `.latest-candidate-${process.pid}-${(0, crypto_1.randomBytes)(12).toString("hex")}.json`);
    let priorHistory;
    ...
    try {
        ...
        const rollback = async () => {
            ...
            if (originalText !== null) {
                ...
                await guard.installTrackedRegularFileAtomic(priorHistory.path, (0, path_1.dirname)(priorHistory.path), latestPath, guard.root, "Prior latest pointer restoration");
            }
        };
        try {
            throwIfCancelled(signal);
        ...
        if (guard.trackedRegularFile(candidatePath))
            await guard.discardTrackedRegularFile(candidatePath, guard.root, "Failed latest pointer candidate discard").catch(() => undefined);
