...
exports.buildRewardItemAudit = exports.REWARD_ITEM_AUDIT_TABLES = void 0;
const crypto_1 = require("crypto");
const promises_1 = require("fs/promises");
const path_1 = require("path");
const game_db_source_1 = require("./game-db-source");
const game_db_stage_drop_preview_1 = require("./game-db-stage-drop-preview");
...
exports.REWARD_ITEM_AUDIT_TABLES = [...new Set([
...
function id(row) { return (0, game_db_source_1.normalizeDbId)(row.id) ?? "row-without-id"; }
function dimension(status, evidence) { return { status, evidence }; }
function coverage(itemType) {
    if (["Card", "EquipmentSkillItem", "LinkSkillLvUpItem"].includes(itemType))
        return {
    ...
    if (["TrainingItem", "TreasureItem"].includes(itemType))
        return {
            metadataAvailable: true,
...
function requiredRelationId(row, column, label) {
    const result = (0, game_db_source_1.normalizeDbId)(row[column]);
    if (!result)
        throw new Error(`${label} has no ${column}`);
    return result;
}
function validateDeliveredWallpaper(reward, surface) {
...
function collectDeliveredWallpapers(dataset) {
    const result = [];
    const add = (surface, key, reward) => {
        if (reward.itemType !== "WallpaperItem")
            return;
        validateDeliveredWallpaper(reward, surface);
        result.push({ surface, key, itemId: reward.itemId });
    };
    for (const stage of dataset.entries) {
        stage.bossDrops?.forEach(reward => add("questLevelBossDrops", `map:${stage.id}:row:${reward.sourceRowId}:item:${reward.itemId}`, reward));
        stage.dropPreviews?.forEach(preview => preview.items.forEach(reward => add("questLevelDropPreviews", `map:${stage.id}:row:${preview.sourceRowId}:item:${reward.itemId}`, reward)));
    }
    for (const stage of dataset.zBattles ?? []) {
        stage.checkpoints?.forEach(checkpoint => checkpoint.repeatRewards.forEach(reward => add("zBattleCheckpointRewards", `stage:${stage.id}:checkpoint:${checkpoint.id}:group:${checkpoint.normalRewardTableGroupId}` +
            `:item:${reward.itemId}:quantity:${reward.quantity ?? "n/a"}`, reward)));
        stage.firstRewards?.forEach(range => range.rewards.forEach(reward => add("zBattleFirstRewards", `stage:${stage.id}:range:${range.id}:set:${range.rewardSetId}` +
            `:item:${reward.itemId}:quantity:${reward.quantity ?? "n/a"}`, reward)));
    }
    for (const mission of dataset.eventMissions ?? [])
        mission.rewards.forEach(reward => add("eventMissions", `mission:${mission.id}:item:${reward.itemId}:quantity:${reward.quantity ?? "n/a"}`, reward));
    return result;
}
...
function buildRewardItemAudit(options) {
    if (Number.isNaN(Date.parse(options.generatedAt)) || !/^\d+$/.test(options.sourceSnapshotVersion)
        ...
        throw new Error("Invalid reward audit provenance");
    if (options.stageDataset.sourceSnapshotVersion !== options.sourceSnapshotVersion
        || options.stageDataset.sourceDatabaseSha256 !== options.sourceDatabaseSha256)
        throw new Error("Stage dataset does not match reward audit provenance");
    const occurrences = [];
    for (const surface of SURFACES) {
        const rows = options.tables[surface.table];
        if (!rows)
            throw new Error(`Reward audit is missing surface ${surface.table}`);
        for (const row of rows)
            for (const [typeColumn, idColumn] of surface.pairs) {
                const itemType = value(row, typeColumn);
                const itemId = value(row, idColumn);
                if (!itemType && !itemId)
                    continue;
                if (!itemType || !itemId)
                    throw new Error(`${surface.table} row ${id(row)} has incomplete ${typeColumn}/${idColumn}`);
    ...
    for (const occurrence of occurrences)
        byType.set(occurrence.itemType, [...(byType.get(occurrence.itemType) ?? []), occurrence]);
    const matrix = [...byType].sort(([left], [right]) => left.localeCompare(right)).map(([itemType, items]) => {
        ...
        if (catalogTable) {
            ...
            if (!catalogRows)
                throw new Error(`Reward audit is missing catalog ${catalogTable}`);
            const catalog = new Map();
            for (const row of catalogRows) {
                const itemId = (0, game_db_source_1.normalizeDbId)(row.id);
                if (!itemId)
                    throw new Error(`${catalogTable} contains a row without id`);
                if (catalog.has(itemId))
                    throw new Error(`${catalogTable} contains duplicate id ${itemId}`);
            ...
            if (missing.length)
                throw new Error(`${itemType} references missing ${catalogTable} IDs: ${missing.join(", ")}`);
            joinedItemCount = referencedIds.length;
            if (itemType === "WallpaperItem")
                for (const itemId of referencedIds) {
                    const row = catalog.get(itemId);
                    if (!value(row, "name") || !value(row, "description"))
    ...
    if (assetPresentationIds.length !== options.wallpaperAssetPresentationIds.length
        || JSON.stringify(assetPresentationIds) !== JSON.stringify(catalogIds))
        throw new Error("Wallpaper asset presentations do not cover the exact wallpaper catalog");
    ...
    const stageDeliverableMissions = missionWallpaperOccurrences.filter(item => {
        if (!item.missionId)
            throw new Error(`mission_rewards row ${item.rowId} has no mission_id`);
        const mission = missions.get(item.missionId);
        if (!mission)
            throw new Error(`mission_rewards row ${item.rowId} references missing mission ${item.missionId}`);
        const areaId = (0, game_db_source_1.normalizeDbId)(mission.area_id);
    ...
    const sourceRow = (occurrence) => {
        ...
        if (!row)
            throw new Error(`${occurrence.table} is missing source row ${occurrence.rowId}`);
        return row;
    };
    ...
    const mapsByQuest = new Map();
    for (const map of options.tables.sugoroku_maps ?? []) {
        const questId = (0, game_db_source_1.normalizeDbId)(map.quest_id);
        if (!questId)
            continue;
    ...
    const firstRangesBySet = new Map();
    ...
    const reconciliations = [
        ...
        reconcileStageWallpaperSurface({
            ...
            expected: previewSources.flatMap(item => {
                ...
                return (mapsByQuest.get(questId) ?? []).flatMap(map => {
                    if (!(0, game_db_stage_drop_preview_1.stageDropPreviewDifficultiesForMap)(row, map))
                        return [];
                    return [{
                            source: item,
        ...
        reconcileStageWallpaperSurface({
            ...
            expected: normalSources.flatMap(item => {
                ...
                if (!table)
                    throw new Error(`Z-Battle normal reward ${item.rowId} references missing table ${tableId}`);
    ...
    const classifiedOutside = outside.map(item => {
        if (item.table === "mission_category_rewards")
            return { ...item, disposition: "surface-missing" };
        if (item.table === "rmbattle_mission_rewards")
            return { ...item, disposition: "contract-missing" };
        if (item.table === "mission_rewards")
            return { ...item, disposition: "out-of-scope" };
    ...
    const unboundSupportedCount = classifiedOutside.filter(item => ["sugoroku_map_boss_drop_items", "quest_drop_item_views", "z_battle_normal_rewards", "z_battle_first_rewards"].includes(item.table)).length;
    const outsideClassifications = [
    ...
    if (unboundSupportedCount > 0)
        outsideClassifications.push({
    ...
    return {
        ...
        wallpaper: {
            ...
            quantitiesSha256: (0, crypto_1.createHash)("sha256").update(quantityEvidence.join("\n")).digest("hex"),
...
exports.buildRewardItemAudit = buildRewardItemAudit;
async function main() {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!["--source-data-dir", "--source-snapshot-version", "--source-database-sha256", "--stage-dataset", "--wallpaper-manifest", "--output", "--generated-at"].includes(name))
            throw new Error(`Unexpected reward audit argument: ${name}`);
        const item = separator >= 0 ? args[index].slice(separator + 1) : args[++index];
        if (!item || values.has(name))
            throw new Error(`Missing or duplicate reward audit argument: ${name}`);
        values.set(name, item);
    }
    for (const required of ["--source-data-dir", "--source-snapshot-version", "--source-database-sha256", "--stage-dataset", "--wallpaper-manifest", "--output"])
        if (!values.has(required))
            throw new Error(`Missing reward audit argument: ${required}`);
    const source = (0, path_1.resolve)(values.get("--source-data-dir"));
    const tables = Object.fromEntries(await Promise.all(exports.REWARD_ITEM_AUDIT_TABLES.map(async (table) => [table, await (0, game_db_source_1.readGameDbTable)({ sourceRoot: source, dataDir: source }, table)])));
    const stageDataset = JSON.parse(await (0, promises_1.readFile)((0, path_1.resolve)(values.get("--stage-dataset")), "utf8"));
    const wallpaperManifest = (0, game_db_wallpaper_assets_1.validateWallpaperAssetManifest)(JSON.parse(await (0, promises_1.readFile)((0, path_1.resolve)(values.get("--wallpaper-manifest")), "utf8")));
    if (wallpaperManifest.source.databaseSnapshotVersion !== values.get("--source-snapshot-version")
        || wallpaperManifest.source.databaseSha256 !== values.get("--source-database-sha256"))
        throw new Error("Wallpaper asset manifest does not match reward audit provenance");
    ...
    await (0, promises_1.mkdir)((0, path_1.dirname)(output), { recursive: true });
    await (0, promises_1.writeFile)(output, `${JSON.stringify(audit, null, 2)}\n`, { encoding: "utf8", flag: "wx" });
...
//# sourceMappingURL=game-db-reward-item-audit.js.map