...
describe("game DB manual database artifact acquisition", function () {
        ...
        try {
            ...
            for (const member of ["database.db", "metadata.json", "commit-marker.json"]) {
                ...
                try {
                    ...
                    (0, assert_1.equal)((0, fs_1.readFileSync)(first.latestPointerPath, "utf8"), pointerBefore, member);
                }
                finally {
                    (0, fs_1.rmSync)(root, { recursive: true, force: true });
                }
    ...
    it("rolls latest back when the pointed commit is corrupted immediately after pointer installation", async () => {
        ...
        try {
            ...
            const pointerBefore = (0, fs_1.readFileSync)(first.latestPointerPath, "utf8");
            const signal = boundarySignal(() => {
                if (corrupted || !(0, fs_1.existsSync)(first.latestPointerPath))
                    return false;
                let pointer;
                try {
                    pointer = latest(root);
                ...
                if (pointer.currentIdentity !== seed.identity)
                    return false;
    ...
    it("fails closed for nlink greater than one, writable final members and a corrupt latest consumer", async () => {
        for (const variant of ["nlink", "writable", "latest-corrupt"]) {
            ...
            try {
                ...
                await (0, assert_1.rejects)((0, game_db_download_database_artifact_1.validateAcquiredDatabaseArtifact)(options), /hard link|read-only|marker|commit|identity/i, variant);
            }
            finally {
                (0, fs_1.rmSync)(root, { recursive: true, force: true });
            }
            ...
            for (const variant of variants) {
                ...
                try {
                    ...
                    if (variant.valid) {
                        ...
                        (0, assert_1.deepEqual)(latest(root), { schemaVersion: 1, contract: "dokkan-game-db-local-latest", contractVersion: "1.0.0", currentIdentity: seed.identity, previousIdentity: first.identity });
                    }
                    else {
    ...
    it("rejects arbitrary, missing, cyclic and corrupt latest references without replacing the pointer", async () => {
        const variants = [
            ...
            { name: "corrupt current metadata", mutate(root, _first, second) { const path = (0, path_1.join)(root, "artifacts", second.identity, "metadata.json"); (0, fs_1.chmodSync)(path, 0o644); (0, fs_1.writeFileSync)(path, "{}\n"); } },
            { name: "corrupt previous marker", mutate(root, first) { const path = (0, path_1.join)(root, "artifacts", first.identity, "commit-marker.json"); (0, fs_1.chmodSync)(path, 0o644); (0, fs_1.writeFileSync)(path, "{}\n"); } },
        ];
        for (const variant of variants) {
            const root = temp();
            try {
