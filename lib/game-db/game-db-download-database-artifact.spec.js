            ...
            const variants = [
                ...
                { name: "valid same-identity commit", valid: true, introduce(target) {
                        ...
                        for (const name of ["database.db", "metadata.json", "commit-marker.json"])
                            (0, fs_1.copyFileSync)((0, path_1.join)(seedRoot, "artifacts", seed.identity, name), (0, path_1.join)(target, name));
                    } },
            ...
            for (const variant of variants) {
                ...
                try {
                    ...
                    if (variant.valid) {
                        ...
                        await (0, assert_1.rejects)((0, game_db_download_database_artifact_1.acquireDatabaseArtifact)({ descriptor: descriptor(), storeRoot: root, transport: fakeTransport(nextBytes), signal }), /destination|directory|members|metadata|regular|symlink|junction|reparse/i, variant.name);
