...
exports.buildGameDbLaneRefreshCandidate = exports.applyAdditiveCategoryAssignments = exports.resolveOfficialCategoryAssignments = exports.LANE_REFRESH_GAME_DB_TABLES = exports.parseGameDbLaneRefreshArgs = exports.buildCreatedDomainEnrichedLaneCharacterArtifact = exports.mergeReleaseProjections = void 0;
...
function mergeReleaseProjections(releaseRoots, relatedForms) {
    const merged = new Map();
    for (const projection of releaseRoots) {
        if (merged.has(projection.id))
            throw new Error(`duplicate release-root projection ${projection.id}`);
...
exports.mergeReleaseProjections = mergeReleaseProjections;
function buildCreatedDomainEnrichedLaneCharacterArtifact(inputCharacters, projection, generatedAt) {
    const enrichment = (0, game_db_dokkan_field_created_domain_1.applySnapshotAuditedCreatedDomainsToCharacters)(inputCharacters, projection);
    ...
    return {
        ...
        artifact: (0, dataset_artifacts_1.buildCharacterDatasetArtifact)(enrichment.characters, {
...
exports.buildCreatedDomainEnrichedLaneCharacterArtifact = buildCreatedDomainEnrichedLaneCharacterArtifact;
...
exports.LANE_REFRESH_GAME_DB_TABLES = [...new Set([
        ...game_db_experiment_1.REQUIRED_GAME_DB_TABLES,
...
function loadInventoriedGameDbTables(inventory) {
    return Object.fromEntries(exports.LANE_REFRESH_GAME_DB_TABLES.map(tableName => {
...
async function buildGameDbLaneRefreshCandidate(options) {
    ...
    const createdDomainProjection = (0, game_db_dokkan_field_created_domain_1.buildCurrentSnapshotAuditedCreatedDomainProjection)({
        sourceSnapshotId: `glb-db-${metadata.dbVersion}`,
        fieldTables: Object.fromEntries(game_db_dokkan_field_sidecar_1.DOKKAN_FIELD_SIDECAR_TABLES.map(table => [
            table,
            tables[table],
        ])),
    ...
    const releaseRootProjections = requestedProjections.filter(projection => options.releaseStateCardIds.includes(projection.id));
    const newRelatedIds = [...new Set(newProjections.flatMap(projection => projection.transformations.map(item => item.id)))];
    const releaseRelatedIds = [...new Set(releaseRootProjections.flatMap(projection => projection.transformations.map(item => item.id)))];
    ...
    const releaseProjections = mergeReleaseProjections(releaseRootProjections, relatedProjections.filter(projection => releaseRelatedIdSet.has(projection.id)));
    ...
    const passiveModeFormIds = (0, game_db_character_passive_mode_overlay_1.selectDeliveredPassiveModeFormIds)(passiveModeInputCharacters, tables);
    const passiveModeProjections = passiveModeFormIds.length > 0
        ? (0, game_db_app_projection_1.projectGameDbCharactersToDokkanpanion)((0, game_db_experiment_1.buildGameDbCharacterSnapshots)(passiveModeFormIds, tables), { sourceVersion: metadata.dbVersion })
        : [];
    const passiveModes = (0, game_db_character_passive_mode_overlay_1.overlayGameDbCharacterPassiveModes)(passiveModeInputCharacters, passiveModeProjections);
    const generatedAt = new Date().toISOString();
    const laneCharacters = buildCreatedDomainEnrichedLaneCharacterArtifact(passiveModes.characters, createdDomainProjection, generatedAt);
