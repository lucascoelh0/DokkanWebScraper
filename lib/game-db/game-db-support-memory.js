...
exports.SUPPORT_MEMORY_FIRST_PARTY_CONTRACT_VERSION = "1.0.0";
...
function officialMissionSource(join, legacy) {
    return {
        ...(legacy ? clone(legacy) : {}),
...
function mergeMissionAcquisition(legacy, itemType, itemId, requiredQuantity, joins) {
    const official = joins.filter(join => join.itemType === itemType && join.itemId === itemId);
    if (official.length === 0)
        return legacy ? clone(legacy) : undefined;
    const legacyByRewardId = new Map();
    for (const source of legacy?.sources ?? []) {
        const parts = source.sourceKey.split(":");
        if (parts[0] === "event-mission" && parts.length >= 5)
            legacyByRewardId.set(parts[3], source);
    }
