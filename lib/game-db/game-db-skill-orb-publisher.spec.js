...
const CANDIDATE = (0, path_1.resolve)("data/skill-orbs/candidate-sko01-1788329250-v5");
...
async function realStoreWithExpected(mode = "sko01-preflight") {
    ...
    for (const object of candidate.objects) {
        const absent = mode === "sko01-preflight" && (object.kind === "manifest" || object.kind === "payload"
...
async function storeWithExpected(mode = "sko01-preflight") {
...
describe("SKO-02 Skill Orb publisher", function () {
    ...
    realCandidateIt("rejects snapshot, version, hash and size drift", async () => {
        for (const mutate of [
            ...
            try {
                ...
                await (0, assert_1.rejects)((0, game_db_skill_orb_publisher_1.validateSkillOrbCandidate)(root), /pinned SKO-01 release/);
            }
            finally {
    ...
    realCandidateIt("reproduces the 194 reused plus four new read-only plan with no writes", async () => {
        ...
        (0, assert_1.equal)(report.remote.newNetBytes, 306602);
        (0, assert_1.equal)(report.remote.futureWriteBytes, 306602);
