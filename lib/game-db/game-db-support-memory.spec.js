"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const assert_1 = require("assert");
const mocha_1 = require("mocha");
const game_db_support_memory_1 = require("./game-db-support-memory");
const row = (values) => Object.fromEntries(Object.entries(values).map(([key, value]) => [key, value === null ? "" : String(value)]));
function fixture() {
    return {
        cards: [
            row({ id: 1, card_unique_info_id: 1 }), row({ id: 2, card_unique_info_id: 2 }),
            row({ id: 3, card_unique_info_id: 3 }), row({ id: 4, card_unique_info_id: 4 }),
        ],
        card_card_categories: [
            row({ id: 1, card_id: 1, card_category_id: 16 }),
            row({ id: 2, card_id: 2, card_category_id: 48 }),
            row({ id: 3, card_id: 3, card_category_id: 98 }),
            row({ id: 4, card_id: 4, card_category_id: 16 }),
            row({ id: 5, card_id: 4, card_category_id: 48 }),
        ],
        card_categories: [
            row({ id: 16, name: "Movie Bosses" }), row({ id: 48, name: "Revenge" }), row({ id: 98, name: "DAIMA" }),
        ],
        card_unique_info_set_relations: [row({ id: 1, card_unique_info_set_id: 98, card_unique_info_id: 3 })],
        mission_categories: [row({ id: 798, name: "Broly missions" })],
        mission_rewards: [row({ id: 43837, mission_id: 25331, item_id: 50015, item_type: "SupportMemory", quantity: 100 })],
        missions: [row({
                id: 25331, mission_category_id: 798, name: "Official mission", description: "Official description",
                start_at: "2024-09-20 06:00:00", end_at: "2038-01-01 00:00:00",
                conditions: '{"sugoroku_map_ids":[1302001]}', area_id: 130, z_battle_stage_id: 42,
            })],
        sub_target_type_sets: [row({ id: 31 }), row({ id: 618 }), row({ id: 619 })],
        sub_target_types: [
            row({ id: 1, sub_target_type_set_id: 31, target_value_type: 1, target_value: 16 }),
            row({ id: 2, sub_target_type_set_id: 618, target_value_type: 2, target_value: 16 }),
            row({ id: 3, sub_target_type_set_id: 618, target_value_type: 1, target_value: 48 }),
            row({ id: 4, sub_target_type_set_id: 619, target_value_type: 4, target_value: 98 }),
        ],
        support_films: [row({ id: 5, name: "Film (Green)", description: "Green film" })],
        support_memories: [row({
                id: 50015, name: "Broly's Inner Power", description: "Official memory text", support_film_id: 5,
                cost: 200, unlock_quantity: 100, open_at: "2024-09-20 06:00:00",
            })],
        support_memory_enhancement_items: [],
        support_memory_enhancement_levels: [],
        support_memory_enhancement_require_items: [],
        support_memory_skills: [
            row({ id: 500151, support_memory_id: 50015, efficacy_type: 3, efficacy_values: "[10,10,0]", target_type: 2, calc_option: 2, turn: -1, probability: 100, sub_target_type_set_id: 31 }),
            row({ id: 500152, support_memory_id: 50015, efficacy_type: 3, efficacy_values: "[10,10,0]", target_type: 2, calc_option: 2, turn: -1, probability: 100, sub_target_type_set_id: 618 }),
            row({ id: 500153, support_memory_id: 50015, efficacy_type: 5, efficacy_values: "[1,0,0]", target_type: 2, calc_option: 0, turn: 7, probability: 100, sub_target_type_set_id: 619 }),
        ],
    };
}
function previous() {
    return {
        generatedAt: "old",
        source: "dokkan.fyi",
        count: 1,
        entries: [{
                id: "50015", name: "Old", description: "Old", maxLevel: 1, enhancementChain: [], effects: [],
                categoryIds: ["16", "48", "98"], categoryNames: ["DAIMA", "Movie Bosses", "Revenge"],
                applicableCharacterIds: ["1"], unlockMethod: "direct-item",
                dokkanInfo: { detailUrl: "https://dokkan.fyi/old", levelDescriptions: [], enhancementItems: [] },
                unlockAcquisition: {
                    itemKey: "SupportMemory:50015", itemType: "SupportMemory", itemId: "50015", sourceModel: "acquisition-item",
                    requiredQuantity: 1, groupCount: 1, sourceCount: 1,
                    groups: [{ groupKey: "event-mission-category:798", groupKind: "event-mission-category", title: "Old mission", sourceCount: 1, sourceKeys: ["event-mission:798:25331:43837:50015"] }],
                    sources: [{ sourceKey: "event-mission:798:25331:43837:50015", sourceKind: "event-mission", groupKey: "event-mission-category:798", groupKind: "event-mission-category", title: "Old mission", description: "Old description", quantity: 1 }],
                },
            }],
    };
}
const options = (tables) => ({
    generatedAt: "2026-08-31T12:00:00.000Z",
    sourceSnapshotVersion: "1787900894",
    sourceDatabaseSha256: "a".repeat(64),
    tables,
    previousDataset: previous(),
});
(0, mocha_1.describe)("Support Memory first-party candidate", function () {
    (0, mocha_1.it)("uses structural target sets, removes the false DAIMA relation, and overlays official mission facts", () => {
        const candidate = (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)(options(fixture()));
        const memory = candidate.dataset.entries[0];
        (0, assert_1.equal)(candidate.dataset.source, "dokkan-game-db");
        (0, assert_1.deepEqual)(memory.categoryIds, ["16", "48"]);
        (0, assert_1.deepEqual)(memory.categoryNames, ["Movie Bosses", "Revenge"]);
        (0, assert_1.equal)(memory.categoryTargetSource, "game-db-structural");
        (0, assert_1.deepEqual)(memory.applicableCharacterIds, ["1", "2", "3", "4"]);
        (0, assert_1.equal)(memory.applicableCharacterSource, "game-db-structural");
        (0, assert_1.equal)(memory.lastsEntireBattle, true);
        (0, assert_1.equal)(memory.unlockAcquisition?.sources[0].title, "Official mission");
        (0, assert_1.equal)(memory.unlockAcquisition?.sources[0].quantity, 100);
        (0, assert_1.equal)(memory.unlockAcquisition?.groups[0].title, "Broly missions");
        (0, assert_1.deepEqual)(memory.unlockAcquisition?.sources[0].officialStageRelations, [
            { targetKind: "area", targetId: "130", relation: "mission-owner" },
            { targetKind: "quest-level", targetId: "1302001", relation: "direct-stage-condition" },
            { targetKind: "z-battle", targetId: "42", relation: "mission-owner" },
        ]);
        (0, assert_1.equal)(memory.dokkanInfo, undefined);
        (0, assert_1.deepEqual)(candidate.audit.compatibility.categoryChangedIds, ["50015"]);
        (0, assert_1.equal)(candidate.audit.entries[0].targetRules.length, 3);
        (0, assert_1.equal)(candidate.audit.counts.officialPresentationCount, 0);
    });
    (0, mocha_1.it)("never copies acquisition or presentation fields from the comparison dataset", () => {
        const tables = fixture();
        tables.mission_rewards = [];
        tables.missions = [];
        const candidate = (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)(options(tables));
        const memory = candidate.dataset.entries[0];
        (0, assert_1.equal)(memory.unlockMethod, "film-only");
        (0, assert_1.equal)(memory.unlockAcquisition, undefined);
        (0, assert_1.equal)(memory.dokkanInfo, undefined);
        (0, assert_1.equal)(candidate.audit.counts.unresolvedUnlockMemoryCount, 1);
    });
    (0, mocha_1.it)("resolves transitive official stage relations without text parsing", () => {
        const tables = fixture();
        tables.missions[0].conditions = '{"mission_ids":[25330]}';
        tables.missions[0].area_id = "";
        tables.missions[0].z_battle_stage_id = "";
        tables.missions.push(row({
            id: 25330, mission_category_id: 798, name: "Required mission", description: "Required",
            conditions: '{"sugoroku_map_ids":[1302002]}', area_id: null, z_battle_stage_id: null,
        }));
        const memory = (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)(options(tables)).dataset.entries[0];
        (0, assert_1.deepEqual)(memory.unlockAcquisition?.sources[0].officialStageRelations, [
            { targetKind: "quest-level", targetId: "1302002", relation: "transitive-mission-condition" },
        ]);
    });
    (0, mocha_1.it)("accepts only a complete official game-asset presentation map", () => {
        const presentation = {
            detailUrl: "",
            levelDescriptions: [{ level: 1, description: "Official memory text" }],
            enhancementItems: [],
        };
        const candidate = (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)({
            ...options(fixture()),
            presentations: new Map([["50015", presentation]]),
        });
        (0, assert_1.equal)(candidate.dataset.entries[0].presentationSource, "game-assets");
        (0, assert_1.deepEqual)(candidate.dataset.entries[0].dokkanInfo, presentation);
        (0, assert_1.equal)(candidate.audit.counts.officialPresentationCount, 1);
        (0, assert_1.throws)(() => (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)({
            ...options(fixture()),
            presentations: new Map(),
        }), /Missing official Support Memory presentation 50015/);
    });
    (0, mocha_1.it)("fails closed for unknown structural target semantics", () => {
        const tables = fixture();
        tables.sub_target_types[0].target_value_type = "3";
        (0, assert_1.throws)(() => (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)(options(tables)), /Unsupported Support Memory sub-target value type/);
    });
    (0, mocha_1.it)("fails closed for malformed effect values", () => {
        const tables = fixture();
        tables.support_memory_skills[0].efficacy_values = "not-json";
        (0, assert_1.throws)(() => (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)(options(tables)), /Invalid efficacy_values JSON/);
    });
    (0, mocha_1.it)("fails closed for a broken enhancement chain", () => {
        const tables = fixture();
        tables.support_memories.push(row({ id: 500152, name: "Enhanced", description: "Enhanced", support_film_id: 5, cost: 200, unlock_quantity: 1, open_at: "2024-09-20 06:00:00" }));
        tables.support_memory_enhancement_levels.push(row({ id: 9, root_support_memory_id: 50015, level: 3, support_memory_id: 50015, enhanced_support_memory_id: 500152 }));
        (0, assert_1.throws)(() => (0, game_db_support_memory_1.buildSupportMemoryFirstPartyCandidate)(options(tables)), /Broken Support Memory enhancement chain/);
    });
});
//# sourceMappingURL=game-db-support-memory.spec.js.map