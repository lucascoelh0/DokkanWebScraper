...
exports.validateStageDeliveryRoutes = exports.buildStageDelivery = exports.DEFAULT_STAGE_ASSET_BASE_URL = exports.DEFAULT_STAGE_SHARD_MAX_EXPANDED_BYTES = exports.STAGE_DELIVERY_CONTRACT_VERSION = exports.STAGE_DELIVERY_CONTRACT = void 0;
const crypto_1 = require("crypto");
const zlib_1 = require("zlib");
exports.STAGE_DELIVERY_CONTRACT = "dokkan-stage-delivery";
exports.STAGE_DELIVERY_CONTRACT_VERSION = "1.1.0";
exports.DEFAULT_STAGE_SHARD_MAX_EXPANDED_BYTES = 2 * 1024 * 1024;
exports.DEFAULT_STAGE_ASSET_BASE_URL = "https://assets.dokkanstats.com/assets/global/en";
function buildStageDelivery(dataset, shardMaxExpandedBytes = exports.DEFAULT_STAGE_SHARD_MAX_EXPANDED_BYTES, compressionLevel = 9, assetBaseUrl = exports.DEFAULT_STAGE_ASSET_BASE_URL) {
    ...
    const shards = pendingShards.map(shard => {
        ...
        for (const item of shard.items) {
            ...
            if (shardIdByKey.has(key))
                throw new Error(`Duplicate Stage delivery route ${key}`);
    ...
    const catalog = {
        schemaVersion: 1,
        contract: exports.STAGE_DELIVERY_CONTRACT,
        contractVersion: exports.STAGE_DELIVERY_CONTRACT_VERSION,
        datasetVersion,
        ...
        entries: [
            ...
            ...(dataset.zBattles ?? []).map(stage => zBattleCatalogEntry(stage, relations, requireShardId(shardIdByKey, "z-battle", stage.id))),
        ],
...
exports.buildStageDelivery = buildStageDelivery;
function buildCharacterDrops(dataset) {
    const questEntriesByIdentity = new Map();
    ...
    const drops = new Map();
    const add = (stageId, rawReward) => {
        if (rawReward.itemType !== "Card")
            return;
...
function characterReward(reward) {
    return {
...
function buildAwakeningMedalAcquisitionSources(dataset) {
    const questEntriesByIdentity = new Map();
    for (const stage of dataset.entries) {
        const key = `${stage.areaId}:${stage.questId}`;
        questEntriesByIdentity.set(key, [...(questEntriesByIdentity.get(key) ?? []), stage]);
    }
    const stageDropTargets = new Map();
    const addStageDrop = (medalId, stage, stageIds) => {
        const key = [medalId, stage.areaId, stage.questId].join(":");
        const targets = stageDropTargets.get(key) ?? new Set();
        stageIds.forEach(stageId => targets.add(stageId));
        stageDropTargets.set(key, targets);
    };
    for (const stage of dataset.entries) {
        for (const drop of stage.bossDrops ?? []) {
            if (drop.itemType === "AwakeningItem")
                addStageDrop(drop.itemId, stage, [stage.id]);
        }
        for (const preview of stage.dropPreviews ?? []) {
            for (const item of preview.items) {
                if (item.itemType !== "AwakeningItem")
                    continue;
                const stageIds = (questEntriesByIdentity.get(`${stage.areaId}:${stage.questId}`) ?? [])
                    .filter(candidate => candidate.difficultyRaw != null
                    && preview.difficultyValues.includes(candidate.difficultyRaw))
    ...
    for (const zBattle of dataset.zBattles ?? []) {
        for (const rewardLevel of zBattle.firstRewards ?? []) {
            for (const reward of rewardLevel.rewards) {
                if (reward.itemType !== "AwakeningItem")
                    continue;
    ...
    const groupedZBattleRows = new Map();
    for (const row of zBattleRows) {
        const key = [row.kind, row.medalId, row.targetId].join(":");
        const levelsByQuantity = groupedZBattleRows.get(key) ?? new Map();
    ...
    const zBattleSources = [...groupedZBattleRows.entries()].map(([key, levelsByQuantity]) => {
        const [kind, medalId, targetId] = key.split(":");
        return {
            kind,
            medalId,
            targetKind: "z-battle",
...
function deliveryItems(dataset) {
    const questItems = [...dataset.entries]
    ...
    const zBattleItems = [...(dataset.zBattles ?? [])]
...
function partitionItems(items, datasetVersion, maximumBytes) {
    ...
    for (const item of items) {
        ...
        if (pending.length > 0 && estimatedBytes + separatorBytes + item.serializedBytes > maximumBytes)
            flush();
...
function questCatalogEntry(stage, shardId) {
    ...
    return {
        ...
        hasRewards: Boolean(stage.bossDrops?.length
...
function questChapterImagePath(chapterId) {
    if (!/^\d+$/.test(chapterId))
        throw new Error(`Quest chapter ID must be numeric: ${chapterId}`);
    return `outgame/extension/adventure/chapter/${chapterId}/${chapterId}001.png`;
}
function questBrowseCategory(stage) {
    if (stage.areaType === "Area::MainArea" && stage.chapter?.id)
        return "quests";
    if (stage.areaType === "Area::DbStory")
        return "db-story";
    if (stage.areaType !== "Area::EventArea")
        return undefined;
    if ([2, 7].includes(stage.areaCategoryRaw))
        return "story";
    if ([4, 6].includes(stage.areaCategoryRaw))
        return "bonus";
    if ([1, 8, 9, 10, 11, 12].includes(stage.areaCategoryRaw))
        return "growth";
    if (stage.areaCategoryRaw === 16)
        return "limited";
    if (stage.areaCategoryRaw === 20)
        return "challenge";
    return undefined;
}
function zBattleCatalogEntry(stage, relations, shardId) {
...
function validateStageDeliveryRoutes(catalog, manifest) {
    const shards = new Map(manifest.shards.map(shard => [shard.id, shard]));
    for (const entry of catalog.entries) {
        const shard = shards.get(entry.detailShardId);
        if (!shard)
            throw new Error(`Stage catalog entry ${entry.key} references a missing shard`);
        const ids = entry.kind === "quest-level" ? shard.questLevelIds : shard.zBattleIds;
        if (!ids.includes(entry.id))
            throw new Error(`Stage catalog entry ${entry.key} is absent from its shard manifest`);
    }
    ...
    const questEntriesById = new Map(catalog.entries.filter(entry => entry.kind === "quest-level").map(entry => [entry.id, entry]));
    ...
    for (const relation of catalog.supportMemoryRelations) {
        ...
        if (!resolved)
            throw new Error(`Support Memory relation ${relationKey(relation)} has no catalog target`);
    }
    ...
    const characterDropKeys = new Set();
    for (const drop of characterDrops) {
        ...
        if (characterDropKeys.has(key))
            throw new Error(`Duplicate Character drop ${key}`);
    ...
    for (const source of catalog.awakeningMedalSources) {
        ...
        if (sourceKeys.has(key))
            throw new Error(`Duplicate Awakening Medal acquisition source ${key}`);
        sourceKeys.add(key);
    }
}
exports.validateStageDeliveryRoutes = validateStageDeliveryRoutes;
function requireShardId(map, kind, id) {
    const key = catalogKey(kind, id);
    const shardId = map.get(key);
    if (!shardId)
        throw new Error(`Stage delivery route ${key} was not partitioned`);
    return shardId;
}
...
function characterDropKey(drop) {
    return `${drop.stageId}:${JSON.stringify(characterReward(drop.reward))}`;
}
function compareCharacterDrops(left, right) {
...
function awakeningMedalSourceKey(source) {
    return [
        source.medalId,
        source.kind,
        source.targetKind,
...
function compareAwakeningMedalSources(left, right) {
...
function awakeningMedalRewardEntryKey(entry) {
    return `${entry.quantity}@${entry.levels.join(",")}`;
}
function compareAwakeningMedalRewardEntries(left, right) {
...
function uniqueText(values, comparator = (left, right) => left.localeCompare(right)) {
...
//# sourceMappingURL=game-db-stage-delivery.js.map