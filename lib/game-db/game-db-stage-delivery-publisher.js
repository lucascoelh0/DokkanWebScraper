...
exports.scopedObjectKey = exports.validateStageDeliveryCatalogPayload = exports.buildStageDeliveryPublishPlan = exports.parseStageDeliveryPublishArgs = exports.STAGE_DELIVERY_HTTP_CONTENT_TYPE = void 0;
...
async function validateObjectFile(releaseDir, object, manifest) {
    ...
    if (object.objectKey === manifest.catalog.objectKey) {
        validateStageDeliveryCatalogPayload(payload, manifest);
    }
    else {
...
function validateStageDeliveryCatalogPayload(catalog, manifest) {
    ...
    (0, game_db_stage_delivery_1.validateStageDeliveryRoutes)(catalog, manifest);
}
exports.validateStageDeliveryCatalogPayload = validateStageDeliveryCatalogPayload;
function rewardEntryKey(entry) {
    return `${entry.quantity}@${entry.levels.join(",")}`;
}
function compareRewardEntries(left, right) {
