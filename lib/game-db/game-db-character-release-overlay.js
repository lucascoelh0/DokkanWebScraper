...
function applyTransformationProjection(transformation, projection, baseCharacterId, inheritedReleaseDates) {
    ...
    if (projection.ezaPassive) {
        ...
        transformation.ezaPassiveDetails = (0, game_db_transformation_passive_details_1.rebindTransformationPassiveDetails)(projection.ezaPassiveDetails, baseCharacterId, projection.id);
    ...
    if (projection.sezaPassive) {
        ...
        transformation.sezaPassiveDetails = (0, game_db_transformation_passive_details_1.rebindTransformationPassiveDetails)(projection.sezaPassiveDetails, baseCharacterId, projection.id);
...
function overlayGameDbCharacterReleaseStates(baselineCharacters, projections, targetCardIds) {
    ...
    for (const cardId of targetIds) {
        ...
        const fields = applyProjection(character, projectionById.get(cardId));
        for (const transformation of character.transformations ?? []) {
            const transformationProjection = projectionById.get(transformation.id);
            if (!transformationProjection)
                continue;
            const transformationFields = applyTransformationProjection(transformation, transformationProjection, cardId, character);
