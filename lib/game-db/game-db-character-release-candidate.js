...
exports.buildGameDbCharacterReleaseCandidate = exports.assertTeamAnalysisBoundToCharacterArtifact = exports.reparseCharacterLeaderSkillDetails = exports.assertFreshCandidateOutput = exports.parseGameDbCharacterReleaseCandidateArgs = exports.GAME_DB_CHARACTER_RELEASE_CANDIDATE_ROOT = void 0;
...
function reparseCharacterLeaderSkillDetails(characters) {
    const seenIds = new Set();
    const changedCharacterIds = [];
    const leaderSkillDetailsChangedCharacterIds = [];
    const ezaLeaderSkillDetailsChangedCharacterIds = [];
    const reparsed = characters.map(character => {
        ...
        if (typeof character.leaderSkill === "string" && character.leaderSkill.trim()) {
            const details = (0, scraper_1.parseLeaderSkillDetails)(character.leaderSkill);
        ...
        if (typeof character.ezaLeaderSkill === "string" && character.ezaLeaderSkill.trim()) {
            const details = (0, scraper_1.parseLeaderSkillDetails)(character.ezaLeaderSkill);
...
exports.reparseCharacterLeaderSkillDetails = reparseCharacterLeaderSkillDetails;
function characterWithoutLeaderSkillDetails(character) {
    return Object.fromEntries(Object.entries(character).filter(([key]) => key !== "leaderSkillDetails" && key !== "ezaLeaderSkillDetails"));
}
function disabledLeaderSkillReparse(characters) {
    return {
        characters,
        report: {
...
function noReleaseStateOverlay(characters) {
    return {
        characters,
        patches: [],
        checks: {
...
async function buildGameDbCharacterReleaseCandidate(options) {
    ...
    const overlay = options.cardIds.length > 0
        ? (0, game_db_character_release_overlay_1.overlayGameDbCharacterReleaseStates)(baseline.characters, (0, game_db_app_projection_1.projectGameDbCharactersToDokkanpanion)((0, game_db_experiment_1.buildGameDbCharacterSnapshots)(options.cardIds, tables), { sourceVersion: metadata.dbVersion }), options.cardIds)
    ...
    const artifact = (0, dataset_artifacts_1.buildCharacterDatasetArtifact)(leaderSkillReparse.characters, {
