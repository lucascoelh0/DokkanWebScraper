...
function rewardPresentation(itemType, itemId, cards, canonicalAwakenedCards, treasureItems, linkSkillLvUpItems, equipmentSkillItems, equipmentSkillPresentations, wallpaperItems, wallpaperPresentations) {
    if (itemType === "Card") {
    ...
    if (itemType === "LinkSkillLvUpItem") {
        ...
        if (!item)
            throw new Error(`Stage reward references missing Link Skill level-up item ${itemId}`);
    ...
    if (itemType === "EquipmentSkillItem") {
        ...
        if (!equipmentSkill)
            throw new Error(`Stage reward references missing equipment skill presentation ${itemId}`);
        return {
            ...(text(item.name) ? { name: text(item.name) } : {}),
    ...
    if (itemType === "WallpaperItem") {
        ...
        if (!item)
            throw new Error(`Stage reward references missing wallpaper item ${itemId}`);
        const name = text(item.name);
        const description = text(item.description);
        if (!name || !description) {
        ...
        if (!presentation)
            throw new Error(`Stage reward wallpaper ${itemId} lacks verified official assets`);
...
function equipmentLevelAssetPath(levels) {
...
function exactConditionIds(conditions, key, context) {
...
function equipmentRestriction(row, cardCategories, cards, uniqueInfoRelationsBySet, uniqueInfos) {
    ...
    if (rawType === "EquipmentSkillLimitation::CardCategoryLimitation") {
        ...
        if (names.some(name => !name))
            throw new Error(`Equipment limitation ${sourceRowId} references an unnamed category`);
    ...
    if (rawType === "EquipmentSkillLimitation::CardLimitation") {
        ...
        if (names.some(name => !name))
            throw new Error(`Equipment limitation ${sourceRowId} references an unnamed card`);
    ...
    if (rawType === "EquipmentSkillLimitation::CardUniqueInfoSetLimitation") {
        ...
        const names = cardUniqueInfoSetIds.flatMap(uniqueSetId => {
            ...
            if (!relations?.length)
                throw new Error(`Equipment limitation ${sourceRowId} references missing card unique info set ${uniqueSetId}`);
        ...
        if (names.some(name => !name))
            throw new Error(`Equipment limitation ${sourceRowId} references an unnamed character identity`);
...
function equipmentSkillPresentations(tables, equipmentItems, cards, cardCategories) {
    ...
    for (const [itemId, item] of equipmentItems) {
        ...
        if (skillRows.length === 0)
            throw new Error(`Equipment skill item ${itemId} has no skill rows`);
        const skills = skillRows.map(skillRow => {
            ...
            if (Boolean(potentialSkillId) === Boolean(statusType))
                throw new Error(`Equipment skill ${id(skillRow)} must identify exactly one effect`);
            if (statusType && statusType !== "hp" && statusType !== "attack" && statusType !== "defense")
                throw new Error(`Equipment skill ${id(skillRow)} has unsupported status_type ${statusType}`);
            const level = integer(skillRow, "level");
            if (level <= 0)
                throw new Error(`Equipment skill ${id(skillRow)} has invalid level ${level}`);
            const identity = potentialSkillId ? `potential:${potentialSkillId}` : `status:${statusType}`;
            const priority = displayPriority.get(identity);
            if (!priority)
                throw new Error(`Equipment skill ${id(skillRow)} has no proved native display priority for ${identity}`);
            return {
        ...
        if (!limitationSetId)
            throw new Error(`Equipment skill item ${itemId} has no limitation set`);
        const limitationRows = limitationsBySet.get(limitationSetId) ?? [];
        if (limitationRows.length === 0)
            throw new Error(`Equipment skill item ${itemId} limitation set ${limitationSetId} has no rows`);
    ...
    for (const skillRow of tables.equipment_skills)
        if (!equipmentItems.has(id(skillRow, "equipment_skill_item_id")))
            throw new Error(`Equipment skill ${id(skillRow)} references missing item`);
    return result;
}
...
function buildStageFirstPartyCandidate(options) {
    ...
    const wallpaperPresentations = options.wallpaperPresentations ?? new Map();
    ...
    for (const mapRow of [...maps.values()].sort((left, right) => numericCompare(id(left), id(right)))) {
        ...
        const dropPreviews = (dropViewsByQuest.get(questId) ?? []).flatMap(row => {
            const difficultyValues = (0, game_db_stage_drop_preview_1.stageDropPreviewDifficultiesForMap)(row, mapRow);
            if (!difficultyValues)
                return [];
        ...
        entries.push({
            ...
            bossDrops: (dropsByMap.get(mapId) ?? []).map(drop => ({
                ...
                ...rewardPresentation(text(drop.item_type), id(drop, "item_id"), cards, canonicalAwakenedCards, treasureItems, linkSkillLvUpItems, equipmentSkillItems, equipmentPresentations, wallpaperItems, wallpaperPresentations),
            })),
            dropPreviews,
            categoryBonuses,
    ...
    const rewardItem = (row) => ({
        ...
        ...rewardPresentation(text(row.item_type), id(row, "item_id"), cards, canonicalAwakenedCards, treasureItems, linkSkillLvUpItems, equipmentSkillItems, equipmentPresentations, wallpaperItems, wallpaperPresentations),
    });
