...
function rewardPresentation(itemType, itemId, cards, canonicalAwakenedCards, treasureItems, equipmentSkillItems, equipmentSkillPresentations) {
    if (itemType === "Card") {
...
function buildStageFirstPartyCandidate(options) {
    ...
    for (const mapRow of [...maps.values()].sort((left, right) => numericCompare(id(left), id(right)))) {
        ...
        entries.push({
            ...
            bossDrops: (dropsByMap.get(mapId) ?? []).map(drop => ({
                ...
                ...rewardPresentation(text(drop.item_type), id(drop, "item_id"), cards, canonicalAwakenedCards, treasureItems, equipmentSkillItems, equipmentPresentations),
            })),
            dropPreviews,
            categoryBonuses,
    ...
    const rewardItem = (row) => ({
        ...
        ...rewardPresentation(text(row.item_type), id(row, "item_id"), cards, canonicalAwakenedCards, treasureItems, equipmentSkillItems, equipmentPresentations),
    });
