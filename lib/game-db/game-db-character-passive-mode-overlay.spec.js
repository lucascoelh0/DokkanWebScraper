...
(0, mocha_1.describe)("game DB character passive-mode overlay", () => {
    ...
    (0, mocha_1.it)("treats a source snapshot version refresh as non-semantic when mode evidence is otherwise identical", () => {
        const baselineMode = mode("standard", "Same passive", "10");
        const projectedMode = JSON.parse(JSON.stringify(baselineMode));
        ...
        const result = (0, game_db_character_passive_mode_overlay_1.overlayGameDbCharacterPassiveModes)(baseline, [projection("10", {
                passiveDetails: { text: "Same passive", modes: [projectedMode] },
            })]);
        (0, assert_1.equal)(result.coverage.alreadyPresentStateCount, 1);
        (0, assert_1.equal)(result.coverage.patchedStateCount, 0);
        (0, assert_1.equal)(result.characters[0].passiveDetails?.modes?.[0].structuralSource?.evidence[0].provenance.sourceVersion, "1");
    });
