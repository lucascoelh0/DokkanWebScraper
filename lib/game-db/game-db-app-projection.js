...
function gameDbPassiveConditionEvidence(rawText, normalizedText, context) {
...
function passiveDescriptionDetails(passiveSkillSet, rawText, payloadField, context) {
...
function splitGameDbPassiveSections(rawText, normalizedLines) {
    const sections = [];
    let current;
    let foundStructuralHeader = false;
    const pushCurrent = () => {
        if (current && (current.label || current.lines.length > 0))
    ...
    for (const rawLine of rawText.split(/\r?\n/)) {
        ...
        if (structuralHeader) {
            pushCurrent();
            current = {
                label: (0, scraper_1.cleanMultilineText)(structuralHeader[1]) || undefined,
                lines: [],
            };
        ...
        if (bullet) {
            if (!current)
                current = { lines: [] };
            const line = (0, scraper_1.cleanMultilineText)(bullet[1]);
            if (line)
                current.lines.push(line);
            continue;
        }
        const continuation = (0, scraper_1.cleanMultilineText)(trimmed);
        if (!continuation)
            continue;
        if (!current)
            current = { lines: [] };
    ...
    pushCurrent();
    return foundStructuralHeader ? sections : (0, scraper_1.splitPassiveSections)(normalizedLines);
}
function passiveDetailsFromSkillSet(passiveSkillSet, context) {
    const basic = passiveDescriptionDetails(passiveSkillSet, passiveSkillSet?.itemizedDescription, "passive_skill_sets.itemized_description", context);
    if (!basic)
        return undefined;
    const standard = passiveDescriptionDetails(passiveSkillSet, passiveSkillSet?.groupItemizedDescription, "passive_skill_sets.group_itemized_description", context);
    const survival = passiveDescriptionDetails(passiveSkillSet, passiveSkillSet?.characterItemizedDescription, "passive_skill_sets.character_itemized_description", context);
    const modes = [];
    if (standard) {
        modes.push({
            mode: "standard",
