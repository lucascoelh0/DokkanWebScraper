...
exports.validateSkillOrbCatalog = exports.buildSkillOrbCatalog = exports.requiredSkillOrbAssetPaths = exports.computeAssetInventorySha256 = exports.ELEMENT_LIMITATION_BITS = exports.PINNED_SKILL_ORB_PROFILE = exports.SKILL_ORB_CONTRACT_VERSION = void 0;
const crypto_1 = require("crypto");
const game_db_source_1 = require("./game-db-source");
exports.SKILL_ORB_CONTRACT_VERSION = "1.2.0";
exports.PINNED_SKILL_ORB_PROFILE = {
...
exports.ELEMENT_LIMITATION_BITS = [
...
function requiredId(row, field = "id", context = field) {
    const value = (0, game_db_source_1.normalizeDbId)(row[field]);
    if (!value || !/^\d+$/.test(value) || Number(value) <= 0)
        throw new Error(`${context} must be a positive structural ID`);
    return value;
}
function optionalId(row, field) {
    const raw = text(row[field]);
    if (!raw)
        return undefined;
    const value = (0, game_db_source_1.normalizeDbId)(raw);
    if (!value || !/^\d+$/.test(value) || Number(value) <= 0)
        throw new Error(`${field} must be a positive structural ID`);
    return value;
}
...
function uniqueRows(rows, context, field = "id") {
    const result = new Map();
    for (const row of rows) {
        const rowId = requiredId(row, field, `${context}.${field}`);
        if (result.has(rowId))
            throw new Error(`Duplicate ${context} ID ${rowId}`);
        result.set(rowId, row);
    }
    return result;
}
function canonicalAwakeningOwners(routes, cards) {
    const visibleCardId = (cardId) => {
        const numeric = Number(cardId);
        if (!Number.isSafeInteger(numeric) || numeric <= 0)
            throw new Error(`Invalid card ID ${cardId}`);
        const releaseState = numeric % 10;
        if (releaseState !== 0 && releaseState !== 1)
            throw new Error(`Unsupported card release-state variant ${cardId}`);
    ...
    const representativeByVisibleId = new Map();
    for (const cardId of cards.keys()) {
        const visibleId = visibleCardId(cardId);
        const previous = representativeByVisibleId.get(visibleId);
        if (!previous || numericCompare(previous, cardId) < 0)
            representativeByVisibleId.set(visibleId, cardId);
    }
    ...
    for (const row of routes) {
        ...
        if (previous && previous !== targetId)
            throw new Error(`Ambiguous card awakening route for ${sourceId}`);
    ...
    for (const cardId of cards.keys()) {
        ...
        while (outgoing.has(current)) {
            if (!visited.add(current))
                throw new Error(`Cyclic card awakening route at ${current}`);
            current = outgoing.get(current);
        }
        const representative = representativeByVisibleId.get(current);
        if (!representative)
            throw new Error(`Missing canonical card representative for ${current}`);
        owners.set(cardId, representative);
    }
    return owners;
}
function groupRows(rows, field, context) {
    const result = new Map();
    for (const row of rows) {
...
function conditionIds(value, context) {
    if (!Array.isArray(value) || value.length === 0)
        throw new Error(`${context} must be a non-empty array`);
    const ids = value.map((entry, index) => {
        const normalized = (0, game_db_source_1.normalizeDbId)(String(entry ?? ""));
        if (!normalized || !/^\d+$/.test(normalized) || Number(normalized) <= 0)
            throw new Error(`${context}[${index}] must be a positive ID`);
        return normalized;
    });
    if (new Set(ids).size !== ids.length)
        throw new Error(`${context} must not contain duplicate IDs`);
    return ids;
}
function exactConditionIds(raw, key, context) {
    if (Object.keys(raw).length !== 1 || !(key in raw))
        throw new Error(`${context} has unsupported condition fields`);
    return conditionIds(raw[key], `${context}.${key}`).sort(numericCompare);
}
function conciseNames(names) {
    const unique = [...new Set(names)];
    if (unique.length <= 2)
        return unique.join(" / ");
    return `${unique.slice(0, 2).join(" / ")} +${unique.length - 2}`;
}
function levelAssetPath(effects) {
...
function buildCondition(row, cards, categories, uniqueInfos, uniqueInfoRelationsBySet, cardsByUniqueInfo, categoryRelationsByCategory, canonicalOwnerByCardId) {
    ...
    if (rawType === "EquipmentSkillLimitation::ElementLimitation") {
        ...
        const matched = exports.ELEMENT_LIMITATION_BITS.filter(value => (elementBitPattern & value.bit) !== 0);
        const knownMask = matched.reduce((sum, value) => sum + value.bit, 0);
        if (knownMask !== elementBitPattern)
            throw new Error(`Equipment limitation ${sourceRowId} has unknown element bitpattern ${elementBitPattern}`);
    ...
    if (rawType === "EquipmentSkillLimitation::CardCategoryLimitation") {
        ...
        const resolvedCategories = cardCategoryIds.map(id => {
            ...
            if (!row || !name)
                throw new Error(`Equipment limitation ${sourceRowId} references missing/unnamed category ${id}`);
            const relations = categoryRelationsByCategory.get(id);
            if (!relations?.length)
                throw new Error(`Equipment limitation ${sourceRowId} category ${id} has no first-party card relations`);
    ...
    if (rawType === "EquipmentSkillLimitation::CardLimitation") {
        ...
        const canonicalOwnerCardIds = cardIds.map(id => {
            ...
            if (!ownerId)
                throw new Error(`Equipment limitation ${sourceRowId} lacks an awakening owner for card ${id}`);
            return ownerId;
        });
        const resolvedCards = cardIds.map(id => {
            ...
            const name = text(row?.name);
            if (!row || !name)
                throw new Error(`Equipment limitation ${sourceRowId} references missing/unnamed card ${id}`);
            return { id, name };
        });
    ...
    if (rawType === "EquipmentSkillLimitation::CardUniqueInfoSetLimitation") {
        ...
        const resolvedCardUniqueInfoSets = cardUniqueInfoSetIds.map(id => {
            ...
            if (!relations?.length)
                throw new Error(`Equipment limitation ${sourceRowId} references missing unique-info set ${id}`);
            ...
            if (new Set(uniqueInfoIds).size !== uniqueInfoIds.length)
                throw new Error(`Unique-info set ${id} repeats an identity`);
            const names = uniqueInfoIds.map(uniqueInfoId => {
                const name = text(uniqueInfos.get(uniqueInfoId)?.name);
                if (!name)
                    throw new Error(`Unique-info set ${id} references missing/unnamed identity ${uniqueInfoId}`);
                return name;
            });
            ...
            if (!eligibleCardIds.length)
                throw new Error(`Unique-info set ${id} expands to no eligible cards`);
...
function addReverse(index, cardId, orbId) {
    const values = index.get(cardId) ?? new Set();
    values.add(orbId);
...
function computeAssetInventorySha256(assets) {
...
exports.computeAssetInventorySha256 = computeAssetInventorySha256;
function requiredSkillOrbAssetPaths(tables) {
    const effectRowsByItem = groupRows(tables.equipment_skills, "equipment_skill_item_id", "equipment_skills");
    const foregroundIcons = new Set();
    const gradeBackgrounds = new Set();
    const levelAssets = new Set();
    const infinityAssets = new Set();
    for (const item of tables.equipment_skill_items) {
        ...
        if (grade !== "bronze" && grade !== "silver" && grade !== "gold")
            throw new Error(`Equipment Skill Orb ${itemId} has unknown grade ${grade}`);
        ...
        const effects = (effectRowsByItem.get(itemId) ?? []).map(row => {
            ...
            if (!priority)
                throw new Error(`Equipment skill ${requiredId(row)} has unknown effect ${identity}`);
        ...
        if (!effects.length)
            throw new Error(`Equipment Skill Orb ${itemId} has no effects`);
        levelAssets.add(`derived/equipment/levels/lv-${effects.map(effect => effect.level).join("-")}.png`);
        const eternal = text(item.is_eternal);
        if (eternal !== "0" && eternal !== "1")
            throw new Error(`Equipment Skill Orb ${itemId} has unknown is_eternal ${eternal}`);
        if (eternal === "1")
            infinityAssets.add(`${UI_ROOT}/equ_infinite_icon_${grade}.png`);
    }
    const restrictionBadges = [
        ...exports.ELEMENT_LIMITATION_BITS.map(value => `layout/en/image/character/cha_type_icon_${value.assetCode}.png`),
        `${UI_ROOT}/equ_icon_category.png`, `${UI_ROOT}/equ_icon_same_chara.png`, `${UI_ROOT}/equ_icon_specific_chara.png`,
    ];
    const sort = (values) => [...values].sort((left, right) => left.localeCompare(right, "en", { numeric: true }));
    return {
...
exports.requiredSkillOrbAssetPaths = requiredSkillOrbAssetPaths;
function buildSkillOrbCatalog(options) {
    if (options.snapshotVersion !== exports.PINNED_SKILL_ORB_PROFILE.snapshotVersion)
        throw new Error(`Unsupported Skill Orb snapshot ${options.snapshotVersion}`);
    if (options.sourceDatabaseSha256.toLowerCase() !== exports.PINNED_SKILL_ORB_PROFILE.sourceDatabaseSha256)
        throw new Error("Skill Orb SQLite SHA-256 mismatch");
    ...
    const categoryRelationsByCategory = groupRows(options.tables.card_card_categories.filter(row => cards.has(requiredId(row, "card_id"))), "card_category_id", "card_card_categories");
    const canonicalOwnerByCardId = canonicalAwakeningOwners(options.tables.card_awakening_routes, cards);
    for (const row of options.tables.equipment_skills)
        if (!itemsById.has(requiredId(row, "equipment_skill_item_id")))
            throw new Error(`Equipment skill ${requiredId(row)} references a missing item`);
    for (const row of options.tables.card_unique_info_set_relations)
        if (!uniqueInfos.has(requiredId(row, "card_unique_info_id")))
            throw new Error(`Unique-info relation ${requiredId(row)} references a missing identity`);
    for (const row of options.tables.cards)
        if (!uniqueInfos.has(requiredId(row, "card_unique_info_id")))
            throw new Error(`Card ${requiredId(row)} references a missing unique identity`);
    uniqueRows(options.tables.card_card_categories, "card_card_categories");
    for (const row of options.tables.card_card_categories) {
        if (!categories.has(requiredId(row, "card_category_id")))
            throw new Error(`Card-category relation ${requiredId(row)} references a missing category`);
    }
    const limitationSets = [...limitationsBySet.entries()].sort(([left], [right]) => numericCompare(left, right)).map(([setId, rows]) => {
        const conditions = [...rows].sort((left, right) => numericCompare(requiredId(left), requiredId(right)))
            .map(row => buildCondition(row, cards, categories, uniqueInfos, uniqueInfoRelationsBySet, cardsByUniqueInfo, categoryRelationsByCategory, canonicalOwnerByCardId));
    ...
    const items = [...itemsById.values()].sort((left, right) => numericCompare(requiredId(left), requiredId(right))).map((row) => {
        const itemId = requiredId(row);
        const name = text(row.name);
        const description = text(row.description);
        if (!name || !description)
            throw new Error(`Equipment Skill Orb ${itemId} has empty official text`);
        const grade = text(row.grade).toLowerCase();
        if (grade !== "bronze" && grade !== "silver" && grade !== "gold")
            throw new Error(`Equipment Skill Orb ${itemId} has unknown grade ${grade}`);
        ...
        if (!limitationSetById.has(limitationSetId))
            throw new Error(`Equipment Skill Orb ${itemId} references missing limitation set ${limitationSetId}`);
        ...
        const effectsWithPriority = effectRows.map(effectRow => {
            ...
            if (Boolean(potentialSkillId) === Boolean(statusType))
                throw new Error(`Equipment skill ${sourceRowId} must have exactly one typed identity`);
            if (statusType && statusType !== "hp" && statusType !== "attack" && statusType !== "defense")
                throw new Error(`Equipment skill ${sourceRowId} has unknown status_type ${statusType}`);
            const identity = potentialSkillId ? `potential:${potentialSkillId}` : `status:${statusType}`;
            const priority = EFFECT_PRIORITY.get(identity);
            const label = EFFECT_LABEL.get(identity);
            if (!priority || !label)
                throw new Error(`Equipment skill ${sourceRowId} has unknown effect ${identity}`);
            const value = statusType ? nonNegativeInteger(row[statusType], `Equipment Skill Orb ${itemId}.${statusType}`) : undefined;
            if (statusType && !value)
                throw new Error(`Equipment skill ${sourceRowId} has no positive ${statusType} value`);
            const effect = {
                sourceRowId,
        ...
        if (isEternalRaw !== "0" && isEternalRaw !== "1")
            throw new Error(`Equipment Skill Orb ${itemId} has unknown is_eternal ${isEternalRaw}`);
    ...
    for (const item of items) {
        ...
        for (const condition of limitation.conditions) {
            if (condition.kind === "card")
                for (const cardId of condition.cardIds ?? [])
                    addReverse(exact, cardId, item.id);
            if (condition.kind === "card-unique-info-set") {
                for (const cardId of condition.resolvedCardUniqueInfoSets?.flatMap(value => value.eligibleCardIds) ?? [])
                    addReverse(family, cardId, item.id);
            }
        }
        if (!limitation.isUnrestricted && limitation.conditions.length === 1) {
            ...
            if (condition.kind === "card") {
                ...
                if (owners.length === 1)
                    addReverse(exclusive, owners[0], item.id);
            }
        }
    }
    ...
    const categoryEligibleCardId = Object.fromEntries([...restrictedCategoryIds].sort(numericCompare).map(categoryId => {
        ...
        if (!cardIds.length)
            throw new Error(`Restricted category ${categoryId} has no eligible cards`);
        return [categoryId, cardIds];
    }));
    const catalog = {
        schemaVersion: 1,
        parserVersion: exports.SKILL_ORB_CONTRACT_VERSION,
...
exports.buildSkillOrbCatalog = buildSkillOrbCatalog;
...
function equalCounts(actual, expected, context) {
    for (const [key, value] of Object.entries(expected))
        if (actual[key] !== value)
            throw new Error(`${context}.${key}: expected ${value}, observed ${actual[key] ?? 0}`);
}
function validateSkillOrbCatalog(catalog) {
    if (catalog.schemaVersion !== 1 || !["1.0.0", "1.1.0", exports.SKILL_ORB_CONTRACT_VERSION].includes(catalog.parserVersion))
        throw new Error("Unsupported Skill Orb contract");
    if (catalog.provenance.snapshotVersion !== exports.PINNED_SKILL_ORB_PROFILE.snapshotVersion || catalog.provenance.sourceDatabaseSha256 !== exports.PINNED_SKILL_ORB_PROFILE.sourceDatabaseSha256)
        throw new Error("Skill Orb provenance mismatch");
    if (catalog.items.length !== exports.PINNED_SKILL_ORB_PROFILE.itemCount)
        throw new Error(`Expected ${exports.PINNED_SKILL_ORB_PROFILE.itemCount} Skill Orbs`);
    if (Math.max(...catalog.items.map(item => Number(item.id))) !== exports.PINNED_SKILL_ORB_PROFILE.maxItemId)
        throw new Error("Unexpected maximum Skill Orb ID");
    if (catalog.items.some(item => !item.name.trim() || !item.description.trim()))
        throw new Error("Skill Orb official text must not be empty");
    for (const item of catalog.items) {
        for (const effect of item.effects) {
            const isStatus = Boolean(effect.statusType);
            if (catalog.parserVersion === exports.SKILL_ORB_CONTRACT_VERSION && isStatus !== Boolean(effect.value && effect.value > 0)) {
                throw new Error(`Skill Orb ${item.id} effect value mismatch`);
            }
            if (!isStatus && effect.value !== undefined)
                throw new Error(`Potential effect ${effect.sourceRowId} cannot have a flat value`);
        }
    }
    const effectCount = catalog.items.reduce((sum, item) => sum + item.effects.length, 0);
    if (effectCount !== exports.PINNED_SKILL_ORB_PROFILE.effectRowCount)
        throw new Error(`Unexpected effect count ${effectCount}`);
    ...
    if (single !== exports.PINNED_SKILL_ORB_PROFILE.singleEffectItemCount || dual !== exports.PINNED_SKILL_ORB_PROFILE.dualEffectItemCount || single + dual !== catalog.items.length)
        throw new Error("Unexpected single/dual effect distribution");
    if (catalog.items.filter(item => item.isEternal).length !== exports.PINNED_SKILL_ORB_PROFILE.eternalItemCount)
        throw new Error("Unexpected reusable Skill Orb count");
    equalCounts(countBy(catalog.items.map(item => item.grade)), exports.PINNED_SKILL_ORB_PROFILE.gradeCounts, "gradeCounts");
    if (catalog.limitationSets.length !== exports.PINNED_SKILL_ORB_PROFILE.limitationSetCount)
        throw new Error("Unexpected limitation set count");
    const limitationRows = catalog.limitationSets.flatMap(set => set.conditions);
    if (limitationRows.length !== exports.PINNED_SKILL_ORB_PROFILE.limitationRowCount)
        throw new Error("Unexpected limitation row count");
    const requireSortedIds = (ids, context) => {
        if (ids && [...ids].sort(numericCompare).join(",") !== ids.join(","))
            throw new Error(`Non-deterministic ${context}`);
    };
    for (const row of limitationRows) {
        ...
        for (const resolved of row.resolvedCardUniqueInfoSets ?? [])
            requireSortedIds(resolved.eligibleCardIds, `eligibleCardIds for set ${resolved.id}`);
        if (row.resolvedCards && row.resolvedCards.map(value => value.id).join(",") !== (row.cardIds ?? []).join(","))
            throw new Error("Resolved exact-card targets are not aligned");
        if (catalog.parserVersion === exports.SKILL_ORB_CONTRACT_VERSION && row.kind === "card"
            && row.canonicalOwnerCardIds?.length !== row.cardIds?.length)
            throw new Error("Canonical exact-card owners are not aligned");
        if (row.resolvedCategories && row.resolvedCategories.map(value => value.id).join(",") !== (row.cardCategoryIds ?? []).join(","))
            throw new Error("Resolved category targets are not aligned");
        if (row.resolvedCardUniqueInfoSets && row.resolvedCardUniqueInfoSets.map(value => value.id).join(",") !== (row.cardUniqueInfoSetIds ?? []).join(","))
            throw new Error("Resolved family targets are not aligned");
    }
    const setById = new Map(catalog.limitationSets.map(set => [set.id, set]));
    if (setById.size !== catalog.limitationSets.length)
        throw new Error("Duplicate normalized limitation set");
    equalCounts(countBy(limitationRows.map(row => row.kind)), exports.PINNED_SKILL_ORB_PROFILE.limitationRowsByKind, "limitationRowsByKind");
    equalCounts(countBy(catalog.items.map(item => setById.get(item.limitationSetId).conditions[0].kind)), exports.PINNED_SKILL_ORB_PROFILE.itemCountsByLimitationKind, "itemCountsByLimitationKind");
    const exactRows = limitationRows.filter(row => row.kind === "card");
    const exactReferences = exactRows.flatMap(row => row.cardIds ?? []);
    if (exactReferences.length !== exports.PINNED_SKILL_ORB_PROFILE.exactCardReferenceCount || new Set(exactReferences).size !== exports.PINNED_SKILL_ORB_PROFILE.exactUniqueCardCount)
        throw new Error("Unexpected exact-card limitation coverage");
    if (limitationRows.filter(row => row.kind === "card-unique-info-set").length !== 7)
        throw new Error("Unexpected unique-info set limitation coverage");
    if (new Set(catalog.items.map(item => item.iconImageId)).size !== exports.PINNED_SKILL_ORB_PROFILE.foregroundIconCount)
        throw new Error("Unexpected foreground icon count");
    if (new Set(catalog.items.map(item => item.levelAssetPath)).size !== exports.PINNED_SKILL_ORB_PROFILE.levelCombinationCount)
        throw new Error("Unexpected level asset combination count");
    const itemIds = new Set(catalog.items.map(item => item.id));
    const validateIndex = (index, kind) => {
        for (const [cardId, orbIds] of Object.entries(index)) {
            if (!/^\d+$/.test(cardId) || !orbIds.length || new Set(orbIds).size !== orbIds.length)
                throw new Error(`Invalid ${kind} reverse index entry ${cardId}`);
            if ([...orbIds].sort(numericCompare).join(",") !== orbIds.join(",") || orbIds.some(id => !itemIds.has(id)))
                throw new Error(`Invalid ${kind} reverse index values ${cardId}`);
        }
    };
    validateIndex(catalog.indexes.exactCardId, "card");
    validateIndex(catalog.indexes.familyEligibleCardId, "card-unique-info-set");
    if (catalog.parserVersion === exports.SKILL_ORB_CONTRACT_VERSION) {
        const categoryIndex = catalog.indexes.categoryEligibleCardId;
        if (!categoryIndex)
            throw new Error("Missing category eligibility index");
        const expectedCategoryIds = [...new Set(limitationRows.flatMap(row => row.cardCategoryIds ?? []))].sort(numericCompare);
        if (Object.keys(categoryIndex).join(",") !== expectedCategoryIds.join(","))
            throw new Error("Category eligibility index keys mismatch");
        ...
        const expectedExclusive = new Map();
        for (const item of catalog.items) {
            const set = setById.get(item.limitationSetId);
            if (set.isUnrestricted || set.conditions.length !== 1 || set.conditions[0].kind !== "card")
                continue;
            const owners = [...new Set(set.conditions[0].canonicalOwnerCardIds ?? [])];
            if (owners.length === 1)
                addReverse(expectedExclusive, owners[0], item.id);
        }
    ...
    if (new Set(assets.map(asset => asset.path)).size !== assets.length || assets.some(asset => !/^[a-f0-9]{64}$/.test(asset.sha256) || asset.sizeBytes <= 0))
        throw new Error("Invalid Skill Orb asset inventory");
    if (catalog.assetInventory.inventorySha256 !== computeAssetInventorySha256(assets) || catalog.assetInventory.totalBytes !== assets.reduce((sum, asset) => sum + asset.sizeBytes, 0))
        throw new Error("Skill Orb asset inventory digest mismatch");
...
exports.validateSkillOrbCatalog = validateSkillOrbCatalog;
//# sourceMappingURL=game-db-skill-orb-catalog.js.map