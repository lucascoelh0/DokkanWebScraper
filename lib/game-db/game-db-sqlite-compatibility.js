...
async function readDescriptorBoundSnapshotBytes(handle, expectedSize, expectedSha256) {
    let bytes;
    try {
        bytes = Buffer.allocUnsafe(expectedSize);
    }
    catch {
        throw new Error("C4 snapshot is too large for descriptor-bound inspection");
    }
    let offset = 0;
    while (offset < expectedSize) {
        const result = await handle.read(bytes, offset, expectedSize - offset, offset);
        if (result.bytesRead <= 0)
            throw new Error("C4 snapshot ended during descriptor-bound inspection read");
        offset += result.bytesRead;
    }
    const extra = Buffer.alloc(1);
    if ((await handle.read(extra, 0, 1, expectedSize)).bytesRead !== 0)
        throw new Error("C4 snapshot grew during descriptor-bound inspection read");
    if ((0, crypto_1.createHash)("sha256").update(bytes).digest("hex") !== expectedSha256)
        throw new Error("C4 descriptor-bound inspection bytes do not match the AQ commit");
    return bytes;
}
...
async function buildGameDbSqliteCompatibility(options) {
    ...
    try {
        ...
        if (snapshot.readableSqliteHeader) {
            ...
            inspection = await sqlite_readonly_adapter_1.ReadOnlySqliteAdapter.fromDescriptorBoundBytes(snapshot.path, inspectionBytes).inspect();
        }
        await verifyPrivateInspectionSnapshot(snapshot, acquired.metadata.observedSizeBytes, acquired.metadata.localSha256);
