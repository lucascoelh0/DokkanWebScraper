...
function evaluateSqliteProfile(input) {
    ...
    const status = input.artifactState === "encrypted_or_packaged"
        ? "unknown"
        : missing.length > 0
            ? "incompatible"
            : exact
...
async function hashHandle(handle, expectedSize) {
    ...
    while (offset < expectedSize) {
        ...
        if (bytesRead <= 0)
            throw new Error("C4 snapshot ended before the committed source size");
    ...
    if ((await handle.read(extra, 0, 1, expectedSize)).bytesRead !== 0)
        throw new Error("C4 snapshot exceeds the committed source size");
...
async function copyHandle(source, destination, expectedSize) {
    ...
    while (offset < expectedSize) {
        ...
        if (bytesRead <= 0)
            throw new Error("Committed source ended during C4 snapshot creation");
        const chunk = buffer.subarray(0, bytesRead);
        hash.update(chunk);
        let written = 0;
    ...
    if ((await source.read(extra, 0, 1, expectedSize)).bytesRead !== 0)
        throw new Error("Committed source exceeds its size during C4 snapshot creation");
...
async function createPrivateInspectionSnapshot(storeRoot, artifactPath, expectedSize, expectedSha256) {
    ...
    try {
        ...
        if (!sameIdentity(sourceIdentity, sourcePathIdentity))
            throw new Error("C4 authority source pathname does not match its opened descriptor");
        assertSnapshotFileIdentity(sourceIdentity, "C4 authority source database");
        if (sourceIdentity.sizeBytes !== expectedSize)
            throw new Error("C4 authority source observed size does not match deterministic metadata");
        ...
        if (verified.sha256 !== expectedSha256)
            throw new Error("C4 snapshot hash does not match committed metadata");
        await source.close();
        return {
            directory,
...
function isPlainRuntimeOptions(value) {
...
async function buildGameDbSqliteCompatibility(options) {
    if (arguments.length !== 1 || !isPlainRuntimeOptions(options))
    ...
    if (!explicitDerived && (typeof options.storeRoot !== "string" || !options.storeRoot || options.storeRoot.includes("\0"))) {
    ...
    if (explicitDerived) {
        ...
        if (typeof derivedOptions.derivedStoreRoot !== "string" || !derivedOptions.derivedStoreRoot || derivedOptions.derivedStoreRoot.includes("\0"))
            throw new Error("SQLite compatibility derivedStoreRoot is invalid");
        if (typeof derivedOptions.sourceStoreRoot !== "string" || !derivedOptions.sourceStoreRoot || derivedOptions.sourceStoreRoot.includes("\0"))
            throw new Error("SQLite compatibility sourceStoreRoot is invalid");
        if (typeof derivedOptions.derivedArtifactIdentity !== "string" || !SHA256_PATTERN.test(derivedOptions.derivedArtifactIdentity))
            throw new Error("SQLite compatibility derivedArtifactIdentity is invalid");
    }
    ...
    if (explicitDerived) {
        const derivedOptions = options;
        const derived = await (0, game_db_derived_sqlite_artifact_validator_1.validateDerivedSqliteArtifact)({
        ...
        try {
            const boundDerived = await (0, game_db_derived_sqlite_artifact_validator_1.validateDerivedSqliteArtifact)({
                storeRoot: derivedOptions.derivedStoreRoot,
                sourceStoreRoot: derivedOptions.sourceStoreRoot,
            ...
            if (!snapshot.readableSqliteHeader)
                throw new Error("Validated DQ output is no longer readable SQLite");
            if (options.signal?.aborted)
                throw new Error("SQLite compatibility was cancelled");
            const inspection = await sqlite_readonly_adapter_1.ReadOnlySqliteAdapter.fromDescriptorBoundHandle(snapshot.path, snapshot.handle, inspectionSize, derived.metadata.output.sha256, {
            ...
            if (options.signal?.aborted)
                throw new Error("SQLite compatibility was cancelled");
            await verifyPrivateInspectionSnapshot(snapshot, inspectionSize, derived.metadata.output.sha256);
            const revalidated = await (0, game_db_derived_sqlite_artifact_validator_1.validateDerivedSqliteArtifact)({
                storeRoot: derivedOptions.derivedStoreRoot,
                sourceStoreRoot: derivedOptions.sourceStoreRoot,
                artifactIdentity: derived.identity,
            });
    ...
    const acquired = await (0, game_db_download_database_artifact_1.validateAcquiredDatabaseArtifact)(explicitLatest
        ...
        : { storeRoot: aqOptions.storeRoot, artifactIdentity: aqOptions.artifactIdentity });
...
function parseGameDbSqliteCompatibilityArgs(argv) {
    ...
    let useLatest = false;
    let derivedStoreRoot;
    let sourceStoreRoot;
    let derivedArtifactIdentity;
    let outputFile;
    const seen = new Set();
    for (let index = 0; index < argv.length; index += 1) {
        ...
        else if (name === "--derived-store-root")
            derivedStoreRoot = value;
        else if (name === "--source-store-root")
            sourceStoreRoot = value;
        else if (name === "--derived-artifact-identity")
            derivedArtifactIdentity = value;
    ...
    if (!hasAqSelector && !hasDqSelector)
        throw new Error("Missing --store-root or complete DQ selector");
    if (hasAqSelector === hasDqSelector)
        throw new Error("Choose exactly one AQ or DQ selector");
    let selector;
    if (hasDqSelector) {
        ...
        if (!SHA256_PATTERN.test(derivedArtifactIdentity))
            throw new Error("DQ derived artifact identity is invalid");
        ...
        if (artifactIdentity && !SHA256_PATTERN.test(artifactIdentity))
            throw new Error("AQ artifact identity is invalid");
        selector = useLatest ? { storeRoot, useLatest: true } : { storeRoot, artifactIdentity: artifactIdentity };
    }
