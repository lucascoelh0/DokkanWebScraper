...
function mapChapter(chapter, infoTitle, infoBannerPath, battleById, portraits, cardSkins) {
    return {
        id: chapter.id,
...
function enrichMissions(missions, cardSkins) {
    if (!cardSkins)
        return missions;
...
function enrichCardSkinReward(reward, cardSkins) {
    if (reward.itemType !== "CardSkinItem")
        return reward;
    ...
    if (!skin)
        throw new Error(`Frontier card-skin reward ${itemId} is missing from DokkanStats.`);
...
function cardSkinPortraitSpec(skin) {
    const type = skin.type?.trim().toUpperCase();
    const characterClass = skin.characterClass?.trim();
    const rarity = skin.rarity?.trim().toUpperCase();
    const typeDigit = type === character_1.Types.AGL ? 0
        : type === character_1.Types.TEQ ? 1
            : type === character_1.Types.INT ? 2
                : type === character_1.Types.STR ? 3
                    : type === character_1.Types.PHY ? 4
                        : undefined;
    if (typeDigit === undefined || !Object.values(character_1.Classes).includes(characterClass)
        || !Object.values(character_1.Rarities).includes(rarity)) {
        throw new Error(`DokkanStats card skin ${skin.id} has incomplete portrait metadata.`);
    }
    return (0, portrait_asset_contract_1.portraitSpecFromTypeAndClass)(skin.cardId, rarity, typeDigit, characterClass);
}
function cardSkinLookup(cardSkins) {
    return uniqueMap(cardSkins, value => value.id, "DokkanStats card skin");
}
function validateCardSkinCoverage(chapters, cardSkins) {
    const missions = [
...
function requiredRewardId(value, label) {
    const normalized = value?.trim() ?? "";
    if (!/^\d+$/.test(normalized))
        throw new Error(`Frontier card-skin reward ${label} is missing or invalid.`);
    return normalized;
}
...
async function readDefaultInputs() {
    const [dokkanInfo, dokkanFyiSeries, dokkanFyiChapters, dokkanStats] = await Promise.all([
        ...
        readOptionalJson("data/dokkanstats-frontier/latest/frontier.json"),
    ]);
