...
exports.validateDd6 = exports.buildDd6 = exports.validateDd6SourceLock = exports.readDd6CanonicalGitBlob = void 0;
...
function dd2Identity(dd2) {
    const bytes = Buffer.from(canonicalJson(dd2), "utf8");
    return { contract: "dokkan-data-download-asset-contracts", contractVersion: "0.1.0", identityPolicy: "recursive_lexicographic_json_v1", sizeBytes: bytes.length, sha256: (0, crypto_1.createHash)("sha256").update(bytes).digest("hex") };
}
function git(root, args) { return new Promise((done, reject) => (0, child_process_1.execFile)("git", ["-C", root, ...args], { encoding: "buffer", maxBuffer: 16 * 1024 * 1024, windowsHide: true }, (error, stdout) => error ? reject(new Error("DD6 canonical Git source is unavailable")) : done(Buffer.isBuffer(stdout) ? stdout : Buffer.from(stdout)))); }
function samePath(left, right) { return process.platform === "win32" ? (0, path_1.resolve)(left).toLowerCase() === (0, path_1.resolve)(right).toLowerCase() : (0, path_1.resolve)(left) === (0, path_1.resolve)(right); }
async function readDd6CanonicalGitBlob(root, fileName) { if (typeof root !== "string" || !root || typeof fileName !== "string" || !/^[A-Za-z0-9._/-]+$/.test(fileName) || fileName.startsWith("/") || fileName.split("/").some(part => !part || part === "." || part === ".."))
    throw new Error("DD6 canonical Git source path is invalid"); const topLevel = (await git(root, ["rev-parse", "--show-toplevel"])).toString("utf8").trim(); if (!samePath(topLevel, root))
    throw new Error("DD6 root must be the canonical Git checkout root"); return git(root, ["cat-file", "blob", `HEAD:${fileName}`]); }
exports.readDd6CanonicalGitBlob = readDd6CanonicalGitBlob;
async function validateDd6SourceLock(root, lock) { if (!lock || typeof lock !== "object" || !exactKeys(lock, ["schemaVersion", "contract", "contractVersion", "identityPolicy", "artifacts"]) || lock.schemaVersion !== 1 || lock.contract !== "dokkan-data-download-shadow-source-lock" || lock.contractVersion !== "0.8.0" || lock.identityPolicy !== "git_blob_bytes_v1" || !Array.isArray(lock.artifacts) || JSON.stringify(lock.artifacts.map(value => [value.key, value.fileName])) !== JSON.stringify(expectedSources) || lock.artifacts.some(item => !exactKeys(item, ["key", "fileName", "sizeBytes", "sha256"]) || !Number.isSafeInteger(item.sizeBytes) || item.sizeBytes <= 0 || !/^[a-f0-9]{64}$/.test(item.sha256)))
    throw new Error("DD6 source lock contract mismatch"); for (const item of lock.artifacts) {
    const bytes = await readDd6CanonicalGitBlob(root, item.fileName);
    if (bytes.byteLength !== item.sizeBytes || (0, crypto_1.createHash)("sha256").update(bytes).digest("hex") !== item.sha256)
...
async function buildDd6(root, lock, dd2) {
    ...
    const dd2Validation = (0, data_download_dd2_1.validateDd2)(dd2);
    if (!dd2Validation.valid)
        throw new Error(`DD6 DD2 input validation failed: ${dd2Validation.failures.join(", ")}`);
    const rows = [
    ...
    const dataset = { schemaVersion: 1, contract: "dokkan-data-download-shadow-parity", contractVersion: "0.9.0", generatedAt: dd2.generatedAt, collectionMode: "offline_pinned_document_and_code_comparison", defaultEnabled: false, productionMutation: false, identityPolicy: "structural_endpoint_field_and_exact_path_only_no_names_or_localized_text", sourceIdentityPolicy: lock.identityPolicy, sourceLineage: lock.artifacts, inputLineage: dd2Identity(dd2), rows, totals, completenessWarning: "zero_confirmed_conflicts_never_implies_completeness" };
    const validation = validateDd6(dataset, lock, dd2);
    if (!validation.valid)
        throw new Error(`DD6 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildDd6 = buildDd6;
function validateDd6(value, lock, dd2) {
    const failures = [];
    const classifications = ["agreement", "representation_gain", "representation_mismatch", "confirmed_conflict", "unknown", "unjoinable"];
    if (!value || typeof value !== "object" || !exactKeys(value, ["schemaVersion", "contract", "contractVersion", "generatedAt", "collectionMode", "defaultEnabled", "productionMutation", "identityPolicy", "sourceIdentityPolicy", "sourceLineage", "inputLineage", "rows", "totals", "completenessWarning"])
        ...
        || value.completenessWarning !== "zero_confirmed_conflicts_never_implies_completeness")
        failures.push("DD6 parity contract");
    ...
    if (!lockValid || !Array.isArray(value?.sourceLineage) || JSON.stringify(value.sourceLineage) !== JSON.stringify(lock.artifacts)
        || value.sourceLineage.some(item => !exactKeys(item, ["key", "fileName", "sizeBytes", "sha256"]) || typeof item.fileName !== "string" || !item.fileName
            || !Number.isSafeInteger(item.sizeBytes) || item.sizeBytes <= 0 || !/^[a-f0-9]{64}$/.test(item.sha256)))
        failures.push("DD6 source lineage");
    const dd2Valid = Boolean(dd2 && (0, data_download_dd2_1.validateDd2)(dd2).valid);
    if (!dd2Valid || !value?.inputLineage || !exactKeys(value.inputLineage, ["contract", "contractVersion", "identityPolicy", "sizeBytes", "sha256"])
        || JSON.stringify(value.inputLineage) !== JSON.stringify(dd2Identity(dd2)) || value.generatedAt !== dd2.generatedAt)
        failures.push("DD6 DD2 input lineage");
    ...
    if (!Array.isArray(value?.rows) || value.rows.length !== expectedRows.length || value.rows.some((row, index) => !exactKeys(row, ["subject", "prior", "classification", "units", "reason"])
        ...
        || row.units !== expectedRows[index].units || row.reason !== reasons[index]))
        failures.push("DD6 parity rows");
    ...
    if (!totalsValid || JSON.stringify(value.totals) !== JSON.stringify(recalculated))
        failures.push("DD6 parity totals");
