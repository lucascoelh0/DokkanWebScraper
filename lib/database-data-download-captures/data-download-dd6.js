...
async function validateDd6SourceLock(root, lock) { if (!lock || typeof lock !== "object" || !exactKeys(lock, ["schemaVersion", "contract", "contractVersion", "identityPolicy", "artifacts"]) || lock.schemaVersion !== 1 || lock.contract !== "dokkan-data-download-shadow-source-lock" || lock.contractVersion !== "0.8.0" || lock.identityPolicy !== "git_blob_bytes_v1" || !Array.isArray(lock.artifacts) || JSON.stringify(lock.artifacts.map(value => value.key)) !== JSON.stringify(expectedKeys) || new Set(lock.artifacts.map(value => value.fileName)).size !== lock.artifacts.length || lock.artifacts.some(item => !exactKeys(item, ["key", "fileName", "sizeBytes", "sha256"]) || !Number.isSafeInteger(item.sizeBytes) || item.sizeBytes <= 0 || !/^[a-f0-9]{64}$/.test(item.sha256)))
...
async function buildDd6(root, lock, dd2) {
    ...
    const dataset = { schemaVersion: 1, contract: "dokkan-data-download-shadow-parity", contractVersion: "0.8.0", generatedAt: dd2.generatedAt, collectionMode: "offline_pinned_document_and_code_comparison", defaultEnabled: false, productionMutation: false, identityPolicy: "structural_endpoint_field_and_exact_path_only_no_names_or_localized_text", sourceIdentityPolicy: lock.identityPolicy, sourceLineage: lock.artifacts, rows, totals, completenessWarning: "zero_confirmed_conflicts_never_implies_completeness" };
    const validation = validateDd6(dataset, lock);
    if (!validation.valid)
        throw new Error(`DD6 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildDd6 = buildDd6;
function validateDd6(value, lock) {
    const failures = [];
    const classifications = ["agreement", "representation_gain", "representation_mismatch", "confirmed_conflict", "unknown", "unjoinable"];
    if (!value || typeof value !== "object" || !exactKeys(value, ["schemaVersion", "contract", "contractVersion", "generatedAt", "collectionMode", "defaultEnabled", "productionMutation", "identityPolicy", "sourceIdentityPolicy", "sourceLineage", "rows", "totals", "completenessWarning"])
        || value.schemaVersion !== 1 || value.contract !== "dokkan-data-download-shadow-parity" || value.contractVersion !== "0.8.0"
    ...
    if (!Array.isArray(value?.rows) || value.rows.length !== expectedRows.length || value.rows.some((row, index) => !exactKeys(row, ["subject", "prior", "classification", "units", "reason"])
        ...
        || row.units !== expectedRows[index].units || typeof row.reason !== "string" || row.reason.length === 0))
