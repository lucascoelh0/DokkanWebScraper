...
function validateVersionableTarget(name, text) {
    if (typeof name !== "string" || !name || typeof text !== "string" || name.includes("\0"))
...
function readContainedExternalSource(captureRoot, lock) {
    ...
    if (!rootLink.isDirectory() || rootLink.isSymbolicLink())
        throw new Error("DD0 external source root must be a regular directory");
...
function loadDdExternalSources(captureRoot, lock) { validateDd0SourceLock(lock); return lock.externalConfidentialSources.map(value => readContainedExternalSource(captureRoot, value)); }
...
function loadDdCaptures(captureRoot, lock) {
    ...
    return lock.captures.map(capture => {
        const before = (0, fs_1.statSync)(`${captureRoot}\\${capture.fileName}`, { bigint: true });
        const link = (0, fs_1.lstatSync)(`${captureRoot}\\${capture.fileName}`);
        if (!before.isFile() || link.isSymbolicLink())
            throw new Error(`DD0 source is not a regular contained file: ${capture.captureId}`);
        const snapshot = (0, capture_h0_audit_1.readValidatedCaptureSnapshot)(captureRoot, capture.fileName, capture.captureId);
        const after = (0, fs_1.statSync)(`${captureRoot}\\${capture.fileName}`, { bigint: true });
        if (before.dev !== after.dev || before.ino !== after.ino || before.size !== after.size || before.mtimeNs !== after.mtimeNs)
            throw new Error(`DD0 source changed during read: ${capture.captureId}`);
        if (Buffer.byteLength(snapshot.text) !== capture.sizeBytes || sha256(snapshot.text) !== capture.sha256 || Number(before.size) !== capture.sizeBytes || Math.abs(Number(before.mtimeNs / 1000000n) - Date.parse(capture.modifiedAt)) > 0)
            throw new Error(`DD0 source identity mismatch: ${capture.captureId}`);
...
function stagedDdVersionableTargets() {
    const names = (0, child_process_1.execFileSync)("git", ["diff", "--cached", "--name-only", "-z"], { encoding: "utf8" }).split("\0").filter(Boolean);
    if (names.length === 0)
        throw new Error("DD staged secret scan requires at least one staged target");
    return names.map(name => ({ name, text: (0, child_process_1.execFileSync)("git", ["show", `:${name}`], { encoding: "utf8", maxBuffer: 64 * 1024 * 1024 }) }));
}
