...
exports.mapCategorySupportMemoryRefFromFyi = exports.mapCategoryCharacterRefFromFyi = exports.mergeCategorySupportFromSupportOnly = exports.mapCategoryFromFyi = exports.buildCategoryDataset = exports.writeDokkanFyiCategories = exports.getDokkanFyiCategories = void 0;
...
class DokkanFyiCategoryClient {
    async fetchCategories(limit) {
        ...
        return mapWithConcurrency(limitedCategories, requestedSupportOnlyConcurrency(), async (category) => this.fetchCategoryWithSupport(category));
    }
    ...
    async fetchCategoryWithSupport(category) {
    ...
    async fetchSupportOnlyCharacters(categoryId) {
    ...
    async fetchSupportOnlyPage(categoryId, page) {
        const query = new URLSearchParams({
            support_only: "1",
            page: page.toString(),
        });
        const html = await fetchDokkanFyiHtml(`${DOKKAN_FYI_BASE_URL}/categories/${categoryId}?${query.toString()}`, `category ${categoryId} support-only page ${page}`);
        return extractPagePayload(html);
    }
}
...
function mergeCategorySupportFromSupportOnly(category, supportCharacters) {
    return {
        ...category,
...
exports.mergeCategorySupportFromSupportOnly = mergeCategorySupportFromSupportOnly;
...
function requestedSupportOnlyConcurrency() {
...
function uniqueCharacterSummaries(values) {
    const byId = new Map();
