...
exports.validateCaptureH6 = exports.buildCaptureH6 = void 0;
...
function unsafeRawPath(path) { return /[\\%]/.test(path) || path.includes("//") || path.split("/").some(segment => segment === "." || segment === ".."); }
function sanitizedAssetPath(raw) {
    if (typeof raw !== "string" || raw.length === 0 || raw.length > 2048 || /[\u0000-\u001f\\]/.test(raw))
        return null;
    let pathname;
    try {
        ...
        if (scheme) {
            ...
            if (unsafeRawPath(rawPath))
                return null;
        }
        const url = new URL(raw);
        if (url.protocol !== "https:" || url.hostname.toLowerCase() !== CDN_HOST || url.port || url.username || url.password)
            return null;
        pathname = url.pathname;
    }
    catch {
        if (/[?#]/.test(raw) || unsafeRawPath(raw))
            return null;
        pathname = raw.startsWith("/") ? raw : `/${raw}`;
    }
    if (pathname.includes("..") || pathname.includes("//") || pathname.includes("%") || !assetPathPattern.test(pathname))
        return null;
    return pathname;
}
function extension(path) { return path.slice(path.lastIndexOf(".") + 1).toLowerCase(); }
function descriptorEvidenceSha256(version, algorithm, upstreamHashOpaque, assetPath) { return sha256(`$.version|$.algorithm|$.hash|$.url\n${JSON.stringify({ version, algorithm, upstreamHashOpaque, assetPath })}`); }
function provenance(context, jsonPath, assetPath, hostClass = "official_api") {
...
function addReference(target, context, raw, jsonPath, referenceKind = "product_json_reference") {
    const path = sanitizedAssetPath(raw);
    if (!path || !referenceCoordinates.has(`${context.normalizedEndpoint}\0${jsonPath}`) && referenceKind !== "client_database_url")
        return;
...
function addObjectFields(target, context, values, fields) { for (const value of values)
    for (const [key, path] of fields)
        addReference(target, context, value?.[key], path); }
function collectJsonReferences(target, descriptors, context) {
    const body = context.body;
    if (!body || typeof body !== "object")
        return;
    if (context.normalizedEndpoint === "/client_assets/database") {
        const assetPath = sanitizedAssetPath(body.url);
        if (!assetPath || !Number.isSafeInteger(body.version) || body.version < 0 || body.version > 10 ** 12 || typeof body.algorithm !== "string" || !/^[a-z0-9_-]{1,16}$/.test(body.algorithm) || typeof body.hash !== "string" || !/^[A-Za-z0-9_-]{1,128}$/.test(body.hash))
            return;
    ...
    else if (context.normalizedEndpoint === "/gashas")
        addObjectFields(target, context, objects(body.gashas), [["banner_url", "$.gashas[].banner_url"]]);
    ...
    else if (context.normalizedEndpoint === "/title/banners")
        addObjectFields(target, context, objects(body.banners), [["image", "$.banners[].image"]]);
}
function exactH0(expected, current) {
    if (expected.contract !== current.contract || expected.contractVersion !== current.contractVersion || expected.captures.length !== current.captures.length)
        throw new Error("H6 H0 contract drift");
    for (const capture of current.captures) {
        ...
        if (!prior || prior.sizeBytes !== capture.sizeBytes || prior.entryCount !== capture.entryCount || prior.structuralFingerprint !== capture.structuralFingerprint || prior.schemaFingerprint !== capture.schemaFingerprint || prior.sourceIdentityFingerprint !== capture.sourceIdentityFingerprint)
            throw new Error(`H6 capture drift for ${capture.captureId}`);
    }
}
function collectCdnRequests(target, manifest, roots, h0) {
    const current = (0, capture_h0_audit_1.auditCaptureManifest)(manifest, roots);
    exactH0(h0, current);
    const root = roots[manifest.inputRoot];
    if (!root)
        throw new Error("H6 root is not allowlisted");
    for (const input of manifest.captures) {
        ...
        if (!lineage.capturedAtStart)
            throw new Error(`H6 capture ${input.captureId} has no timestamp`);
        ...
        if (snapshot.sourceIdentityFingerprint !== lineage.sourceIdentityFingerprint)
            throw new Error(`H6 source identity drift for ${input.captureId}`);
        const har = JSON.parse(snapshot.text);
        if (!Array.isArray(har?.log?.entries))
            throw new Error(`H6 invalid HAR ${input.captureId}`);
        for (const raw of har.log.entries) {
            const safe = (0, capture_h0_audit_1.sanitizeHarEntryStructure)(raw);
            if (!safe || safe.hostClass !== "official_cdn" || safe.method !== "GET" || !safe.capturedAt || safe.classification !== "asset_delivery" || !(safe.status === 304 || safe.status >= 200 && safe.status <= 299))
                continue;
            const assetPath = sanitizedAssetPath(raw?.request?.url);
            if (!assetPath)
                continue;
            const deliveryEvidence = safe.status === 304 ? "http_304_revalidation_observed" : "http_2xx_response_observed";
            const context = { captureId: input.captureId, captureFingerprint: lineage.structuralFingerprint, captureSchemaFingerprint: lineage.schemaFingerprint, captureSourceIdentityFingerprint: lineage.sourceIdentityFingerprint, captureTimestamp: lineage.capturedAtStart, normalizedEndpoint: safe.normalizedEndpoint, method: "GET", observedAt: safe.capturedAt, httpStatus: safe.status, endpointClassification: "asset_delivery", hostname: CDN_HOST, pathname: assetPath, body: null };
...
function projectionRows(observations, descriptors) {
    const rows = new Map();
    const add = (captureId, row) => rows.set(captureId, [...(rows.get(captureId) ?? []), JSON.stringify(row)]);
    for (const value of observations) {
        const p = value.provenance;
        add(p.captureId, { assetPath: value.assetPath, extension: value.extension, referenceKind: value.referenceKind, deliveryEvidence: value.deliveryEvidence, endpoint: p.normalizedEndpoint, observedAt: p.observedAt, status: p.httpStatus, jsonPath: p.jsonPath });
    }
    for (const value of descriptors) {
        const p = value.provenance;
        add(p.captureId, { descriptor: true, version: value.version, algorithm: value.algorithm, upstreamHashOpaque: value.upstreamHashOpaque, assetPath: value.assetPath, observedAt: p.observedAt, status: p.httpStatus });
    }
    return rows;
}
function publicFingerprints(observations, descriptors) { return new Map([...projectionRows(observations, descriptors)].map(([captureId, rows]) => [captureId, sha256(rows.sort((a, b) => a.localeCompare(b)).join("\n"))])); }
function observationId(value) { return sha256(JSON.stringify(value)); }
function descriptorId(value) { return sha256(JSON.stringify(value)); }
function buildCaptureH6(manifest, roots, h0) {
    const observations = [], descriptors = [];
    (0, capture_product_core_1.forEachCaptureProductResponse)(manifest, roots, h0, context => { if (context.hostname === "ishin-global.aktsk.com")
        collectJsonReferences(observations, descriptors, context); });
    ...
    for (const value of [...uniqueObservations, ...uniqueDescriptors])
        value.provenance.capturePublicValueFingerprint = fingerprints.get(value.provenance.captureId);
...
function validateCaptureH6(dataset, h0) {
    ...
    for (const value of dataset.observations) {
        const { observationId: _id, ...withoutId } = value;
        const p = value.provenance, match = value.assetPath.match(assetPathPattern);
        if (ids.has(value.observationId) || value.observationId !== observationId(withoutId))
            failures.push("observation identity");
        ids.add(value.observationId);
        if (!referenceKinds.has(value.referenceKind) || !deliveryEvidenceKinds.has(value.deliveryEvidence))
            failures.push("observation enum");
        if (!match || value.extension !== extension(value.assetPath) || value.assetPathSha256 !== sha256(value.assetPath) || /[?#\\%]/.test(value.assetPath) || value.assetPath.includes("..") || value.assetPath.includes("//"))
        ...
        if (value.referenceKind === "captured_cdn_request") {
            ...
            if (p.hostClass !== "official_cdn" || p.endpointClassification !== "asset_delivery" || p.normalizedEndpoint !== `/asset/${value.extension}` || p.jsonPath !== "$request.url.pathname" || !validStatusEvidence)
                failures.push("CDN evidence boundary");
        }
        else {
            referenceCount += 1;
            referencedPaths.add(value.assetPath);
            const expectedClassification = mixedApiEndpoints.has(p.normalizedEndpoint) ? "mixed_product_and_user_state" : "product_catalog";
            if (!(value.referenceKind === "product_json_reference" || value.referenceKind === "client_database_url") || p.hostClass !== "official_api" || p.endpointClassification !== expectedClassification || p.httpStatus < 200 || p.httpStatus > 299 || value.deliveryEvidence !== "reference_only")
                failures.push("reference evidence boundary");
            if (value.referenceKind === "client_database_url") {
                if (p.normalizedEndpoint !== "/client_assets/database" || p.jsonPath !== "$.url" || value.extension !== "db")
                    failures.push("database URL coordinate");
            }
            else if (!referenceCoordinates.has(`${p.normalizedEndpoint}\0${p.jsonPath}`))
                failures.push("reference coordinate allowlist");
        }
    }
    for (const value of dataset.databaseDescriptors) {
        const { descriptorId: _id, ...withoutId } = value, p = value.provenance;
        if (ids.has(value.descriptorId) || value.descriptorId !== descriptorId(withoutId))
            failures.push("descriptor identity");
        ids.add(value.descriptorId);
        if (!Number.isSafeInteger(value.version) || value.version < 0 || value.version > 10 ** 12 || !/^[a-z0-9_-]{1,16}$/.test(value.algorithm) || !/^[A-Za-z0-9_-]{1,128}$/.test(value.upstreamHashOpaque) || sanitizedAssetPath(value.assetPath) !== value.assetPath || extension(value.assetPath) !== "db")
            failures.push("descriptor value domain");
        ...
        if (p.valueEvidenceSha256 !== descriptorEvidenceSha256(value.version, value.algorithm, value.upstreamHashOpaque, value.assetPath))
            failures.push("descriptor evidence digest");
        if (p.hostClass !== "official_api" || p.endpointClassification !== "product_catalog" || p.normalizedEndpoint !== "/client_assets/database" || p.jsonPath !== "$.version|$.algorithm|$.hash|$.url" || p.httpStatus < 200 || p.httpStatus > 299)
            failures.push("descriptor coordinate");
    }
    ...
    if (dataset.observations.length === 0 || referenceCount === 0 || capturedCdnRequestCount === 0)
        failures.push("empty evidence class");
...
exports.validateCaptureH6 = validateCaptureH6;
//# sourceMappingURL=capture-h6-assets.js.map