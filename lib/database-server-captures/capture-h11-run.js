...
const expectedLockPaths = { h7: "data/database-server-captures/h7/capture-h7-shadow-readiness.json", h8: "data/database-server-captures/h8/capture-h8-inventory.json", h9: "data/database-server-captures/h9/capture-h9-observed-schemas.json", h10: "data/database-server-captures/h10/capture-h10-domains.json", h13: "data/database-server-captures/h13/capture-h13-gasha-audit.json" };
if (lock.value.schemaVersion !== 1 || lock.value.contract !== "dokkan-official-capture-h11-source-lock" || lock.value.contractVersion !== "0.12.1" || JSON.stringify(lock.value.artifacts.map((value) => value.key).sort()) !== JSON.stringify(Object.keys(expectedLockPaths).sort()))
...
const h13 = read("data/database-server-captures/h13/capture-h13-gasha-audit.json").value;
...
if (!(0, capture_h13_gasha_audit_1.validateCaptureH13)(h13).valid)
    throw new Error("H11 requires a valid H13 corrective audit");
...
const dataset = (0, capture_h11_parity_1.buildCaptureH11)(lock.text, h7, h10, h3, h4, h5, h6, h8.captures.map(value => value.captureId), h13);
