...
const expectedPaths = { h8: "data/database-server-captures/h8/capture-h8-inventory.json", h9: "data/database-server-captures/h9/capture-h9-observed-schemas.json", h10: "data/database-server-captures/h10/capture-h10-domains.json", h11: "data/database-server-captures/h11/capture-h11-shadow-parity.json" };
if (lock.value.schemaVersion !== 1 || lock.value.contract !== "dokkan-official-capture-h12-source-lock" || lock.value.contractVersion !== "0.13.0" || JSON.stringify(lock.value.artifacts.map((value) => value.key).sort()) !== JSON.stringify(Object.keys(expectedPaths).sort()))
...
const h8 = files.get("h8").value, h9 = files.get("h9").value, h10 = files.get("h10").value, h11 = files.get("h11").value;
if (!(0, capture_h11_parity_1.validateCaptureH11)(h11).valid)
    throw new Error("H12 requires valid H11");
const sourceLockSha256 = (0, crypto_1.createHash)("sha256").update(lock.text).digest("hex"), h11File = files.get("h11");
const dataset = (0, capture_h12_readiness_1.buildCaptureH12)(h8, h9, h10, h11, sourceLockSha256, h11File.sha256, h11File.sizeBytes), validation = (0, capture_h12_readiness_1.validateCaptureH12)(dataset), text = `${JSON.stringify(dataset, null, 2)}\n`;
