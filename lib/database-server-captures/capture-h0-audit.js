...
exports.auditCaptureManifest = exports.auditSanitizedCaptureEntries = exports.loadSanitizedCaptureEntries = exports.readValidatedCaptureSnapshot = exports.readValidatedCaptureForSecretScanner = exports.sanitizeHarEntryStructure = exports.classifyEndpoint = void 0;
...
function sanitizeHarEntryStructure(raw) {
...
exports.sanitizeHarEntryStructure = sanitizeHarEntryStructure;
...
function readValidatedCapture(captureId, filePath, expectedRoot) {
    ...
    try {
        ...
        return { sizeBytes: opened.size, text: (0, fs_1.readFileSync)(descriptor, "utf8"), sourceIdentityFingerprint };
    }
    finally {
...
function readValidatedCaptureSnapshot(rootPath, relativePath, captureId) {
    const resolved = resolveCapture(rootPath, relativePath);
...
exports.readValidatedCaptureSnapshot = readValidatedCaptureSnapshot;
function loadSanitizedCaptureEntries(rootPath, relativePath, captureId) {
    ...
    return { sizeBytes: validated.sizeBytes, entryCount: rawEntries.length, sourceIdentityFingerprint: validated.sourceIdentityFingerprint, entries: rawEntries.map(sanitizeHarEntryStructure).filter((value) => value !== null) };
}
exports.loadSanitizedCaptureEntries = loadSanitizedCaptureEntries;
function auditSanitizedCaptureEntries(captureId, sizeBytes, entryCount, entries, sourceIdentityFingerprint) {
...
function auditOne(captureId, filePath, expectedRoot) {
    ...
    if (!Array.isArray(rawEntries))
        throw new Error(`capture ${captureId} is not a HAR with log.entries`);
    const entries = rawEntries.map(sanitizeHarEntryStructure).filter((value) => value !== null);
