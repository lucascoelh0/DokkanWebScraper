...
describe("capture H13 conservative gasha classification", () => {
    it("classifies a same-dimension value change across temporal windows as temporal_change", () => {
        ...
        assert.equal((0, capture_h13_gasha_audit_1.classifyCanonicalGashaFact)(200, [current], [prior], true).classification, "temporal_change");
    });
    it("classifies a legacy pool union versus a dimensional element as representation_mismatch", () => {
        ...
        assert.equal((0, capture_h13_gasha_audit_1.classifyCanonicalGashaFact)(200, [current], [prior], true).classification, "representation_mismatch");
    });
    ...
    it("confirms only an incompatible value in the same dimension and proven window", () => {
        ...
        assert.equal((0, capture_h13_gasha_audit_1.classifyCanonicalGashaFact)(200, [current], [prior], true).classification, "confirmed_conflict");
    });
    it("classifies absence of an endpoint baseline as coverage_gap", () => {
        assert.equal((0, capture_h13_gasha_audit_1.classifyCanonicalGashaFact)(200, [observation()], [], true).classification, "coverage_gap");
    });
});
//# sourceMappingURL=capture-h13-gasha-audit.spec.js.map