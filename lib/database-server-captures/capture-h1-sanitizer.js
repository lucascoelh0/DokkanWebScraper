...
function buildCaptureH1(manifest, roots, h0) {
    if (h0.contract !== "dokkan-official-capture-structural-inventory" || h0.contractVersion !== "0.1.0" || h0.productionMutation !== false || h0.captures.length !== manifest.captures.length)
    ...
    const captures = manifest.captures.map(input => {
        ...
        const current = (0, capture_h0_audit_1.auditSanitizedCaptureEntries)(input.captureId, loaded.sizeBytes, loaded.entryCount, loaded.entries);
        if (current.entryCount !== lineage.entryCount || current.sizeBytes !== lineage.sizeBytes || current.structuralFingerprint !== lineage.structuralFingerprint || current.schemaFingerprint !== lineage.schemaFingerprint)
