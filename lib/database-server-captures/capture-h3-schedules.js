...
exports.validateCaptureH3 = exports.buildCaptureH3 = void 0;
...
function normalizedValue(kind, raw) {
    ...
    if (kind === "timestamp" && Number.isSafeInteger(raw) && raw >= 0 && raw <= 10 ** 13)
        return raw;
    if (kind === "positiveIntArray" && Array.isArray(raw) && raw.length <= 4096 && raw.every(item => Number.isSafeInteger(item) && item > 0 && item <= 10 ** 12))
        return [...new Set(raw)].sort((a, b) => a - b);
    if (kind === "weekdayArray" && Array.isArray(raw) && raw.length <= 7 && raw.every(item => typeof item === "string" && /^(?:sunday|monday|tuesday|wednesday|thursday|friday|saturday)$/i.test(item)))
        return [...new Set(raw.map(item => item.toLowerCase()))].sort((a, b) => a.localeCompare(b));
    if (kind === "observedTrue" && raw === true)
        return true;
    if (kind === "rate" && typeof raw === "number" && Number.isFinite(raw) && raw >= 0 && raw <= 1000)
        return raw;
    if (kind === "scheduleType" && Number.isSafeInteger(raw) && raw >= 0 && raw <= 1000)
        return raw;
    return undefined;
}
function record(candidates, context, entityType, entityId, base, values) {
    if (!Number.isSafeInteger(entityId) || entityId <= 0)
        return;
    const id = entityId;
    for (const [field, raw, path] of [["identity", id, `${base}.id`], ...values]) {
        const fieldRule = rules[entityType]?.[field];
        if (!fieldRule || raw === undefined)
            continue;
        const value = normalizedValue(fieldRule.kind, raw);
        if (value !== undefined)
            candidates.push({ entityType, entityId: id, fact: (0, capture_product_core_1.makeProductFact)(entityType, id, field, value, context, path) });
    }
}
function objects(value) { return Array.isArray(value) ? value.filter(item => item && typeof item === "object") : []; }
function ids(value) { return objects(value).flatMap(item => Number.isSafeInteger(item.id) && item.id > 0 ? [item.id] : []); }
function buildCaptureH3(manifest, roots, h0) {
    const candidates = [];
    (0, capture_product_core_1.forEachCaptureProductResponse)(manifest, roots, h0, context => {
        ...
        if (context.normalizedEndpoint === "/events") {
            for (const item of objects(body.events))
                record(candidates, context, "event", item.id, "$.events[]", [["startAt", item.start_at, "$.events[].start_at"], ["endAt", item.end_at, "$.events[].end_at"], ["weekdays", item.wday, "$.events[].wday"], ["weekdayStartAt", item.wday_start_at, "$.events[].wday_start_at"], ["weekdayEndAt", item.wday_end_at, "$.events[].wday_end_at"], ["announcementId", item.announcement_id, "$.events[].announcement_id"], ["questIds", ids(item.quests), "$.events[].quests[].id"]]);
            ...
            for (const item of objects(body.eventkagi_events))
                record(candidates, context, "event_key_event", item.id, "$.eventkagi_events[]", [["endAt", item.end_at, "$.eventkagi_events[].end_at"], ["announcementId", item.announcement_id, "$.eventkagi_events[].announcement_id"], ["questIds", ids(item.quests), "$.eventkagi_events[].quests[].id"], ["referenceObserved", true, "$.eventkagi_events[]"]]);
            for (const item of objects(body.eventkagi_z_battle_stages))
                record(candidates, context, "event_key_z_battle", item.id, "$.eventkagi_z_battle_stages[]", [["announcementId", item.announcement_id, "$.eventkagi_z_battle_stages[].announcement_id"], ["referenceObserved", true, "$.eventkagi_z_battle_stages[]"]]);
        }
        else if (context.normalizedEndpoint === "/bonus_schedules") {
            for (const item of objects(body.bonus_schedules))
                record(candidates, context, "bonus_schedule", item.id, "$.bonus_schedules[]", [["startAt", item.start_at, "$.bonus_schedules[].start_at"], ["endAt", item.end_at, "$.bonus_schedules[].end_at"], ["scheduleType", item.type, "$.bonus_schedules[].type"], ["rate", item.rate, "$.bonus_schedules[].rate"]]);
        }
            ...
            if (rm && typeof rm === "object")
                record(candidates, context, "rmbattle", rm.id, "$.rmbattles", [["startAt", rm.start_at, "$.rmbattles.start_at"], ["endAt", rm.end_at, "$.rmbattles.end_at"], ["announcementId", rm.announcement_id, "$.rmbattles.announcement_id"], ["requiredRank", rm.required_rank, "$.rmbattles.required_rank"]]);
            const budokai = body.budokai;
            if (budokai && typeof budokai === "object")
                record(candidates, context, "budokai", budokai.id, "$.budokai", [["startAt", budokai.start_at, "$.budokai.start_at"], ["endAt", budokai.end_at, "$.budokai.end_at"], ["announcementId", budokai.announcement_id, "$.budokai.announcement_id"]]);
            for (const item of objects(body.genkai_battles?.genkai_battles))
                record(candidates, context, "genkai_battle", item.id, "$.genkai_battles.genkai_battles[]", [["startAt", item.start_at, "$.genkai_battles.genkai_battles[].start_at"], ["endAt", item.end_at, "$.genkai_battles.genkai_battles[].end_at"], ["areaId", item.area_id, "$.genkai_battles.genkai_battles[].area_id"], ["scheduleId", item.genkai_battle_schedule_id, "$.genkai_battles.genkai_battles[].genkai_battle_schedule_id"], ["mapId", item.sugoroku_map_id, "$.genkai_battles.genkai_battles[].sugoroku_map_id"]]);
        }
    });
    const dataset = { schemaVersion: 1, contract: "dokkan-official-capture-schedules-availability", contractVersion: "0.4.0", generatedAt: h0.generatedAt, generatedAtPolicy: "latest_capture_timestamp_for_deterministic_bytes", collectionMode: "offline_allowlisted_product_values_no_requests", productionMutation: false, defaultEnabled: false, authority: "capture_observation_partial_database_identity_remains_authoritative", discardedUserFields: [...discardedUserFields], entities: (0, capture_product_core_1.groupProductFacts)(candidates) };
    const validation = validateCaptureH3(dataset, h0);
    if (!validation.valid)
        throw new Error(`H3 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildCaptureH3 = buildCaptureH3;
function validateCaptureH3(dataset, h0) {
    const failures = [];
    let factCount = 0, supportedCount = 0, partialCount = 0, userDerivedAuthorityCount = 0;
    const entityKeys = new Set(), factIds = new Set();
    ...
    for (const entity of dataset.entities) {
        ...
        if (entityKeys.has(entityKey) || !rules[entity.entityType] || !Number.isSafeInteger(entity.entityId) || entity.entityId <= 0)
        ...
        for (const fact of entity.facts) {
            ...
            if (factIds.has(fact.factId) || !/^[a-f0-9]{64}$/.test(fact.factId) || !fieldRule || fact.factId !== (0, capture_product_core_1.productFactId)(entity.entityType, entity.entityId, withoutId))
                failures.push("fact identity or field");
            factIds.add(fact.factId);
            if (!fieldRule || JSON.stringify(normalizedValue(fieldRule.kind, fact.value)) !== JSON.stringify(fact.value) || (fact.field === "identity" && fact.value !== entity.entityId))
                failures.push("fact value domain");
            if (!fieldRule?.endpoints.includes(fact.provenance.normalizedEndpoint) || !fieldRule?.paths.includes(fact.provenance.jsonPath))
                failures.push("coordinate allowlist");
            if (discardedUserFields.some(value => fact.field.toLowerCase().includes(value) || fact.provenance.jsonPath.toLowerCase().includes(value)))
                failures.push("user field leakage");
            if (fact.provenance.confidence === "partial")
                partialCount += 1;
            else
                supportedCount += 1;
            if (fact.provenance.userDerivedAuthority !== false)
...
exports.validateCaptureH3 = validateCaptureH3;
//# sourceMappingURL=capture-h3-schedules.js.map