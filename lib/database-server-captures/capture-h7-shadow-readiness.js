...
exports.validateCaptureH7 = exports.buildCaptureH7 = void 0;
const metricKeys = ["agreement", "representationGain", "confirmedConflict", "unknown", "unjoinable"];
...
function counts(agreement = 0, representationGain = 0, confirmedConflict = 0, unknown = 0, unjoinable = 0) { return { agreement, representationGain, confirmedConflict, unknown, unjoinable }; }
function numberSet(values) { return new Set(values.flatMap(value => Array.isArray(value) ? value : [value]).map(Number).filter(value => Number.isSafeInteger(value) && value > 0)); }
function values(dataset, entityTypes, fields) { return numberSet(dataset.entities.filter(entity => entityTypes.includes(entity.entityType)).flatMap(entity => entity.facts.filter((fact) => fields.includes(fact.field)).map((fact) => fact.value))); }
function entityIds(dataset, entityTypes) { return numberSet(dataset.entities.filter(entity => entityTypes.includes(entity.entityType)).map(entity => entity.entityId)); }
function difference(left, right) { return [...left].filter(value => !right.has(value)).length; }
function intersection(left, right) { return [...left].filter(value => right.has(value)).length; }
function compareSets(left, right) { return counts(intersection(left, right), difference(left, right), 0, difference(right, left), 0); }
function comparison(key, left, right, unit, basis, value, boundary, includedInTotals = true) { return { key, left, right, unit, basis, includedInTotals, counts: value, boundary }; }
function canonicalDatabasePath(value) { if (!value || value.length > 2048 || /[?#\\%]/.test(value) || value.includes("..") || value.includes("//") || value.startsWith("http:") || value.startsWith("https:"))
    return null; return value.startsWith("/") ? value : `/${value}`; }
function priorS6Counts(input) { return counts(input.agreement ?? 0, input.representationGain ?? 0, input.confirmedConflict ?? 0, input.unknown ?? 0, input.unjoinable ?? 0); }
function priorE7Counts(values) {
    const result = counts();
    for (const value of values) {
        if (value === "agreement")
            result.agreement += 1;
        else if (value === "representation_gain")
            result.representationGain += 1;
        else if (value === "confirmed_conflict")
            result.confirmedConflict += 1;
        else if (value === "unknown")
            result.unknown += 1;
        else if (value === "unjoinable")
            result.unjoinable += 1;
    }
    return result;
}
...
function buildCaptureH7(generatedAt, externalSourceLockSha256, input, lineage, h3, h4, h5, h6) {
    const comparisons = [];
    ...
    const eventKinds = new Set(["area", "chapter", "db_story_group", "origin_episode", "origin_series"]), byId = new Map();
    for (const identity of input.e1Identities.filter(value => eventKinds.has(value.kind)))
        byId.set(identity.id, new Set([...(byId.get(identity.id) ?? []), identity.kind]));
    const captureEvents = entityIds(h3, ["event", "event_key_event"]);
    let eventAgreement = 0, eventGain = 0, eventUnjoinable = 0;
    for (const id of captureEvents) {
    ...
    for (const period of input.s1GashaPeriods.filter(value => commonGashas.has(value.id))) {
        ...
        for (const [field, expected] of [["openAt", period.startAt], ["endAt", period.endAt]]) {
            const observed = [...new Set(entity.facts.filter(fact => fact.field === field && typeof fact.value === "number").map(fact => fact.value))];
            if (observed.length !== 1)
                periodUnknown += 1;
            else if (observed[0] === expected || observed[0] === expected * 1000)
                periodAgreement += 1;
            else
                periodConflict += 1;
        }
    ...
    const comparisonCellTotals = counts();
    for (const item of comparisons.filter(value => value.includedInTotals))
        for (const key of metricKeys)
            comparisonCellTotals[key] += item.counts[key];
    const dataset = { schemaVersion: 1, contract: "dokkan-official-capture-shadow-readiness", contractVersion: "0.8.0", generatedAt, generatedAtPolicy: "latest_capture_timestamp_for_deterministic_bytes", collectionMode: "offline_pinned_artifact_comparison_no_requests", productionMutation: false, defaultEnabled: false, authority: "shadow_parity_only_zero_conflicts_does_not_prove_completeness", aggregationPolicy: "non_exclusive_sum_of_comparison_cells_units_may_overlap", externalSourceLockPath: "database-server-captures/capture-h7-source-lock.json", externalSourceLockSha256, sourceLineage: [...lineage].sort((a, b) => a.key.localeCompare(b.key)), comparisons, comparisonCellTotals, decisions: decisions() };
    const validation = validateCaptureH7(dataset);
    if (!validation.valid)
        throw new Error(`H7 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildCaptureH7 = buildCaptureH7;
function validateCaptureH7(dataset) {
    const failures = [];
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-official-capture-shadow-readiness" || dataset.contractVersion !== "0.8.0" || dataset.generatedAtPolicy !== "latest_capture_timestamp_for_deterministic_bytes" || dataset.collectionMode !== "offline_pinned_artifact_comparison_no_requests" || Number.isNaN(Date.parse(dataset.generatedAt)) || dataset.productionMutation !== false || dataset.defaultEnabled !== false || dataset.authority !== "shadow_parity_only_zero_conflicts_does_not_prove_completeness" || dataset.aggregationPolicy !== "non_exclusive_sum_of_comparison_cells_units_may_overlap" || dataset.externalSourceLockPath !== "database-server-captures/capture-h7-source-lock.json" || !/^[a-f0-9]{64}$/.test(dataset.externalSourceLockSha256))
        failures.push("dataset contract");
    if (JSON.stringify(dataset.sourceLineage.map(value => value.key).sort()) !== JSON.stringify(sourceKeys))
        failures.push("source set");
    for (const source of dataset.sourceLineage) {
        const expected = sourceContracts[source.key];
        if (!expected || source.artifactContract !== expected[0] || source.artifactContractVersion !== expected[1] || source.sourceClass !== expected[2] || !/^[a-z0-9][a-z0-9_-]{0,31}$/.test(source.key) || !/^[A-Za-z0-9_.\/-]+$/.test(source.artifactPath) || source.artifactPath.includes("..") || !/^[a-f0-9]{64}$/.test(source.artifactSha256) || !Number.isSafeInteger(source.artifactSizeBytes) || source.artifactSizeBytes <= 0 || source.manifestSha256 !== null && !/^[a-f0-9]{64}$/.test(source.manifestSha256) || source.validationSha256 !== null && !/^[a-f0-9]{64}$/.test(source.validationSha256) || source.sourceClass === "capture_sidecar" !== (source.manifestPath === null && source.manifestSha256 === null && source.validationSha256 === null) || source.manifestPath !== null && (!/^[A-Za-z0-9_.\/-]+$/.test(source.manifestPath) || source.manifestPath.includes("..")))
            failures.push("source lineage");
    }
    if (JSON.stringify(dataset.comparisons.map(value => value.key).sort()) !== JSON.stringify(comparisonKeys))
        failures.push("comparison set");
    const recomputed = counts();
    for (const item of dataset.comparisons) {
        if (!/^[a-z0-9_]{1,80}$/.test(item.key) || typeof item.includedInTotals !== "boolean" || ![item.left, item.right, item.unit, item.basis, item.boundary].every(value => typeof value === "string" && value.length > 0 && value.length <= 512 && !/[\u0000-\u001f]/.test(value)))
            failures.push("comparison contract");
        for (const key of metricKeys) {
            if (!Number.isSafeInteger(item.counts[key]) || item.counts[key] < 0)
                failures.push("comparison count");
            else if (item.includedInTotals)
                recomputed[key] += item.counts[key];
        }
    }
    if (JSON.stringify(dataset.comparisonCellTotals) !== JSON.stringify(recomputed))
        failures.push("comparison cell totals");
    if (JSON.stringify(dataset.decisions.map(value => value.key).sort()) !== JSON.stringify(decisionKeys))
        failures.push("decision set");
    const expectedStatuses = { android_shadow_mode: "NO_GO", asset_delivery: "NO_GO", future_authenticated_automation: "UNRESOLVED", local_sanitized_fixtures: "GO", merge_disabled_infrastructure: "GO", mission_reward_authority: "NO_GO", r2_publication: "NO_GO", remove_fyi_dokkaninfo: "NO_GO", replace_banners: "NO_GO", replace_schedules: "NO_GO" };
    for (const decision of dataset.decisions)
        if (decision.status !== expectedStatuses[decision.key] || typeof decision.rationale !== "string" || decision.rationale.length === 0 || decision.rationale.length > 1024 || /[\u0000-\u001f]/.test(decision.rationale) || !Array.isArray(decision.exitCriteria) || !decision.exitCriteria.every(value => typeof value === "string" && value.length > 0 && value.length <= 256 && !/[\u0000-\u001f]/.test(value)))
...
exports.validateCaptureH7 = validateCaptureH7;
//# sourceMappingURL=capture-h7-shadow-readiness.js.map