...
const decisionKeys = ["android_shadow", "authenticated_refresh", "disabled_infrastructure", "local_sanitized_fixtures", "r2_publication", "remove_fyi_dokkaninfo", "scraper_replacement"];
function decisions() {
    return [
...
function buildCaptureH12(h8, h9, h10, h11, h13, sourceLockSha, h11Sha, h11Size, h13Sha, h13Size) {
    const dataset = {
    ...
    const validation = validateCaptureH12(dataset);
    if (!validation.valid)
        throw new Error(`H12 validation failed: ${validation.failures.join(", ")}`);
    return dataset;
}
exports.buildCaptureH12 = buildCaptureH12;
function validateCaptureH12(dataset) {
    const failures = [];
    ...
    if (JSON.stringify(dataset.decisions.map(value => value.key)) !== JSON.stringify(decisionKeys))
    ...
    if (dataset.evidence.uniqueGashaAuditFactCount !== 627 || dataset.evidence.legacyUniqueGashaConflictFactCount !== 627 || dataset.evidence.legacyNonExclusiveGashaConflictCellCount !== 1254 || JSON.stringify(dataset.evidence.uniqueGashaAuditCounts) !== JSON.stringify(expectedUnique) || dataset.evidence.nonExclusiveComparisonCellTotals.confirmedConflict !== 0)
        failures.push("corrective gasha evidence");
    const expected = { android_shadow: "NO_GO", authenticated_refresh: "NO_GO", disabled_infrastructure: "GO", local_sanitized_fixtures: "GO", r2_publication: "NO_GO", remove_fyi_dokkaninfo: "NO_GO", scraper_replacement: "NO_GO" };
    for (const decision of dataset.decisions)
        if (decision.status !== expected[decision.key] || !decision.rationale || decision.status === "NO_GO" && decision.exitCriteria.length === 0)
            failures.push("decision policy");
    if (Object.values(dataset.security).some(value => value !== false && value !== 0))
        failures.push("security boundary");
