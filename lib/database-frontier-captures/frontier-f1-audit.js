...
exports.validateFrontierF1 = exports.buildFrontierF1 = exports.selectFrontierDictionaryProofFromDiscovery = exports.scanFrontierDictionaryArtifacts = void 0;
...
function scanFrontierDictionaryArtifacts(lock, roots) {
    ...
    if (!Array.isArray(parsed.scans) || !Array.isArray(parsed.candidates) || parsed.scans.length !== lock.artifacts.length)
    ...
    for (const item of lock.artifacts) {
        const matches = parsed.scans.filter(value => value.artifactRole === item.role), scan = matches[0];
        if (matches.length !== 1 || !scan || scan.sizeBytes !== item.sizeBytes || scan.sha256 !== item.sha256 || ![scan.archiveEntryCount, scan.archiveExpandedBytes, scan.dictionaryHeaderCount].every(value => Number.isSafeInteger(value) && value >= 0))
            throw new Error(`F1 artifact changed during scan ${item.role}`);
    }
    for (const candidate of parsed.candidates) {
        const safeArchiveEntry = candidate.archiveEntry === null || typeof candidate.archiveEntry === "string" && candidate.archiveEntry.length <= 512 && !candidate.archiveEntry.includes("\0") && !candidate.archiveEntry.split(/[\\/]/).includes(".."), identityShapeInvalid = candidate.identityStatus === "embedded_header_length_unknown" ? candidate.sizeBytes !== null || candidate.sha256 !== null : !Number.isSafeInteger(candidate.sizeBytes) || candidate.sizeBytes <= 0 || !/^[a-f0-9]{64}$/.test(candidate.sha256 ?? "");
        if (!roles.has(candidate.artifactRole) || !["file", "archive_entry"].includes(candidate.source) || !safeArchiveEntry || !Number.isSafeInteger(candidate.offset) || candidate.offset < 0 || !Number.isSafeInteger(candidate.dictionaryId) || candidate.dictionaryId < 0 || !["whole_file_dictionary", "whole_archive_entry_dictionary", "embedded_header_length_unknown"].includes(candidate.identityStatus) || identityShapeInvalid)
...
function selectFrontierDictionaryProofFromDiscovery(_candidates, _observedDictionaryId) { return null; }
exports.selectFrontierDictionaryProofFromDiscovery = selectFrontierDictionaryProofFromDiscovery;
...
function validateFrontierF1(dataset, f0, f0Text) { const failures = []; if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-frontier-zstd-frame-and-dictionary-audit" || dataset.contractVersion !== "0.2.0" || dataset.generatedAtPolicy !== "inherits_f0_capture_timestamp" || dataset.collectionMode !== "offline_local_har_no_requests_no_replay" || dataset.productionMutation !== false || dataset.defaultEnabled !== false || dataset.frameFormat !== "zstandard_rfc8878" || !/^[a-f0-9]{64}$/.test(dataset.f0ArtifactSha256) || !Number.isSafeInteger(dataset.f0ArtifactSizeBytes) || dataset.f0ArtifactSizeBytes <= 0)
    ...
    failures.push("frame evidence"); if (dataset.dictionary.identityStatus !== "unknown" || dataset.dictionary.proof !== null || dataset.dictionary.candidateProofPolicy !== "discovery_candidates_never_grant_identity_explicit_pinned_dictionary_required" || typeof dataset.dictionary.unknownReason !== "string" || dataset.dictionary.unknownReason.length === 0)
