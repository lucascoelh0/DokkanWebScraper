...
async function writeProjection(path, projection) {
    ...
    const write = async (chunk) => { rawBytes += Buffer.byteLength(chunk); rawHash.update(chunk, "utf8"); await writeChunk(gzip, chunk); };
    ...
    return { sha256: await (0, source_1.sha256File)(path), sizeBytes: metadata.size, uncompressedSha256: rawHash.digest("hex"), uncompressedSizeBytes: rawBytes };
}
...
async function run() {
    ...
    try {
        ...
        if (!(0, shadow_release_1.manifestMatchesPinnedCharacterShadowRelease)(manifest))
            throw new Error(`K10-K14 release identity changed from the pinned offline profile: ${JSON.stringify(manifest)}`);
        await Promise.all([
