...
exports.validateCharacterCompactArtifact = exports.buildCharacterCompactReadiness = exports.validateCharacterCompactProjection = exports.assertPinnedCharacterCompactReleaseManifest = exports.pinnedCharacterCompactLineage = void 0;
...
function assertPinnedCharacterCompactReleaseManifest(manifest, manifestBytes) {
...
exports.assertPinnedCharacterCompactReleaseManifest = assertPinnedCharacterCompactReleaseManifest;
...
async function readArtifactSnapshot(root, fileName, exactName) {
...
async function validateCharacterCompactArtifact(root) {
    ...
    if (payloadBytes.length !== manifest.sizeBytes || hash(payloadBytes) !== manifest.sha256
        ...
        || payloadBytes.length > compact_contract_1.CHARACTER_COMPACT_GZIP_BUDGET_BYTES)
    ...
    if (raw.length !== manifest.uncompressedSizeBytes || hash(raw) !== manifest.uncompressedSha256
        || raw.length !== release.rawSizeBytes || hash(raw) !== release.rawSha256)
    ...
    const finalManifest = JSON.parse(manifestAfter.bytes.toString("utf8"));
    ...
    for (const [snapshot, sha256, size, label] of [
        ...
        if (snapshot.bytes.length !== size || hash(snapshot.bytes) !== sha256) {
