...
async function verifyK8(directory) {
    for (const file of K8_FILES)
        await (0, source_1.assertPinnedArtifactFile)((0, path_1.resolve)(directory, file.fileName), file);
    const [manifest, validation] = await Promise.all([
        (0, promises_1.readFile)((0, path_1.resolve)(directory, "database-characters-k8-manifest.json"), "utf8").then(JSON.parse),
        (0, promises_1.readFile)((0, path_1.resolve)(directory, "database-characters-k8-validation.json"), "utf8").then(JSON.parse),
    ]);
    if (manifest.sha256 !== K8_FILES[0].sha256 || manifest.sidecarCount !== 8 || manifest.projectedSidecarBytes !== 10166877 || validation.valid !== true || validation.failures.length !== 0)
