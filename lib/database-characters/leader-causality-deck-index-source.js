...
exports.assertLeaderCausalityDeckIndexNativeStable = exports.loadCharacterLeaderCausalityDeckIndexNativeProof = void 0;
...
function evidencePath() {
    const compiled = (0, path_1.resolve)(__dirname, "..", "database-experiment", "native-leader-causality-deck-index.json");
    return (0, fs_1.existsSync)(compiled) ? compiled : (0, path_1.resolve)(__dirname, "..", "..", "database-experiment", "native-leader-causality-deck-index.json");
}
function branchTarget(callerVma, instruction) {
    if (instruction.length !== 4)
        throw new Error(`K54 malformed call instruction at ${callerVma}`);
    const word = instruction.readUInt32LE();
    if (((word & 0xfc000000) >>> 0) !== 0x94000000)
        throw new Error(`K54 native call opcode changed at ${callerVma}`);
...
function pltRelocationOffset(pltVma, bytes) {
    if (bytes.length !== 16)
        throw new Error(`K54 malformed PLT size at ${pltVma}`);
    ...
    if (((adrp & 0x9f00001f) >>> 0) !== 0x90000010 || ((ldr & 0xffc003ff) >>> 0) !== 0xf9400211
        ...
        throw new Error(`K54 malformed PLT at ${pltVma}`);
...
async function loadCharacterLeaderCausalityDeckIndexNativeProof(nativeRuntimePath) {
    if (!nativeRuntimePath)
        throw new Error("K54 requires explicit native runtime path");
    ...
    const [native, evidenceBytes] = await Promise.all([
        (0, leader_scope_source_1.readPinnedLeaderScopeMember)((0, path_1.resolve)(nativeRuntimePath), { sha256: pin.nativeSha256, sizeBytes: pin.nativeSizeBytes }, "K54 native runtime"),
        (0, leader_scope_source_1.readPinnedLeaderScopeMember)(evidencePath(), { sha256: pin.nativeEvidenceSha256, sizeBytes: pin.nativeEvidenceSizeBytes }, "K54 native evidence"),
    ]);
    ...
    for (const vtable of evidence.vtables) {
        ...
        if (typeof vtable.role !== "string" || symbols.length !== 1 || symbols[0].value !== vtable.vma
            ...
            throw new Error(`K54 native vtable changed: ${vtable.role}`);
...
exports.loadCharacterLeaderCausalityDeckIndexNativeProof = loadCharacterLeaderCausalityDeckIndexNativeProof;
function assertLeaderCausalityDeckIndexNativeStable(before, after) {
    if (JSON.stringify(before) !== JSON.stringify(after))
        throw new Error("K54 native deck-index evidence changed");
}
exports.assertLeaderCausalityDeckIndexNativeStable = assertLeaderCausalityDeckIndexNativeStable;
//# sourceMappingURL=leader-causality-deck-index-source.js.map