...
exports.buildCharacterLeaderLifecycleSemanticsReport = exports.assertPinnedCharacterLeaderLifecycleSemantics = exports.assertConservativeK54ForLifecycle = void 0;
...
const K54_GO = ["deckIndexRuntimeArgumentProvenance", "deckIndexIndependentFromTargetType"];
const K54_NO_GO = [
...
function assertConservativeK54ForLifecycle(report) {
    ...
    for (const key of K54_GO)
        if (report.readiness[key] !== "GO")
            throw new Error(`K55 requires K54 ${key} GO`);
    for (const key of K54_NO_GO)
        if (report.readiness[key] !== "NO-GO")
            throw new Error(`K55 requires conservative K54 ${key} NO-GO`);
    const conditional = report.upstreamK53.upstreamK52.scope;
    if (conditional.type82NonNullRows !== leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_PIN.conditionalType82Effects
        || conditional.k48References !== leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_PIN.conditionalType82References
        || report.scope.observedType82CausalityRows !== leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_PIN.conditionalType82Effects
        || report.scope.staticallyBoundRuntimeDeckIndexRows !== 0
        || leader_native_semantics_contract_1.CHARACTER_LEADER_NATIVE_PIN.type82Rows !== leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_PIN.totalType82Effects
        || leader_native_semantics_contract_1.CHARACTER_LEADER_NATIVE_PIN.type82References !== leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_PIN.totalType82References
...
exports.assertConservativeK54ForLifecycle = assertConservativeK54ForLifecycle;
function assertPinnedCharacterLeaderLifecycleSemantics(upstreamK54, native) {
...
exports.assertPinnedCharacterLeaderLifecycleSemantics = assertPinnedCharacterLeaderLifecycleSemantics;
function buildCharacterLeaderLifecycleSemanticsReport(upstreamK54, native) {
    ...
    const report = {
        ...
        contractVersion: leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_CONTRACT_VERSION,
        ...
        inputIntegrity: {
            ...
            reportMaximumBytesExclusive: leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_RSS_LIMIT_BYTES,
    ...
    if (Buffer.byteLength(`${JSON.stringify(report)}\n`) >= leader_lifecycle_semantics_contract_1.CHARACTER_LEADER_LIFECYCLE_SEMANTICS_REPORT_LIMIT_BYTES) {
        throw new Error("K55 report byte limit reached");
    }
    return report;
}
exports.buildCharacterLeaderLifecycleSemanticsReport = buildCharacterLeaderLifecycleSemanticsReport;
//# sourceMappingURL=leader-lifecycle-semantics.js.map