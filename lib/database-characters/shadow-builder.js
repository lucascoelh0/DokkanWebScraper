...
function applyCharacterShadowInMemory(characters, projection) {
    ...
    const byId = new Map();
    const visit = (character) => { if (character?.id)
        byId.set(String(character.id), character); (character?.transformations ?? []).forEach(visit); };
    clone.forEach(visit);
    for (const patch of projection.fields) {
        if (!patch.characterField || patch.authority !== "database_candidate" || patch.evidenceStatus !== "supported" || !["agreement", "representation_gain"].includes(patch.comparison))
            continue;
        const target = byId.get(patch.cardId);
        if (target)
            target[patch.characterField] = patch.databaseValue;
    }
    return clone;
}
