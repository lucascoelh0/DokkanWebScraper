...
exports.buildDatabaseCharacterSkillsCoverage = exports.buildDatabaseCharacterSkillsDataset = exports.stateContainsPassiveSkill = exports.characterStateId = void 0;
const integration_c2_builder_1 = require("../database-integration/integration-c2-builder");
const source_1 = require("./source");
const characterStateId = (cardId, state) => state.growthStep ? `card-state:${cardId}:growth:${state.growthStep.provenance.rowId}` : `card-state:${cardId}:initial`;
exports.characterStateId = characterStateId;
const supportedKey = (rule) => `${rule.identity.stateKey}|${rule.identity.ruleKey}|${rule.identity.efficacyType}|${rule.identity.effectOrdinal}`;
function stateContainsPassiveSkill(state, passiveSkillId) {
    return state.passiveSkill?.relations.some(item => item.skill?.table === "passive_skills" && item.skill.rowId === passiveSkillId) ?? false;
}
exports.stateContainsPassiveSkill = stateContainsPassiveSkill;
function latestReleasedState(states) {
...
async function buildDatabaseCharacterSkillsDataset(options) {
    if (options.c1.sourceSnapshotVersion !== options.source.snapshotVersion || options.c1.sourceDatabaseSha256 !== options.source.databaseSha256)
        throw new Error("C1 source identity does not match DB1");
    if (options.c2.sourceSnapshotVersion !== options.source.snapshotVersion || options.c2.sourceDatabaseSha256 !== options.source.databaseSha256 || options.c2.auditSidecar.sha256 !== options.c1Artifact.sha256)
        throw new Error("C2 source identity does not match DB1/C1");
    const stateSkills = [];
    const actionSkills = [];
    ...
    const ref = (row) => {
        ...
        if (previous && JSON.stringify(previous) !== JSON.stringify(row))
            throw new Error(`Conflicting source rows for ${key}`);
        rawRows.set(key, row);
    ...
    for await (const card of options.cardsInput ?? (0, source_1.streamDb1Cards)(options.source.artifactPath)) {
        for (const state of card.skillStates)
            stateSkills.push({ stateId: (0, exports.characterStateId)(card.cardId, state), sourceStateKey: state.stateKey, cardId: card.cardId, releaseState: state.releaseState,
        ...
        if (card.activeSkills.length || card.standbySkills.length || card.finishSkills.length)
            actionSkills.push({ cardId: card.cardId,
    ...
    for (const state of stateSkills)
        statesByCard.set(state.cardId, [...(statesByCard.get(state.cardId) ?? []), state]);
    const supportedMechanics = options.c2.rules.map(rule => {
        const state = latestReleasedState(statesByCard.get(rule.identity.formId) ?? []);
        if (!state)
            throw new Error(`C2 state does not structurally join: ${rule.identity.stateKey}`);
        const projectedReleaseState = rule.identity.formId === rule.identity.cardId ? state.releaseState : "initial";
        if (projectedReleaseState !== rule.identity.releaseState)
            throw new Error(`C2 release-state projection does not join: ${rule.identity.stateKey}`);
        if (!stateContainsPassiveSkill(state, rule.identity.passiveSkillId))
            throw new Error(`C2 passive skill does not join selected state: ${supportedKey(rule)}`);
...
exports.buildDatabaseCharacterSkillsDataset = buildDatabaseCharacterSkillsDataset;
function buildDatabaseCharacterSkillsCoverage(dataset) {
    const attackCounts = { super: 0, ultra: 0, unit: 0, ex: 0, unknown: 0 };
    for (const state of dataset.stateSkills)
        for (const attack of state.attacks)
            attackCounts[attack.variant.value] += 1;
    ...
    return { schemaVersion: 1, rawRowCount: dataset.rawRows.length, stateCount: dataset.stateSkills.length, leaderSkillStateCount: dataset.stateSkills.filter(state => state.leaderSkill).length, passiveSkillStateCount: dataset.stateSkills.filter(state => state.passiveSkill).length,
        ...
        forbiddenConsumerFieldCount: (0, integration_c2_builder_1.countForbiddenConsumerFields)(dataset.supportedMechanics.map(({ stateId, sourceTeamAnalysisStateKey, sourceReleaseState, ...rule }) => rule)), duplicateStateIdentityCount: dataset.stateSkills.length - new Set(dataset.stateSkills.map(state => state.stateId)).size,
        duplicateSupportedRuleIdentityCount: supportedKeys.length - new Set(supportedKeys).size };
}
exports.buildDatabaseCharacterSkillsCoverage = buildDatabaseCharacterSkillsCoverage;
//# sourceMappingURL=skills-builder.js.map