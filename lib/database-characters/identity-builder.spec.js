...
function card(cardId, formTarget) {
    const source = (table, rowId, values = { id: rowId }) => ({ values, provenance: { table, rowId, columns: Object.keys(values) } });
    return {
        cardId,
        recordKind: Number(cardId) >= 4000000 ? "form" : "collectable",
...
describe("database character identity builder", () => {
    it("keeps card, character, form, state and UI grouping identities separate", async () => {
        ...
        const dataset = await (0, identity_builder_1.buildDatabaseCharacterIdentityDataset)({
        ...
        const coverage = (0, identity_builder_1.buildDatabaseCharacterIdentityCoverage)(dataset);
    ...
    it("rejects a truncated state projection", async () => {
        ...
        const coverage = (0, identity_builder_1.buildDatabaseCharacterIdentityCoverage)(dataset);
        assert.strictEqual((0, identity_validator_1.validateDatabaseCharacterIdentityDataset)(dataset, coverage).valid, false);
        assert.ok((0, identity_validator_1.validateDatabaseCharacterIdentityDataset)(dataset, coverage).failures.some(item => item.includes("10654")));
    });
});
//# sourceMappingURL=identity-builder.spec.js.map