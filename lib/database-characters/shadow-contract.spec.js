...
describe("database character K10 shadow authority", () => {
    it("keeps K0-K2 candidates distinct from mandatory external fallback", () => {
        ...
        for (const field of ["leaderSkill", "passive", "baseHP", "portraitURL", "releaseDate", "equipment", "transformations", "awakeningCards", "previousAwakenings", "nextAwakenings"]) {
            (0, assert_1.equal)(byField.get(field)?.authority, "external_fallback");
        }
        (0, assert_1.equal)(shadow_contract_1.CHARACTER_FIELD_AUTHORITY_MATRIX.filter(rule => rule.owner === "external").length, 74);
        (0, assert_1.equal)(new Set(shadow_contract_1.CHARACTER_FIELD_AUTHORITY_MATRIX.flatMap(rule => rule.characterField ? [rule.characterField] : [])).size, 82);
