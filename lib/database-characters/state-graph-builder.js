...
exports.buildDatabaseCharacterStateGraphCoverage = exports.buildDatabaseCharacterStateGraphDataset = void 0;
const source_1 = require("./source");
function numeric(value) {
    const parsed = Number(value);
    return Number.isFinite(parsed) ? parsed : undefined;
}
function stateId(cardId, state) {
...
function awakeningKind(route) {
    if (route.values.type === "CardAwakeningRoute::Zet")
        return "z_awaken";
    if (route.values.type === "CardAwakeningRoute::Dokkan")
        return "dokkan_awaken";
    if (route.values.type === "CardAwakeningRoute::Optimal" && route.values.optimal_awakening_type === 1)
        return "eza";
    if (route.values.type === "CardAwakeningRoute::Optimal" && route.values.optimal_awakening_type === 2)
        return "seza";
    return "unknown";
}
function formKind(value) {
    ...
    if (value === "giant-or-rage")
        return "giant_or_rage";
    if (value === "reversible-exchange")
        return "reversible_exchange";
    return "unknown";
}
async function buildDatabaseCharacterStateGraphDataset(source, cardsInput = (0, source_1.streamDb1Cards)(source.artifactPath)) {
    const cards = [];
    for await (const card of cardsInput)
        cards.push(card);
    const knownCards = new Set(cards.map(card => card.cardId));
    const states = cards.flatMap(card => card.skillStates.map(state => ({
    ...
    for (const card of cards) {
        ...
        for (const route of card.awakeningPaths.outgoing) {
            if (seenRoutes.has(route.provenance.rowId))
                continue;
...
exports.buildDatabaseCharacterStateGraphDataset = buildDatabaseCharacterStateGraphDataset;
function buildDatabaseCharacterStateGraphCoverage(dataset) {
    const awakeningTransitionCounts = { z_awaken: 0, dokkan_awaken: 0, eza: 0, seza: 0, unknown: 0 };
    const formTransitionCounts = { transformation: 0, giant_or_rage: 0, reversible_exchange: 0, unknown: 0 };
...
exports.buildDatabaseCharacterStateGraphCoverage = buildDatabaseCharacterStateGraphCoverage;
//# sourceMappingURL=state-graph-builder.js.map