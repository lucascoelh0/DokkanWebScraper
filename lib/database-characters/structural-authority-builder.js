...
exports.buildStructuralAuthorityAudit = exports.StructuralAuthorityAuditBuilder = exports.indexProductiveCharacterRecords = exports.compareCharacterClass = exports.compareStructuralCollection = void 0;
const structural_authority_contract_1 = require("./structural-authority-contract");
const sourceFields = new Set(["characterClass", "categoryIds", "categories", "linkIds", "links"]);
...
function compareStructuralCollection(databaseIds, projectedLabels, productiveRaw) {
    ...
    const exclusions = [];
    if (databaseDuplicateIds.length)
        exclusions.push("duplicate_structural_ids");
    if (databaseDuplicateLabels.length)
        exclusions.push("ambiguous_id_to_label_projection");
    if (!projectedLabels)
        exclusions.push("missing_id_to_label_projection");
    if (productiveRaw !== null && productiveRaw !== undefined && !productiveLabels)
        exclusions.push("productive_value_is_not_string_array");
    if (productiveDuplicateLabels.length)
        exclusions.push("duplicate_productive_labels");
    if (exclusions.length)
        return { classification: "unknown", comparison, exclusions };
    if (productiveRaw === null || productiveRaw === undefined)
        return { classification: "representation_gain", comparison, exclusions };
    if (comparison.ordered === "equal")
        return { classification: "agreement", comparison, exclusions };
    if (comparison.set === "equal")
        return { classification: "representation_mismatch", comparison, exclusions: ["same_set_different_order"] };
...
exports.compareStructuralCollection = compareStructuralCollection;
function compareCharacterClass(databaseValue, productiveRaw) {
    ...
    if (productiveRaw === null || productiveRaw === undefined) {
        if (databaseValue === "unawakened")
            return { classification: "unknown", exclusions: ["unawakened_has_no_character_projection"] };
    ...
    if (databaseValue === productiveRaw)
        return { classification: "agreement", exclusions: [] };
    if (databaseValue === "unawakened")
        return { classification: "representation_mismatch", exclusions: ["unawakened_is_not_coerced"] };
    return { classification: "confirmed_conflict", exclusions: ["different_supported_class"] };
}
exports.compareCharacterClass = compareCharacterClass;
function productiveRecord(value, sourceRecordPath, recordKind) {
...
const productiveSignature = (value) => stringify([
...
function indexProductiveCharacterRecords(characters) {
    ...
    for (const id of ids) {
        ...
        if (primary.length === 1)
            ...
            if (alternatives.length && new Set(alternatives.map(productiveSignature)).size === 1)
...
exports.indexProductiveCharacterRecords = indexProductiveCharacterRecords;
function stateIdentity(fields) {
    const stateIds = new Set(fields.map(item => item.stateId).filter(Boolean));
    const stateKeys = new Set(fields.flatMap(item => item.provenance.flatMap(provenance => ["k0", "k1", "k2"].includes(provenance.sidecar) && provenance.sourceState ? [provenance.sourceState.sourceStateKey] : [])));
    const recordKinds = new Set(fields.map(item => item.recordKind));
    const exclusions = [];
    if (stateIds.size !== 1)
        exclusions.push("ambiguous_state_id");
    if (stateKeys.size !== 1)
        exclusions.push("ambiguous_state_key");
    if (recordKinds.size !== 1)
        exclusions.push("ambiguous_record_kind");
    return {
...
function aggregateStatus(fields) {
    if (fields.some(item => item.evidenceStatus === "unknown"))
        return "unknown";
    if (fields.some(item => item.evidenceStatus === "partial"))
        return "partial";
    return "supported";
}
function compactK11Provenance(fields) {
...
function validateK11K2Provenance(fields, cardId, stateId, stateKey, expectedK2Sha256) {
    const exclusions = [];
    for (const item of fields) {
        const evidence = item.provenance.filter(provenance => provenance.sidecar === "k2");
        if (!evidence.length)
            exclusions.push(`${item.field}:missing_k2_provenance`);
        if (evidence.some(provenance => provenance.sidecarSha256 !== expectedK2Sha256 || provenance.sourceSnapshotVersion === ""))
            exclusions.push(`${item.field}:k2_provenance_lineage_changed`);
        if (evidence.some(provenance => provenance.sourceState?.stateId !== stateId || provenance.sourceState?.sourceStateKey !== stateKey))
            exclusions.push(`${item.field}:field_state_binding_changed`);
        if (evidence.some(provenance => provenance.rowId === "" || provenance.table === ""))
            exclusions.push(`${item.field}:incomplete_field_provenance`);
    }
    if (fields.some(item => item.cardId !== cardId))
        exclusions.push("card_binding_changed");
...
function taxonomyIndexes(taxonomy) {
    const unique = (values, label) => {
        ...
        values.forEach(value => {
            if (result.has(value.id))
                throw new Error(`duplicate K2 ${label} ID ${value.id}`);
    ...
    taxonomy.cards.forEach(card => {
        if (cards.has(card.cardId))
            throw new Error(`duplicate K2 card ID ${card.cardId}`);
...
function collectionEvidence(card, idsField, labelsField, field, dictionary, ambiguousLabels) {
    ...
    const exclusions = [];
    if (!Array.isArray(idsField.databaseValue) || !idsField.databaseValue.every(value => typeof value === "string"))
        exclusions.push("K11_structural_ids_are_not_string_array");
    if (!card)
        exclusions.push("missing_K2_card");
    const k2Ids = field === "categories" ? card?.categoryAssignments.map(item => item.categoryId) : card?.links.map(item => item.linkSkillId);
    if (k2Ids && stringify(ids) !== stringify(k2Ids))
        exclusions.push("K11_K2_structural_order_changed");
    if (labels.some(value => value === undefined))
        exclusions.push("missing_id_to_label_projection");
    if (labels.some(value => value !== undefined && ambiguousLabels.has(value)))
        exclusions.push("ambiguous_id_to_label_projection");
    const k11Labels = stringArray(labelsField.databaseValue);
    if (k11Labels && labels.every(value => value !== undefined) && stringify(k11Labels) !== stringify(labels))
        exclusions.push("K11_K2_label_projection_changed");
    ...
    if (assignmentsSupported === false)
        exclusions.push("K2_assignment_not_supported");
    const assignmentProvenance = field === "categories"
    ...
    const labelProvenance = dictionaryEntries.flatMap((entry, index) => entry ? [{
            role: "dictionary_label",
    ...
    return {
        ids,
        labels: labels.every(value => value !== undefined) && !labels.some(value => ambiguousLabels.has(value)) ? labels : null,
...
function candidateFor(classification, databaseValue, ids) {
    if (classification !== "agreement" && classification !== "representation_gain")
        return null;
    return {
...
function productiveProvenance(lineage, productive) {
...
class StructuralAuthorityAuditBuilder {
    cards = new Map();
    accept(item) {
        if (!item || !sourceFields.has(item.field))
            return;
        const field = item.field;
        const card = this.cards.get(item.cardId) ?? { cardId: item.cardId, fields: {} };
        if (card.fields[field])
            throw new Error(`duplicate K11 structural authority field ${item.cardId}:${field}`);
        card.fields[field] = {
            cardId: item.cardId,
    ...
    finish(taxonomy, productiveIndex, lineage, generatedAt) {
        ...
        for (const [cardId, accumulator] of [...this.cards].sort(([left], [right]) => numeric(left, right))) {
            const required = ["characterClass", "categoryIds", "categories", "linkIds", "links"];
            if (required.some(field => !accumulator.fields[field]))
                throw new Error(`incomplete K11 structural authority fields for ${cardId}`);
            const source = accumulator.fields;
            ...
            if (!card)
                classExclusions.push("missing_K2_card");
            if (card && source.characterClass.databaseValue !== card.characterClass.value)
                classExclusions.push("K11_K2_class_projection_changed");
            if (card?.characterClass.status !== "supported")
                classExclusions.push("K2_class_not_supported");
            ...
            if (ambiguousPaths || !productive || identity.exclusions.length)
                classResult = { classification: "unjoinable", exclusions: [ambiguousPaths ? "ambiguous_productive_card_id_binding" : !productive ? "productive_card_id_missing" : "ambiguous_K11_state_binding"] };
            else if (classStatus !== "supported" || classExclusions.length)
                classResult = { classification: "unknown", exclusions: classExclusions };
            ...
            for (const field of ["categories", "links"]) {
                ...
                const evidence = collectionEvidence(card, idsField, labelsField, field, field === "categories" ? taxonomyIndex.categories : taxonomyIndex.links, field === "categories" ? taxonomyIndex.ambiguousCategoryLabels : taxonomyIndex.ambiguousLinkLabels);
                ...
                else if (evidence.status !== "supported" || baseExclusions.length)
                    result = { classification: "unknown", comparison: result.comparison, exclusions: baseExclusions };
        ...
        const fieldSummaries = structural_authority_contract_1.STRUCTURAL_AUTHORITY_FIELDS.map(field => {
            ...
            return {
                ...
                fullCardIdComparableScopeEvidence: cardIdComparableFactCount > 0 && blockers === 0 ? "GO" : "NO-GO",
        ...
        for (const field of ["categories", "links"]) {
            facts.filter(item => item.field === field).forEach(item => {
                ...
                if (value.ordered === "equal")
                    collectionComparisons[field].orderedEqual++;
                else if (value.ordered === "different")
                    collectionComparisons[field].orderedDifferent++;
                if (value.set === "equal")
                    collectionComparisons[field].setEqual++;
                else if (value.set === "different")
                    collectionComparisons[field].setDifferent++;
                if (value.ordered === "different" && value.set === "equal")
                    collectionComparisons[field].sameSetDifferentOrder++;
                if (value.ordered === "unavailable" || value.set === "unavailable")
                    collectionComparisons[field].unavailable++;
            });
        }
        ...
        return {
            ...
            contractVersion: structural_authority_contract_1.STRUCTURAL_AUTHORITY_CONTRACT_VERSION,
            generatedAt,
...
exports.StructuralAuthorityAuditBuilder = StructuralAuthorityAuditBuilder;
function buildStructuralAuthorityAudit(fields, taxonomy, productiveIndex, lineage, generatedAt) {
...
exports.buildStructuralAuthorityAudit = buildStructuralAuthorityAudit;
//# sourceMappingURL=structural-authority-builder.js.map