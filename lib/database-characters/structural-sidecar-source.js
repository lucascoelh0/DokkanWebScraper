...
exports.loadCharacterStructuralSidecarSource = exports.indexProductiveCardIds = exports.validateCharacterStructuralTaxonomy = exports.validateCharacterStructuralProductiveManifest = exports.validateCharacterStructuralK2Manifest = void 0;
...
async function regularRoot(value, label) {
    ...
    if (!metadata.isDirectory() || metadata.isSymbolicLink())
        throw new Error(`K32 ${label} root must be a regular non-link directory`);
    const canonical = await (0, promises_1.realpath)(path);
    if (!samePath(path, canonical))
        throw new Error(`K32 ${label} root symlink or junction rejected`);
    return canonical;
}
async function readInputSnapshot(root, fileName) {
    const path = await (0, artifact_path_1.resolveCharacterInputFile)(root, fileName, fileName);
    const directPath = (0, path_1.join)(root, fileName);
    if (!samePath(path, directPath))
        throw new Error(`K32 ${fileName} symlink or junction rejected`);
    const before = await (0, promises_1.lstat)(directPath);
    if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1)
        throw new Error(`K32 ${fileName} must be a single-link regular file`);
    const handle = await (0, promises_1.open)(directPath, fs_1.constants.O_RDONLY | (fs_1.constants.O_NOFOLLOW ?? 0));
    try {
        const opened = await handle.stat();
        if (!opened.isFile() || opened.nlink !== 1 || !sameFile(before, opened))
            throw new Error(`K32 ${fileName} identity changed while opening`);
        const bytes = await handle.readFile();
        const after = await handle.stat();
        const afterPath = await (0, promises_1.lstat)(directPath);
        if (!sameFile(opened, after) || !sameFile(opened, afterPath) || after.size !== bytes.length || afterPath.size !== bytes.length
            || after.mtimeMs !== opened.mtimeMs || after.ctimeMs !== opened.ctimeMs)
            throw new Error(`K32 ${fileName} changed while reading`);
...
function exact(snapshot, expected, label) {
    if (snapshot.bytes.length !== expected.sizeBytes || hash(snapshot.bytes) !== expected.sha256)
        throw new Error(`K32 ${label} identity changed`);
}
...
function validateCharacterStructuralK2Manifest(manifest) {
    const pin = structural_sidecar_contract_1.CHARACTER_STRUCTURAL_SIDECAR_SOURCE_PIN.k2;
    if (manifest?.schemaVersion !== 1 || manifest.contractVersion !== pin.contractVersion || manifest.generatedAt !== pin.generatedAt
        ...
        || manifest.sourceSnapshotVersion !== structural_sidecar_contract_1.CHARACTER_STRUCTURAL_SIDECAR_SOURCE_PIN.snapshotVersion
...
exports.validateCharacterStructuralK2Manifest = validateCharacterStructuralK2Manifest;
function validateCharacterStructuralProductiveManifest(manifest) {
    ...
    if (manifest?.schemaVersion !== 1 || manifest.datasetVersion !== pin.datasetVersion || manifest.generatedAt !== pin.datasetVersion
        ...
        || manifest.sizeBytes !== pin.payloadSizeBytes || manifest.uncompressedSizeBytes !== pin.uncompressedSizeBytes
        || manifest.characterCount !== pin.topLevelCount)
        throw new Error("K32 productive Character manifest lineage changed");
}
exports.validateCharacterStructuralProductiveManifest = validateCharacterStructuralProductiveManifest;
function validateCharacterStructuralTaxonomy(taxonomy) {
    ...
    if (taxonomy?.schemaVersion !== 1 || taxonomy.contract !== "dokkan-database-characters-taxonomy" || taxonomy.contractVersion !== "1.0.0"
        ...
        || !Array.isArray(taxonomy.links) || taxonomy.links.length !== pin.k2.linkCount)
        throw new Error("K32 K2 taxonomy lineage or cardinality changed");
    if (taxonomy.cards.some(card => typeof card.cardId !== "string" || !/^\d+$/.test(card.cardId)))
        throw new Error("K32 K2 cardId contract changed");
}
exports.validateCharacterStructuralTaxonomy = validateCharacterStructuralTaxonomy;
function indexProductiveCardIds(characters) {
    if (!Array.isArray(characters))
        throw new Error("K32 productive Character payload must be an array");
    const cardIds = new Set();
    let occurrenceCount = 0;
    const accept = (value, path) => {
        if (!value || (typeof value !== "object"))
            throw new Error(`K32 malformed productive Character record at ${path}`);
        if (value.id === undefined || value.id === null || !/^\d+$/.test(String(value.id)))
            throw new Error(`K32 malformed productive cardId at ${path}`);
        const cardId = String(value.id);
        occurrenceCount++;
        if (cardIds.has(cardId))
            throw new Error(`K32 ambiguous productive cardId ${cardId}`);
        cardIds.add(cardId);
        const transformations = value.transformations;
        if (transformations !== undefined && !Array.isArray(transformations))
            throw new Error(`K32 malformed productive transformations at ${path}`);
        (transformations ?? []).forEach((item, index) => accept(item, `${path}.transformations[${index}]`));
    };
    characters.forEach((value, index) => accept(value, `$[${index}]`));
    return { topLevelCount: characters.length, occurrenceCount, cardIds };
}
exports.indexProductiveCardIds = indexProductiveCardIds;
function lineage(productive) {
    const pin = structural_sidecar_contract_1.CHARACTER_STRUCTURAL_SIDECAR_SOURCE_PIN;
    return {
...
async function loadCharacterStructuralSidecarSource(options) {
    ...
    const pin = structural_sidecar_contract_1.CHARACTER_STRUCTURAL_SIDECAR_SOURCE_PIN;
    const names = [pin.k2.manifestFile, pin.k2.payloadFile, pin.k2.coverageFile, pin.k2.validationFile];
    ...
    if (k2CoverageValue?.cardCount !== pin.k2.cardCount || k2CoverageValue.categoryAssignmentCount !== pin.k2.categoryAssignmentCount
        || k2CoverageValue.linkAssignmentCount !== pin.k2.linkAssignmentCount)
        throw new Error("K32 K2 coverage cardinality changed");
    const k2ValidationValue = JSON.parse(k2Validation.bytes.toString("utf8"));
    if (k2ValidationValue?.valid !== true || !Array.isArray(k2ValidationValue.failures) || k2ValidationValue.failures.length !== 0)
        throw new Error("K32 K2 validation is not successful");
    const taxonomyRaw = (0, zlib_1.gunzipSync)(k2Payload.bytes);
    if (taxonomyRaw.length !== pin.k2.uncompressedSizeBytes)
        throw new Error("K32 K2 uncompressed size changed");
    const taxonomy = JSON.parse(taxonomyRaw.toString("utf8"));
    validateCharacterStructuralTaxonomy(taxonomy);
    const productiveRaw = (0, zlib_1.gunzipSync)(productivePayload.bytes);
    if (productiveRaw.length !== pin.productiveCharacters.uncompressedSizeBytes)
        throw new Error("K32 productive uncompressed size changed");
    const productiveCharacters = JSON.parse(productiveRaw.toString("utf8"));
    if (!Array.isArray(productiveCharacters) || productiveCharacters.length !== pin.productiveCharacters.topLevelCount)
        throw new Error("K32 productive top-level cardinality changed");
...
exports.loadCharacterStructuralSidecarSource = loadCharacterStructuralSidecarSource;
//# sourceMappingURL=structural-sidecar-source.js.map