...
exports.buildCharacterLeaderTargetSemanticsReport = exports.assertPinnedCharacterLeaderTargetSemantics = exports.evaluateCharacterLeaderTargetSemantics = void 0;
const leader_target_semantics_contract_1 = require("./leader-target-semantics-contract");
const emptySet = (value) => value === null || value === "0";
function evaluateCharacterLeaderTargetSemantics(k48, k3, targets) {
    ...
    const nonzeroSets = new Set(type82.map(row => row.subTargetTypeSetId).filter(value => !emptySet(value)));
    ...
    for (const ref of references) {
        ...
        if (ref.targetSetId !== effect.subTargetTypeSetId)
            throw new Error(`K51 target-set join changed for ${ref.effectRowId}`);
        if (emptySet(ref.targetSetId)) {
            if (ref.targetRowIds.length)
                throw new Error(`K51 empty target set expanded for ${ref.effectRowId}`);
            continue;
        }
        for (const rowId of ref.targetRowIds) {
            const row = rowsById.get(rowId);
            if (!row || row.targetSetId !== ref.targetSetId)
                throw new Error(`K51 target row join changed for ${rowId}`);
            targetOccurrences++;
        }
    }
    const targetTypeRows = [
        [2, "team_allies"], [12, "super_class_allies"], [13, "extreme_class_allies"],
    ].map(([raw, scope]) => ({ raw, rows: type82.filter(row => row.targetType === raw).length, scope }));
    return {
...
exports.evaluateCharacterLeaderTargetSemantics = evaluateCharacterLeaderTargetSemantics;
function assertPinnedCharacterLeaderTargetSemantics(scope, native) {
    ...
    for (const [label, actual, expected] of checks)
        if (actual !== expected)
            throw new Error(`K51 ${label} hypothesis changed`);
    ...
    if (JSON.stringify(scope.targetTypeRows) !== JSON.stringify(expectedTargets))
        throw new Error("K51 target type corpus changed");
    if (native.targetDispatchEvidenceSha256 !== pin.targetDispatchEvidenceSha256 || native.subTargetEvidenceSha256 !== pin.subTargetEvidenceSha256
        ...
        || !native.targetTypesBound || !native.subTargetTypesBound || !native.leaderRuntimeBridgeBound)
        throw new Error("K51 native target proof changed");
}
exports.assertPinnedCharacterLeaderTargetSemantics = assertPinnedCharacterLeaderTargetSemantics;
function buildCharacterLeaderTargetSemanticsReport(k48, k3, targetFingerprint, native, scope) {
    ...
    const report = {
        ...
        contractVersion: leader_target_semantics_contract_1.CHARACTER_LEADER_TARGET_SEMANTICS_CONTRACT_VERSION, mode: "offline_local_explicit_opt_in_stdout_only",
        ...
        inputIntegrity: { k48SourceBoundBefore: "NOT_EXECUTED", k48SourceBoundAfter: "NOT_EXECUTED", k48K3NativeAndTargetRowsStable: false, nativeEvidenceExactPinned: true, reportTimestampIncluded: false, reportMaximumBytesExclusive: leader_target_semantics_contract_1.CHARACTER_LEADER_TARGET_SEMANTICS_REPORT_LIMIT_BYTES, rssMaximumBytesExclusive: leader_target_semantics_contract_1.CHARACTER_LEADER_TARGET_SEMANTICS_RSS_LIMIT_BYTES },
    ...
    if (Buffer.byteLength(`${JSON.stringify(report, null, 2)}\n`) >= leader_target_semantics_contract_1.CHARACTER_LEADER_TARGET_SEMANTICS_REPORT_LIMIT_BYTES)
        throw new Error("K51 report byte limit reached");
    return report;
}
exports.buildCharacterLeaderTargetSemanticsReport = buildCharacterLeaderTargetSemanticsReport;
//# sourceMappingURL=leader-target-semantics.js.map