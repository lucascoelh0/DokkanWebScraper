...
async function runCharacterStateProductScopeAudit(options) {
    ...
    let initial = await (0, shadow_source_1.loadCharacterShadowInputs)(loadOptions);
    const scope = (0, state_product_scope_1.evaluateCharacterStateProductScope)(initial);
    const fingerprintSha256 = (0, state_product_scope_1.fingerprintCharacterStateProductScopeInputs)(initial);
    const sources = {
        fingerprintSha256,
    ...
    if (global.gc)
        global.gc();
    let reloaded = await (0, shadow_source_1.loadCharacterShadowInputs)(loadOptions);
    const reloadedSources = {
        fingerprintSha256: (0, state_product_scope_1.fingerprintCharacterStateProductScopeInputs)(reloaded),
    ...
    if (JSON.stringify(sources) !== JSON.stringify(reloadedSources))
        throw new Error("K42 sources changed after evaluation");
    reloaded = undefined;
    if (global.gc)
        global.gc();
