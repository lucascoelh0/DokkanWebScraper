...
exports.buildDatabaseCharacterTaxonomyCoverage = exports.buildDatabaseCharacterTaxonomyDataset = void 0;
const source_1 = require("./source");
function text(value) { return value === null || value === undefined ? "" : String(value).trim(); }
function label(row, column) { return { value: text(row.values[column]), sourceLocale: "global_snapshot_default", source: { table: row.provenance.table, rowId: row.provenance.rowId, column } }; }
function rowId(row) { return String(row.id); }
async function buildDatabaseCharacterTaxonomyDataset(options) {
    const sourceCards = [];
    for await (const card of options.cardsInput ?? (0, source_1.streamDb1Cards)(options.source.artifactPath))
        sourceCards.push(card);
    const cardById = new Map(sourceCards.map(card => [card.cardId, card]));
    const zSources = new Map();
    for (const card of sourceCards)
        for (const route of card.awakeningPaths.outgoing)
            if (route.values.type === "CardAwakeningRoute::Zet") {
    ...
    const roots = (cardId, visited = new Set()) => {
        if (visited.has(cardId))
            return { cardIds: [], routeRowIds: [], cycle: true };
        visited.add(cardId);
        const sources = zSources.get(cardId) ?? [];
        if (sources.length === 0)
            return { cardIds: [cardId], routeRowIds: [], cycle: false };
    ...
    for (const card of sourceCards) {
        card.categories.forEach(item => { if (item.category)
            categoryRows.set(item.category.provenance.rowId, item.category); });
        card.links.forEach(item => { if (item.skill)
            linkRows.set(item.skill.provenance.rowId, item.skill); });
    }
    const levelsByLink = new Map();
    for (const row of options.linkLevels)
        levelsByLink.set(String(row.link_skill_id), [...(levelsByLink.get(String(row.link_skill_id)) ?? []), row]);
    const effectsByLevel = new Map();
    for (const row of options.linkEfficacies)
        effectsByLevel.set(String(row.link_skill_lv_id), [...(effectsByLevel.get(String(row.link_skill_lv_id)) ?? []), row]);
    const categories = [...categoryRows].map(([id, row]) => ({ id, label: label(row, "name") })).sort((a, b) => Number(a.id) - Number(b.id));
    const links = [...linkRows].map(([id, row]) => ({
        ...
        levels: (levelsByLink.get(id) ?? []).sort((a, b) => Number(a.skill_lv) - Number(b.skill_lv)).map(levelRow => ({
            ...
            description: { value: text(levelRow.description), sourceLocale: "global_snapshot_default", source: { table: "link_skill_lvs", rowId: rowId(levelRow), column: "description" } },
            effects: (effectsByLevel.get(rowId(levelRow)) ?? []).sort((a, b) => Number(a.id) - Number(b.id)).map(effect => ({ rowId: rowId(effect), raw: effect, status: "supported_structural_raw_semantics_uninterpreted" })),
        })),
    ...
    const cards = sourceCards.map(card => {
        ...
        return {
            ...
            rarity: { raw: card.rarity.raw, value: card.rarity.value, status: (card.rarity.value === "unknown" ? "unknown" : "supported") },
            originalRarity: { status: (rootEvidence.cycle ? "unknown" : rootIds.length === 1 ? "supported" : "partial"), sourceCardIds: rootIds, zRouteRowIds: rootEvidence.routeRowIds.sort((a, b) => Number(a) - Number(b)), cycleDetected: rootEvidence.cycle, rawValues: rootCards.map(item => item.rarity.raw), values: rootCards.map(item => item.rarity.value) },
            type: { raw: card.type.raw, value: card.type.value, status: (card.type.value === "unknown" ? "unknown" : "supported") },
            characterClass: { raw: card.characterClass.raw, value: card.characterClass.value, status: (card.characterClass.value === "unknown" ? "unknown" : "supported") },
            categoryAssignments: card.categories.map(item => ({ categoryId: String(item.relation.values.card_category_id), relationRowId: item.relation.provenance.rowId, status: (item.category ? "supported" : "unknown") })).sort((a, b) => Number(a.relationRowId) - Number(b.relationRowId)),
            links: card.links.map(item => ({ slot: item.slot, linkSkillId: String(card.card.values[`link_skill${item.slot}_id`]), status: (item.skill ? "supported" : "unknown"), sourceColumn: `link_skill${item.slot}_id` })),
        };
...
exports.buildDatabaseCharacterTaxonomyDataset = buildDatabaseCharacterTaxonomyDataset;
function buildDatabaseCharacterTaxonomyCoverage(dataset) {
...
exports.buildDatabaseCharacterTaxonomyCoverage = buildDatabaseCharacterTaxonomyCoverage;
//# sourceMappingURL=taxonomy-builder.js.map