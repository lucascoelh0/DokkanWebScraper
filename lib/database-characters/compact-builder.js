...
function classifyExclusion(card) {
    ...
    if (values.some(item => !item))
        return "incomplete";
    const fields = values;
    if (fields.some(item => item.productionJoin?.status !== "joined" || item.comparison === "unjoinable"))
        return "unjoinable";
    if (fields.some(item => item.evidenceStatus === "partial"))
        return "partial";
    if (fields.some(item => item.evidenceStatus !== "supported" || item.comparison === "unknown"))
        return "unknown";
    if (fields.some(item => item.comparison === "confirmed_conflict" || item.sourceComparisons?.production === "confirmed_conflict"))
        return "conflict";
    if (fields.some(item => !["agreement", "representation_gain"].includes(item.comparison)))
        return "mismatch";
    if (fields.some(item => item.authority !== "database_candidate" || item.characterField !== item.field))
        return "mismatch";
    if (!rarities.has(String(card.fields.rarity.databaseValue)) || !types.has(String(card.fields.type.databaseValue)))
        return "invalidEnum";
    return null;
}
