...
exports.parseCharacterLeaderSupportedPublicShadowCli = void 0;
...
function parseCharacterLeaderSupportedPublicShadowCli(args) {
    const allowed = new Set(["--opt-in-k61", "--remote-read-only", ...VALUES]);
    for (let index = 0; index < args.length; index++) {
        const argument = args[index];
        if (!allowed.has(argument))
            throw new Error(`K61 unsupported argument ${argument}`);
...
exports.parseCharacterLeaderSupportedPublicShadowCli = parseCharacterLeaderSupportedPublicShadowCli;
async function main() {
    const report = await (0, leader_supported_public_shadow_1.runCharacterLeaderSupportedPublicShadow)(parseCharacterLeaderSupportedPublicShadowCli(process.argv.slice(2)));
    const stdout = `${JSON.stringify(report, null, 2)}\n`;
    if (Buffer.byteLength(stdout) >= leader_supported_public_shadow_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLIC_SHADOW_REPORT_LIMIT_BYTES)
        throw new Error("K61 stdout report limit reached");
    process.stdout.write(stdout);
}
if (require.main === module)
    main().catch(error => {
        console.error(`K61 failed: ${(error instanceof Error ? error.message : String(error)).slice(0, 512)}`);
        process.exitCode = 1;
    });
//# sourceMappingURL=leader-supported-public-shadow-run.js.map