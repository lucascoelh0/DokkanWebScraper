...
exports.buildCharacterShadowReadiness = void 0;
function buildCharacterShadowReadiness(projection, coverage, validation) {
    const fallbackProven = validation.valid && coverage.productionUnjoinableCount === 1463
    ...
    const fields = projection.authorityMatrix.map(rule => {
        ...
        if (!field)
            throw new Error(`readiness coverage missing for ${rule.field}`);
        ...
        const blockers = [];
        if (!criteria.unequivocalStructuralBinding)
            blockers.push(rule.owner === "external" ? "K0-K2 are not the field owner" : "state binding is ambiguous");
        if (!criteria.supportedEvidence)
            blockers.push("not all database cards have supported evidence");
        if (!criteria.sufficientParity)
            blockers.push("joined production values include mismatch, unknown, or conflict");
        if (!criteria.fallbackProven)
            blockers.push("fallback safety validation is not green");
        if (!criteria.zeroUnresolvedConflict)
            blockers.push("unresolved field conflict exists");
        if (!criteria.productFieldExists)
            blockers.push("shadow-only structural dimension has no Character field");
        return { field: rule.field, decision: Object.values(criteria).every(Boolean) ? "GO" : "NO-GO", criteria, blockers, patchableCharacterCount: field.patchableCharacterCount };
    });
    return {
        schemaVersion: 1,
...
exports.buildCharacterShadowReadiness = buildCharacterShadowReadiness;
//# sourceMappingURL=shadow-readiness-builder.js.map