...
exports.runCharacterLeaderSupportedPublisherDryRun = exports.assertCharacterLeaderSupportedPublisherSourceStable = exports.maximumIndividualCharacterLeaderSupportedPublisherProcessPeakRss = exports.validateCharacterLeaderSupportedPublisherDryRunArtifact = exports.writeCharacterLeaderSupportedPublisherDryRunArtifacts = exports.assertCharacterLeaderSupportedPublisherDryRunArtifacts = exports.buildCharacterLeaderSupportedPublisherDryRun = void 0;
...
function assertParsed(bytes, value, label) {
    let parsed;
    try {
        parsed = JSON.parse(bytes.toString("utf8"));
    }
    catch {
        throw new Error(`K58 ${label} JSON rejected`);
    }
    if (json(parsed) !== json(value))
        throw new Error(`K58 ${label} bytes/object mismatch`);
}
function assertStructuralK56(artifacts) {
    if (artifacts.raw.length >= leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_RAW_LIMIT_BYTES
        ...
        throw new Error("K58 K56 byte budget rejected");
    if (!(0, zlib_1.gunzipSync)(artifacts.gzip, { maxOutputLength: leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_RAW_LIMIT_BYTES }).equals(artifacts.raw)
        || !(0, zlib_1.gzipSync)(artifacts.raw, { level: 9 }).equals(artifacts.gzip))
        throw new Error("K58 K56 raw/payload identity rejected");
    ...
    if (!/^database-characters-k56-leader-supported-projection\.[a-f0-9]{64}\.json\.gz$/.test(manifest.fileName)
        ...
        || manifest.coverageFile !== leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_FILES.coverage
        ...
        || json(manifest.counts) !== json(artifacts.coverage.counts))
        throw new Error("K58 K56 manifest lineage rejected");
    ...
    if (json(counts) !== json({
        ...
        !== json(leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_PIN.conditionalEffectRowIds)
...
function immutableObjects(artifacts) {
    const sources = [
        ...
        { kind: "manifest", name: leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_FILES.manifest, bytes: artifacts.manifestBytes, contentType: "application/json" },
    ];
    return sources.map((source, index) => {
        ...
        return {
            ...
            objectKey: `${leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE}/objects/sha256/${sha256}/${source.name}`,
            sha256, sizeBytes: source.bytes.length, contentType: source.contentType,
            cacheControl: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_IMMUTABLE_CACHE_CONTROL,
...
function materialize(artifacts, dryRun) {
    assertStructuralK56(artifacts);
    const objects = immutableObjects(artifacts);
    const source = {
        fullArtifactFingerprintSha256: (0, leader_supported_shadow_1.characterLeaderSupportedShadowArtifactFingerprint)(artifacts),
        lineageFingerprintSha256: (0, leader_supported_shadow_1.characterLeaderSupportedShadowLineageFingerprint)(artifacts),
        rawIdentity: {
            sha256: artifacts.manifest.uncompressedSha256,
            sizeBytes: artifacts.manifest.uncompressedSizeBytes,
            persistedOrRemoteObject: false,
        },
    ...
    const candidateManifest = {
        ...
        contractVersion: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_DRY_RUN_CONTRACT_VERSION,
        namespace: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE, source, counts,
        immutableObjects: objects,
        policy: {
            candidateOnly: true, manifestObjectKey: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_MANIFEST_KEY,
    ...
    if (projectedRemoteBytes >= leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE_BUDGET_BYTES) {
    ...
    const plan = {
        ...
        namespace: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE, source, counts, immutableObjects: objects,
        mutableManifest: {
            order: "LAST_AFTER_ALL_IMMUTABLE_OBJECTS", objectKey: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_MANIFEST_KEY,
        ...
        budget: {
            conservativeNamespaceBudgetBytes: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE_BUDGET_BYTES,
            projectedRemoteBytes,
            conservativeNamespaceHeadroomBytes: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE_BUDGET_BYTES - projectedRemoteBytes,
    ...
    const marker = {
        schemaVersion: 1, contract: "dokkan-database-character-leader-supported-publisher-dry-run-marker",
        contractVersion: leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_DRY_RUN_CONTRACT_VERSION,
        planSha256: hash(planBytes), receiptSha256: hash(receiptBytes), candidateManifestSha256: hash(candidateManifestBytes),
        localArtifactSetComplete: true, markerWrittenLast: true,
        remotePreflight: "NOT_EXECUTED", remoteMutationCount: 0,
    };
...
function buildCharacterLeaderSupportedPublisherDryRun(artifacts) {
    return materialize(artifacts, "NOT_EXECUTED");
}
exports.buildCharacterLeaderSupportedPublisherDryRun = buildCharacterLeaderSupportedPublisherDryRun;
function assertCharacterLeaderSupportedPublisherDryRunArtifacts(artifacts) {
    for (const [label, value, bytes] of [
        ...
        if (!jsonBytes(value).equals(bytes) || bytes.length >= leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_REPORT_LIMIT_BYTES) {
    ...
    const expectedNames = [
        ...
        leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_FILES.coverage,
        leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_FILES.validation,
        leader_supported_projection_contract_1.CHARACTER_LEADER_SUPPORTED_PROJECTION_FILES.manifest,
    ];
    if (artifacts.candidateManifest.schemaVersion !== 1
        ...
        || artifacts.candidateManifest.contractVersion !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_DRY_RUN_CONTRACT_VERSION
        || artifacts.candidateManifest.namespace !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE
        || artifacts.plan.schemaVersion !== 1
        || artifacts.plan.contract !== "dokkan-database-character-leader-supported-publisher-dry-run-plan"
        || artifacts.plan.contractVersion !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_DRY_RUN_CONTRACT_VERSION
        || artifacts.plan.mode !== "explicit_opt_in_offline_local_dry_run_only_timestamp_free"
        || artifacts.plan.namespace !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE
        ...
        || object.cacheControl !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_IMMUTABLE_CACHE_CONTROL
        ...
        || artifacts.plan.mutableManifest.objectKey !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_MANIFEST_KEY
        ...
        || artifacts.plan.budget.conservativeNamespaceBudgetBytes !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE_BUDGET_BYTES
        || artifacts.plan.budget.conservativeNamespaceHeadroomBytes
            !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_NAMESPACE_BUDGET_BYTES - projectedRemoteBytes
    ...
    if (artifacts.receipt.contract !== "dokkan-database-character-leader-supported-publisher-dry-run-receipt"
        || artifacts.receipt.contractVersion !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_DRY_RUN_CONTRACT_VERSION
        || artifacts.marker.contract !== "dokkan-database-character-leader-supported-publisher-dry-run-marker"
        || artifacts.marker.contractVersion !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_DRY_RUN_CONTRACT_VERSION
...
exports.assertCharacterLeaderSupportedPublisherDryRunArtifacts = assertCharacterLeaderSupportedPublisherDryRunArtifacts;
...
async function inspectRoot(value, label) {
    ...
    if (!metadata.isDirectory() || metadata.isSymbolicLink())
        throw new Error(`K58 ${label} must be an existing regular non-link directory`);
    const canonical = await (0, promises_1.realpath)(path);
    if (!samePath(path, canonical))
        throw new Error(`K58 ${label} symlink or junction rejected`);
...
async function writeCreateOnly(root, name, bytes) {
    ...
    if (!samePath(path, (0, path_1.resolve)(root.path, name)))
        throw new Error("K58 local member escaped output root");
    ...
    try {
        ...
        if (!opened.isFile() || opened.nlink !== 1 || opened.size !== bytes.length || !sameFile(opened, visible)
            ...
            throw new Error("K58 create-only member identity rejected");
    }
    finally {
        await handle.close();
    }
...
async function writeCharacterLeaderSupportedPublisherDryRunArtifacts(outputRoot, artifacts) {
    assertCharacterLeaderSupportedPublisherDryRunArtifacts(artifacts);
    const root = await inspectRoot(outputRoot, "output root");
    const files = [
        [leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_FILES.candidateManifest, artifacts.candidateManifestBytes],
        [leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_FILES.plan, artifacts.planBytes],
        [leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_FILES.receipt, artifacts.receiptBytes],
        [leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_FILES.marker, artifacts.markerBytes],
    ];
    for (const [name] of files) {
        try {
            await (0, promises_1.lstat)((0, path_1.join)(root.path, name));
            throw new Error(`K58 local output already exists: ${name}`);
        }
        catch (error) {
    ...
    for (const [name, bytes] of files) {
        await writeCreateOnly(root, name, bytes);
        written.push(name);
    }
    return Object.freeze([...written]);
}
exports.writeCharacterLeaderSupportedPublisherDryRunArtifacts = writeCharacterLeaderSupportedPublisherDryRunArtifacts;
async function readK58Member(root, name) {
    await checkpoint(root);
    const path = (0, path_1.join)(root.path, name);
    if (!samePath(path, (0, path_1.resolve)(root.path, name)))
        throw new Error("K58 source-bound member escaped artifact root");
    const before = await (0, promises_1.lstat)(path);
    if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1
        || before.size >= leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_REPORT_LIMIT_BYTES) {
...
async function readCharacterLeaderSupportedPublisherDryRunArtifactSet(artifactRoot) {
    ...
    const [candidateManifestBytes, planBytes, receiptBytes, markerBytes] = await Promise.all(names.map(name => readK58Member(root, name)));
    let candidateManifest, plan, receipt, marker;
    try {
        ...
        throw new Error("K58 source-bound artifact JSON rejected");
    }
    await checkpoint(root);
...
async function validateCharacterLeaderSupportedPublisherDryRunArtifact(options) {
    if (!options.k56Root || !options.artifactRoot)
        throw new Error("K58 validator requires explicit K56 and K58 artifact roots");
    const k56 = await (0, leader_supported_projection_source_1.validateCharacterLeaderSupportedProjectionArtifact)({
    ...
    if (k56.sourceBoundValidation !== "GO")
        throw new Error("K58 validator requires K56 source-bound GO");
    (0, leader_supported_shadow_1.assertExactCharacterLeaderSupportedShadowK56Identity)(k56.artifacts);
...
exports.validateCharacterLeaderSupportedPublisherDryRunArtifact = validateCharacterLeaderSupportedPublisherDryRunArtifact;
async function validateOutputSeparation(options) {
    ...
    for (const [label, value] of [["native runtime", options.nativeRuntime], ["database", options.database]]) {
        const metadata = await (0, promises_1.lstat)((0, path_1.resolve)(value));
...
class ParentRssGuard {
    ...
    observe() {
        ...
        this.exceeded ||= this.peak >= leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_RSS_LIMIT_BYTES;
    }
    sample() { this.observe(); if (this.exceeded)
        throw new Error(`K58 parent per-process RSS limit reached: ${this.peak}`); }
    stop() { clearInterval(this.timer); this.sample(); return this.peak; }
    dispose() { clearInterval(this.timer); }
}
function maximumIndividualCharacterLeaderSupportedPublisherProcessPeakRss(...peaks) {
    if (peaks.length !== 3 || peaks.some(peak => !Number.isSafeInteger(peak) || peak <= 0
        || peak >= leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_RSS_LIMIT_BYTES))
        throw new Error("K58 per-process RSS peak rejected");
    return Math.max(...peaks);
}
exports.maximumIndividualCharacterLeaderSupportedPublisherProcessPeakRss = maximumIndividualCharacterLeaderSupportedPublisherProcessPeakRss;
function assertCharacterLeaderSupportedPublisherSourceStable(beforeFingerprint, beforeLineageFingerprint, after) {
    if (beforeFingerprint !== (0, leader_supported_shadow_1.characterLeaderSupportedShadowArtifactFingerprint)(after)
        || beforeLineageFingerprint !== (0, leader_supported_shadow_1.characterLeaderSupportedShadowLineageFingerprint)(after)) {
        throw new Error("K58 K56 source fingerprint or lineage drifted across dry-run");
    }
}
exports.assertCharacterLeaderSupportedPublisherSourceStable = assertCharacterLeaderSupportedPublisherSourceStable;
const validateCharacterLeaderSupportedProjectionFromSources = leader_supported_projection_source_1.validateCharacterLeaderSupportedProjectionArtifact;
async function runCharacterLeaderSupportedPublisherDryRun(options) {
    if (options?.optIn !== true)
        throw new Error("K58 requires explicit opt-in");
    ...
    if (typeof global.gc !== "function")
        throw new Error("K58 requires Node --expose-gc");
    ...
    try {
        ...
        if (before.sourceBoundValidation !== "GO")
            throw new Error("K58 before source-bound K56 validation did not return GO");
        ...
        const beforeLineage = (0, leader_supported_shadow_1.characterLeaderSupportedShadowLineageFingerprint)(before.artifacts);
        ...
        if (direct.plan.readiness.dryRun !== "NOT_EXECUTED")
            throw new Error("K58 direct builder auto-authorized dry-run");
        before = undefined;
        global.gc();
        rss.sample();
        ...
        if (after.sourceBoundValidation !== "GO")
            throw new Error("K58 after source-bound K56 validation did not return GO");
        ...
        if (!authorized.candidateManifestBytes.equals(authorizedAgain.candidateManifestBytes)
            ...
            || !direct.candidateManifestBytes.equals(authorized.candidateManifestBytes))
            throw new Error("K58 materialization is not deterministic");
        after = undefined;
        global.gc();
        rss.sample();
        const written = await writeCharacterLeaderSupportedPublisherDryRunArtifacts(options.outputRoot, authorized);
        if (written[written.length - 1] !== leader_supported_publisher_dry_run_contract_1.CHARACTER_LEADER_SUPPORTED_PUBLISHER_FILES.marker)
            throw new Error("K58 marker was not written last");
        const parentPeak = rss.stop();
        const maximumIndividualProcessPeakRssBytes = maximumIndividualCharacterLeaderSupportedPublisherProcessPeakRss(beforePeak, afterPeak, parentPeak);
        return {
            outputRoot: (0, path_1.resolve)(options.outputRoot), planSha256: hash(authorized.planBytes),
...
exports.runCharacterLeaderSupportedPublisherDryRun = runCharacterLeaderSupportedPublisherDryRun;
//# sourceMappingURL=leader-supported-publisher-dry-run.js.map