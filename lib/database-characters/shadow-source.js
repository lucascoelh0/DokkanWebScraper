...
exports.CHARACTER_SHADOW_REQUIRED_GATES = exports.loadCharacterShadowInputs = exports.compactCharacters = void 0;
...
function compactCharacter(value, sourceRecordPath) {
...
const comparableExternal = ({ sourceRecordPath: _path, ...value }) => JSON.stringify(value);
...
function compactCharacters(input) {
    ...
    input.forEach((value, index) => {
        ...
        if (topLevelIds.has(id))
            throw new Error(`duplicate top-level external character ID ${id}`);
        topLevelIds.add(id);
    ...
    const visitNested = (value, path) => {
        ...
        transformations.forEach((item, index) => {
            ...
            else if (!topLevelIds.has(id) && comparableExternal(previous) !== comparableExternal(compact))
                throw new Error(`ambiguous duplicate nested external character ID ${id}`);
...
exports.compactCharacters = compactCharacters;
