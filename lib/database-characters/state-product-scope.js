...
exports.assertCharacterStateProductScopeInputsUnchanged = exports.fingerprintCharacterStateProductScopeInputs = exports.assertCharacterStateProductScopePins = exports.evaluateStateProductScope = exports.evaluateCharacterStateProductScope = void 0;
const crypto_1 = require("crypto");
const state_product_scope_contract_1 = require("./state-product-scope-contract");
const selection = () => ({ included: 0, excluded: 0 });
const structuralIdOrder = (left, right) => left < right ? -1 : left > right ? 1 : 0;
function accept(total, grouped, included) {
    const key = included ? "included" : "excluded";
    total[key]++;
    grouped[key]++;
}
function increment(count, included) {
    const key = included ? "included" : "excluded";
    count[key]++;
}
function unique(values, label) {
    if (new Set(values).size !== values.length)
        throw new Error(`K42 duplicate ${label}`);
}
function limited(values) {
    return values.sort(structuralIdOrder).slice(0, state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_SAMPLE_LIMIT);
}
/**
 ...
 */
function evaluateCharacterStateProductScope(inputs) {
    ...
    if (!inputs.k7 || !Array.isArray(inputs.k7.cards))
        throw new Error("K42 malformed K7 coverage");
    ...
    if (k0States.size !== inputs.k0.states.length)
        throw new Error("K42 duplicate K0 state identity");
    const states = {
        ...selection(),
    ...
    const excludedStateIds = [];
    const excludedReleaseIds = [];
    ...
    for (const state of graph.states) {
        ...
        if (!included)
            excludedStateIds.push(state.stateId);
    }
    for (const transition of graph.releaseStateTransitions) {
        ...
        if (!included)
            excludedReleaseIds.push(transition.transitionId);
    }
    for (const transition of graph.awakeningTransitions) {
        ...
        if (!included)
            excludedAwakeningIds.push(transition.transitionId);
    }
    for (const transition of graph.formTransitions) {
        ...
        if (!included)
            excludedFormIds.push(transition.transitionId);
    }
    let agreement = 0;
    let unjoinable = 0;
    for (const card of inputs.k7.cards) {
        if (card.production.identity === "agreement")
            agreement++;
        else if (card.production.identity === "unjoinable")
            unjoinable++;
        else
            throw new Error(`K42 unsupported K7 production identity ${card.cardId}`);
    }
    return {
        states,
        releaseTransitions,
        ...
        excludedStructuralIds: {
            ...
            limitPerScope: state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_SAMPLE_LIMIT,
        },
    };
}
exports.evaluateCharacterStateProductScope = evaluateCharacterStateProductScope;
exports.evaluateStateProductScope = evaluateCharacterStateProductScope;
function assertCharacterStateProductScopePins(scope) {
    const comparisons = [
        ["states", scope.states, state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_PIN.states],
        ["release transitions", scope.releaseTransitions, state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_PIN.releaseTransitions],
        ["awakening transitions", scope.awakeningTransitions, state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_PIN.awakeningTransitions],
        ["form transitions", scope.formTransitions, state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_PIN.formTransitions],
    ];
    ...
    if (scope.productionCoverage.agreement !== state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_PIN.productionCoverage.agreement
        || scope.productionCoverage.unjoinable !== state_product_scope_contract_1.CHARACTER_STATE_PRODUCT_SCOPE_PIN.productionCoverage.unjoinable) {
        throw new Error("K42 K7 production coverage pin changed");
    }
}
exports.assertCharacterStateProductScopePins = assertCharacterStateProductScopePins;
function structuralFingerprintValue(inputs) {
    const sortedKeys = (values) => [...values].sort(structuralIdOrder);
    return {
...
function fingerprintCharacterStateProductScopeInputs(inputs) {
    return (0, crypto_1.createHash)("sha256").update(JSON.stringify(structuralFingerprintValue(inputs)), "utf8").digest("hex");
}
exports.fingerprintCharacterStateProductScopeInputs = fingerprintCharacterStateProductScopeInputs;
function assertCharacterStateProductScopeInputsUnchanged(before, after) {
    ...
    if (beforeFingerprint !== afterFingerprint)
        throw new Error("K42 structural source fingerprint changed after evaluation");
    return beforeFingerprint;
}
exports.assertCharacterStateProductScopeInputsUnchanged = assertCharacterStateProductScopeInputsUnchanged;
//# sourceMappingURL=state-product-scope.js.map