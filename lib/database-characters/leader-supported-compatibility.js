...
function validateCharacterLeaderSupportedCompatibilityReport(report) {
    const failures = [];
    if (report.contractVersion !== leader_supported_compatibility_contract_1.CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_CONTRACT_VERSION
        || report.checkpoint !== "K62.1")
        failures.push("K62.1 contract identity changed");
    if (report.lineage.sourceStability !== "CHECKPOINTED_PERSISTENT_DRIFT_ONLY"
        || report.lineage.transientABADriftDetection !== "NO-GO"
        || "stableAcrossAudit" in report.lineage)
        failures.push("source stability overclaim");
    ...
    if (!androidSource || androidSource.repositoryUrl !== leader_supported_compatibility_golden_1.CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_ANDROID_SOURCE_PIN.repositoryUrl
        || androidSource.commit !== leader_supported_compatibility_golden_1.CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_ANDROID_SOURCE_PIN.commit
        || androidSource.access !== "git_object_database_only" || androidSource.checkoutBytesRead !== false
        || json(androidSource.files) !== json(leader_supported_compatibility_golden_1.CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_ANDROID_SOURCE_PIN.files)
        ...
        if (fingerprintSha256 !== hash(json(fingerprintInput)))
            failures.push("Android Git object source provenance changed");
    }
