...
exports.buildTaxonomyProjection = exports.projectSupportedTaxonomyRecords = exports.assertPinnedTaxonomyProjectionCardScope = exports.assertPinnedTaxonomyProjectionSources = void 0;
...
function emptyDimensionCoverage() {
    return {
        includedCardCount: 0,
        includedFactCount: 0,
        excludedCardCount: 0,
...
function exclude(coverage, reason, cardId, factCount) {
    coverage.excludedCardCount++;
    coverage.excludedFactCount += factCount;
    coverage.exclusions[reason]++;
    if (coverage.examples[reason].length < taxonomy_projection_contract_1.TAXONOMY_PROJECTION_MAX_EXAMPLES)
        coverage.examples[reason].push(cardId);
}
function pinnedK32ManifestBytes(source) {
    return prettyBytes(source.manifest);
}
function assertPinnedTaxonomyProjectionSources(sources) {
    ...
    if (k32.sidecar.records.length !== pin.expected.cardCount || coverage.cardCount !== pin.expected.cardCount
        ...
        || coverage.linkEntryCount !== pin.expected.linkEntryCount)
        throw new Error("K35 K32 supported coverage changed");
...
exports.assertPinnedTaxonomyProjectionSources = assertPinnedTaxonomyProjectionSources;
function assertPinnedTaxonomyProjectionCardScope(report, stdout) {
    const pin = taxonomy_projection_contract_1.TAXONOMY_PROJECTION_SOURCE_PIN;
    (0, card_scope_evaluator_1.validateCardScopeReport)(report);
    const canonicalK34 = (0, card_scope_evaluator_1.serializeCardScopeReport)(report);
    if (canonicalK34 !== stdout || Buffer.byteLength(canonicalK34) !== pin.k34.reportSizeBytes
        || hash(canonicalK34) !== pin.k34.reportSha256)
        throw new Error("K35 K34 exact report pin changed");
    ...
    if (gates.reportExecution !== "GO" || gates.productiveAuthority !== "NO-GO" || gates.applyOrCharacterMutation !== "NO-GO"
        ...
        || gates.dokkanInfoRemoval !== "NO-GO")
        throw new Error("K35 accepts K34 only with every productive gate NO-GO");
}
exports.assertPinnedTaxonomyProjectionCardScope = assertPinnedTaxonomyProjectionCardScope;
function buildTaxonomyProjectionLineage(sources) {
    const k32 = sources.k32.manifest;
    const k34 = sources.k34Report;
    return {
        profileId: taxonomy_projection_contract_1.TAXONOMY_PROJECTION_SOURCE_PIN.profileId,
        k32: {
...
function projectSupportedTaxonomyRecords(sourceRecordsInput) {
    const records = [];
    const coverage = {
        schemaVersion: 1,
    ...
    for (const source of sourceRecords) {
        const record = { cardId: source.cardId };
...
exports.projectSupportedTaxonomyRecords = projectSupportedTaxonomyRecords;
function buildTaxonomyProjection(sources) {
    ...
    if (coverage.sourceCardCount !== expected.cardCount || coverage.projectedCardCount !== expected.cardCount
        ...
        || linkCoverage.includedFactCount !== expected.linkEntryCount)
        throw new Error("K35 projected supported coverage changed");
    return {
        projection: {
            schemaVersion: 1,
            contract: "dokkan-database-character-taxonomy-projection",
            contractVersion: taxonomy_projection_contract_1.TAXONOMY_PROJECTION_CONTRACT_VERSION,
            generatedAt: taxonomy_projection_contract_1.TAXONOMY_PROJECTION_SOURCE_PIN.generatedAt,
...
exports.buildTaxonomyProjection = buildTaxonomyProjection;
//# sourceMappingURL=taxonomy-projection-builder.js.map