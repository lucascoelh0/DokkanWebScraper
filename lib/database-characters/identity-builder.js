...
exports.buildDatabaseCharacterIdentityCoverage = exports.buildDatabaseCharacterIdentityDataset = void 0;
const source_1 = require("./source");
function relation(name, table, assignments, note) {
...
async function buildDatabaseCharacterIdentityDataset(source, cardsInput = (0, source_1.streamDb1Cards)(source.artifactPath)) {
    const cards = [];
    const states = [];
    const characterCards = new Map();
    const characterUniqueInfos = new Map();
    const characterRows = new Map();
    const linkAssignments = [];
    const categoryAssignments = [];
    const awakeningAssignments = [];
    ...
    for await (const card of cardsInput) {
        ...
        const uniqueInfos = characterUniqueInfos.get(card.ids.characterId) ?? new Set();
        ...
        for (const item of card.formRelations)
            formAssignments.push(assignment({ relation: "form", sourceId: card.cardId, targetId: item.targetCardId, table: item.provenance.table, rowId: item.provenance.rowId, columns: item.provenance.columns, resolved: false }));
    ...
    const characters = [...characterCards].map(([characterId, cardIds]) => ({
        characterId,
        ...
        source: { table: "characters", rowId: characterRows.get(characterId) },
    ...
    for (const item of awakeningAssignments)
        item.status = item.targetId && knownCardIds.has(item.targetId) ? "supported" : "partial";
    for (const item of awakeningAssignments)
        item.missing = item.status === "supported" ? [] : ["target_not_in_selected_identity_payload"];
    for (const item of formAssignments)
        item.status = item.targetId && knownCardIds.has(item.targetId) ? "supported" : "unknown";
    for (const item of formAssignments)
        item.missing = item.status === "supported" ? [] : [item.targetId ? "target_not_in_selected_identity_payload" : "target_id_or_join_missing"];
...
exports.buildDatabaseCharacterIdentityDataset = buildDatabaseCharacterIdentityDataset;
function buildDatabaseCharacterIdentityCoverage(dataset) {
...
exports.buildDatabaseCharacterIdentityCoverage = buildDatabaseCharacterIdentityCoverage;
//# sourceMappingURL=identity-builder.js.map