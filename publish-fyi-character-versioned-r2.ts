...
import {
    ...
    readValidatedFyiCharacterReleasePortrait,
...
import {
    ...
    FyiCharacterVersionedPreflightK27,
} from "./fyi-character-versioned-portraits";

export const FYI_CHARACTER_K28_CONTRACT = "dokkan-fyi-character-versioned-r2-publisher-k28";
...
export interface FyiCharacterVersionedPublisherOptions {
    releaseId: string;
    mode: "dry-run" | "publish";
    confirmedDeliveryId?: string;
}

export interface FyiCharacterR2Object {
    bytes: Buffer;
    etag: string;
    contentType?: string;
    cacheControl?: string;
...
export interface FyiCharacterConditionalR2Writer {
    read(objectKey: string, maxBytes: number): Promise<FyiCharacterR2Object | undefined>;
    createIfAbsent(input: FyiCharacterR2WriteInput): Promise<"written" | "precondition_failed">;
    replaceIfMatch(input: FyiCharacterR2WriteInput, etag: string): Promise<"written" | "precondition_failed">;
}

export interface FyiCharacterR2WriteInput {
    objectKey: string;
    bytes: Buffer;
    contentType: string;
    cacheControl: string;
...
export interface FyiCharacterVersionedPublishSummary {
    schemaVersion: 1;
    contract: typeof FYI_CHARACTER_K28_CONTRACT;
    contractVersion: "1.1.0";
    mode: "dry-run" | "publish";
    deliveryId: string;
    preflight: FyiCharacterVersionedPreflightK27;
    ...
    checks: {
        ...
        everyImmutableDirectlyVerified: boolean;
        cacheMetadataVerified: boolean;
        contentAddressedObjectsBeforeManifest: true;
        everyUploadVerified: boolean;
        noDeletes: true;
    ...
    readiness: {
        dryRun: "GO";
        publication: "NOT_EXECUTED" | "COMPLETED";
        production: "NO-GO";
        android: "NO-GO";
    };
}

export function parseFyiCharacterVersionedPublisherArgs(args: string[]): FyiCharacterVersionedPublisherOptions {
    const common = args.length >= 5
        && args[0] === "--opt-in-k28"
        && args[1] === "--release-id"
        && RELEASE_ID.test(args[2])
        && args[3] === "--remote";
    if (!common) throw new Error("K28 requires an exact explicit remote command");
...
export async function publishFyiCharacterVersionedR2(options: {
    fyiRoot: string;
    publisher: FyiCharacterVersionedPublisherOptions;
    remoteReader?: FyiCharacterRemoteReader;
    bucketSizeReader?: FyiCharacterBucketSizeReader;
    conditionalWriter?: FyiCharacterConditionalR2Writer;
    checkedAt?: string;
}): Promise<FyiCharacterVersionedPublishSummary> {
    const validated = await readValidatedFyiCharacterRelease(options.fyiRoot, options.publisher.releaseId);
    ...
    const preflight = await buildFyiCharacterVersionedPreflightK27({
        validated,
        delivery,
        reader: options.remoteReader ?? createFyiCharacterPublicHttpsReader(),
        bucketSizeReader: options.bucketSizeReader ?? createFyiCharacterWranglerBucketSizeReader(),
        checkedAt: options.checkedAt,
    });
    if (preflight.readiness.versionedRemotePreflight !== "GO") throw new Error("K28 remote preflight is not GO");
    ...
    if (options.publisher.mode === "dry-run") return summary("dry-run", delivery, preflight, plannedUploads, 0, 0, false);
    ...
    if (!manifestObject) throw new Error("K28 manifest object missing");
    ...
    for (const object of immutable) {
        ...
        if (!remote) {
            ...
            if (outcome === "written") uploaded += 1;
            remote = await writer.read(object.objectKey, object.sizeBytes + 1);
            if (!remote) throw new Error(`K28 immutable object missing after conditional create: ${object.objectKey}`);
        }
    ...
    if (!initialManifest || !remoteMatches(initialManifest, manifestObject.sha256, manifestObject.sizeBytes, manifestMetadata)) {
        ...
        if (outcome === "written") manifestPromoted = true;
    }
    const finalManifest = await writer.read(manifestObject.objectKey, manifestObject.sizeBytes + 1);
    if (!finalManifest) throw new Error("K28 manifest missing after conditional promotion");
...
async function bytesForObject(
    validated: Awaited<ReturnType<typeof readValidatedFyiCharacterRelease>>,
    delivery: FyiCharacterVersionedDelivery,
    objectKey: string,
): Promise<Buffer> {
    const object = delivery.plan.objects.find(candidate => candidate.objectKey === objectKey);
    if (!object || object.kind === "manifest") throw new Error(`K28 immutable object rejected: ${objectKey}`);
    if (object.kind === "payload") return delivery.payload;
    if (!object.sourceFileName) throw new Error(`K28 portrait source missing: ${objectKey}`);
    return readValidatedFyiCharacterReleasePortrait(validated, object.sourceFileName);
}

function metadataFor(kind: "payload" | "portrait" | "manifest"): Pick<FyiCharacterR2WriteInput, "contentType" | "cacheControl"> {
    return {
...
function assertBytesMatch(label: string, bytes: Buffer, expectedSha256: string, expectedSizeBytes: number): void {
...
function remoteMatches(
    remote: FyiCharacterR2Object,
    expectedSha256: string,
    expectedSizeBytes: number,
    metadata: Pick<FyiCharacterR2WriteInput, "contentType" | "cacheControl">,
): boolean {
...
function assertManifestMatchesPreflight(
    direct: FyiCharacterR2Object | undefined,
    preflight: FyiCharacterVersionedPreflightK27,
    expectedSha256: string,
    expectedSizeBytes: number,
): void {
    if (!direct) {
        if (preflight.manifest.status !== "missing") throw new Error("K28 remote manifest disappeared after preflight");
        return;
    }
    const directHash = sha256(direct.bytes);
    if (directHash === expectedSha256 && direct.bytes.length === expectedSizeBytes) return;
...
async function bodyToBuffer(body: unknown, maxBytes: number): Promise<Buffer> {
    if (!body || !(Symbol.asyncIterator in Object(body))) throw new Error("K28 R2 response body is not streamable");
    const chunks: Buffer[] = [];
    let total = 0;
    for await (const chunk of body as AsyncIterable<Uint8Array>) {
        const bytes = Buffer.from(chunk);
        total += bytes.length;
        if (total > maxBytes) throw new Error("K28 R2 object exceeds its validated size bound");
        chunks.push(bytes);
    }
    return Buffer.concat(chunks, total);
}

export function createFyiCharacterS3ConditionalWriter(options: {
    accountId: string;
    accessKeyId: string;
    secretAccessKey: string;
}): FyiCharacterConditionalR2Writer {
    if (!/^[a-f0-9]{32}$/.test(options.accountId)) throw new Error("K28 CLOUDFLARE_ACCOUNT_ID is invalid");
    if (!options.accessKeyId || !options.secretAccessKey) throw new Error("K28 R2 S3 credentials are missing");
    ...
    return {
        read: async (objectKey, maxBytes) => {
            ...
            try {
                const output = await client.send(new GetObjectCommand({ Bucket: FYI_CHARACTER_K24_BUCKET, Key: objectKey }));
                ...
                if (!output.ETag) throw new Error(`K28 R2 object has no ETag: ${objectKey}`);
                return {
...
async function conditionalPut(
    client: S3Client,
    input: FyiCharacterR2WriteInput,
    ...
    try {
        ...
        if (isStatus(error, 412)) return "precondition_failed";
        throw error;
    }
}

export function createFyiCharacterS3ConditionalWriterFromEnvironment(): FyiCharacterConditionalR2Writer {
