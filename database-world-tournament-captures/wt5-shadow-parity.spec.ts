...
describe("world tournament WT5", () => {
    ...
    it("validates an integrated observed definition branch", () => { const value = fixture(), mission = value.comparisons.find(item => item.key === "mission_relation")!; mission.status = "unknown"; mission.matchedCount = 1; value.rewards.definitions = rewardDefinitionBoundary(63001, true, 631, false); value.totals.coverage_gap--; value.totals.unknown++; assert.equal(validateWt5(value).valid, true); });
    it("rejects an inter-Budokai structural ID collision", () => { assert.equal(parentedStructuralMatches([{ missionId: 63001, budokaiId: 62 }], "missionId", [63001], 63), 0); assert.equal(parentedStructuralMatches([{ missionId: 63001, budokaiId: 63 }], "missionId", [63001], 63), 1); });
