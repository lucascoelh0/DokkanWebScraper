...
export function buildWt5(wt2: Wt2Dataset, wt2Text: string, wt3: Wt3Dataset, wt3Text: string, wt4: Wt4Dataset, wt4Text: string, lockText: string, sources: Wt5Sources): Wt5Dataset {
    ...
    if (eventId !== 63 || JSON.stringify(mapIds) !== JSON.stringify([631,632,633,634]) || JSON.stringify(boxIds) !== JSON.stringify([631]) || JSON.stringify(scheduleIds) !== JSON.stringify([5]) || JSON.stringify(missionIds) !== JSON.stringify([63001])) throw new Error("WT5 pinned structural coordinates changed");
