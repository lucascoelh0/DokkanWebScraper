...
import { DatabaseTeamAnalysisDb46Dataset } from "./team-analysis-db46-contract";
import { DatabaseTeamAnalysisDb47Coverage, DatabaseTeamAnalysisDb47Dataset, Db47NativeEvidence, Db47RuleTiming } from "./team-analysis-db47-contract";
...
const branchTarget = (bytes: Buffer, vma: number) => { const word = bytes.readUInt32LE(); if (word >>> 26 !== 37) return null; let immediate = word & 0x3ffffff; if (immediate & 0x2000000) immediate -= 0x4000000; return vma + immediate * 4; };

export function projectDb47Timing(value: unknown): Db47RuleTiming["executionTiming"] | null { return integer(value) === 15 ? { status: "supported", event: EVENT.name, sequence: EVENT.sequence } : null; }

export function validateDb47Evidence(inspection: NativeRuntimeElfInspection, evidence: Db47NativeEvidence, nativeSha256: string): void {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.auditScope !== "execution-timing-15-puzzle-attack-move-end" || JSON.stringify(evidence.regions) !== JSON.stringify(REGIONS) || JSON.stringify(evidence.event) !== JSON.stringify(EVENT) || JSON.stringify(evidence.directCalls) !== JSON.stringify(DIRECT_CALLS) || JSON.stringify(evidence.virtualCall) !== JSON.stringify(VIRTUAL_CALL) || JSON.stringify(evidence.doesNotImply) !== JSON.stringify(DOES) || JSON.stringify(evidence.unknowns) !== JSON.stringify(UNKNOWNS)) throw Error("DB47 evidence identity");
    for (const region of REGIONS) { const symbol = inspection.symbols.find(value => value.name === region.symbol); const bytes = inspection.readVirtualBytes(region.vma, region.sizeBytes); if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes || hash(bytes) !== region.codeSha256) throw Error("DB47 code region"); }
    for (const instruction of EVENT.instructions) if (inspection.readVirtualBytes(instruction.vma, 4).toString("hex") !== instruction.hex) throw Error("DB47 instruction");
    for (const call of DIRECT_CALLS) {
        if (branchTarget(inspection.readVirtualBytes(call.callVma, 4), call.callVma) !== call.pltVma || inspection.readVirtualBytes(call.pltVma, 16).toString("hex") !== call.pltHex) throw Error("DB47 direct call");
        const relocations = inspection.relocations.filter(value => value.offset === call.relocation.offset);
        if (relocations.length !== 1 || JSON.stringify(relocations[0]) !== JSON.stringify({ offset: call.relocation.offset, type: call.relocation.type, symbolName: call.relocation.symbol, symbolValue: call.relocation.symbolValue, addend: call.relocation.addend })) throw Error("DB47 direct call relocation");
    }
    if (inspection.readVirtualBytes(VIRTUAL_CALL.callVma, 4).toString("hex") !== VIRTUAL_CALL.callHex) throw Error("DB47 virtual call");
    const vtable = inspection.symbols.find(value => value.name === VIRTUAL_CALL.vtable.symbol);
    if (!vtable || vtable.value !== VIRTUAL_CALL.vtable.vma || vtable.size !== VIRTUAL_CALL.vtable.sizeBytes || hash(inspection.readVirtualBytes(vtable.value, vtable.size)) !== VIRTUAL_CALL.vtable.rawSha256) throw Error("DB47 virtual call vtable");
    const virtualRelocations = inspection.relocations.filter(value => value.offset === VIRTUAL_CALL.relocation.offset);
    if (VIRTUAL_CALL.relocation.offset !== VIRTUAL_CALL.vtable.vma + 16 + VIRTUAL_CALL.vptrSlotOffset || virtualRelocations.length !== 1 || JSON.stringify(virtualRelocations[0]) !== JSON.stringify({ offset: VIRTUAL_CALL.relocation.offset, type: VIRTUAL_CALL.relocation.type, symbolName: VIRTUAL_CALL.relocation.symbol, symbolValue: VIRTUAL_CALL.relocation.symbolValue, addend: VIRTUAL_CALL.relocation.addend })) throw Error("DB47 virtual call relocation");
}

export function buildDatabaseTeamAnalysisDb47Dataset(source: DatabaseTeamAnalysisDb46Dataset, sourceSha256: string, inspection: NativeRuntimeElfInspection, nativeSha256: string, nativeSizeBytes: number, evidence: Db47NativeEvidence, evidenceSha256: string): DatabaseTeamAnalysisDb47Dataset {
    if (source.contractVersion !== "0.45.0" || sourceSha256 !== DB46_SHA) throw Error("DB47 lineage");
    validateDb47Evidence(inspection, evidence, nativeSha256);
    const ruleTimings = source.ruleTimings.map(value => { const projection = projectDb47Timing(value.rawExecutionTimingType); return projection ? { ...value, executionTiming: projection, provenance: { database: value.provenance.database, runtime: { fileName: "libcocos2dcpp.so" as const, sha256: nativeSha256, evidenceFile: "native-puzzle-move-end-timing-semantics.json", evidenceSha256, proofRoles: REGIONS.map(region => region.role) } } } : value; });
