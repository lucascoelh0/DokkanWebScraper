...
function branchTarget(bytes: Buffer, instructionVma: number): number {
    const word = bytes.readUInt32LE(0);
    if (((word & 0xfc000000) >>> 0) !== 0x94000000) throw new Error(`Expected BL at ${instructionVma}`);
    let immediate = word & 0x03ffffff;
    if (immediate & 0x02000000) immediate -= 0x04000000;
    return instructionVma + immediate * 4;
}

export function validateNativeSpecialActionBreakEvidence(
    ...
    if (JSON.stringify(evidence.stringLiterals?.map((value: any) => [value.value, value.enumValue]))
    ...
    if (JSON.stringify((evidence.codeRegions ?? []).map((region: any) => [region.role, region.symbol, region.vma, region.sizeBytes, region.codeSha256])) !== JSON.stringify(EXPECTED_CODE_REGIONS)) {
    ...
    if (JSON.stringify((evidence.structuralAssertions ?? []).map((assertion: any) => [assertion.role, assertion.vma, assertion.hex])) !== JSON.stringify(EXPECTED_ASSERTIONS)) {
    ...
    if (JSON.stringify((evidence.directCalls ?? []).map((call: any) => [call.role, call.callVma, call.callHex, call.targetPltVma, call.gotRelocationOffset, call.targetSymbol, call.targetVma])) !== JSON.stringify(EXPECTED_CALLS)) {
