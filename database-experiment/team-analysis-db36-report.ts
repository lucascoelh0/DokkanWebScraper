import {DatabaseTeamAnalysisDb36Coverage} from "./team-analysis-db36-contract";const rows=(x:Record<string,number>)=>Object.entries(x).map(([k,v])=>`| \`${k}\` | ${v} |`).join("\n");
export function renderDatabaseTeamAnalysisDb36Report(c:DatabaseTeamAnalysisDb36Coverage){return `# Database Team Analysis experiment — DB36 sub-target semantics
