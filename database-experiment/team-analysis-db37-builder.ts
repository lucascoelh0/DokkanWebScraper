...
import { DatabaseTeamAnalysisDb37Coverage, DatabaseTeamAnalysisDb37Dataset, Db37DurationProjection, Db37NativeEvidence, Db37OnceOnlyProjection } from "./team-analysis-db37-contract";
...
const integer = (value: unknown) => typeof value === "number" && Number.isSafeInteger(value) && value >= -2147483648 && value <= 2147483647
    ? value
...
function validateEvidence(inspection: NativeRuntimeElfInspection, evidence: Db37NativeEvidence, nativeSha256: string) {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.auditScope !== "passive-skill-turn-is-once-sqlite-runtime-lifecycle") throw new Error("DB37 evidence identity mismatch");
    if (JSON.stringify(evidence.sqliteBinding) !== JSON.stringify(EXPECTED_SQLITE_BINDING) || JSON.stringify(evidence.runtimeStorage) !== JSON.stringify(EXPECTED_RUNTIME_STORAGE) || JSON.stringify(evidence.onceOnly) !== JSON.stringify(EXPECTED_ONCE_ONLY) || JSON.stringify(evidence.duration) !== JSON.stringify(EXPECTED_DURATION) || JSON.stringify(evidence.executedThisTurn) !== JSON.stringify(EXPECTED_EXECUTED_THIS_TURN) || JSON.stringify(evidence.resetBoundary) !== JSON.stringify(EXPECTED_RESET_BOUNDARY) || JSON.stringify(evidence.vtable) !== JSON.stringify(EXPECTED_VTABLE) || JSON.stringify(evidence.unknowns) !== JSON.stringify(EXPECTED_UNKNOWNS)) throw new Error("DB37 evidence boundary mismatch");
    const roles = new Set(evidence.codeRegions.map(region => region.role));
    if (roles.size !== evidence.codeRegions.length || roles.size !== Object.keys(EXPECTED_REGIONS).length) throw new Error("DB37 evidence role cardinality mismatch");
    for (const region of evidence.codeRegions) {
        const expected = EXPECTED_REGIONS[region.role];
        if (!expected || JSON.stringify([region.symbol, region.vma, region.sizeBytes, region.codeSha256]) !== JSON.stringify(expected)) throw new Error(`DB37 evidence role mismatch ${region.role}`);
        const symbol = inspection.symbols.find(value => value.name === region.symbol);
        if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes || sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw new Error(`DB37 native code mismatch ${region.role}`);
    }
    const vtable = inspection.symbols.find(value => value.name === evidence.vtable.symbol);
    if (!vtable || vtable.value !== evidence.vtable.vma || vtable.size !== evidence.vtable.sizeBytes || sha256(inspection.readVirtualBytes(vtable.value, vtable.size)) !== evidence.vtable.rawSha256) throw new Error("DB37 vtable identity mismatch");
    for (const relocation of evidence.vtable.relocations) if (inspection.relocations.filter(value => value.offset === evidence.vtable.vma + relocation.offset && value.symbolName === relocation.symbol).length !== 1) throw new Error(`DB37 vtable relocation mismatch ${relocation.symbol}`);
}

export function projectDb37OnceOnly(rawIsOnce: unknown): Db37OnceOnlyProjection {
    const raw = integer(rawIsOnce);
    if (raw === undefined) return { status: "unknown", rawIsOnce: rawIsOnce as any, normalizedIsOnce: null, enabled: null, viabilityPredicate: "unknown", successfulExecutionMutation: "unknown", execCountReset: { operation: "unknown", trigger: "unknown" } };
    const enabled = raw !== 0;
    return { status: "supported", rawIsOnce: rawIsOnce as any, normalizedIsOnce: enabled ? 1 : 0, enabled, viabilityPredicate: enabled ? "exec_count_less_than_1" : "no_additional_is_once_gate", successfulExecutionMutation: enabled ? "increment_exec_count" : "independent", execCountReset: { operation: "set_zero", trigger: "unknown" } };
}

export function projectDb37Duration(rawTurn: unknown): Db37DurationProjection {
    const raw = integer(rawTurn);
    if (raw === undefined || raw < -1) return { status: "unknown", rawTurn: rawTurn as any, initialCurrentTurn: null, maximumTurn: null, successfulExecutionMutation: "unknown", endTurnMutation: "unknown", endTurnPredicate: "unknown" };
    if (raw === -1) return { status: "partial", rawTurn: rawTurn as any, initialCurrentTurn: -1, maximumTurn: -1, successfulExecutionMutation: "current_turn_equals_maximum_turn", endTurnMutation: "sentinel_minus_one_not_decremented_when_update_eligible", endTurnPredicate: "current_turn_less_than_1" };
    return { status: "supported", rawTurn: rawTurn as any, initialCurrentTurn: raw, maximumTurn: raw, successfulExecutionMutation: "current_turn_equals_maximum_turn", endTurnMutation: "decrement_one_then_deactivate_at_nonpositive_when_update_eligible", endTurnPredicate: "current_turn_less_than_1" };
}

export function buildDatabaseTeamAnalysisDb37Dataset(options: { db36: DatabaseTeamAnalysisDb36Dataset, db36Sha256: string, db20: DatabaseTeamAnalysisDb20Dataset, db20Sha256: string, tables: DatabaseExperimentTables, inspection: NativeRuntimeElfInspection, nativeSha256: string, nativeSizeBytes: number, evidence: Db37NativeEvidence, evidenceSha256: string }): DatabaseTeamAnalysisDb37Dataset {
    ...
    const ruleLifecycles = options.db36.ruleSubTargets.map(rule => {
        ...
        if (!row) throw new Error(`DB37 passive row missing ${rule.passiveSkillId}`);
        ...
        return {
            ...
            simulationStatus: onceOnly.status === "unknown" || duration.status === "unknown" ? "unknown" as const : "partial" as const,
            onceOnly, duration,
            executedThisTurn: { status: "supported" as const, successfulExecutionMutation: "set_true" as const, endTurnMutation: "set_false" as const, independentFromExecCount: true as const },
            independentDimensions: { condition: "independent" as const, target: "inherited_db36" as const, timing: "independent" as const, operation: "independent" as const, unit: "independent" as const, calculationBucket: "independent" as const, recurrence: "partial" as const, stacking: "unknown" as const },
            provenance: { database: { table: "passive_skills" as const, rowId: rule.passiveSkillId, columns: ["turn", "is_once"] as ["turn", "is_once"] }, runtime: { fileName: "libcocos2dcpp.so" as const, sha256: options.nativeSha256, evidenceFile: "native-passive-lifecycle-semantics.json" as const, evidenceSha256: options.evidenceSha256, proofRoles } },
        };
    });
...
function distribution(items: DatabaseTeamAnalysisDb37Dataset["ruleLifecycles"], selector: (item: DatabaseTeamAnalysisDb37Dataset["ruleLifecycles"][number]) => unknown, effects = false) {
    const output: Record<string, number> = {};
    for (const item of items) { const key = String(selector(item)); output[key] = (output[key] ?? 0) + (effects ? item.effectCount : 1); }
    return Object.fromEntries(Object.entries(output).sort(([left], [right]) => Number(left) - Number(right)));
}

export function buildDatabaseTeamAnalysisDb37Coverage(dataset: DatabaseTeamAnalysisDb37Dataset): DatabaseTeamAnalysisDb37Coverage {
