...
function fixture() {
    ...
    const candidate = (stateKey: string, value: number, databaseRuleKey: string) => ({ stateKey, value, negated: false, databaseRuleKey, databaseLowerSignature: JSON.stringify({ logicalContext: "all", negated: false }), currentExactSignatures: [signature(value)] });
    const db12 = { contractVersion: "0.11.0", generatedAt: "x", sourceDb11: { sha256: "db11" }, sourceCurrentTeamAnalysis: { sha256: "current" }, sourceDatabaseSha256: "database", semanticPromotionCount: 0, matchedStateCount: 3,
        ...
        diagnosticCurrentExactTurnAtoms: [
            { stateKey: "1:1:initial", atom: { value: 3, negated: false, structuralSignature: signature(3) }, sourceRuleKeys: ["current-exact"] },
            { stateKey: "3:3:initial", atom: { value: 5, negated: false, structuralSignature: signature(5) }, sourceRuleKeys: ["current-ambiguous"] },
        ] } as any;
