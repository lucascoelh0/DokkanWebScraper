import { DatabaseTeamAnalysisDb11Coverage, DatabaseTeamAnalysisDb11Parity } from "./team-analysis-db11-contract";
import { DatabaseTeamAnalysisDb10Coverage } from "./team-analysis-db10-contract";

function percent(value: number, total: number): string { return total === 0 ? "0.0%" : `${(value * 100 / total).toFixed(1)}%`; }
function compact(values: string[]): string {
    const visible = values.slice(0, 8); const remaining = values.length - visible.length;
    return `${visible.join("; ") || "none"}${remaining > 0 ? `; … +${remaining}` : ""}`;
}
export function renderDatabaseTeamAnalysisDb11Report(coverage: DatabaseTeamAnalysisDb11Coverage, parity: DatabaseTeamAnalysisDb11Parity, parserVersion: string, db10Coverage: DatabaseTeamAnalysisDb10Coverage): string {
