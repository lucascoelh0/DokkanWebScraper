...
import { compareDatabaseTeamAnalysisDb5 } from "./team-analysis-db5-parity";

function projection(subject: Record<string, unknown>, value: number, length: number) {
    return {
...
function database(countedScaling: any[]) {
    return {
        states: [{
...
function siteAudit() {
    return { formProjectionAliases: [] } as any;
}

describe("database Team Analysis DB5 parity", function () {
    it("compares an observed contribution with structured current stackCap without promoting a semantic cap", () => {
        ...
        const parity = compareDatabaseTeamAnalysisDb5(database([projection(subject, 10, 3)]), current, siteAudit());
        equal(parity.currentBaseEffectShapeMatchCount, 1);
        equal(parity.currentUnknownConditionShapeMatchCount, 1);
        equal(parity.capComparisonCounts.exact_observed_contribution, 1);
        equal(parity.currentStructuredSelectorMatchCount, 0);
    });

    it("matches a structured Ki Sphere selector only from structured current scaling", () => {
        ...
        const parity = compareDatabaseTeamAnalysisDb5(database([projection(subject, 5, 5)]), current, siteAudit());
        equal(parity.currentStructuredSelectorMatchCount, 1);
        equal(parity.capComparisonCounts.no_structured_cap, 1);
    });

    it("does not call a known portion of a partial mask a structured selector match", () => {
    ...
    it("does not propose a structured base candidate when calculation buckets are absent", () => {
        const subject = { kind: "category", scope: "team", categoryId: "40", categoryName: "Bond", rawSelector: 40, status: "supported", evidence: "first-party-row-join" };
        const current = {
            states: [{ stateKey: "1:1:initial", passive: { rules: [
        ...
        const parity = compareDatabaseTeamAnalysisDb5(database([projection(subject, 10, 3)]), current, siteAudit());
        equal(parity.capComparisonCounts.mismatch, 1);
        equal(parity.capComparisonCounts.candidate_observed_plus_structured_base, 0);
    });
});
