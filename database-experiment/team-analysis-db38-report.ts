import{DatabaseTeamAnalysisDb38Coverage,DatabaseTeamAnalysisDb38Dataset}from"./team-analysis-db38-contract";const rows=(x:Record<string,number>)=>Object.entries(x).map(([k,v])=>`| \`${k}\` | ${v} |`).join("\n");
export function renderDatabaseTeamAnalysisDb38Report(d:DatabaseTeamAnalysisDb38Dataset,c:DatabaseTeamAnalysisDb38Coverage){const examples=d.ruleIncrements.filter(r=>r.legacyComparison.status==="confirmed_conflict").slice(0,8).map(r=>`- \`${r.stateKey}\` / passive \`${r.passiveSkillId}\`: legacy \`ki\`, native \`modifier_battle_gauge\`.`).join("\n");return`# Database Team Analysis experiment — DB38 incremental status
