import{DatabaseTeamAnalysisDb42Coverage,DatabaseTeamAnalysisDb42Dataset}from"./team-analysis-db42-contract";const rows=(x:Record<string,number>)=>Object.entries(x).map(([k,v])=>`| \`${k}\` | ${v} |`).join("\n");
export function renderDatabaseTeamAnalysisDb42Report(d:DatabaseTeamAnalysisDb42Dataset,c:DatabaseTeamAnalysisDb42Coverage){const examples=d.resolutions.slice(0,8).map(r=>{const s=r.predicate.selector;return`- \`${r.stateKey}\` / causality \`${r.causalityId}\`: all seven resolved party slots must match ${s.kind==="card_category_id"?`category ID \`${s.categoryId}\``:`class mask \`${s.rawMask}\` (${s.class})`}.`}).join("\n");return`# Database Team Analysis experiment — DB42 all-team-member conditions
