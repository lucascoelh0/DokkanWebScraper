...
import { DatabaseTeamAnalysisDb31Dataset, Db31RuleProjection } from "./team-analysis-db31-contract";
import { DatabaseTeamAnalysisDb33Dataset, Db33ExecutionTiming } from "./team-analysis-db33-contract";
import { DatabaseTeamAnalysisDb34Coverage, DatabaseTeamAnalysisDb34Dataset, Db34Bucket, Db34LegacyCandidateConflict, Db34NativeEvidence, Db34Stat, Db34StatApplication } from "./team-analysis-db34-contract";
...
const int = (value: unknown) => typeof value === "number" && Number.isSafeInteger(value) ? value : typeof value === "string" && value.trim() && Number.isSafeInteger(Number(value)) ? Number(value) : undefined;
...
const relocation = (inspection: NativeRuntimeElfInspection, offset: number, symbol: string) => inspection.relocations.filter(value => value.offset === offset).length === 1 && inspection.relocations.find(value => value.offset === offset)?.symbolName === symbol;

export function projectDb34CalculationBucket(value: unknown): { status: "supported", value: Db34Bucket } | { status: "unknown", value: "unknown" } {
    const raw = int(value);
    if ([1, 3, 11, 15, 18].includes(raw ?? -1)) return { status: "supported", value: "former_passive_stat" };
    if ([4, 5, 6, 7, 9, 14].includes(raw ?? -1)) return { status: "supported", value: "latter_passive_stat" };
    return { status: "unknown", value: "unknown" };
}
export function projectDb34BasicStatShape(efficacyType: unknown): Array<{ stat: Db34Stat, sourceColumn: "eff_value1" | "eff_value2" }> {
    const value = int(efficacyType);
    if (value === 1) return [{ stat: "attack" as const, sourceColumn: "eff_value1" as const }];
    if (value === 2) return [{ stat: "defense" as const, sourceColumn: "eff_value1" as const }];
    if (value === 3) return [{ stat: "attack" as const, sourceColumn: "eff_value1" as const }, { stat: "defense" as const, sourceColumn: "eff_value2" as const }];
    return [];
}
export function db34OperandUnit(operation: Db31RuleProjection["operation"]) {
    if (operation.status !== "supported") return { status: "unknown" as const, value: "unknown" as const };
    return { status: "supported" as const, value: operation.value === "add_percent_of_lhs" || operation.value === "subtract_percent_of_lhs_floor_zero" ? "percent_points_of_current_stat" as const : "stat_points" as const };
}
function validateEvidence(inspection: NativeRuntimeElfInspection, evidence: Db34NativeEvidence, nativeSha: string) {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha || evidence.auditScope !== "basic-passive-atk-def-native-calculation-buckets" || JSON.stringify(evidence.sqliteBindings) !== JSON.stringify(EXPECTED_BINDINGS) || JSON.stringify(evidence.conclusions) !== JSON.stringify(EXPECTED_CONCLUSIONS) || JSON.stringify(evidence.unknowns) !== JSON.stringify(EXPECTED_UNKNOWNS)) throw new Error("DB34 native evidence identity mismatch");
    for (const binding of evidence.sqliteBindings) if (!inspection.readVirtualBytes(binding.literalVma, binding.column.length + 1).equals(Buffer.from(`${binding.column}\0`))) throw new Error(`DB34 SQLite literal mismatch ${binding.column}`);
    if (evidence.codeRegions.length !== Object.keys(EXPECTED_REGIONS).length || new Set(evidence.codeRegions.map(value => value.role)).size !== evidence.codeRegions.length) throw new Error("DB34 code region cardinality mismatch");
    for (const region of evidence.codeRegions) {
        if (EXPECTED_REGIONS[region.role] !== region.symbol) throw new Error(`DB34 code region role mismatch ${region.role}`);
        const symbol = inspection.symbols.find(value => value.name === region.symbol);
        if (region.symbol.startsWith("local@")) { if (sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw new Error(`DB34 local code mismatch ${region.role}`); }
        else if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes || sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw new Error(`DB34 native code mismatch ${region.role}`);
    }
    if (evidence.efficacyDispatch.tableVma !== 92049840 || JSON.stringify(evidence.efficacyDispatch.slots) !== JSON.stringify(EXPECTED_EFFICACY_DISPATCH)) throw new Error("DB34 efficacy dispatch metadata mismatch");
    for (const expected of EXPECTED_EFFICACY_DISPATCH) { const region = evidence.codeRegions.find(value => value.role === expected.handlerRole); if (!region || !relocation(inspection, 92049840 + expected.raw * 8, region.symbol)) throw new Error(`DB34 efficacy dispatch slot mismatch ${expected.raw}`); }
    if (evidence.doubleCalculationDispatch.tableVma !== 92053528 || evidence.doubleCalculationDispatch.dispatcherRole !== "double_calc_dispatcher" || JSON.stringify(evidence.doubleCalculationDispatch.operationRoles) !== JSON.stringify(["double_calc_add", "double_calc_subtract", "double_calc_percent_add", "double_calc_percent_subtract", "double_calc_assign"]) || evidence.doubleCalculationDispatch.materialization !== "sqlite_numeric_to_double_then_handler_float32_then_widened_double") throw new Error("DB34 double dispatch metadata mismatch");
    for (let raw = 0; raw < 5; raw++) { const role = evidence.doubleCalculationDispatch.operationRoles[raw], region = evidence.codeRegions.find(value => value.role === role); if (!region || !relocation(inspection, 92053528 + raw * 8, region.symbol)) throw new Error(`DB34 double dispatch slot mismatch ${raw}`); }
    if (evidence.bucketGroups.length !== 2) throw new Error("DB34 bucket cardinality mismatch");
    for (const expected of EXPECTED_BUCKETS) {
        const group = evidence.bucketGroups.find(value => value.bucket === expected.bucket);
        if (!group || JSON.stringify(group.timingValues) !== JSON.stringify(expected.timingValues) || group.timingPairLiteralVma !== expected.timingPairLiteralVma || group.timingPairLiteralHex !== expected.timingPairLiteralHex || group.attackSummaryRole !== expected.attackSummaryRole || JSON.stringify(group.attackIntegrationCallSites.map(value => value.callVma)) !== JSON.stringify(expected.attackVmas) || JSON.stringify(group.defenseIntegrationCallSites.map(value => value.callVma)) !== JSON.stringify(expected.defenseVmas) || inspection.readVirtualBytes(group.timingPairLiteralVma, 8).toString("hex") !== group.timingPairLiteralHex) throw new Error(`DB34 bucket metadata mismatch ${expected.bucket}`);
        if (JSON.stringify(group.attackIntegrationCallSites.map(value => value.timing)) !== JSON.stringify(expected.callTimingValues) || JSON.stringify(group.defenseIntegrationCallSites.map(value => value.timing)) !== JSON.stringify(expected.callTimingValues)) throw new Error(`DB34 bucket timing order mismatch ${expected.bucket}`);
        for (const call of [...group.attackIntegrationCallSites, ...group.defenseIntegrationCallSites]) if (inspection.readVirtualBytes(call.callVma, 4).toString("hex") !== call.instructionHex) throw new Error(`DB34 bucket call bytes mismatch ${call.callVma}`);
    }
    ...
    if (JSON.stringify(evidence.accumulator) !== JSON.stringify(expectedAccumulator)) throw new Error("DB34 accumulator metadata mismatch");
    if (evidence.abilityEfficacyInfoVtable.symbol !== "_ZTV19AbilityEfficacyInfo" || evidence.abilityEfficacyInfoVtable.vma !== 92053824 || evidence.abilityEfficacyInfoVtable.sizeBytes !== 624 || JSON.stringify(evidence.abilityEfficacyInfoVtable.slots) !== JSON.stringify(EXPECTED_EFFICACY_INFO_VTABLE_SLOTS)) throw new Error("DB34 vtable identity mismatch");
    const vtable = inspection.symbols.find(value => value.name === evidence.abilityEfficacyInfoVtable.symbol); if (!vtable || vtable.value !== evidence.abilityEfficacyInfoVtable.vma || vtable.size !== evidence.abilityEfficacyInfoVtable.sizeBytes) throw new Error("DB34 vtable ELF mismatch");
    for (const slot of evidence.abilityEfficacyInfoVtable.slots) if (!relocation(inspection, vtable.value + slot.offset, slot.symbol)) throw new Error(`DB34 vtable slot mismatch ${slot.offset}`);
}
function validateLineage(options: { db11: DatabaseTeamAnalysisDb11Dataset, db11Sha256: string, db13: DatabaseTeamAnalysisDb13Dataset, db13Sha256: string, db31: DatabaseTeamAnalysisDb31Dataset, db31Sha256: string, db33: DatabaseTeamAnalysisDb33Dataset, db33Sha256: string, currentSha256: string, nativeSha256: string, nativeSizeBytes: number }) {
    if (options.db11.contractVersion !== "0.10.0" || options.db13.contractVersion !== "0.12.1" || options.db31.contractVersion !== "0.30.0" || options.db33.contractVersion !== "0.32.0" || options.db33Sha256 !== EXPECTED_DB33_SHA256) throw new Error("DB34 source contract mismatch");
    if (options.db13.sourceDb11.sha256 !== options.db11Sha256 || options.db31.sourceDb11.sha256 !== options.db11Sha256 || options.db33.sourceDatabaseSha256 !== options.db11.sourceSha256 || options.db13.sourceDatabaseSha256 !== options.db11.sourceSha256 || options.db31.sourceDatabaseSha256 !== options.db11.sourceSha256 || options.db33.currentTeamAnalysis.sha256 !== options.currentSha256 || options.db13.sourceCurrentTeamAnalysis.sha256 !== options.currentSha256 || options.db31.currentTeamAnalysis.sha256 !== options.currentSha256 || options.db33.nativeRuntime.sha256 !== options.nativeSha256 || options.db33.nativeRuntime.sizeBytes !== options.nativeSizeBytes) throw new Error("DB34 source lineage mismatch");
}
export function buildDatabaseTeamAnalysisDb34Dataset(options: {
    db11: DatabaseTeamAnalysisDb11Dataset, db11Sha256: string, db13: DatabaseTeamAnalysisDb13Dataset, db13Sha256: string, db31: DatabaseTeamAnalysisDb31Dataset, db31Sha256: string, db33: DatabaseTeamAnalysisDb33Dataset, db33Sha256: string,
    tables: DatabaseExperimentTables, current: { states: Array<{ stateKey: string, passive?: { rules?: Array<{ ruleKey?: string, id?: string, effects?: Array<Record<string, unknown>> }> } }> }, currentSha256: string,
    inspection: NativeRuntimeElfInspection, nativeSha256: string, nativeSizeBytes: number, evidence: Db34NativeEvidence, evidenceSha256: string,
}): DatabaseTeamAnalysisDb34Dataset {
    validateLineage(options); validateEvidence(options.inspection, options.evidence, options.nativeSha256);
    ...
    const statApplications: Db34StatApplication[] = [], basicRuleKeys = new Set<string>();
    ...
    for (const state of options.db11.states) for (const rule of state.passive?.rules ?? []) {
        const efficacyType = int(rule.source.efficacyType); if (![1, 2, 3].includes(efficacyType ?? -1)) continue;
        ...
        if (!row || !timing || !operation || id(row) !== rule.source.passiveSkillId || int(row.efficacy_type) !== efficacyType || int(row.exec_timing_type) !== int(rule.source.executionTimingType) || JSON.stringify(operation.operation) !== JSON.stringify(timing.calculationOperation) || operation.passiveSkillId !== rule.source.passiveSkillId || timing.passiveSkillId !== rule.source.passiveSkillId || shape.length !== rule.effects.length) throw new Error(`DB34 source rule mismatch ${key}`);
        const operandUnit = db34OperandUnit(operation.operation); if (operandUnit.status !== "supported") throw new Error(`DB34 unsupported calculation operation ${key}`);
        basicRuleKeys.add(key);
        for (const item of shape) {
            const rawModifier = row[item.sourceColumn], modifier = numeric(rawModifier); if (modifier === undefined) throw new Error(`DB34 invalid modifier ${key}|${item.sourceColumn}`);
            statApplications.push({ stateKey: state.stateKey, ruleKey: rule.ruleKey, passiveSkillId: rule.source.passiveSkillId, efficacyType: efficacyType as 1 | 2 | 3, stat: item.stat, sourceColumn: item.sourceColumn, rawModifier: rawModifier ?? null, runtimeModifierFloat32: Math.fround(modifier), rawExecutionTimingType: rule.source.executionTimingType, calculationBucket: bucket, calculationOperation: operation.operation, operandUnit, executionTiming: timing.executionTiming as Db33ExecutionTiming, semanticStatus: "partial", independentDimensions: { target: "unknown", duration: "unknown", recurrence: "unknown", expiry: "unknown", reset: "unknown", crossBucketOrder: "unknown" }, provenance: { database: { table: "passive_skills", rowId: rule.source.passiveSkillId, columns: ["efficacy_type", "exec_timing_type", "calc_option", item.sourceColumn] }, runtime: { fileName: "libcocos2dcpp.so", sha256: options.nativeSha256, evidenceFile: "native-basic-stat-bucket-semantics.json", evidenceSha256: options.evidenceSha256, proofRoles } } });
        }
    }
    ...
    const currentRules = new Map<string, Array<{ effects?: Array<Record<string, unknown>> }>>();
    for (const state of options.current.states) for (const rule of state.passive?.rules ?? []) { const key = `${state.stateKey}|${rule.ruleKey ?? rule.id}`; const values = currentRules.get(key) ?? []; values.push(rule); currentRules.set(key, values); }
    const applicationsByRule = new Map<string, Db34StatApplication>(); for (const value of statApplications) applicationsByRule.set(`${value.stateKey}|${value.ruleKey}`, value);
    const crossTab: Record<string, number> = {}, candidateConflicts: Db34LegacyCandidateConflict[] = []; let alignedBasicStatRuleCount = 0;
    for (const alignment of options.db13.ruleAlignments) {
        if (alignment.kind !== "exact_effect_set_unique") continue;
        const application = applicationsByRule.get(`${alignment.stateKey}|${alignment.databaseRuleKey}`), matches = currentRules.get(`${alignment.stateKey}|${alignment.currentRuleKey}`); if (!application || !matches || matches.length !== 1) continue;
        alignedBasicStatRuleCount++;
        const legacyBuckets = [...new Set((matches[0].effects ?? []).map(effect => { const bucket = effect.calculationBucket as Record<string, unknown> | undefined; return typeof bucket?.bucket === "string" ? bucket.bucket : "absent"; }))].sort();
        const legacySignature = legacyBuckets.join("+") || "absent", crossKey = `${application.calculationBucket.value}|${legacySignature}`; crossTab[crossKey] = (crossTab[crossKey] ?? 0) + 1;
        if (application.calculationBucket.value === "latter_passive_stat" && legacySignature === "passive_start_of_turn") candidateConflicts.push({ stateKey: alignment.stateKey, databaseRuleKey: alignment.databaseRuleKey, currentRuleKey: alignment.currentRuleKey, passiveSkillId: application.passiveSkillId, rawExecutionTimingType: application.rawExecutionTimingType, firstPartyBucket: "latter_passive_stat", legacyBucket: "passive_start_of_turn", status: "candidate", boundary: "db13_unique_effect_set_alignment_is_diagnostic_not_identity_proof" });
    }
...
const counts = <T>(values: T[], key: (value: T) => string) => { const result: Record<string, number> = {}; for (const value of values) { const name = key(value); result[name] = (result[name] ?? 0) + 1; } return Object.fromEntries(Object.entries(result).sort(([left], [right]) => left.localeCompare(right, "en", { numeric: true }))); };
export function buildDatabaseTeamAnalysisDb34Coverage(dataset: DatabaseTeamAnalysisDb34Dataset): DatabaseTeamAnalysisDb34Coverage {
