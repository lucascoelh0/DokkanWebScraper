import {DatabaseTeamAnalysisDb35Coverage,DatabaseTeamAnalysisDb35Dataset} from "./team-analysis-db35-contract";const rows=(x:Record<string,number>)=>Object.entries(x).map(([k,v])=>`| \`${k}\` | ${v} |`).join("\n");
export function renderDatabaseTeamAnalysisDb35Report(d:DatabaseTeamAnalysisDb35Dataset,c:DatabaseTeamAnalysisDb35Coverage){return `# Database Team Analysis experiment — DB35 passive target dispatch
