...
import { buildCoverage, buildDatabaseExperimentDataset, CONSUMED_TABLE_COLUMNS, DatabaseExperimentTables } from "./builder";

function fixtureTables(): DatabaseExperimentTables {
    const tables = Object.fromEntries(Object.keys(CONSUMED_TABLE_COLUMNS).map(table => [table, []])) as DatabaseExperimentTables;
    tables.cards = [
...
describe("database experiment builder", function () {
    it("joins structured rows, preserves unknowns and is deterministic", () => {
        ...
        const first = buildDatabaseExperimentDataset(options);
        const second = buildDatabaseExperimentDataset(options);
        deepEqual(first, second);
        equal(new Set(first.cards.map(card => card.cardId)).size, first.cards.length);
        const base = first.cards.find(card => card.cardId === "1000000")!;
        ok(base);
        equal(base.characterClass.value, "unawakened");
        equal(base.skillStates[1].releaseState, "seza");
        equal(base.skillStates[1].release.availableAt, "2024-01-01 00:00:00");
        equal(base.catalog.isProjectedPrimary, true);
        deepEqual(base.skillStates[1].leaderSkill?.structuredPercentValues, [170]);
        equal(base.skillStates[0].passiveSkill?.relations[0].causalities[0].values.id, 80);
        deepEqual(base.skillStates[1].attacks.map(attack => attack.variant.value), ["super", "ultra", "unit", "ex"]);
        ok(base.formRelations.some(relation => relation.kind.value === "reversible-exchange"));
        ok(base.formRelations.some(relation => relation.kind.value === "transformation"));
        equal(first.cards.find(card => card.cardId === "4000000")?.grouping.hardDuplicateGroupId, base.grouping.hardDuplicateGroupId);
        const coverage = buildCoverage(first);
        equal(coverage.confirmedEzaStateCount, 0);
        equal(coverage.confirmedSezaStateCount, 1);
        equal(coverage.unknownReleaseStateCount, 0);
    });
});
