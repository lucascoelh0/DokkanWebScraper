...
import { DatabaseTeamAnalysisDb50Coverage, DatabaseTeamAnalysisDb50Dataset, Db50Evidence } from "./team-analysis-db50-contract";
...
const CALLS: Array<[string, number, string, number, string, number, string, number]> = [
...
export const decodeDb50BlTarget = (bytes: Buffer, vma: number) => { const word = bytes.readUInt32LE(); if (word >>> 26 !== 37) return null; let immediate = word & 0x03ffffff; if (immediate & 0x02000000) immediate -= 0x04000000; return vma + immediate * 4; };

export const selectDb50Counter = <T extends { resistDamageRate: number }>(values: T[]) => values.reduce<T | null>((best, value) => best === null || value.resistDamageRate > best.resistDamageRate ? value : best, null);
export const applyDb50CounterResistance = (preDamage: number, resistDamageRate: number) => preDamage - Math.trunc(preDamage * resistDamageRate / 100);
export const doesDb50RateSetFlag = (resistDamageRate: number) => resistDamageRate > 99;
export const hasDb50JoinIdentity = (passiveSkillId: string, effectCount: number, ...values: Array<{ passiveSkillId: string; effectCount: number }>) => values.every(value => value.passiveSkillId === passiveSkillId && value.effectCount === effectCount);
export const hasDb50PayloadIdentity = (source: DatabaseTeamAnalysisDb24Dataset["counterBehaviorResolutions"][number]) => source.efficacyType === 120 && [source.payload.resistDamageRate, source.payload.increaseDamagePercent, source.payload.battleScriptNo].every(field => field.status === "supported" && Number.isSafeInteger(field.runtimeInteger)) && source.payload.resistDamageRate.sqliteColumn === "eff_value1" && source.payload.resistDamageRate.nativeField === "resistDamageRate" && source.payload.increaseDamagePercent.sqliteColumn === "eff_value2" && source.payload.increaseDamagePercent.nativeField === "increaseDamagePercent" && source.payload.battleScriptNo.sqliteColumn === "eff_value3" && source.payload.battleScriptNo.nativeField === "battleScriptNo";

export function validateDb50Evidence(inspection: NativeRuntimeElfInspection, evidence: Db50Evidence, nativeSha256: string): void {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.auditScope !== "efficacy-120-counter-selection-and-enemy-damage-resistance-consumer" || JSON.stringify(evidence.registrationGate) !== JSON.stringify(SEMANTICS.gate) || JSON.stringify(evidence.normalSelection) !== JSON.stringify(SEMANTICS.selection) || JSON.stringify(evidence.preference) !== JSON.stringify(SEMANTICS.preference) || JSON.stringify(evidence.damageConsumer) !== JSON.stringify(SEMANTICS.damage) || JSON.stringify(evidence.unknowns) !== JSON.stringify(SEMANTICS.unknowns)) throw Error("DB50 evidence identity");
    ...
    if (JSON.stringify(evidence.codeRegions) !== JSON.stringify(regions) || JSON.stringify(evidence.directCalls) !== JSON.stringify(calls)) throw Error("DB50 evidence tuples");
    for (const region of evidence.codeRegions) {
        if (sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw Error(`DB50 region ${region.role}`);
        if (!region.symbol.startsWith("local@")) {
            const symbol = inspection.symbols.find(value => value.name === region.symbol);
            if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes) throw Error(`DB50 symbol ${region.role}`);
        }
    }
    for (const call of evidence.directCalls) {
        const callBytes = inspection.readVirtualBytes(call.callVma, 4);
        if (callBytes.toString("hex") !== call.callHex || decodeDb50BlTarget(callBytes, call.callVma) !== call.pltVma || inspection.readVirtualBytes(call.pltVma, 16).toString("hex") !== call.pltHex || inspection.relocations.filter(value => value.offset === call.relocation.offset && value.type === call.relocation.type && value.symbolName === call.relocation.symbol && value.symbolValue === call.relocation.symbolValue && value.addend === call.relocation.addend).length !== 1) throw Error(`DB50 call ${call.role}`);
    }
}

export function buildDatabaseTeamAnalysisDb50Dataset(options: { db24: DatabaseTeamAnalysisDb24Dataset; db24Sha256: string; db35: DatabaseTeamAnalysisDb35Dataset; db35Sha256: string; db47: DatabaseTeamAnalysisDb47Dataset; db47Sha256: string; inspection: NativeRuntimeElfInspection; nativeSha256: string; nativeSizeBytes: number; evidence: Db50Evidence; evidenceSha256: string }): DatabaseTeamAnalysisDb50Dataset {
    ...
    const projections = options.db24.counterBehaviorResolutions.map(source => {
        ...
        if (!hasDb50PayloadIdentity(source)) throw Error(`DB50 payload ${key(source)}`);
        if (!target || target.target.status !== "supported" || !timing || !hasDb50JoinIdentity(source.passiveSkillId, target.effectCount, target, timing)) throw Error(`DB50 join ${key(source)}`);
        return {
            stateKey: source.stateKey,
            ruleKey: source.ruleKey,
            passiveSkillId: source.passiveSkillId,
            efficacyType: 120 as const,
            sourceEffectCount: target.effectCount,
            payload: source.payload,
            registrationGate: { status: "supported" as const, callChangeParamOffset: 4 as const, registerWhenZero: true as const, nonzeroBehavior: "skip_registration" as const, fieldSemantic: "unknown" as const },
            target: { status: "supported" as const, value: target.target.value },
            executionTiming: timing.executionTiming,
            selection: { status: "supported" as const, filters: ["efficacy_type_120", "deck_index_input"] as ["efficacy_type_120", "deck_index_input"], ranking: "highest_resist_damage_rate" as const, tieBehavior: "first_in_efficacy_info_order" as const, preference: "efficacy_128_dodge_then_efficacy_120_normal" as const, callerBoolean: "unknown" as const, externalActivation: "unknown" as const },
            damage: { status: "supported" as const, bucket: "enemy_source_after_efficacy_13_before_defense_and_guard" as const, formula: "pre_minus_trunc_toward_zero(pre_times_resist_damage_rate_div_100)" as const, rateAbove99SetsFlag: true as const, finalHpApplication: "unknown" as const },
            rawActivation: source.activation,
            simulationStatus: "partial" as const,
            provenance: { database: source.provenance.database, runtime: { fileName: "libcocos2dcpp.so" as const, sha256: options.nativeSha256, evidenceFile: "native-counter-consumer-semantics.json" as const, evidenceSha256: options.evidenceSha256, proofRoles: options.evidence.codeRegions.map(value => value.role) }, inherited: { db24Sha256: options.db24Sha256, db35Sha256: options.db35Sha256, db47Sha256: options.db47Sha256 } },
        };
    });
...
export function buildDatabaseTeamAnalysisDb50Coverage(dataset: DatabaseTeamAnalysisDb50Dataset): DatabaseTeamAnalysisDb50Coverage {
