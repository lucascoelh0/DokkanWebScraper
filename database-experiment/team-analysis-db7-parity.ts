...
import { DatabaseTeamAnalysisDb7Coverage, DatabaseTeamAnalysisDb7Dataset, DatabaseTeamAnalysisDb7Parity, Db7SelectorCondition } from "./team-analysis-db7-contract";
...
function signature(scope: string, selectorKind: "class" | "type", selectorValue: string, count: number): string {
    return JSON.stringify({ scope, selectorKind, selectorValue, comparator: "gte", minimumCount: count });
}

function databaseSignature(value: Db7SelectorCondition): { kind: string, signature: string } | undefined {
    if (value.status !== "supported") return undefined;
    if (value.selector.kind === "class" && value.selector.classes.length === 1) return { kind: "class", signature: signature(value.scope, "class", value.selector.classes[0], value.minimumCount) };
    if (value.selector.kind === "type" && value.selector.types.length === 1) return { kind: "type", signature: signature(value.scope, "type", value.selector.types[0], value.minimumCount) };
    return undefined;
}

function currentSignature(predicate: JsonObject): { kind: string, signature: string } | undefined {
    ...
    if (!selectorKind || typeof predicate.scope !== "string") return undefined;
    ...
    if (!Number.isSafeInteger(rawCount) || rawCount < 1 || comparator !== "gte") return undefined;
    return { kind: selectorKind, signature: signature(predicate.scope, selectorKind, values[0], rawCount) };
}

function walkCurrent(value: unknown, visit: (value: { kind: string, signature: string }) => void): void {
    const expression = object(value);
    if (!expression) return;
    if (expression.op === "predicate") {
        const predicate = object(expression.predicate); const normalized = predicate ? currentSignature(predicate) : undefined;
        if (normalized) visit(normalized);
    } else if ((expression.op === "all" || expression.op === "any") && Array.isArray(expression.children)) expression.children.forEach(child => walkCurrent(child, visit));
...
export function compareDatabaseTeamAnalysisDb7(database: DatabaseTeamAnalysisDb7Dataset, current: CurrentTeamAnalysisDataset, siteAudit: SiteAuditFixtures): DatabaseTeamAnalysisDb7Parity {
    const aliases = new Map(siteAudit.formProjectionAliases.map(alias => [alias.databaseCardId, alias.projectedCardId]));
    const projectedKey = (state: DatabaseTeamAnalysisDb7Dataset["states"][number]) => `${state.characterId}:${aliases.get(state.formId) ?? state.formId}:${state.releaseState}`;
    ...
    const examples: DatabaseTeamAnalysisDb7Parity["examples"] = [];
    let matchedStateCount = 0; let partialNameTokenOccurrenceCount = 0;
    for (const state of database.states) {
        const stateKey = projectedKey(state); const currentState = currentByKey.get(stateKey); if (!currentState) continue;
        matchedStateCount += 1;
        ...
        for (const selector of state.passive?.rules.flatMap(rule => rule.selectorConditions) ?? []) {
            if (selector.causalityType === 41) partialNameTokenOccurrenceCount += 1;
            const normalized = databaseSignature(selector); if (!normalized) continue;
            databaseAll.add(`${stateKey}|${normalized.signature}`); databaseState.add(normalized.signature);
            const bucket = kinds.get(normalized.kind) ?? { database: new Set<string>(), current: new Set<string>() };
            bucket.database.add(stateKey); kinds.set(normalized.kind, bucket);
        }
        for (const rule of currentState.passive?.rules ?? []) walkCurrent(rule.condition, normalized => {
            currentAll.add(`${stateKey}|${normalized.signature}`); currentStateSet.add(normalized.signature);
            const bucket = kinds.get(normalized.kind) ?? { database: new Set<string>(), current: new Set<string>() };
            bucket.current.add(stateKey); kinds.set(normalized.kind, bucket);
        });
        const databaseOnlySelectors = difference(databaseState, currentStateSet); const currentOnlySelectors = difference(currentStateSet, databaseState);
        if (databaseOnlySelectors.length || currentOnlySelectors.length) examples.push({ stateKey, databaseOnlySelectors, currentOnlySelectors });
    }
    return {
...
export function renderDatabaseTeamAnalysisDb7Report(coverage: DatabaseTeamAnalysisDb7Coverage, parity: DatabaseTeamAnalysisDb7Parity, parserVersion: string): string {
