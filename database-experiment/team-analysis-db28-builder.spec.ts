...
function fixture() {
    ...
    const tables = { skill_causalities: [{ id: 47, causality_type: 47, cau_val1: 99, cau_val2: 98, cau_val3: 97 }, { id: 54, causality_type: 54, cau_val1: 0, cau_val2: 0, cau_val3: 0 }] } as any;
    ...
    const inspection = { symbols, relocations: [{ offset: 188, symbolName: "deck" }, { offset: 240, symbolName: "pure" }, { offset: 288, symbolName: "back" }], readVirtualBytes: () => bytes } as any;
    return { db8, db9, db11, tables, inspection, evidence };
}

function build(value: ReturnType<typeof fixture>) { return buildDatabaseTeamAnalysisDb28Dataset({ ...value, db8Sha256: "db8", db9Sha256: "db9", db11Sha256: "db11", nativeSha256: "native", nativeSizeBytes: 1, evidenceSha256: "evidence" }); }

describe("database Team Analysis DB28 revival counters", function () {
    it("projects owner and party activation-count predicates", () => {
        const dataset = build(fixture());
        deepStrictEqual(dataset.resolutions.map(value => value.predicate), [
        ...
        const coverage = buildDatabaseTeamAnalysisDb28Coverage(dataset, 2); equal(coverage.resolutionCount, 2); equal(coverage.supportedPredicateCount, 2); equal(coverage.partialResolutionCount, 2);
    });
    it("preserves nonzero party polarity as requires-none", () => { const value = fixture(); value.tables.skill_causalities[1].cau_val1 = 1; const dataset = build(value); const predicate = dataset.resolutions[1].predicate; equal(predicate.scope === "party_pure_and_back_current_records" && predicate.expected, false); equal(buildDatabaseTeamAnalysisDb28Coverage(dataset, 2).nonzeroPolarityCount, 1); });
    it("allows ignored type-47 payload values without reinterpretation", () => { const dataset = build(fixture()); deepStrictEqual(dataset.resolutions[0].raw, { cauVal1: 99, cauVal2: 98, cauVal3: 97 }); });
    it("rejects a mutated increment writer", () => { const value = fixture(); value.evidence.counter.incrementWriter.codeSha256 = "0".repeat(64); throws(() => build(value), /increment writer native code mismatch/); });
    it("rejects a wrong current-record vtable relocation", () => { const value = fixture(); value.inspection.relocations[1].symbolName = "other"; throws(() => build(value), /InGameData vtable evidence mismatch/); });
    it("rejects an incomplete parameter audit", () => { const value = fixture(); value.evidence.handlers[0].ignoredParameters = ["cau_val1", "cau_val2"]; throws(() => build(value), /handler semantics mismatch/); });
    it("rejects a nonnumeric type-54 polarity", () => { const value = fixture(); value.tables.skill_causalities[1].cau_val1 = "unknown"; throws(() => build(value), /invalid polarity/); });
    it("reconciles unique causality cardinality", () => { const value = fixture(); value.db8.causalityGaps[0].uniqueCausalityCount = 2; throws(() => build(value), /source gap accounting mismatch/); });
    it("rejects a null SQLite identity", () => { const value = fixture(); value.tables.skill_causalities.push({ id: null, causality_type: 1, cau_val1: 0, cau_val2: 0, cau_val3: 0 }); throws(() => build(value), /has no id/); });
});
