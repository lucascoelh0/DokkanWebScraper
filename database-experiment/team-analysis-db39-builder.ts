...
import { DatabaseTeamAnalysisDb39Coverage, DatabaseTeamAnalysisDb39Dataset, Db39NativeEvidence, Db39RuleProjection } from "./team-analysis-db39-contract";
...
const integer = (value: unknown) => typeof value === "number" && Number.isSafeInteger(value) ? value : typeof value === "string" && value.trim() !== "" && Number.isSafeInteger(Number(value)) ? Number(value) : undefined;
...
export function validateDb39NativeEvidence(inspection: NativeRuntimeElfInspection, evidence: Db39NativeEvidence, nativeSha256: string) {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.efficacyType !== 61 || evidence.auditScope !== "passive-efficacy-61-atk-def-per-obtained-ball-raw-type-11") throw new Error("DB39 evidence identity mismatch");
    if (JSON.stringify(evidence.dispatch) !== JSON.stringify(EXPECTED_DISPATCH) || JSON.stringify(evidence.unknowns) !== JSON.stringify(EXPECTED_UNKNOWNS)) throw new Error("DB39 evidence dispatch or unknown boundary mismatch");
    if (JSON.stringify(evidence.sqliteBinding) !== JSON.stringify({ table: "passive_skills", eff_value1: { callChangeParamOffset: 40, role: "attack_modifier_per_count" }, eff_value2: { callChangeParamOffset: 48, role: "defense_modifier_per_count" }, eff_value3: { callChangeParamOffset: 56, role: "ignored_by_handler" }, calc_option: { callChangeParamOffset: 24, role: "post_scaling_stat_operation" } })) throw new Error("DB39 SQLite binding mismatch");
    if (JSON.stringify(evidence.emittedFields) !== JSON.stringify({ attack: "eff_value1 converted double_to_float32 and emitted through modifier_attack", defense: "eff_value2 converted double_to_float32 and emitted through modifier_defense", skillEfficacyType: 1 }) || JSON.stringify(evidence.proportionalFlag) !== JSON.stringify({ callChangeParamOffset: 28, writtenValue: 1, abilityEfficacyInfoOffset: 40, defaultRawBallType: 11, defaultRawBitpattern: 0 })) throw new Error("DB39 emitted field or proportional flag mismatch");
    if (JSON.stringify(evidence.countConsumer) !== JSON.stringify({ function: "_ZN25AbilityManagerStatHelpers27getDiffFromEfficacyInfoListERKNSt6__ndk18functionIFbR19AbilityEfficacyInfoEEEN14AbilityManager8CoefTypeEii17SkillCategoryTypeb", enabledWhen: "is_energy_ball_proportional_true_and_consumer_boolean_argument_zero", mapGetter: "_ZNK15InGameCharaData26getObtainedBallTypeNumbersEv", mapKey: 11, missingMapKeyValue: 0, modifierArithmetic: "SQLite numeric to double; handler narrows to float32; consumer widens to double and multiplies by signed int32 map count before calc_option", rawBallTypeSemanticName: "unknown" }) || JSON.stringify(evidence.inheritedBuckets) !== JSON.stringify({ sourceContractVersion: "0.33.0", timing1: "former_passive_stat", timing4: "latter_passive_stat" })) throw new Error("DB39 consumer or bucket evidence mismatch");
    if (evidence.codeRegions.length !== Object.keys(EXPECTED_REGIONS).length || new Set(evidence.codeRegions.map(value => value.role)).size !== evidence.codeRegions.length) throw new Error("DB39 code role cardinality mismatch");
    for (const region of evidence.codeRegions) {
        const expected = EXPECTED_REGIONS[region.role];
        if (!expected || JSON.stringify([region.symbol, region.vma, region.sizeBytes, region.codeSha256]) !== JSON.stringify(expected)) throw new Error(`DB39 native role identity mismatch ${region.role}`);
        const symbol = inspection.symbols.find(value => value.name === region.symbol);
        if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes || sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw new Error(`DB39 native code mismatch ${region.role}`);
    }
    if (inspection.relocations.filter(value => value.offset === EXPECTED_DISPATCH.slotVma && value.symbolName === EXPECTED_DISPATCH.symbol).length !== 1) throw new Error("DB39 dispatch relocation mismatch");
    if (JSON.stringify(evidence.vtableRelocations) !== JSON.stringify(EXPECTED_VTABLES)) throw new Error("DB39 vtable evidence identity mismatch");
    for (const relocation of evidence.vtableRelocations) if (inspection.relocations.filter(value => value.offset === relocation.vtableVma + relocation.relocationOffset && value.symbolName === relocation.symbol).length !== 1) throw new Error(`DB39 vtable relocation mismatch ${relocation.symbol}`);
    const count = evidence.countConsumer as Record<string, unknown>, flag = evidence.proportionalFlag as Record<string, unknown>;
    if (count.mapKey !== 11 || count.missingMapKeyValue !== 0 || count.rawBallTypeSemanticName !== "unknown" || flag.defaultRawBallType !== 11 || flag.defaultRawBitpattern !== 0 || flag.writtenValue !== 1) throw new Error("DB39 proportional count semantics mismatch");
}

export function projectDb39Bucket(rawTiming: unknown): Db39RuleProjection["calculationBucket"] {
    if (integer(rawTiming) === 1) return { status: "supported", value: "former_passive_stat" };
    if (integer(rawTiming) === 4) return { status: "supported", value: "latter_passive_stat" };
    return { status: "unknown", value: "unknown" };
}

export function db39OperandUnit(operation: Db31RuleProjection["operation"]): Db39RuleProjection["operandUnit"] {
    const boundary = "raw_ball_type_11_semantic_name_unknown" as const;
    if (operation.status !== "supported") return { status: "partial", value: "unknown", boundary };
...
export function projectDb39Modifier(stat: "attack" | "defense", rawModifier: unknown) {
    const value = numeric(rawModifier), sourceColumn = stat === "attack" ? "eff_value1" as const : "eff_value2" as const;
    const float32 = value === undefined ? NaN : Math.fround(value);
    if (!Number.isFinite(float32)) return { status: "unknown" as const, stat, sourceColumn, rawModifierPerCount: rawModifier as any, runtimeModifierFloat32: null, scaledModifierFormula: "unknown" as const };
    return { status: "supported" as const, stat, sourceColumn, rawModifierPerCount: rawModifier as any, runtimeModifierFloat32: float32, scaledModifierFormula: "float32_modifier_widened_to_double_times_signed_int32_obtained_count" as const };
}

export function scaleDb39Modifier(rawModifier: unknown, rawCount: unknown): number | null {
    const modifier = numeric(rawModifier), count = integer(rawCount);
    const float32 = modifier === undefined ? NaN : Math.fround(modifier);
    if (!Number.isFinite(float32) || count === undefined || count < -2147483648 || count > 2147483647) return null;
    return float32 * count;
}

export function buildDatabaseTeamAnalysisDb39Dataset(options: { db11: DatabaseTeamAnalysisDb11Dataset, db11Sha256: string, db31: DatabaseTeamAnalysisDb31Dataset, db31Sha256: string, db34: DatabaseTeamAnalysisDb34Dataset, db34Sha256: string, db37: DatabaseTeamAnalysisDb37Dataset, db37Sha256: string, tables: DatabaseExperimentTables, inspection: NativeRuntimeElfInspection, nativeSha256: string, nativeSizeBytes: number, evidence: Db39NativeEvidence, evidenceSha256: string }): DatabaseTeamAnalysisDb39Dataset {
    if (options.db11.contractVersion !== "0.10.0" || options.db31.contractVersion !== "0.30.0" || options.db34.contractVersion !== "0.33.0" || options.db37.contractVersion !== "0.36.0" || options.db11Sha256 !== DB11_SHA || options.db31Sha256 !== DB31_SHA || options.db34Sha256 !== DB34_SHA || options.db37Sha256 !== DB37_SHA) throw new Error("DB39 artifact lineage mismatch");
    if ([options.db11.sourceSha256, options.db31.sourceDatabaseSha256, options.db34.sourceDatabaseSha256, options.db37.sourceDatabaseSha256].some(value => value !== SOURCE_SHA) || new Set([options.db11.generatedAt, options.db31.generatedAt, options.db34.generatedAt, options.db37.generatedAt]).size !== 1) throw new Error("DB39 snapshot lineage mismatch");
    ...
    const legacy = new Map<string, string | null>(); for (const state of options.db11.states) for (const rule of state.passive?.rules ?? []) legacy.set(`${state.stateKey}|${rule.ruleKey}`, rule.effects[0]?.kind ?? null);
    const proofRoles = options.evidence.codeRegions.map(region => region.role);
    const ruleProjections: Db39RuleProjection[] = [];
    for (const lifecycle of options.db37.ruleLifecycles) {
        const row = rows.get(lifecycle.passiveSkillId); if (!row || integer(row.efficacy_type) !== 61) continue;
        const key = `${lifecycle.stateKey}|${lifecycle.ruleKey}`, operation = operations.get(key); if (!operation || operation.passiveSkillId !== lifecycle.passiveSkillId) throw new Error(`DB39 missing operation ${key}`);
...
const distribution = <T>(items: T[], key: (item: T) => string) => { const result: Record<string, number> = {}; for (const item of items) { const value = key(item); result[value] = (result[value] ?? 0) + 1; } return Object.fromEntries(Object.entries(result).sort(([left], [right]) => left.localeCompare(right, "en", { numeric: true }))); };
export function buildDatabaseTeamAnalysisDb39Coverage(dataset: DatabaseTeamAnalysisDb39Dataset): DatabaseTeamAnalysisDb39Coverage {
