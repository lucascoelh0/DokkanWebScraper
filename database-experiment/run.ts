...
import { buildDatabaseTeamAnalysisDb44Coverage, buildDatabaseTeamAnalysisDb44Dataset } from "./team-analysis-db44-builder";
import { DatabaseTeamAnalysisDb44ArtifactManifest, Db44NativeEvidence } from "./team-analysis-db44-contract";
import { validateDatabaseTeamAnalysisDb44Goldens } from "./team-analysis-db44-golden";
import { renderDatabaseTeamAnalysisDb44Report } from "./team-analysis-db44-report";
import { validateDatabaseTeamAnalysisDb44Dataset } from "./team-analysis-db44-validator";
import { buildDatabaseTeamAnalysisDb45Coverage, buildDatabaseTeamAnalysisDb45Dataset } from "./team-analysis-db45-builder";
import { DatabaseTeamAnalysisDb45ArtifactManifest, Db45NativeEvidence } from "./team-analysis-db45-contract";
import { validateDatabaseTeamAnalysisDb45Goldens } from "./team-analysis-db45-golden";
import { renderDatabaseTeamAnalysisDb45Report } from "./team-analysis-db45-report";
import { validateDatabaseTeamAnalysisDb45Dataset } from "./team-analysis-db45-validator";
import { buildDatabaseTeamAnalysisDb46Coverage, buildDatabaseTeamAnalysisDb46Dataset } from "./team-analysis-db46-builder";
import { DatabaseTeamAnalysisDb46Manifest, Db46NativeEvidence } from "./team-analysis-db46-contract";
import { validateDatabaseTeamAnalysisDb46Goldens } from "./team-analysis-db46-golden";
import { renderDatabaseTeamAnalysisDb46Report } from "./team-analysis-db46-report";
import { validateDatabaseTeamAnalysisDb46Dataset } from "./team-analysis-db46-validator";
import { buildDatabaseTeamAnalysisDb47Coverage, buildDatabaseTeamAnalysisDb47Dataset } from "./team-analysis-db47-builder";
import { DatabaseTeamAnalysisDb47Manifest, Db47NativeEvidence } from "./team-analysis-db47-contract";
import { validateDatabaseTeamAnalysisDb47Goldens } from "./team-analysis-db47-golden";
import { renderDatabaseTeamAnalysisDb47Report } from "./team-analysis-db47-report";
import { validateDatabaseTeamAnalysisDb47Dataset } from "./team-analysis-db47-validator";
import { buildDatabaseTeamAnalysisDb48Coverage, buildDatabaseTeamAnalysisDb48Dataset } from "./team-analysis-db48-builder";
import { DatabaseTeamAnalysisDb48Manifest, Db48NativeEvidence } from "./team-analysis-db48-contract";
import { validateDatabaseTeamAnalysisDb48Goldens } from "./team-analysis-db48-golden";
import { renderDatabaseTeamAnalysisDb48Report } from "./team-analysis-db48-report";
import { validateDatabaseTeamAnalysisDb48Dataset } from "./team-analysis-db48-validator";
import { buildDatabaseTeamAnalysisDb49Coverage, buildDatabaseTeamAnalysisDb49Dataset } from "./team-analysis-db49-builder";
import { DatabaseTeamAnalysisDb49Manifest, Db49NativeEvidence } from "./team-analysis-db49-contract";
import { validateDatabaseTeamAnalysisDb49Goldens } from "./team-analysis-db49-golden";
import { renderDatabaseTeamAnalysisDb49Report } from "./team-analysis-db49-report";
import { validateDatabaseTeamAnalysisDb49Dataset } from "./team-analysis-db49-validator";
...
export async function runDatabaseExperiment(options: RunOptions): Promise<{
    ...
    teamAnalysisDb44Manifest: DatabaseTeamAnalysisDb44ArtifactManifest,
    teamAnalysisDb45Manifest: DatabaseTeamAnalysisDb45ArtifactManifest,
    teamAnalysisDb46Manifest: DatabaseTeamAnalysisDb46Manifest,
    teamAnalysisDb47Manifest: DatabaseTeamAnalysisDb47Manifest,
    teamAnalysisDb48Manifest: DatabaseTeamAnalysisDb48Manifest,
    teamAnalysisDb49Manifest: DatabaseTeamAnalysisDb49Manifest,
    teamAnalysisDb50Manifest: DatabaseTeamAnalysisDb50Manifest,
    outputDir: string,
    ...
    const singleStatProportionalEvidencePath = resolve(__dirname, "native-single-stat-energy-ball-proportional-semantics.json"); const singleStatProportionalEvidenceBytes = await readFile(singleStatProportionalEvidencePath); const singleStatProportionalEvidence = JSON.parse(singleStatProportionalEvidenceBytes.toString("utf8")) as Db44NativeEvidence; const singleStatProportionalEvidenceSha256 = sha256(singleStatProportionalEvidenceBytes);
    const buildTeamAnalysisDb44 = () => buildDatabaseTeamAnalysisDb44Dataset({ db8: teamAnalysisDb8Dataset, db8Sha256: sha256(teamAnalysisDb8Gzip), db9: teamAnalysisDb9Dataset, db9Sha256: sha256(teamAnalysisDb9Gzip), db11: teamAnalysisDb11Dataset, db11Sha256: sha256(teamAnalysisDb11Gzip), db31: teamAnalysisDb31Dataset, db31Sha256: sha256(teamAnalysisDb31Gzip), db34: teamAnalysisDb34Dataset, db34Sha256: sha256(teamAnalysisDb34Gzip), db35: teamAnalysisDb35Dataset, db35Sha256: sha256(teamAnalysisDb35Gzip), db36: teamAnalysisDb36Dataset, db36Sha256: sha256(teamAnalysisDb36Gzip), db37: teamAnalysisDb37Dataset, db37Sha256: sha256(teamAnalysisDb37Gzip), db39: teamAnalysisDb39Dataset, db39Sha256: sha256(teamAnalysisDb39Gzip), tables, inspection: nativeInspection, nativeSha256: nativeBefore.sha256, nativeSizeBytes: nativeBefore.sizeBytes, evidence: singleStatProportionalEvidence, evidenceSha256: singleStatProportionalEvidenceSha256 }); const teamAnalysisDb44Dataset = buildTeamAnalysisDb44(); const firstTeamAnalysisDb44Json = `${JSON.stringify(teamAnalysisDb44Dataset)}\n`; const secondTeamAnalysisDb44Json = `${JSON.stringify(buildTeamAnalysisDb44())}\n`; if (sha256(firstTeamAnalysisDb44Json) !== sha256(secondTeamAnalysisDb44Json)) throw new Error("DB44 determinism check failed: two single-stat proportional projections differ"); const teamAnalysisDb44Gzip = gzipSync(Buffer.from(firstTeamAnalysisDb44Json, "utf8"), { level: 9 }); const secondTeamAnalysisDb44Gzip = gzipSync(Buffer.from(secondTeamAnalysisDb44Json, "utf8"), { level: 9 }); if (!teamAnalysisDb44Gzip.equals(secondTeamAnalysisDb44Gzip)) throw new Error("DB44 determinism check failed: gzip bytes differ"); const teamAnalysisDb44Validation = validateDatabaseTeamAnalysisDb44Dataset(teamAnalysisDb44Dataset, teamAnalysisDb11Dataset, teamAnalysisDb31Dataset, teamAnalysisDb35Dataset, teamAnalysisDb36Dataset, teamAnalysisDb37Dataset, tables, { db8Sha256: sha256(teamAnalysisDb8Gzip), db9Sha256: sha256(teamAnalysisDb9Gzip), db11Sha256: sha256(teamAnalysisDb11Gzip), db31Sha256: sha256(teamAnalysisDb31Gzip), db34Sha256: sha256(teamAnalysisDb34Gzip), db35Sha256: sha256(teamAnalysisDb35Gzip), db36Sha256: sha256(teamAnalysisDb36Gzip), db37Sha256: sha256(teamAnalysisDb37Gzip), db39Sha256: sha256(teamAnalysisDb39Gzip), db39NativeEvidenceSha256: proportionalStatEvidenceSha256, nativeSha256: nativeBefore.sha256, nativeSizeBytes: nativeBefore.sizeBytes, evidence: singleStatProportionalEvidence, evidenceSha256: singleStatProportionalEvidenceSha256 }); if (!teamAnalysisDb44Validation.valid) throw new Error(`DB44 payload validation failed: ${JSON.stringify(teamAnalysisDb44Validation.failures.slice(0, 10))}`); const teamAnalysisDb44Coverage = buildDatabaseTeamAnalysisDb44Coverage(teamAnalysisDb44Dataset); const teamAnalysisDb44Goldens = await validateDatabaseTeamAnalysisDb44Goldens(teamAnalysisDb44Dataset, teamAnalysisDb44Coverage); if (teamAnalysisDb44Goldens.failures.length > 0) throw new Error(`DB44 golden fixture validation failed: ${JSON.stringify(teamAnalysisDb44Goldens.failures)}`); const teamAnalysisDb44Report = renderDatabaseTeamAnalysisDb44Report(teamAnalysisDb44Dataset, teamAnalysisDb44Coverage);
    const postDamageHpFactorEvidencePath = resolve(__dirname, "native-player-attack-post-damage-hp-factor-semantics.json"); const postDamageHpFactorEvidenceBytes = await readFile(postDamageHpFactorEvidencePath); const postDamageHpFactorEvidence = JSON.parse(postDamageHpFactorEvidenceBytes.toString("utf8")) as Db45NativeEvidence; const postDamageHpFactorEvidenceSha256 = sha256(postDamageHpFactorEvidenceBytes);
    ...
    const buildTeamAnalysisDb45 = () => buildDatabaseTeamAnalysisDb45Dataset({ db8: teamAnalysisDb8Dataset, db8Sha256: db45SourceHashes.db8, db9: teamAnalysisDb9Dataset, db9Sha256: db45SourceHashes.db9, db11: teamAnalysisDb11Dataset, db11Sha256: db45SourceHashes.db11, db31: teamAnalysisDb31Dataset, db31Sha256: db45SourceHashes.db31, db33: teamAnalysisDb33Dataset, db33Sha256: db45SourceHashes.db33, db35: teamAnalysisDb35Dataset, db35Sha256: db45SourceHashes.db35, db36: teamAnalysisDb36Dataset, db36Sha256: db45SourceHashes.db36, db37: teamAnalysisDb37Dataset, db37Sha256: db45SourceHashes.db37, tables, inspection: nativeInspection, nativeSha256: nativeBefore.sha256, nativeSizeBytes: nativeBefore.sizeBytes, evidence: postDamageHpFactorEvidence, evidenceSha256: postDamageHpFactorEvidenceSha256 });
    const teamAnalysisDb45Dataset = buildTeamAnalysisDb45(); const firstTeamAnalysisDb45Json = `${JSON.stringify(teamAnalysisDb45Dataset)}\n`; const secondTeamAnalysisDb45Json = `${JSON.stringify(buildTeamAnalysisDb45())}\n`; if (sha256(firstTeamAnalysisDb45Json) !== sha256(secondTeamAnalysisDb45Json)) throw new Error("DB45 determinism check failed: two post-damage HP-factor projections differ"); const teamAnalysisDb45Gzip = gzipSync(Buffer.from(firstTeamAnalysisDb45Json, "utf8"), { level: 9 }); const secondTeamAnalysisDb45Gzip = gzipSync(Buffer.from(secondTeamAnalysisDb45Json, "utf8"), { level: 9 }); if (!teamAnalysisDb45Gzip.equals(secondTeamAnalysisDb45Gzip)) throw new Error("DB45 determinism check failed: gzip bytes differ");
    const teamAnalysisDb45Validation = validateDatabaseTeamAnalysisDb45Dataset(teamAnalysisDb45Dataset, teamAnalysisDb11Dataset, teamAnalysisDb31Dataset, teamAnalysisDb33Dataset, teamAnalysisDb35Dataset, teamAnalysisDb36Dataset, teamAnalysisDb37Dataset, tables, { sourceHashes: db45SourceHashes, nativeSha256: nativeBefore.sha256, nativeSizeBytes: nativeBefore.sizeBytes, evidence: postDamageHpFactorEvidence, evidenceSha256: postDamageHpFactorEvidenceSha256 }); if (!teamAnalysisDb45Validation.valid) throw new Error(`DB45 payload validation failed: ${JSON.stringify(teamAnalysisDb45Validation.failures.slice(0, 10))}`); const teamAnalysisDb45Coverage = buildDatabaseTeamAnalysisDb45Coverage(teamAnalysisDb45Dataset); const teamAnalysisDb45Goldens = await validateDatabaseTeamAnalysisDb45Goldens(teamAnalysisDb45Dataset, teamAnalysisDb45Coverage); if (teamAnalysisDb45Goldens.failures.length > 0) throw new Error(`DB45 golden fixture validation failed: ${JSON.stringify(teamAnalysisDb45Goldens.failures)}`); const teamAnalysisDb45Report = renderDatabaseTeamAnalysisDb45Report(teamAnalysisDb45Dataset, teamAnalysisDb45Coverage);
    const enemyAttackTimingEvidenceBytes = await readFile(resolve(__dirname, "native-enemy-attack-timing-semantics.json")); const enemyAttackTimingEvidence = JSON.parse(enemyAttackTimingEvidenceBytes.toString("utf8")) as Db46NativeEvidence; const enemyAttackTimingEvidenceSha256 = sha256(enemyAttackTimingEvidenceBytes);
    const buildTeamAnalysisDb46 = () => buildDatabaseTeamAnalysisDb46Dataset(teamAnalysisDb45Dataset, sha256(teamAnalysisDb45Gzip), nativeInspection, nativeBefore.sha256, nativeBefore.sizeBytes, enemyAttackTimingEvidence, enemyAttackTimingEvidenceSha256); const teamAnalysisDb46Dataset=buildTeamAnalysisDb46(); const firstTeamAnalysisDb46Json=`${JSON.stringify(teamAnalysisDb46Dataset)}\n`; const secondTeamAnalysisDb46Json=`${JSON.stringify(buildTeamAnalysisDb46())}\n`; const teamAnalysisDb46Gzip=gzipSync(Buffer.from(firstTeamAnalysisDb46Json,"utf8"),{level:9}); if(sha256(firstTeamAnalysisDb46Json)!==sha256(secondTeamAnalysisDb46Json)||!teamAnalysisDb46Gzip.equals(gzipSync(Buffer.from(secondTeamAnalysisDb46Json,"utf8"),{level:9})))throw new Error("DB46 determinism check failed"); const teamAnalysisDb46Validation=validateDatabaseTeamAnalysisDb46Dataset(teamAnalysisDb46Dataset,teamAnalysisDb45Dataset,nativeBefore.sha256,nativeBefore.sizeBytes,enemyAttackTimingEvidenceSha256); if(!teamAnalysisDb46Validation.valid)throw new Error(`DB46 payload validation failed: ${JSON.stringify(teamAnalysisDb46Validation.failures.slice(0,10))}`); const teamAnalysisDb46Coverage=buildDatabaseTeamAnalysisDb46Coverage(teamAnalysisDb46Dataset); const teamAnalysisDb46Goldens=validateDatabaseTeamAnalysisDb46Goldens(teamAnalysisDb46Dataset,teamAnalysisDb46Coverage); if(teamAnalysisDb46Goldens.failures.length)throw new Error(`DB46 goldens failed: ${JSON.stringify(teamAnalysisDb46Goldens.failures)}`); const teamAnalysisDb46Report=renderDatabaseTeamAnalysisDb46Report(teamAnalysisDb46Coverage);
    const puzzleMoveEndEvidenceBytes=await readFile(resolve(__dirname,"native-puzzle-move-end-timing-semantics.json"));const puzzleMoveEndEvidence=JSON.parse(puzzleMoveEndEvidenceBytes.toString("utf8"))as Db47NativeEvidence;const puzzleMoveEndEvidenceSha256=sha256(puzzleMoveEndEvidenceBytes);const buildTeamAnalysisDb47=()=>buildDatabaseTeamAnalysisDb47Dataset(teamAnalysisDb46Dataset,sha256(teamAnalysisDb46Gzip),nativeInspection,nativeBefore.sha256,nativeBefore.sizeBytes,puzzleMoveEndEvidence,puzzleMoveEndEvidenceSha256);const teamAnalysisDb47Dataset=buildTeamAnalysisDb47(),firstTeamAnalysisDb47Json=`${JSON.stringify(teamAnalysisDb47Dataset)}\n`,secondTeamAnalysisDb47Json=`${JSON.stringify(buildTeamAnalysisDb47())}\n`,teamAnalysisDb47Gzip=gzipSync(Buffer.from(firstTeamAnalysisDb47Json,"utf8"),{level:9});if(sha256(firstTeamAnalysisDb47Json)!==sha256(secondTeamAnalysisDb47Json)||!teamAnalysisDb47Gzip.equals(gzipSync(Buffer.from(secondTeamAnalysisDb47Json,"utf8"),{level:9})))throw Error("DB47 determinism check failed");const teamAnalysisDb47Validation=validateDatabaseTeamAnalysisDb47Dataset(teamAnalysisDb47Dataset,teamAnalysisDb46Dataset,nativeBefore.sha256,nativeBefore.sizeBytes,puzzleMoveEndEvidenceSha256);if(!teamAnalysisDb47Validation.valid)throw Error(`DB47 validation failed: ${JSON.stringify(teamAnalysisDb47Validation.failures.slice(0,10))}`);const teamAnalysisDb47Coverage=buildDatabaseTeamAnalysisDb47Coverage(teamAnalysisDb47Dataset),teamAnalysisDb47Goldens=validateDatabaseTeamAnalysisDb47Goldens(teamAnalysisDb47Dataset,teamAnalysisDb47Coverage);if(teamAnalysisDb47Goldens.failures.length)throw Error(`DB47 goldens failed: ${JSON.stringify(teamAnalysisDb47Goldens.failures)}`);const teamAnalysisDb47Report=renderDatabaseTeamAnalysisDb47Report(teamAnalysisDb47Coverage);
    const damageMitigationEvidenceBytes=await readFile(resolve(__dirname,"native-damage-mitigation-semantics.json"));const damageMitigationEvidence=JSON.parse(damageMitigationEvidenceBytes.toString("utf8"))as Db48NativeEvidence,damageMitigationEvidenceSha256=sha256(damageMitigationEvidenceBytes),db48Hashes={db11:sha256(teamAnalysisDb11Gzip),db35:sha256(teamAnalysisDb35Gzip),db36:sha256(teamAnalysisDb36Gzip),db37:sha256(teamAnalysisDb37Gzip),db47:sha256(teamAnalysisDb47Gzip)};const buildTeamAnalysisDb48=()=>buildDatabaseTeamAnalysisDb48Dataset({db11:teamAnalysisDb11Dataset,db11Sha256:db48Hashes.db11,db35:teamAnalysisDb35Dataset,db35Sha256:db48Hashes.db35,db36:teamAnalysisDb36Dataset,db36Sha256:db48Hashes.db36,db37:teamAnalysisDb37Dataset,db37Sha256:db48Hashes.db37,db47:teamAnalysisDb47Dataset,db47Sha256:db48Hashes.db47,tables,inspection:nativeInspection,nativeSha256:nativeBefore.sha256,nativeSizeBytes:nativeBefore.sizeBytes,evidence:damageMitigationEvidence,evidenceSha256:damageMitigationEvidenceSha256});const teamAnalysisDb48Dataset=buildTeamAnalysisDb48(),firstTeamAnalysisDb48Json=`${JSON.stringify(teamAnalysisDb48Dataset)}\n`,secondTeamAnalysisDb48Json=`${JSON.stringify(buildTeamAnalysisDb48())}\n`,teamAnalysisDb48Gzip=gzipSync(Buffer.from(firstTeamAnalysisDb48Json,"utf8"),{level:9});if(sha256(firstTeamAnalysisDb48Json)!==sha256(secondTeamAnalysisDb48Json)||!teamAnalysisDb48Gzip.equals(gzipSync(Buffer.from(secondTeamAnalysisDb48Json,"utf8"),{level:9})))throw Error("DB48 determinism check failed");const teamAnalysisDb48Validation=validateDatabaseTeamAnalysisDb48Dataset(teamAnalysisDb48Dataset,teamAnalysisDb11Dataset,teamAnalysisDb35Dataset,teamAnalysisDb36Dataset,teamAnalysisDb37Dataset,teamAnalysisDb47Dataset,tables,{db11Sha256:db48Hashes.db11,db35Sha256:db48Hashes.db35,db36Sha256:db48Hashes.db36,db37Sha256:db48Hashes.db37,db47Sha256:db48Hashes.db47,nativeSha256:nativeBefore.sha256,nativeSizeBytes:nativeBefore.sizeBytes,evidenceSha256:damageMitigationEvidenceSha256,proofRoles:damageMitigationEvidence.codeRegions.map(value=>value.role)});if(!teamAnalysisDb48Validation.valid)throw Error(`DB48 validation failed: ${JSON.stringify(teamAnalysisDb48Validation.failures.slice(0,10))}`);const teamAnalysisDb48Coverage=buildDatabaseTeamAnalysisDb48Coverage(teamAnalysisDb48Dataset),teamAnalysisDb48Goldens=await validateDatabaseTeamAnalysisDb48Goldens(teamAnalysisDb48Dataset,teamAnalysisDb48Coverage);if(teamAnalysisDb48Goldens.failures.length)throw Error(`DB48 goldens failed: ${JSON.stringify(teamAnalysisDb48Goldens.failures)}`);const teamAnalysisDb48Report=renderDatabaseTeamAnalysisDb48Report(teamAnalysisDb48Coverage);
    const forcedGuardEvidenceBytes=await readFile(resolve(__dirname,"native-forced-guard-semantics.json"));const forcedGuardEvidence=JSON.parse(forcedGuardEvidenceBytes.toString("utf8"))as Db49NativeEvidence,forcedGuardEvidenceSha256=sha256(forcedGuardEvidenceBytes),db49Hashes={db11:sha256(teamAnalysisDb11Gzip),db35:sha256(teamAnalysisDb35Gzip),db36:sha256(teamAnalysisDb36Gzip),db37:sha256(teamAnalysisDb37Gzip),db47:sha256(teamAnalysisDb47Gzip)};const buildTeamAnalysisDb49=()=>buildDatabaseTeamAnalysisDb49Dataset({db11:teamAnalysisDb11Dataset,db11Sha256:db49Hashes.db11,db35:teamAnalysisDb35Dataset,db35Sha256:db49Hashes.db35,db36:teamAnalysisDb36Dataset,db36Sha256:db49Hashes.db36,db37:teamAnalysisDb37Dataset,db37Sha256:db49Hashes.db37,db47:teamAnalysisDb47Dataset,db47Sha256:db49Hashes.db47,tables,inspection:nativeInspection,nativeSha256:nativeBefore.sha256,nativeSizeBytes:nativeBefore.sizeBytes,evidence:forcedGuardEvidence,evidenceSha256:forcedGuardEvidenceSha256});const teamAnalysisDb49Dataset=buildTeamAnalysisDb49(),firstTeamAnalysisDb49Json=`${JSON.stringify(teamAnalysisDb49Dataset)}\n`,secondTeamAnalysisDb49Json=`${JSON.stringify(buildTeamAnalysisDb49())}\n`,teamAnalysisDb49Gzip=gzipSync(Buffer.from(firstTeamAnalysisDb49Json,"utf8"),{level:9});if(sha256(firstTeamAnalysisDb49Json)!==sha256(secondTeamAnalysisDb49Json)||!teamAnalysisDb49Gzip.equals(gzipSync(Buffer.from(secondTeamAnalysisDb49Json,"utf8"),{level:9})))throw Error("DB49 determinism check failed");const teamAnalysisDb49Validation=validateDatabaseTeamAnalysisDb49Dataset(teamAnalysisDb49Dataset,teamAnalysisDb11Dataset,teamAnalysisDb35Dataset,teamAnalysisDb36Dataset,teamAnalysisDb37Dataset,teamAnalysisDb47Dataset,tables,{db11Sha256:db49Hashes.db11,db35Sha256:db49Hashes.db35,db36Sha256:db49Hashes.db36,db37Sha256:db49Hashes.db37,db47Sha256:db49Hashes.db47,nativeSha256:nativeBefore.sha256,nativeSizeBytes:nativeBefore.sizeBytes,evidenceSha256:forcedGuardEvidenceSha256,proofRoles:forcedGuardEvidence.codeRegions.map(value=>value.role)});if(!teamAnalysisDb49Validation.valid)throw Error(`DB49 validation failed: ${JSON.stringify(teamAnalysisDb49Validation.failures.slice(0,10))}`);const teamAnalysisDb49Coverage=buildDatabaseTeamAnalysisDb49Coverage(teamAnalysisDb49Dataset),teamAnalysisDb49Goldens=await validateDatabaseTeamAnalysisDb49Goldens(teamAnalysisDb49Dataset,teamAnalysisDb49Coverage);if(teamAnalysisDb49Goldens.failures.length)throw Error(`DB49 goldens failed: ${JSON.stringify(teamAnalysisDb49Goldens.failures)}`);const teamAnalysisDb49Report=renderDatabaseTeamAnalysisDb49Report(teamAnalysisDb49Coverage);
    const counterConsumerEvidenceBytes=await readFile(resolve(__dirname,"native-counter-consumer-semantics.json"));const counterConsumerEvidence=JSON.parse(counterConsumerEvidenceBytes.toString("utf8"))as Db50Evidence,counterConsumerEvidenceSha256=sha256(counterConsumerEvidenceBytes),db50Hashes={db24:sha256(teamAnalysisDb24Gzip),db35:sha256(teamAnalysisDb35Gzip),db47:sha256(teamAnalysisDb47Gzip)};const buildTeamAnalysisDb50=()=>buildDatabaseTeamAnalysisDb50Dataset({db24:teamAnalysisDb24Dataset,db24Sha256:db50Hashes.db24,db35:teamAnalysisDb35Dataset,db35Sha256:db50Hashes.db35,db47:teamAnalysisDb47Dataset,db47Sha256:db50Hashes.db47,inspection:nativeInspection,nativeSha256:nativeBefore.sha256,nativeSizeBytes:nativeBefore.sizeBytes,evidence:counterConsumerEvidence,evidenceSha256:counterConsumerEvidenceSha256});const teamAnalysisDb50Dataset=buildTeamAnalysisDb50(),firstTeamAnalysisDb50Json=`${JSON.stringify(teamAnalysisDb50Dataset)}\n`,secondTeamAnalysisDb50Json=`${JSON.stringify(buildTeamAnalysisDb50())}\n`,teamAnalysisDb50Gzip=gzipSync(Buffer.from(firstTeamAnalysisDb50Json,"utf8"),{level:9});if(sha256(firstTeamAnalysisDb50Json)!==sha256(secondTeamAnalysisDb50Json)||!teamAnalysisDb50Gzip.equals(gzipSync(Buffer.from(secondTeamAnalysisDb50Json,"utf8"),{level:9})))throw Error("DB50 determinism check failed");const teamAnalysisDb50Validation=validateDatabaseTeamAnalysisDb50Dataset(teamAnalysisDb50Dataset,teamAnalysisDb24Dataset,teamAnalysisDb35Dataset,teamAnalysisDb47Dataset,{db24Sha256:db50Hashes.db24,db35Sha256:db50Hashes.db35,db47Sha256:db50Hashes.db47,nativeSha256:nativeBefore.sha256,nativeSizeBytes:nativeBefore.sizeBytes,evidenceSha256:counterConsumerEvidenceSha256,proofRoles:counterConsumerEvidence.codeRegions.map(value=>value.role)});if(!teamAnalysisDb50Validation.valid)throw Error(`DB50 validation failed: ${JSON.stringify(teamAnalysisDb50Validation.failures.slice(0,10))}`);const teamAnalysisDb50Coverage=buildDatabaseTeamAnalysisDb50Coverage(teamAnalysisDb50Dataset),teamAnalysisDb50Goldens=await validateDatabaseTeamAnalysisDb50Goldens(teamAnalysisDb50Dataset,teamAnalysisDb50Coverage);if(teamAnalysisDb50Goldens.failures.length)throw Error(`DB50 goldens failed: ${JSON.stringify(teamAnalysisDb50Goldens.failures)}`);const teamAnalysisDb50Report=renderDatabaseTeamAnalysisDb50Report(teamAnalysisDb50Coverage);
    ...
    await Promise.all([
        ...
        writeFile(resolve(options.outputDir, teamAnalysisDb44Manifest.fileName), teamAnalysisDb44Gzip),
        writeFile(resolve(options.outputDir, "team-analysis-db44-manifest.json"), `${JSON.stringify(teamAnalysisDb44Manifest, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb44Manifest.coverageFile), `${JSON.stringify(teamAnalysisDb44Coverage, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb44Manifest.reportFile), teamAnalysisDb44Report, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb44Manifest.validationFile), `${JSON.stringify(teamAnalysisDb44Validation, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb44Manifest.goldenValidationFile), `${JSON.stringify(teamAnalysisDb44Goldens, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb45Manifest.fileName), teamAnalysisDb45Gzip),
        writeFile(resolve(options.outputDir, "team-analysis-db45-manifest.json"), `${JSON.stringify(teamAnalysisDb45Manifest, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb45Manifest.coverageFile), `${JSON.stringify(teamAnalysisDb45Coverage, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb45Manifest.reportFile), teamAnalysisDb45Report, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb45Manifest.validationFile), `${JSON.stringify(teamAnalysisDb45Validation, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir, teamAnalysisDb45Manifest.goldenValidationFile), `${JSON.stringify(teamAnalysisDb45Goldens, null, 2)}\n`, "utf8"),
        writeFile(resolve(options.outputDir,teamAnalysisDb46Manifest.fileName),teamAnalysisDb46Gzip),writeFile(resolve(options.outputDir,"team-analysis-db46-manifest.json"),`${JSON.stringify(teamAnalysisDb46Manifest,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb46Manifest.coverageFile),`${JSON.stringify(teamAnalysisDb46Coverage,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb46Manifest.reportFile),teamAnalysisDb46Report),writeFile(resolve(options.outputDir,teamAnalysisDb46Manifest.validationFile),`${JSON.stringify(teamAnalysisDb46Validation,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb46Manifest.goldenValidationFile),`${JSON.stringify(teamAnalysisDb46Goldens,null,2)}\n`),
        writeFile(resolve(options.outputDir,teamAnalysisDb47Manifest.fileName),teamAnalysisDb47Gzip),writeFile(resolve(options.outputDir,"team-analysis-db47-manifest.json"),`${JSON.stringify(teamAnalysisDb47Manifest,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb47Manifest.coverageFile),`${JSON.stringify(teamAnalysisDb47Coverage,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb47Manifest.reportFile),teamAnalysisDb47Report),writeFile(resolve(options.outputDir,teamAnalysisDb47Manifest.validationFile),`${JSON.stringify(teamAnalysisDb47Validation,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb47Manifest.goldenValidationFile),`${JSON.stringify(teamAnalysisDb47Goldens,null,2)}\n`),
        writeFile(resolve(options.outputDir,teamAnalysisDb48Manifest.fileName),teamAnalysisDb48Gzip),writeFile(resolve(options.outputDir,"team-analysis-db48-manifest.json"),`${JSON.stringify(teamAnalysisDb48Manifest,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb48Manifest.coverageFile),`${JSON.stringify(teamAnalysisDb48Coverage,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb48Manifest.reportFile),teamAnalysisDb48Report),writeFile(resolve(options.outputDir,teamAnalysisDb48Manifest.validationFile),`${JSON.stringify(teamAnalysisDb48Validation,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb48Manifest.goldenValidationFile),`${JSON.stringify(teamAnalysisDb48Goldens,null,2)}\n`),
        writeFile(resolve(options.outputDir,teamAnalysisDb49Manifest.fileName),teamAnalysisDb49Gzip),writeFile(resolve(options.outputDir,"team-analysis-db49-manifest.json"),`${JSON.stringify(teamAnalysisDb49Manifest,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb49Manifest.coverageFile),`${JSON.stringify(teamAnalysisDb49Coverage,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb49Manifest.reportFile),teamAnalysisDb49Report),writeFile(resolve(options.outputDir,teamAnalysisDb49Manifest.validationFile),`${JSON.stringify(teamAnalysisDb49Validation,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb49Manifest.goldenValidationFile),`${JSON.stringify(teamAnalysisDb49Goldens,null,2)}\n`),
        writeFile(resolve(options.outputDir,teamAnalysisDb50Manifest.fileName),teamAnalysisDb50Gzip),writeFile(resolve(options.outputDir,"team-analysis-db50-manifest.json"),`${JSON.stringify(teamAnalysisDb50Manifest,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb50Manifest.coverageFile),`${JSON.stringify(teamAnalysisDb50Coverage,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb50Manifest.reportFile),teamAnalysisDb50Report),writeFile(resolve(options.outputDir,teamAnalysisDb50Manifest.validationFile),`${JSON.stringify(teamAnalysisDb50Validation,null,2)}\n`),writeFile(resolve(options.outputDir,teamAnalysisDb50Manifest.goldenValidationFile),`${JSON.stringify(teamAnalysisDb50Goldens,null,2)}\n`),
    ]);
