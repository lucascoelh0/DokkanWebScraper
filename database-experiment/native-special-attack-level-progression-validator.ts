...
export function validateNativeSpecialAttackLevelProgressionEvidence(
    ...
    if (JSON.stringify((evidence.codeRegions ?? []).map((region: any) => [
        region.role,
        region.symbol,
        region.vma,
        region.sizeBytes,
    ...
    if (JSON.stringify((evidence.structuralAssertions ?? []).map((assertion: any) => [
        assertion.role,
        assertion.vma,
        assertion.hex,
    ...
    if (JSON.stringify((evidence.directCalls ?? []).map((call: any) => [
        call.role,
        call.callVma,
        call.callHex,
        call.targetPltVma,
