...
import { DatabaseTeamAnalysisDb33Coverage, DatabaseTeamAnalysisDb33Dataset, Db33NativeEvidence } from "./team-analysis-db33-contract";

const sha256 = (value: Buffer) => createHash("sha256").update(value).digest("hex");
const EXPECTED_DB32_SHA256 = "29c408e3ced29f3d07fe8a4715afb88a6d950c52be43cb7fadc7d2b23de4fdfa";
export const parseDb33ExecutionTiming = (value: unknown) => typeof value === "number" ? value : typeof value === "string" && value.trim() ? Number(value) : Number.NaN;
export const projectDb33AttackSetupTiming = (value: unknown) => Number.isSafeInteger(parseDb33ExecutionTiming(value)) && parseDb33ExecutionTiming(value) === 4
    ? { status: "supported" as const, event: "player_attack_setup" as const, sequence: "inside_player_attack_damage_and_action_bank_setup_before_setup_result_consumer" as const }
...
function validateCode(inspection: NativeRuntimeElfInspection, region: { symbol: string, vma: number, sizeBytes: number, codeSha256: string }) {
    const symbol = inspection.symbols.find(value => value.name === region.symbol);
    if (!symbol || symbol.value !== region.vma || symbol.size !== region.sizeBytes || sha256(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw new Error(`DB33 native region mismatch ${region.symbol}`);
}
export function validateDb33Evidence(inspection: NativeRuntimeElfInspection, evidence: Db33NativeEvidence, nativeSha256: string) {
    if (evidence.schemaVersion !== 1 || evidence.sourceSha256 !== nativeSha256 || evidence.auditScope !== "passive-skill-execution-timing-value-4" || JSON.stringify(evidence.unknowns) !== JSON.stringify(EXPECTED_UNKNOWNS)) throw new Error("DB33 evidence identity mismatch");
    if (JSON.stringify(evidence.sqliteField) !== JSON.stringify({ table: "passive_skills", column: "exec_timing_type" })) throw new Error("DB33 SQLite binding mismatch");
    if (JSON.stringify(evidence.filter) !== JSON.stringify({ symbol: "_ZN14AbilityManager21callAbilityStatusExecEi17SkillCategoryType19SkillExecTimingType9SkillTypeN13AbilityStatus14AddtionalParamE", timingArgumentRegister: "w3", getterVirtualCallVma: 64372628, equalityCompareVma: 64372640 })) throw new Error("DB33 filter mismatch");
    if (evidence.supportedValue.raw !== 4 || evidence.supportedValue.event !== "player_attack_setup" || evidence.supportedValue.sequence !== "inside_player_attack_damage_and_action_bank_setup_before_setup_result_consumer" || JSON.stringify(evidence.supportedValue.callSites) !== JSON.stringify(EXPECTED_CALLS)) throw new Error("DB33 supported value mismatch");
    if (JSON.stringify(evidence.setupCallers) !== JSON.stringify(EXPECTED_CALLERS)) throw new Error("DB33 setup callers mismatch");
    ...
    if (JSON.stringify(evidence.passiveStatusFilterProof) !== JSON.stringify(EXPECTED_PASSIVE_FILTER_PROOF)) throw new Error("DB33 passive status filter proof mismatch");
    if (evidence.codeRegions.length !== 12 || new Set(evidence.codeRegions.map(value => value.role)).size !== 12) throw new Error("DB33 code region count mismatch");
    for (const region of evidence.codeRegions) {
        const expected = EXPECTED_REGIONS[region.role];
        if (!expected || JSON.stringify([region.symbol, region.vma, region.sizeBytes, region.codeSha256]) !== JSON.stringify(expected)) throw new Error(`DB33 code region metadata mismatch ${region.role}`);
        validateCode(inspection, region);
    }
    for (const call of evidence.supportedValue.callSites) {
        if (inspection.readVirtualBytes(call.literalVma, 4).toString("hex") !== call.literalInstructionHex || inspection.readVirtualBytes(call.callVma, 4).toString("hex") !== call.callInstructionHex) throw new Error(`DB33 call-site bytes mismatch ${call.callVma}`);
    }
    for (const call of evidence.passiveStatusFilterProof.allPublicCreatorCallSites) if (inspection.readVirtualBytes(call.categoryLiteralVma, 4).toString("hex") !== call.literalInstructionHex) throw new Error(`DB33 passive creator category mismatch ${call.callVma}`);
    ...
    if (managerCreatorRelocation.length !== 1 || managerCreatorRelocation[0].symbolName !== evidence.passiveStatusFilterProof.sharedCreator.symbol || managerCreatorRelocation[0].symbolValue !== 64383460) throw new Error("DB33 passive creator vtable slot mismatch");
    const passiveVtable = inspection.symbols.find(value => value.name === evidence.passiveStatusFilterProof.passiveVtable.symbol);
    if (!passiveVtable || passiveVtable.value !== evidence.passiveStatusFilterProof.passiveVtable.vma || passiveVtable.size !== evidence.passiveStatusFilterProof.passiveVtable.sizeBytes || sha256(inspection.readVirtualBytes(passiveVtable.value, passiveVtable.size)) !== evidence.passiveStatusFilterProof.passiveVtable.rawSha256) throw new Error("DB33 passive vtable mismatch");
    const expectedGetters: Array<[number, string, number]> = [[64, "_ZNK13AbilityStatus13getExecTimingEv", 64480160], [96, "_ZNK13AbilityStatus11getCategoryEv", 64480192], [104, "_ZNK13AbilityStatus12getSkillTypeEv", 64480200]];
    for (const [offset, symbolName, symbolValue] of expectedGetters) { const relocation = inspection.relocations.filter(value => value.offset === passiveVtable.value + offset); if (relocation.length !== 1 || relocation[0].symbolName !== symbolName || relocation[0].symbolValue !== symbolValue) throw new Error(`DB33 passive getter slot mismatch ${offset}`); }
}
export function validateDb33SourceLineage(db32: DatabaseTeamAnalysisDb32Dataset, db32Sha256: string, sourceDatabaseSha256: string, currentSha256: string, nativeSha256: string, nativeSizeBytes: number) {
    if (db32.contractVersion !== "0.31.0" || db32Sha256 !== EXPECTED_DB32_SHA256 || db32.sourceDatabaseSha256 !== sourceDatabaseSha256 || db32.currentTeamAnalysis.sha256 !== currentSha256 || db32.nativeRuntime.sha256 !== nativeSha256 || db32.nativeRuntime.sizeBytes !== nativeSizeBytes) throw new Error("DB33 source lineage mismatch");
}
export function buildDatabaseTeamAnalysisDb33Dataset(options: {
    db32: DatabaseTeamAnalysisDb32Dataset, db32Sha256: string, sourceDatabaseSha256: string,
    current: { states: Array<{ passive?: { rules?: Array<{ effects?: Array<Record<string, unknown>> }> } }> }, currentSha256: string,
    inspection: NativeRuntimeElfInspection, nativeSha256: string, nativeSizeBytes: number, evidence: Db33NativeEvidence, evidenceSha256: string,
}): DatabaseTeamAnalysisDb33Dataset {
    ...
    const ruleTimings = options.db32.ruleTimings.map(source => {
        const raw = parseDb33ExecutionTiming(source.rawExecutionTimingType);
        if (raw === 1 && (source.executionTiming.status !== "supported" || source.executionTiming.event !== "turn_start" || !source.provenance.runtime)) throw new Error(`DB33 inherited timing mismatch ${source.stateKey}|${source.ruleKey}`);
        if (raw !== 1 && source.executionTiming.status !== "unknown") throw new Error(`DB33 unexpected inherited timing ${source.stateKey}|${source.ruleKey}`);
        const promoted = projectDb33AttackSetupTiming(source.rawExecutionTimingType);
        ...
        const provenance = promoted.status === "supported" ? {
            ...
            runtime: { fileName: "libcocos2dcpp.so" as const, sha256: options.nativeSha256, evidenceFile: "native-execution-timing-value-4-semantics.json" as const, evidenceSha256: options.evidenceSha256, proofRoles },
        } : source.provenance;
    ...
    const explicitWhenAttackingEffectCount = options.current.states.flatMap(state => state.passive?.rules ?? []).flatMap(rule => rule.effects ?? []).filter(effect => (effect.activationTiming as any)?.moment === "when_attacking").length;
    return {
...
function counts(values: unknown[]) { const result: Record<string, number> = {}; for (const value of values) { const key = String(value); result[key] = (result[key] ?? 0) + 1; } return Object.fromEntries(Object.entries(result).sort(([a], [b]) => a.localeCompare(b, "en", { numeric: true }))); }
export function buildDatabaseTeamAnalysisDb33Coverage(dataset: DatabaseTeamAnalysisDb33Dataset): DatabaseTeamAnalysisDb33Coverage {
