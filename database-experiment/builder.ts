import {
    ...
    DatabaseAttack,
    DatabaseCardRecord,
    DatabaseCharacterExperimentDataset,
    DatabaseExperimentCoverage,
    DatabaseFormRelation,
    DatabaseSkillState,
    EvidenceKind,
    FormRelationKind,
    MappedValue,
...
import { ReadOnlySqliteAdapter, SqliteRow } from "./sqlite-readonly-adapter";

export const CONSUMED_TABLE_COLUMNS: Record<string, string[]> = {
...
export type DatabaseExperimentTables = Record<string, SqliteRow[]>;

const COLLECTABLE_CARD_ID_MAX_EXCLUSIVE = 4_000_000;
...
function provenance(table: string, row: SqliteRow): RowProvenance {
    return {
        table,
...
function sourced(table: string, row: SqliteRow | undefined): SourcedRow | undefined {
    return row ? { values: row, provenance: provenance(table, row) } : undefined;
}

function byId(rows: SqliteRow[]): Map<string, SqliteRow> {
...
function groupBy(rows: SqliteRow[], column: string): Map<string, SqliteRow[]> {
...
function parseJson(value: SqliteScalar | undefined): unknown {
...
function numericJsonArray(value: SqliteScalar | undefined): number[] {
    const parsed = parseJson(value);
    return Array.isArray(parsed) ? parsed.filter(item => typeof item === "number" && Number.isFinite(item)) : [];
}

function collectExistingCausalityIds(value: unknown, known: Map<string, SqliteRow>, result = new Set<string>()): Set<string> {
    if (typeof value === "number" && known.has(String(value))) result.add(String(value));
    if (Array.isArray(value)) value.forEach(item => collectExistingCausalityIds(item, known, result));
    if (value && typeof value === "object") Object.values(value as Record<string, unknown>).forEach(item => collectExistingCausalityIds(item, known, result));
    return result;
}

function mapRarity(raw: SqliteScalar): MappedValue<"N" | "R" | "SR" | "SSR" | "UR" | "LR"> {
    const values = ["N", "R", "SR", "SSR", "UR", "LR"] as const;
...
function mapType(raw: SqliteScalar): MappedValue<"AGL" | "TEQ" | "INT" | "STR" | "PHY"> {
    const values = ["AGL", "TEQ", "INT", "STR", "PHY"] as const;
    const numeric = numberValue(raw);
...
function mapClass(raw: SqliteScalar): MappedValue<"Super" | "Extreme" | "unawakened"> {
    const numeric = numberValue(raw);
    if (numeric !== null && numeric >= 0 && numeric < 5) return { raw, value: "unawakened", evidence: "first-party-row-join" };
    if (numeric !== null && numeric >= 10 && numeric < 20) return { raw, value: "Super", evidence: "first-party-row-join" };
    if (numeric !== null && numeric >= 20 && numeric < 30) return { raw, value: "Extreme", evidence: "first-party-row-join" };
    return { raw, value: "unknown", evidence: "unknown" };
}

function mapAttackVariant(raw: SqliteScalar): MappedValue<AttackVariant> {
    const mapping: Record<string, AttackVariant> = { Normal: "super", Hyper: "ultra", Condition: "unit", Extra: "ex" };
...
function mapFormKind(channel: DatabaseFormRelation["channel"], raw: SqliteScalar): MappedValue<FormRelationKind> {
    const numeric = numberValue(raw);
    if (numeric === 103) return { raw, value: "transformation", evidence: "first-party-row-join" };
    if (numeric === 79 && (channel === "passive" || channel === "active")) return { raw, value: "giant-or-rage", evidence: "first-party-row-join" };
    if (numeric === 131 && channel === "passive") return { raw, value: "reversible-exchange", evidence: "cross-source-golden" };
    return { raw, value: "unknown", evidence: "unknown" };
}
...
export async function loadDatabaseExperimentTables(adapter: ReadOnlySqliteAdapter): Promise<DatabaseExperimentTables> {
    const tables: DatabaseExperimentTables = {};
...
function buildFormRelations(tables: DatabaseExperimentTables): DatabaseFormRelation[] {
    const result: DatabaseFormRelation[] = [];
    const descriptionByKey = new Map<string, SqliteRow>();
    for (const row of tables.transformation_descriptions) {
        descriptionByKey.set(`${text(row.skill_type)}:${id(row.skill_id)}`, row);
    }
    const push = (channel: DatabaseFormRelation["channel"], sourceCardId: string, skill: SqliteRow, setId: string | undefined, target: SqliteScalar | undefined) => {
    ...
    for (const card of tables.cards) {
        ...
        for (const passiveSetId of passiveSetIds) {
            for (const relation of passiveRelationsBySet.get(passiveSetId) ?? []) {
                ...
                if (skill) push("passive", cardId, skill, passiveSetId, skill.eff_value1);
            }
        }
        for (const relation of activeRelationsByCard.get(cardId) ?? []) {
            const setId = id(relation.active_skill_set_id);
            for (const skill of activeBySet.get(setId ?? "") ?? []) push("active", cardId, skill, setId, skill.eff_val1);
        }
        const standbySetIds: string[] = [];
        for (const relation of standbyRelationsByCard.get(cardId) ?? []) {
            const setId = id(relation.standby_skill_set_id);
            if (setId) standbySetIds.push(setId);
            for (const skill of standbyBySet.get(setId ?? "") ?? []) push("standby", cardId, skill, setId, numericJsonArray(skill.efficacy_values)[0]);
        }
        ...
        for (const setId of finishSetIds) {
            for (const skill of finishBySet.get(setId ?? "") ?? []) push("finish", cardId, skill, setId, numericJsonArray(skill.efficacy_values)[0]);
        }
    }
    return result.sort((a, b) => `${a.sourceCardId}:${a.channel}:${a.sourceSkillId}`.localeCompare(`${b.sourceCardId}:${b.channel}:${b.sourceSkillId}`));
}

function buildAttacks(cardId: string, tables: DatabaseExperimentTables, lookups: ReturnType<typeof buildLookups>): DatabaseAttack[] {
...
function buildLookups(tables: DatabaseExperimentTables) {
    return {
...
function resolveGrowthRelease(
    cardId: string,
    step: number | null,
    lookups: ReturnType<typeof buildLookups>,
    formSourcesByTarget: Map<string, string[]>,
    releasedAtOrBefore: string,
): Pick<DatabaseSkillState, "releaseState" | "releaseStateEvidence" | "release"> {
    if (step === null) {
        return { releaseState: "unknown", releaseStateEvidence: "unknown", release: { availableAt: null, availableAtSnapshot: null, routes: [] } };
    }
    const matchingRoutes = (sourceCardId: string) => (lookups.routesBySource.get(sourceCardId) ?? []).filter(route =>
        text(route.type) === "CardAwakeningRoute::Optimal" && numberValue(route.optimal_awakening_step) === step);
    ...
    if (routes.length === 0) {
        ...
        while (pending.length > 0) {
            ...
            if (visited.has(sourceCardId)) continue;
...
function buildState(
    ...
    const release = growth
        ...
        : {
            ...
            releaseStateEvidence: "first-party-row-join" as const,
            release: {
...
export function buildDatabaseExperimentDataset(options: {
    tables: DatabaseExperimentTables,
    generatedAt: string,
    ...
    for (const relation of allFormRelations) {
        if (selectedIds.has(relation.sourceCardId) && relation.targetCardId && lookups.cards.has(relation.targetCardId)) selectedIds.add(relation.targetCardId);
    }

    const formSourcesByTarget = new Map<string, string[]>();
    for (const relation of allFormRelations) {
        if (!selectedIds.has(relation.sourceCardId) || !relation.targetCardId || !selectedIds.has(relation.targetCardId)) continue;
        const sources = formSourcesByTarget.get(relation.targetCardId) ?? [];
        if (!sources.includes(relation.sourceCardId)) sources.push(relation.sourceCardId);
        formSourcesByTarget.set(relation.targetCardId, sources.sort((left, right) => Number(left) - Number(right)));
    }

    const awakeningTargetsByCard = new Map<string, string[]>();
    for (const route of tables.card_awakening_routes) {
        const source = id(route.card_id); const target = id(route.awaked_card_id);
        if (!source || !target || source === target || !selectedIds.has(source) || !selectedIds.has(target)) continue;
    ...
    const downstreamCollectionCardIds = (cardId: string): string[] => {
        const visited = new Set([cardId]);
        ...
        while (pending.length > 0) {
            ...
            if (collectionListedIds.has(targetId)) result.add(targetId);
    ...
    for (const route of tables.card_awakening_routes) {
        const source = id(route.card_id); const target = id(route.awaked_card_id);
        if (!source || !target || source === target || !selectedIds.has(source) || !selectedIds.has(target)) continue;
        awakening.union(source, target); hardDuplicate.union(source, target);
    }
    for (const relation of allFormRelations) {
        if (selectedIds.has(relation.sourceCardId) && relation.targetCardId && selectedIds.has(relation.targetCardId)) hardDuplicate.union(relation.sourceCardId, relation.targetCardId);
    }

    const cards: DatabaseCardRecord[] = [...selectedIds].sort((a, b) => Number(a) - Number(b)).map(cardId => {
        ...
        const rarity = mapRarity(card.rarity); const type = mapType(card.element); const characterClass = mapClass(card.element);
        if (rarity.value === "unknown") unknowns.push(`rarity:${card.rarity}`);
        if (type.value === "unknown") unknowns.push(`element-type:${card.element}`);
        if (characterClass.value === "unknown") unknowns.push(`element-class:${card.element}`);
        ...
        if (skillStates.slice(1).some(state => state.releaseState === "unknown")) unknowns.push("growth-step-release-state:EZA/SEZA-unproven");
        return {
            cardId,
    ...
    return {
        ...
        releaseStateEnumEvidence: {
            eza: [sourced("help_bodies", byId(tables.help_bodies).get("181")), sourced("missions", byId(tables.missions).get("17293"))].filter((row): row is SourcedRow => Boolean(row)),
            seza: [sourced("help_bodies", byId(tables.help_bodies).get("403")), sourced("missions", byId(tables.missions).get("25264"))].filter((row): row is SourcedRow => Boolean(row)),
        },
...
export function buildCoverage(dataset: DatabaseCharacterExperimentDataset): DatabaseExperimentCoverage {
    const attacks: Record<AttackVariant, number> = { super: 0, ultra: 0, unit: 0, ex: 0, unknown: 0 };
    const forms: Record<FormRelationKind, number> = { transformation: 0, "giant-or-rage": 0, "reversible-exchange": 0, unknown: 0 };
    const unknownEnums: Record<string, number> = {};
    const dangling: Record<string, number> = { character: 0, cardUniqueInfo: 0, linkSkill: 0, category: 0, formTarget: 0 };
    let leader = 0; let passive = 0; let growthCards = 0; let unknownRelease = 0; let eza = 0; let seza = 0; let futureRelease = 0;
    let projectedUnknownRelease = 0; let projectedFutureRelease = 0;
    const enumRaw = new Map<string, Set<SqliteScalar>>();
    const remember = (name: string, value: SqliteScalar | undefined) => {
        if (value === undefined) return;
        const values = enumRaw.get(name) ?? new Set<SqliteScalar>();
        values.add(value);
        enumRaw.set(name, values);
    };
    for (const card of dataset.cards) {
        if (!card.character) dangling.character += 1;
        if (!card.cardUniqueInfo) dangling.cardUniqueInfo += 1;
        ...
        if (card.rarity.value === "unknown") unknownEnums.rarity = (unknownEnums.rarity ?? 0) + 1;
        if (card.type.value === "unknown") unknownEnums.type = (unknownEnums.type ?? 0) + 1;
        if (card.characterClass.value === "unknown") unknownEnums.characterClass = (unknownEnums.characterClass ?? 0) + 1;
        remember("rarity", card.rarity.raw); remember("element", card.type.raw);
        if (card.skillStates.length > 1) growthCards += 1;
        for (const state of card.skillStates) {
            if (state.leaderSkill) leader += 1;
            if (state.passiveSkill) passive += 1;
            if (state.releaseState === "unknown") unknownRelease += 1;
            if (state.releaseState === "eza") eza += 1;
            if (state.releaseState === "seza") seza += 1;
            if (state.release.availableAtSnapshot === false) futureRelease += 1;
            if (card.catalog.isProjectedPrimary && state.releaseState === "unknown") projectedUnknownRelease += 1;
            if (card.catalog.isProjectedPrimary && state.release.availableAtSnapshot === false) projectedFutureRelease += 1;
        ...
        card.formRelations.forEach(relation => {
            ...
            if (relation.targetCardId && !dataset.cards.some(target => target.cardId === relation.targetCardId)) dangling.formTarget += 1;
        });
    }
    const sortedRaw = (name: string) => [...(enumRaw.get(name) ?? [])].sort((left, right) => String(left).localeCompare(String(right), undefined, { numeric: true }));
    const split = (name: string, confirmed: SqliteScalar[], note: string) => ({ confirmed, unknown: sortedRaw(name).filter(value => !confirmed.includes(value)), note });
    return {
        schemaVersion: 1,
