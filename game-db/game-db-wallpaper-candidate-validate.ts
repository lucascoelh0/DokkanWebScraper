...
import { RewardItemAudit } from "./game-db-reward-item-audit";
...
export async function validateWallpaperCandidate(options: {
    stageDatasetPath: string,
    rewardAuditPath: string,
    wallpaperManifestPath: string,
    wallpaperAssetRoot: string,
}): Promise<Record<string, unknown>> {
    const dataset = JSON.parse(await readFile(resolve(options.stageDatasetPath), "utf8")) as StageDetailsDataset;
    const audit = JSON.parse(await readFile(resolve(options.rewardAuditPath), "utf8")) as RewardItemAudit;
    const manifest = validateWallpaperAssetManifest(JSON.parse(await readFile(resolve(options.wallpaperManifestPath), "utf8")));
    if (dataset.sourceSnapshotVersion !== SNAPSHOT || audit.sourceSnapshotVersion !== SNAPSHOT
        ...
        || manifest.source.databaseSha256 !== DATABASE_SHA256) throw new Error("WP-01 artifacts are not bound to the current official snapshot");
    ...
    for (const path of fixturePaths) {
        ...
        const bytes = await readFile(resolve(options.wallpaperAssetRoot, "game-assets", ...path.split("/")));
        if (bytes.byteLength !== entry.sizeBytes || sha256(bytes) !== entry.sha256) throw new Error(`WP-01 fixture asset drifted: ${path}`);
    }
...
async function main(): Promise<void> {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!["--stage-dataset", "--reward-audit", "--wallpaper-manifest", "--wallpaper-asset-root"].includes(name)) throw new Error(`Unexpected WP-01 validation argument: ${name}`);
        const value = separator >= 0 ? args[index].slice(separator + 1) : args[++index];
        if (!value || values.has(name)) throw new Error(`Missing or duplicate WP-01 validation argument: ${name}`);
        values.set(name, value);
    }
    for (const required of ["--stage-dataset", "--reward-audit", "--wallpaper-manifest", "--wallpaper-asset-root"]) if (!values.has(required)) throw new Error(`Missing WP-01 validation argument: ${required}`);
