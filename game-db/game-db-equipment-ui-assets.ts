...
type ArchiveRole = "installed-fonts-en" | "installed-layout-common" | "installed-layout-charamenu" | "installed-layout-character";

export interface EquipmentUiSourceManifest {
    schemaVersion: 2,
    packageName: "com.bandainamcogames.dbzdokkanww",
    versionName: string,
    databaseSnapshotVersion: string,
    assetVersion: string,
    cpkReader: { repository: "https://github.com/Sewer56/CriFsV2Lib", commit: string },
    nativeLevelRenderer: {
        evidenceClientVersionName: string,
        libraryPath: "lib/arm64-v8a/libcocos2dcpp.so",
        librarySizeBytes: number,
        librarySha256: string,
        logicalFontName: "OT-PShinGoPr6N-Heavy.otf",
        englishHeavyFontCandidatePath: typeof FONT_PATH,
        fontResolution: "first-party-supported-inference",
        fillColor: "#e38235",
        outlineColor: "#000000",
        outlineSize: 4,
        shadowColor: "#000000",
        shadowBlur: 0,
        getters: {
            singleBase: string, singleNumber: string, dualBase: string,
            dualBottomOneDigit: string, dualTopOneDigit: string,
            dualBottomTwoDigits: string, dualTopTwoDigits: string,
        },
    },
    archives: Array<{ role: ArchiveRole, path: string, sizeBytes: number, sha256: string }>,
    files: Array<{ path: string, archiveRole: ArchiveRole, entryPath: string, sizeBytes: number, sha256: string }>,
}

interface Rect { width: number, height: number, x: number, y: number }
interface TextRect extends Rect { fontSize: number, effectiveKerning: number, shadowX: number, shadowDown: number }

export interface EquipmentUiAssetEntry {
    path: string,
    sourceUrl: string,
    sha256: string,
    sizeBytes: number,
...
function contained(root: string, path: string): string {
    ...
    if (!target.toLowerCase().startsWith(`${resolve(root)}${sep}`.toLowerCase())) throw new Error(`Equipment UI path escapes root: ${path}`);
    return target;
}

function validateLevel(level: number): void {
    if (!Number.isSafeInteger(level) || level < 0 || level > 99) throw new Error(`Official equipment level layout cannot represent ${level}`);
}

async function renderTextGlyphs(fontFile: string, value: string, node: TextRect): Promise<Buffer> {
    return sharp({
        text: {
...
async function colorizeGlyphs(mask: Buffer, width: number, height: number, color: string): Promise<Buffer> {
...
export async function renderOfficialLevelNumber(fontFile: string, value: number, node: TextRect): Promise<Buffer> {
    validateLevel(value);
    const text = String(value);
    const glyphMask = await renderTextGlyphs(fontFile, text, node);
    const metadata = await sharp(glyphMask).metadata();
    if (!metadata.width || !metadata.height) throw new Error(`Official equipment level font could not render ${text}`);
    ...
    if (width + node.shadowX > node.width || height + node.shadowDown > node.height) throw new Error(`Official equipment level ${text} exceeds its native ${node.width}x${node.height} node`);
    const outlineLayers: sharp.OverlayOptions[] = [];
...
function topInCanvas(canvas: Rect, node: Rect): number {
    return canvas.y + canvas.height - node.y - node.height;
}

async function compositeNativeLayout(canvas: Rect, layers: Array<{ input: Buffer, rect: Rect }>): Promise<Buffer> {
...
type LevelNumberRenderer = (fontFile: string, value: number, node: TextRect) => Promise<Buffer>;

export async function renderEquipmentLevelAsset(
    fontFile: string,
    singleBase: Buffer,
    dualBase: Buffer,
    levels: number[],
    renderNumber: LevelNumberRenderer = renderOfficialLevelNumber,
): Promise<Buffer> {
    ...
    if (levels.length !== 2) throw new Error(`Official equipment level layout cannot represent ${levels.join("/")}`);
...
export async function buildEquipmentUiAssets(options: {
    dataset: StageDetailsDataset,
    sourceDir: string,
    sourceManifest: EquipmentUiSourceManifest,
    outputDir: string,
}): Promise<{ assets: EquipmentUiAssetEntry[], source: EquipmentUiSourceManifest }> {
    const sourceDir = resolve(options.sourceDir);
    const outputDir = resolve(options.outputDir);
    try { await stat(outputDir); throw new Error(`Equipment UI output must not already exist: ${outputDir}`); } catch (error) {
    ...
    for (const path of SOURCE_FILES) {
        ...
        if (!expected) throw new Error(`Official equipment UI manifest is missing ${path}`);
        const bytes = await readFile(contained(sourceDir, path));
        if (bytes.byteLength !== expected.sizeBytes || sha256(bytes) !== expected.sha256) throw new Error(`Official equipment UI source drifted: ${path}`);
    ...
    for (const reward of [
        ...
        if (equipment.infinityAssetPath) requested.add(equipment.infinityAssetPath);
        for (const condition of equipment.restriction.conditions) {
            for (const path of condition.presentation.badgeAssetPaths ?? (condition.presentation.badgeAssetPath ? [condition.presentation.badgeAssetPath] : [])) requested.add(path);
        }
    }
    const assets: EquipmentUiAssetEntry[] = [];
    ...
    for (const path of [...requested].sort((left, right) => left.localeCompare(right, "en", { numeric: true }))) {
        ...
        if (!bytes) throw new Error(`Equipment UI request is not backed by the official source: ${path}`);
    ...
    await writeFormattedJson(resolve(outputDir, "equipment-ui-assets-manifest.json"), { schemaVersion: 1, assets, source: manifest });
...
async function main(): Promise<void> {
    ...
    for (const key of ["--dataset", "--source-dir", "--source-manifest", "--output-dir"]) if (!values.get(key)) throw new Error(`Missing ${key}`);
    const dataset = JSON.parse(await readFile(resolve(values.get("--dataset")!), "utf8")) as StageDetailsDataset;
    const sourceManifest = JSON.parse(await readFile(resolve(values.get("--source-manifest")!), "utf8")) as EquipmentUiSourceManifest;
    ...
    console.log(JSON.stringify({ assetCount: result.assets.length, outputDir: resolve(values.get("--output-dir")!) }, null, 2));
}
