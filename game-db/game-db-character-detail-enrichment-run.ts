...
import {
    buildCharacterDetailEnrichment,
    CharacterDetailEnrichmentBuild,
    serializeCharacterDetailManifest,
} from "./game-db-character-detail-enrichment";
import { loadRequiredGameDbTables, REQUIRED_GAME_DB_TABLES } from "./game-db-experiment";
import { resolveGameDbSourceConfig } from "./game-db-source";
import type { StageCatalogPayload, StageDeliveryManifest } from "./game-db-stage-delivery";

export const CHARACTER_DETAIL_ENRICHMENT_CANDIDATE_ROOT = resolve(
    "game-db",
    "data",
    "character-detail-enrichment",
);

export interface CharacterDetailEnrichmentRunOptions {
...
export function parseCharacterDetailEnrichmentArgs(args: string[]): CharacterDetailEnrichmentRunOptions {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!supported.has(key)) throw new Error(`Unexpected Character detail enrichment argument: ${token}`);
        const value = separator >= 0 ? token.slice(separator + 1) : args[++index];
        if (!value || values.has(key)) throw new Error(`Missing or duplicate Character detail enrichment argument: ${key}`);
    ...
    for (const key of required) {
        if (!values.has(key)) throw new Error(`Missing Character detail enrichment argument: ${key}`);
    }
    ...
    return {
        firstPartyDir: resolve(values.get("--first-party-dir")!),
        primaryManifestPath: resolve(values.get("--primary-manifest")!),
        primaryPayloadPath: resolve(values.get("--primary-payload")!),
        stageManifestPath: resolve(values.get("--stage-manifest")!),
        stageCatalogPath: resolve(values.get("--stage-catalog")!),
        awakeningManifestPath: resolve(values.get("--awakening-manifest")!),
        awakeningPayloadPath: resolve(values.get("--awakening-payload")!),
...
export async function buildCharacterDetailEnrichmentCandidate(
    options: CharacterDetailEnrichmentRunOptions,
): Promise<{
    manifestPath: string,
    reportPath: string,
    build: CharacterDetailEnrichmentBuild,
}> {
    await assertFreshOutput(options.outputDir);
    const metadataBytes = await readFile(resolve(options.firstPartyDir, "metadata.json"));
    ...
    const tables = await loadRequiredGameDbTables(resolveGameDbSourceConfig(options.firstPartyDir));
    ...
    const build = buildCharacterDetailEnrichment({
    ...
    for (const shard of build.shards) await writeObject(options.outputDir, shard.manifest.objectKey, shard.gzip);
    const manifestPath = resolve(options.outputDir, "character-details-manifest.json");
    await writeFile(manifestPath, serializeCharacterDetailManifest(build.manifest), { flag: "wx" });
    const reportPath = resolve(options.outputDir, "character-detail-enrichment-report.json");
...
async function readPrimaryDataset(
    manifestPath: string,
    payloadPath: string,
): Promise<{ manifest: DatasetManifest, characters: Character[] }> {
    ...
    if (raw.byteLength !== manifest.uncompressedSizeBytes) throw new Error("Character detail primary raw size mismatch");
...
async function readStageCatalog(
    manifestPath: string,
    payloadPath: string,
): Promise<{ manifest: StageDeliveryManifest, catalog: StageCatalogPayload, payloadSha256: string }> {
    const manifest = JSON.parse(await readFile(manifestPath, "utf8")) as StageDeliveryManifest;
    const payload = await readFile(payloadPath);
    ...
    const raw = gunzipSync(payload);
    if (raw.byteLength !== manifest.catalog.expandedSizeBytes) throw new Error("Character detail Stage catalog raw size mismatch");
    return { manifest, catalog: JSON.parse(raw.toString("utf8")) as StageCatalogPayload, payloadSha256 };
}

async function readAwakeningCatalog(
    manifestPath: string,
    payloadPath: string,
): Promise<{ manifest: AwakeningMedalManifest, catalog: AwakeningMedalCatalog, payloadSha256: string }> {
    const manifest = JSON.parse(await readFile(manifestPath, "utf8")) as AwakeningMedalManifest;
    const payload = await readFile(payloadPath);
    const payloadSha256 = sha(payload);
    ...
    const raw = gunzipSync(payload);
    if (raw.byteLength !== manifest.payload.expandedSizeBytes) throw new Error("Character detail Awakening catalog raw size mismatch");
    return { manifest, catalog: JSON.parse(raw.toString("utf8")) as AwakeningMedalCatalog, payloadSha256 };
}

async function inventoryFirstPartyTables(firstPartyDir: string): Promise<SourceInventoryEntry[]> {
    const dataDir = resolve(firstPartyDir, "data");
    return Promise.all([...REQUIRED_GAME_DB_TABLES].sort().map(async table => {
        const bytes = await readFile(resolve(dataDir, `${table}.csv`));
...
async function assertFreshOutput(outputDir: string): Promise<void> {
    await mkdir(CHARACTER_DETAIL_ENRICHMENT_CANDIDATE_ROOT, { recursive: true });
    const root = await realpath(CHARACTER_DETAIL_ENRICHMENT_CANDIDATE_ROOT);
...
async function writeObject(outputDir: string, objectKey: string, bytes: Buffer): Promise<void> {
    ...
    if (!isStrictlyContained(outputDir, path)) throw new Error("Character detail candidate object escaped output root");
...
async function validateWrittenCandidate(
    ...
    for (const object of objects) {
        const path = resolve(outputDir, ...object.descriptor.objectKey.split("/"));
        const written = await readFile(path);
        if (!written.equals(object.bytes) || written.byteLength !== object.descriptor.sizeBytes
            || sha(written) !== object.descriptor.sha256
            || gunzipSync(written).byteLength !== object.descriptor.expandedSizeBytes) {
...
async function main(): Promise<void> {
    const result = await buildCharacterDetailEnrichmentCandidate(
        parseCharacterDetailEnrichmentArgs(process.argv.slice(2)),
    );
