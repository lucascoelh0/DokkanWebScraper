...
interface PortraitSpecLike {
    iconId: number,
}

interface AwakeningRouteCardLike {
    id: string,
    portraitSpec: PortraitSpecLike,
}

interface CharacterLike {
    id: string,
    portraitSpec?: PortraitSpecLike,
    portraitLayers?: { thumbURL?: string },
    transformations?: CharacterLike[],
}
...
interface HistoricalCaptureReport {
    source: {
        installedGame: {
            contract: string,
            packageName: string,
        ...
        cpkInventory: { entries: InventoryEntry[] },
        extractedLayerInventory: { entries: InventoryEntry[] },
    },
    portraitLayers: { entries: Array<InventoryEntry & { kind: string, objectKey: string, localPath: string }> },
}

export interface AwakeningRouteFallbackScope {
    routeFormCount: number,
    charactersResolvedFormCount: number,
    fallbackFormCount: number,
    requiredThumbIds: string[],
    fallbackFormsByThumbId: Map<string, number>,
}

export interface HistoricalThumb {
    iconId: string,
    localPath: string,
    sizeBytes: number,
    sha256: string,
...
export interface DeviceProbe {
    ...
    versionName?: string,
    versionCode?: string,
    baseApk?: { path: string, sizeBytes: number, sha256: string },
    assetRoot: string,
    readable: boolean,
    blockedReason?: string,
}

export interface AwakeningRoutePortraitAssetReport {
    schemaVersion: 1,
    contract: "dokkan-awakening-route-portrait-assets",
    contractVersion: "1.0.0",
    status: "complete" | "incomplete-source-inaccessible" | "incomplete-source-missing",
    sourceSnapshotVersion: string,
    sourceDatabaseSha256: string,
    counts: {
        routeForms: number,
        charactersResolvedForms: number,
        fallbackForms: number,
        requiredThumbIds: number,
        foundThumbIds: number,
        extractedThumbIds: number,
        missingThumbIds: number,
        fallbackFormsCovered: number,
        fallbackFormsMissing: number,
    },
    requiredThumbIds: string[],
    foundThumbIds: string[],
    missingThumbIds: string[],
    source: {
        historicalCapture: {
            reportSha256: string,
            packageName: string,
...
function assertId(value: unknown, label: string): string {
    ...
    if (!/^\d+$/.test(id)) throw new Error(`${label} is not a decimal ID`);
    return id;
}

function flattenCharacters(characters: CharacterLike[]): CharacterLike[] {
    return characters.flatMap(character => [character, ...(character.transformations ?? [])]);
}

export function deriveAwakeningRouteFallbackScope(
    routeCards: AwakeningRouteCardLike[],
    characters: CharacterLike[],
): AwakeningRouteFallbackScope {
    const characterIds = new Set(flattenCharacters(characters).map(character => assertId(character.id, "Character ID")));
    const fallbackFormsByThumbId = new Map<string, number>();
    let resolved = 0;
    const seenCardIds = new Set<string>();
    for (const card of routeCards) {
        const cardId = assertId(card.id, "Awakening Route card ID");
        if (seenCardIds.has(cardId)) throw new Error(`Duplicate Awakening Route card ${cardId}`);
        ...
        if (!iconId.endsWith("0")) throw new Error(`Awakening Route card ${cardId} has an unnormalized icon ID`);
...
export function buildHistoricalCaptureIndex(
    report: HistoricalCaptureReport,
    captureCharacters: CharacterLike[],
    captureRoot: string,
): Map<string, HistoricalThumb> {
    ...
    const cpkIds = new Set(report.source.cpkInventory.entries.map(entry =>
        /^thumbs\/card_(\d+)_thumb\.cpk$/.exec(entry.path)?.[1]).filter((id): id is string => Boolean(id)));
    const extractedIds = new Set(report.source.extractedLayerInventory.entries.map(entry =>
        /^thumb\/card_(\d+)_thumb\.png$/.exec(entry.path)?.[1]).filter((id): id is string => Boolean(id)));
    const layerByObjectKey = new Map(report.portraitLayers.entries
        .filter(entry => entry.kind === "thumb")
        .map(entry => [entry.objectKey, entry]));
    const result = new Map<string, HistoricalThumb>();
    for (const character of flattenCharacters(captureCharacters)) {
        ...
        if (previous && previous.sha256 !== layer.sha256) throw new Error(`Historical thumb ${iconId} is ambiguous`);
        result.set(iconId, {
            iconId,
...
function run(executable: string, args: string[], maxBuffer = 16 * 1024 * 1024): { status: number, stdout: string, stderr: string } {
    const result = spawnSync(executable, args, { encoding: "utf8", maxBuffer });
    if (result.error) throw result.error;
    return { status: result.status ?? -1, stdout: result.stdout ?? "", stderr: result.stderr ?? "" };
}

export function probeOfficialDevice(adbPath: string, serial: string): DeviceProbe {
    if (!serial.trim()) throw new Error("An explicit ADB serial is required");
    ...
    const shell = (...args: string[]) => run(adbPath, ["-s", serial, "shell", ...args]);
    ...
    let baseApk: DeviceProbe["baseApk"];
    if (apkPath) {
...
async function readJsonPayload(path: string): Promise<any> {
...
async function validateExtractor(path: string): Promise<void> {
    const directory = dirname(path);
    const [extractor, reader, definitions] = await Promise.all([
        readFile(path),
        readFile(resolve(directory, "CriFsV2Lib.dll")),
        readFile(resolve(directory, "CriFsV2Lib.Definitions.dll")),
    ]);
...
async function acquireDeviceThumbs(options: {
    ...
    iconIds: string[],
    outputDir: string,
}): Promise<{ entries: InventoryEntry[], absentIds: string[] }> {
    if (!options.probe.readable) return { entries: [], absentIds: [] };
    ...
    for (const iconId of options.iconIds) {
        ...
        await copyFile(extractedPath, destination, 1);
...
export async function buildAwakeningRoutePortraitAssets(options: {
    awakeningPayloadPath: string,
    charactersPayloadPath: string,
    captureReportPath: string,
    captureCharactersPayloadPath: string,
    captureRoot: string,
    outputDir: string,
    sourceSnapshotVersion: string,
    sourceDatabaseSha256: string,
...
}): Promise<AwakeningRoutePortraitAssetReport> {
    ...
    const [awakeningBytes, awakening, characters, captureReportBytes, captureReport, captureCharacters] = await Promise.all([
        readFile(options.awakeningPayloadPath),
        readJsonPayload(options.awakeningPayloadPath),
        readJsonPayload(options.charactersPayloadPath),
        readFile(options.captureReportPath),
        readJsonPayload(options.captureReportPath) as Promise<HistoricalCaptureReport>,
        readJsonPayload(options.captureCharactersPayloadPath) as Promise<CharacterLike[]>,
    ]);
    ...
    await writeFile(contained(options.outputDir, ".gitignore"), "*\n", { flag: "wx" });
    ...
    for (const iconId of historicalIds) {
        ...
        const destination = contained(options.outputDir, ...path.split("/"));
        await mkdir(dirname(destination), { recursive: true });
        await copyFile(source.localPath, destination, 1);
        inventory.push({ path, sizeBytes: bytes.byteLength, sha256: source.sha256 });
    }
    let deviceEntries: InventoryEntry[] = [];
    let deviceAbsentIds: string[] = [];
    ...
    const report: AwakeningRoutePortraitAssetReport = {
        schemaVersion: 1,
        contract: "dokkan-awakening-route-portrait-assets",
        contractVersion: "1.0.0",
    ...
    await writeFile(contained(options.outputDir, "awakening-route-portrait-assets-report.json"), `${JSON.stringify(report, null, 2)}\n`, { flag: "wx" });
    await writeFile(contained(options.outputDir, "awakening-route-payload.sha256"), `${sha256(awakeningBytes)}  ${basename(options.awakeningPayloadPath)}\n`, { flag: "wx" });
    return report;
}
...
function parseArgs(args: string[]): CliOptions {
    ...
    for (let index = 0; index < args.length; index += 1) {
        ...
        if (!value || values[key]) throw new Error(`Missing or duplicate --${key}`);
        values[key] = value;
    }
    return values;
}
...
async function main(): Promise<void> {
    ...
    const report = await buildAwakeningRoutePortraitAssets({
        awakeningPayloadPath: resolve(args["awakening-payload"]),
        charactersPayloadPath: resolve(args["characters-payload"]),
        captureReportPath: resolve(args["capture-report"]),
        captureCharactersPayloadPath: resolve(args["capture-characters-payload"]),
        captureRoot: resolve(args["capture-root"]),
        ...
        ...(args["cpk-extractor"] ? { cpkExtractorPath: resolve(args["cpk-extractor"]) } : {}),
    });
    console.log(JSON.stringify({ outputDir: resolve(args["output-dir"]), status: report.status, counts: report.counts }, null, 2));
