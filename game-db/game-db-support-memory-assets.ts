...
import { supportMemoryAssetObjectKey, supportMemoryEnhancementItemKey } from "../support-memory-dokkaninfo-enrichment";
import { GameDbRow, normalizeDbId } from "./game-db-source";

export const SUPPORT_MEMORY_GAME_ASSET_CONTRACT = "dokkan-support-memory-game-assets";
export const SUPPORT_MEMORY_GAME_ASSET_CONTRACT_VERSION = "1.0.0";

export interface SupportMemoryGameAssetSourceIdentity {
...
export interface SupportMemoryGameAssetTables {
...
export interface SupportMemoryGameAssetInventoryEntry {
    path: string,
    sizeBytes: number,
    sha256: string,
}

export interface SupportMemoryGameAssetAudit {
    schemaVersion: 1,
    contract: typeof SUPPORT_MEMORY_GAME_ASSET_CONTRACT,
    contractVersion: typeof SUPPORT_MEMORY_GAME_ASSET_CONTRACT_VERSION,
    generatedAt: string,
    source: SupportMemoryGameAssetSourceIdentity & {
        bundleRoot: string,
        archiveInventory: SupportMemoryGameAssetInventoryEntry[],
        extractedInputInventory: SupportMemoryGameAssetInventoryEntry[],
    },
    output: {
        root: string,
        memoryCount: number,
        ...
        files: SupportMemoryGameAssetInventoryEntry[],
    },
    joins: {
        animationByScriptNameCount: number,
        nonIdentityAnimationIds: Array<{ memoryId: string, scriptAssetId: string }>,
        enhancementItemCount: number,
        filmCount: number,
    },
}

export interface SupportMemoryGameAssetBuildResult {
    presentations: Map<string, SupportMemoryDokkanInfoPresentation>,
    audit: SupportMemoryGameAssetAudit,
}
...
const FILM_CODES: Record<string, string> = {
    "1": "original",
    "2": "z",
    "3": "gt",
    "4": "super",
...
function id(row: GameDbRow, column = "id"): string {
    ...
    if (!value) throw new Error(`Support Memory game-asset row is missing ${column}`);
    return value;
}
...
function integer(row: GameDbRow, column: string): number {
    ...
    if (value === undefined) throw new Error(`Support Memory game-asset row ${id(row)} is missing integer ${column}`);
    return value;
}
...
function normalizedProjectPath(projectRoot: string, absolutePath: string): string {
...
async function inventoryFiles(root: string): Promise<SupportMemoryGameAssetInventoryEntry[]> {
    const files: string[] = [];
    async function walk(directory: string): Promise<void> {
        for (const entry of await readdir(directory, { withFileTypes: true })) {
            const path = contained(directory, entry.name);
            if (entry.isSymbolicLink()) throw new Error(`Support Memory game-asset source contains a symlink: ${path}`);
            if (entry.isDirectory()) await walk(path);
            else if (entry.isFile()) files.push(path);
            else throw new Error(`Unsupported Support Memory game-asset source entry: ${path}`);
        }
    }
    await walk(root);
    const inventory = await Promise.all(files.map(async path => {
        const buffer = await readFile(path);
        return { path: relative(root, path).replace(/\\/g, "/"), sizeBytes: buffer.byteLength, sha256: sha256(buffer) };
    }));
    return inventory.sort((left, right) => left.path.localeCompare(right.path, "en", { numeric: true }));
}

function assetRef(asset: PlannedAsset): SupportMemoryDokkanInfoAssetRef {
    return { remoteUrl: "", localPath: asset.localPath, objectKey: asset.objectKey };
}

function animationAssetId(row: GameDbRow): string {
    const scriptName = row.script_name?.trim();
    const match = /^sm(\d+)$/.exec(scriptName ?? "");
    if (!match) throw new Error(`Support Memory ${id(row)} has unsupported script_name ${scriptName ?? ""}`);
    return match[1];
}

export async function buildSupportMemoryGameAssets(options: {
    generatedAt: string,
    sourceIdentity: SupportMemoryGameAssetSourceIdentity,
    sourceBundleRoot: string,
    outputRoot: string,
    projectRoot: string,
    tables: SupportMemoryGameAssetTables,
}): Promise<SupportMemoryGameAssetBuildResult> {
    if (options.sourceIdentity.packageName !== "com.bandainamcogames.dbzdokkanww") throw new Error("Unsupported Support Memory asset package");
    if (!options.sourceIdentity.versionName.trim() || !/^\d+$/.test(options.sourceIdentity.versionCode)) throw new Error("Invalid Support Memory asset package version");
    if (!/^\d+$/.test(options.sourceIdentity.databaseSnapshotVersion)) throw new Error("Invalid Support Memory asset database snapshot version");
    if (!/^\d{4}-\d{2}-\d{2}T/.test(options.sourceIdentity.acquiredAt)) throw new Error("Invalid Support Memory asset acquisition timestamp");
    ...
    const plannedByObjectKey = new Map<string, PlannedAsset>();

    async function plan(sourceSegments: string[], destinationSegments: string[]): Promise<PlannedAsset> {
        const sourcePath = contained(sourceRoot, ...sourceSegments);
        const sourceStats = await stat(sourcePath).catch(() => undefined);
        if (!sourceStats?.isFile()) throw new Error(`Missing Support Memory official asset: ${sourceSegments.join("/")}`);
        ...
        if (!objectKey) throw new Error(`Invalid Support Memory official asset path: ${localPath}`);
        const existing = plannedByObjectKey.get(objectKey);
        if (existing) {
            if (existing.sourcePath !== sourcePath) throw new Error(`Conflicting Support Memory official asset object key: ${objectKey}`);
            return existing;
        }
        const asset = { sourcePath, destinationPath, localPath, objectKey, buffer: await readFile(sourcePath) };
        plans.push(asset);
        plannedByObjectKey.set(objectKey, asset);
        return asset;
    }

    const presentations = new Map<string, SupportMemoryDokkanInfoPresentation>();
    const nonIdentityAnimationIds: Array<{ memoryId: string, scriptAssetId: string }> = [];
    for (const root of roots) {
        const memoryId = id(root);
        const filmId = id(root, "support_film_id");
        if (!filmRows.has(filmId)) throw new Error(`Support Memory ${memoryId} references missing film ${filmId}`);
        const filmCode = FILM_CODES[filmId];
        if (!filmCode) throw new Error(`Support Memory ${memoryId} has unsupported film ${filmId}`);
        const large = await plan(
            ["extracted", "support_memory", "large", `support_memory_large_${memoryId}.png`],
            [memoryId, "large.png"],
        );
        const complete = await plan(
            ["extracted", "support_memory", "large", `support_memory_large_sepia_${memoryId}.png`],
            [memoryId, "complete.png"],
        );
        const film = await plan(
            ["extracted", "support_memory", "film_icon", `support_memory_film_${filmCode}.png`],
            [memoryId, "film.png"],
        );
        ...
        const enhancementItems: SupportMemoryDokkanInfoEnhancementItem[] = [];
        for (const variantId of variantIds) {
            ...
            for (const requirement of requirements) {
                ...
                if (!enhancementRows.has(itemId)) throw new Error(`Support Memory ${memoryId} references missing enhancement item ${itemId}`);
                const itemAsset = await plan(
                    ["extracted", "support_memory_enhancement", itemId, `${itemId}.png`],
                    [memoryId, "enhancements", `${itemId}.png`],
                );
                enhancementItems.push({
                    itemType: "SupportMemoryEnhancementItem",
                    itemKey: supportMemoryEnhancementItemKey(itemId),
                    id: itemId,
        ...
        if (scriptAssetId !== memoryId) nonIdentityAnimationIds.push({ memoryId, scriptAssetId });
        const animationSource = contained(sourceRoot, "extracted", "animations", scriptAssetId, "en");
        const animationMembers = (await readdir(animationSource, { withFileTypes: true }))
            .filter(entry => entry.isFile())
            .map(entry => entry.name)
            .sort((left, right) => left.localeCompare(right, "en", { numeric: true }));
        if (animationMembers.some(name => !/\.(?:lwf|png)$/i.test(name))) throw new Error(`Unsupported Support Memory animation member for ${memoryId}`);
        ...
        const animationAssets = new Map<string, PlannedAsset>();
        for (const member of animationMembers) {
            animationAssets.set(member, await plan(
                ["extracted", "animations", scriptAssetId, "en", member],
                [memoryId, "animation", member],
            ));
        }
        const animation: SupportMemoryDokkanInfoAnimationAssetSet = {
            sourceType: "lwf",
            status: "mirrored",
            remoteBaseUrl: "",
    ...
    if (archiveInventory.length !== expectedArchiveCount) throw new Error(`Expected ${expectedArchiveCount} official Support Memory CPKs, found ${archiveInventory.length}`);
    ...
    if (JSON.stringify(actualArchives) !== JSON.stringify(expectedArchives)) throw new Error("Official Support Memory CPK inventory does not match the database roots");
    ...
    const files: SupportMemoryGameAssetInventoryEntry[] = plans.map(asset => ({
        path: relative(outputRoot, asset.destinationPath).replace(/\\/g, "/"),
    ...
    const audit: SupportMemoryGameAssetAudit = {
        schemaVersion: 1,
        contract: SUPPORT_MEMORY_GAME_ASSET_CONTRACT,
        contractVersion: SUPPORT_MEMORY_GAME_ASSET_CONTRACT_VERSION,
    ...
    await writeFile(contained(outputRoot, "support-memory-game-assets.json"), `${JSON.stringify(audit, null, 2)}\n`, "utf8");
