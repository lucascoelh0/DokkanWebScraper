...
export const SA_TRAINING_CONTRACT = "dokkan-sa-training";
export const SA_TRAINING_VERSION = "1.0.0";
export const SA_RULES_VERSION = "reference-5.31-v1";
export const SA_MAX_EXPANDED_BYTES = 8 * 1024 * 1024;
export const SA_MAX_COMPRESSED_BYTES = 1024 * 1024;
export const SA_MAX_MANIFEST_BYTES = 64 * 1024;
...
export const SA_RATE_MATRIX: ReadonlyArray<ReadonlyArray<number | null>> = [
...
export interface SaCard {
    id: string; name: string; nameIdentityId: string; rarityRaw: number;
    pathId: string; supportedTarget: boolean;
}
export interface SaPath {
    id: string; cardIds: string[]; nameIdentityIds: string[]; baseRarityRaw: number;
}
export interface SaMaterial {
    sourceCardId: string; formCardIds: string[]; stageIds: string[]; missionIds: string[];
}
export interface SaSourceBinding { datasetVersion: string; payloadSha256: string }
export interface SaBindings {
    primaryCharacters: SaSourceBinding; stages: SaSourceBinding; awakeningGraph: SaSourceBinding;
    firstPartyTableInventorySha256: string;
}
export interface SaCoverage {
    officialCardCount: number; rewardCardCount: number; materialSourceCount: number;
    preparedFormCount: number; supportedTargetCount: number; quarantinedCardCount: number;
    excludedRewardCount: number;
}
export interface SaPayload {
    schemaVersion: 1; contract: string; contractVersion: string; datasetVersion: string;
    generatedAt: string; sourceSnapshotVersion: string; sourceDatabaseSha256: string;
    sourceBindings: SaBindings;
    rules: { version: string; rateMatrix: (number | null)[][]; profileSelection: string;
        evidence: { type: string; ref: string }[] };
    cards: SaCard[]; paths: SaPath[]; materials: SaMaterial[];
    quarantines: { cardIds: string[]; reasons: string[] }[];
    coverage: SaCoverage;
}
export interface SaManifest {
    schemaVersion: 1; contract: string; contractVersion: string; datasetVersion: string;
    generatedAt: string; sourceSnapshotVersion: string; sourceDatabaseSha256: string;
    sourceBindings: SaBindings; rulesVersion: string; coverage: SaCoverage;
    catalog: { objectKey: string; sha256: string; sizeBytes: number; expandedSizeBytes: number;
        contentType: "application/json"; contentEncoding: "gzip" };
}
export interface SaBuildOptions {
    generatedAt: string; sourceSnapshotVersion: string; sourceDatabaseSha256: string;
    firstPartyTableInventorySha256: string; tables: Record<string, GameDbRow[]>;
    primaryCharacters: { id: string | number }[];
    primaryManifest: { datasetVersion: string; sha256: string; characterCount: number };
    stageCatalog: any; stageManifest: any; stagePayloadSha256: string;
    awakeningCatalog: any; awakeningManifest: any; awakeningPayloadSha256: string;
}
export type SaEvaluation =
    | { kind: "same-path"; chancePercent: 100; gain: "transfer" }
    | { kind: "compatible"; chancePercent: number | null; gain: "one-level" }
    | { kind: "incompatible" | "insufficient-evidence" };
const digest = (b: Buffer) => createHash("sha256").update(b).digest("hex");
const sorted = (values: Iterable<string>) => [...new Set(values)].sort();
const isId = (v: unknown): v is string => typeof v === "string" && /^[1-9]\d*$/.test(v);
const rarity = (v: unknown) => Number.isInteger(v) && Number(v) >= 0 && Number(v) <= 5;
function check(ok: unknown, message: string): asserts ok { if (!ok) throw new Error(`SA index: ${message}`); }
const hash = (v: unknown) => typeof v === "string" && /^[a-f0-9]{64}$/.test(v);
const unique = (values: string[]) => new Set(values).size === values.length;

/** Structural exclusion heuristic, deliberately versioned/audited instead of called an official enum. */
function specialReason(row: GameDbRow): string | undefined {
    if (row.is_selling_only === "1") return "selling-only";
    if (row.cost === "99" && row.exp_type === "5" && row.training_exp === "10") return "training-signature-v1";
    if (row.collectable_type !== "1" || row.is_selling_only !== "0") return "unsupported-collectability";
    return undefined;
}

export function buildSaTrainingIndex(o: SaBuildOptions) {
    ...
    const raw = new Map<string, GameDbRow>();
    for (const r of o.tables.cards ?? []) {
        check(isId(r.id) && !raw.has(r.id), "duplicate/invalid official card"); raw.set(r.id, r);
    }
    ...
    const sources = new Map<string, { stages: Set<string>; missions: Set<string> }>();
    function source(id: string) {
        check(isId(id), "invalid reward card ID");
        if (!sources.has(id)) sources.set(id, { stages: new Set(), missions: new Set() });
    ...
    for (const mission of o.stageCatalog.eventMissions) {
        ...
        for (const reward of mission.rewards) if (reward.itemType === "Card") source(reward.itemId).missions.add(mission.id);
    }
    ...
    const adjacency = new Map<string, Set<string>>(), errors = new Map<string, Set<string>>();
    const edge = (map: Map<string, Set<string>>, a: string, b: string) => {
        if (!map.has(a)) map.set(a, new Set()); map.get(a)!.add(b);
    };
    const bad = (id: string, reason: string) => { if (!errors.has(id)) errors.set(id, new Set()); errors.get(id)!.add(reason); };
    const routeIds = new Map<string, string>();
    for (const r of o.tables.card_awakening_routes) {
        if (r.type === "CardAwakeningRoute::Optimal") continue;
        if (r.type !== "CardAwakeningRoute::Zet" && r.type !== "CardAwakeningRoute::Dokkan") {
            if (raw.has(r.card_id)) bad(r.card_id, "unsupported-route-type");
            if (raw.has(r.awaked_card_id)) bad(r.awaked_card_id, "unsupported-route-type");
            continue;
        }
        const a = r.card_id, b = r.awaked_card_id;
        if (!isId(a) || !isId(b)) { if (raw.has(a)) bad(a, "invalid-endpoint"); if (raw.has(b)) bad(b, "invalid-endpoint"); continue; }
        edge(adjacency, a, b); edge(adjacency, b, a);
        edge(outgoing, a, b); edge(incoming, b, a);
        const signature = `${a}:${b}:${r.type}`;
        if (routeIds.has(r.id) && routeIds.get(r.id) !== signature) {
            bad(a, "conflicting-route-id"); bad(b, "conflicting-route-id");
            const previous = routeIds.get(r.id)!.split(":"); bad(previous[0], "conflicting-route-id"); bad(previous[1], "conflicting-route-id");
        }
        routeIds.set(r.id, signature);
        if (a === b) bad(a, "ordinary-self-edge");
        if (!raw.has(a) || !raw.has(b)) { bad(a, "dangling-endpoint"); bad(b, "dangling-endpoint"); }
    ...
    const cards: SaCard[] = [], paths: SaPath[] = [], quarantines: SaPayload["quarantines"] = [];
    const visited = new Set<string>();
    for (const start of sorted(raw.keys())) {
        if (visited.has(start)) continue;
        const pending = [start], component: string[] = [], reasons = new Set<string>();
        while (pending.length) {
            const id = pending.pop()!;
            if (visited.has(id)) continue;
            visited.add(id); component.push(id);
            for (const next of adjacency.get(id) ?? []) pending.push(next);
            for (const reason of errors.get(id) ?? []) reasons.add(reason);
            if ((incoming.get(id)?.size ?? 0) > 1 || (outgoing.get(id)?.size ?? 0) > 1) reasons.add("branch-or-merge");
            const row = raw.get(id);
            if (!row || !rarity(Number(row.rarity)) || !isId(row.card_unique_info_id) || !names.has(row.card_unique_info_id)
                || !row.name || specialReason(row)) reasons.add("unsupported-card-metadata");
        }
        if (!component.some(id => supported.has(id))) continue;
        const roots = component.filter(id => !(incoming.get(id)?.size));
        if (roots.length !== 1) reasons.add("cycle-or-invalid-root");
        if (reasons.size) { quarantines.push({ cardIds: sorted(component), reasons: sorted(reasons) }); continue; }
        const ordered: string[] = [], pathVisited = new Set<string>();
        let current: string | undefined = roots[0];
        while (current && !pathVisited.has(current)) {
            pathVisited.add(current); ordered.push(current); current = [...(outgoing.get(current) ?? [])][0];
        }
        if (ordered.length !== component.length || current) {
            quarantines.push({ cardIds: sorted(component), reasons: ["cycle-or-disconnected"] }); continue;
        }
        const id = ordered[0];
    ...
    cards.sort((a, b) => a.id.localeCompare(b.id)); paths.sort((a, b) => a.id.localeCompare(b.id));
    ...
    const materials: SaMaterial[] = [], excludedRewards: { cardId: string; reason: string }[] = [];
    for (const id of sorted(sources.keys())) {
        const row = raw.get(id), card = cardMap.get(id);
        if (!row || !card) { excludedRewards.push({ cardId: id, reason: row ? specialReason(row) ?? "quarantined-path" : "missing-official-card" }); continue; }
    ...
    const sourceBindings: SaBindings = {
    ...
    const payload: SaPayload = {
        schemaVersion: 1, contract: SA_TRAINING_CONTRACT, contractVersion: SA_TRAINING_VERSION,
        ...
        rules: { version: SA_RULES_VERSION, rateMatrix: SA_RATE_MATRIX.map(row => [...row]),
    ...
    const bytes = Buffer.from(JSON.stringify(payload)), catalogGzip = gzipSync(bytes, { level: 9 });
    check(bytes.length <= SA_MAX_EXPANDED_BYTES && catalogGzip.length <= SA_MAX_COMPRESSED_BYTES, "payload bounds exceeded");
    const sha256 = digest(catalogGzip);
    const manifest: SaManifest = {
        ...
        sourceBindings, rulesVersion: SA_RULES_VERSION, coverage,
...
export function validateSaTrainingIndex(p: SaPayload, m?: SaManifest): void {
    check(p?.schemaVersion === 1 && p.contract === SA_TRAINING_CONTRACT && p.contractVersion === SA_TRAINING_VERSION, "unsupported contract");
    check(p.rules?.version === SA_RULES_VERSION && p.rules.profileSelection === "ordinary-linear-path-initial-rarity-reference-v1"
        && JSON.stringify(p.rules.rateMatrix) === JSON.stringify(SA_RATE_MATRIX), "unsupported or modified rules");
    ...
    for (const key of ["primaryCharacters", "stages", "awakeningGraph"] as const) {
    ...
    const cards = new Map<string, SaCard>(), paths = new Map<string, SaPath>();
    ...
    for (const material of p.materials) {
        ...
        check(path && !sourceIds.has(material.sourceCardId), "invalid source"); sourceIds.add(material.sourceCardId);
    ...
    for (const q of p.quarantines) {
        ...
        for (const id of q.cardIds) { check(!cards.has(id) && !quarantined.has(id), "quarantine overlap"); quarantined.add(id); }
    }
    ...
    const bytes = Buffer.from(JSON.stringify(p)); check(bytes.length <= SA_MAX_EXPANDED_BYTES, "expanded bound");
    if (m) {
        check(Buffer.byteLength(JSON.stringify(m)) <= SA_MAX_MANIFEST_BYTES && m.contract === p.contract
        ...
        check(m.catalog.contentEncoding === "gzip" && m.catalog.contentType === "application/json"
            ...
            && m.catalog.sizeBytes > 0 && m.catalog.sizeBytes <= SA_MAX_COMPRESSED_BYTES && hash(m.catalog.sha256)
...
export function createSaTrainingEvaluator(index: SaPayload) {
    ...
    const evaluate = (targetId: string, materialId: string): SaEvaluation => {
        const target = cards.get(targetId), material = cards.get(materialId);
        if (!target?.supportedTarget || !material) return { kind: "insufficient-evidence" };
        if (target.pathId === material.pathId) return { kind: "same-path", chancePercent: 100, gain: "transfer" };
        const a = paths.get(target.pathId)!, b = paths.get(material.pathId)!;
        if (!a.nameIdentityIds.some(id => names.get(b.id)!.has(id))) return { kind: "incompatible" };
        return { kind: "compatible", chancePercent: index.rules.rateMatrix[a.baseRarityRaw][material.rarityRaw], gain: "one-level" };
    };
    return {
        evaluate,
        findMaterials(targetId: string) {
...
export const evaluateSaTraining = (p: SaPayload, target: string, material: string) => createSaTrainingEvaluator(p).evaluate(target, material);
export const findSaTrainingMaterials = (p: SaPayload, target: string) => createSaTrainingEvaluator(p).findMaterials(target);
