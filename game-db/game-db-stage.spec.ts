...
describe("Stage first-party candidate", () => {
    ...
    it("projects drop previews only onto maps whose difficulty is listed", () => {
        ...
        deepEqual(candidate.dataset.entries.map(stage => ({
            id: stage.id,
    ...
    it("projects official treasure identity for boss drops", () => {
        ...
        equal(candidate.dataset.entries[0].bossDrops?.[0].description, "An official Treasure description.");
        equal(candidate.dataset.entries[0].bossDrops?.[0].rarityRaw, 2);
        equal(
            candidate.dataset.entries[0].bossDrops?.[0].iconAssetPath,
            "item/other/en/thumb/thumb_trade_jewel_00013/thumb_trade_jewel_00013.png",
        );
        deepEqual(candidate.dataset.entries[0].bossDrops?.[0].treasure, {
            sellingExchangePointRaw: 1,
            willExpire: false,
        });
    });

    it("projects the same Treasure detail contract for Event Mission rewards", () => {
        const source = tables();
        ...
        equal(reward?.itemId, "77");
        equal(reward?.quantity, 25);
        equal(reward?.name, "Official Treasure");
        equal(reward?.description, "An official Treasure description.");
        equal(reward?.iconAssetPath, "item/other/en/thumb/thumb_trade_jewel_00013/thumb_trade_jewel_00013.png");
        deepEqual(reward?.treasure, { sellingExchangePointRaw: 1, willExpire: false });
    });

    it("projects all official Link Skill Orb variants for Stage 2 Reunion with Ba", () => {
        const source = tables();
        ...
        const stage = buildStageFirstPartyCandidate({
        ...
        equal(stage.id, "12030021");
        equal(stage.questName, "Reunion with Ba");
        deepEqual(stage.dropPreviews?.[0].items, [
            {
        ...
        equal(stage.bossDrops?.[0].name, "Link Skill Orb (S)");
        equal(stage.bossDrops?.[0].iconAssetPath, "item/other/en/thumb/thumb_linkskill_orb_00001/thumb_linkskill_orb_00001.png");
    });

    it("projects verified official WallpaperItem presentation without changing reward quantity", () => {
        const source = tables();
        ...
        equal(reward?.quantity, 1);
        equal(reward?.name, name);
        deepEqual(reward?.wallpaper, {
    ...
    it("fails closed for missing or ambiguous WallpaperItem joins", () => {
        const source = tables();
        source.mission_rewards.push({ id: "51015", mission_id: "10", item_id: "76", item_type: "WallpaperItem", quantity: "1" });
        const options = {
            generatedAt: "2026-09-04T00:00:00.000Z",
        ...
        throws(() => buildStageFirstPartyCandidate(options), /missing wallpaper item 76/);

        const item = { id: "76", name: "Companions on Planet Vampa", description: "Official description" };
        source.wallpaper_items = [item];
        throws(() => buildStageFirstPartyCandidate(options), /lacks verified official assets/);
