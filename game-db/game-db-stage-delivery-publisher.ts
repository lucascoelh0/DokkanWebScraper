...
import {
    ...
    STAGE_DELIVERY_CONTRACT_VERSION,
    StageCatalogPayload,
...
export const STAGE_DELIVERY_HTTP_CONTENT_TYPE = "application/gzip";
const MAX_DEFAULT_UPLOAD_BYTES = 512 * 1024 * 1024;
const UPLOAD_CONCURRENCY = 4;
const RETRY_ATTEMPTS = 4;
const execFileAsync = promisify(execFile);

export interface StageDeliveryPublishOptions {
    bucket: string,
    objectPrefix: string,
    releaseDir: string,
    manifestPath: string,
...
export interface StageDeliveryPublishState {
    schemaVersion: 2,
    status?: "in-progress" | "complete",
    destination: StageDeliveryPublishDestination,
    datasetVersion: string,
    manifestSha256: string,
    objects: Record<string, { sha256: string, sizeBytes: number }>,
}

export interface StageDeliveryPublishDestination {
    bucket: string,
    objectPrefix: string,
    target: "remote" | "local",
...
export interface ValidatedStageDeliveryFile {
    object: StageDeliveryObject,
    absolutePath: string,
}

export interface StageDeliveryPublishPlan {
    options: StageDeliveryPublishOptions,
    manifest: StageDeliveryManifest,
    manifestBytes: number,
    manifestSha256: string,
    files: ValidatedStageDeliveryFile[],
    changedFiles: ValidatedStageDeliveryFile[],
    totalDatasetBytes: number,
    uploadBytes: number,
    uploadManifest: boolean,
    previousState?: StageDeliveryPublishState,
}

export function parseStageDeliveryPublishArgs(argv: string[]): StageDeliveryPublishOptions {
    ...
    for (let index = 0; index < argv.length; index += 1) {
        ...
        if (!valueOptions.has(name)) throw new Error(`Unexpected Stage delivery publisher argument: ${token}`);
    ...
    const releaseDir = resolve(values.get("--release-dir") ?? DEFAULT_RELEASE_DIR);
    return {
        ...
        releaseDir,
        manifestPath: resolve(values.get("--manifest") ?? resolve(releaseDir, MANIFEST_FILE_NAME)),
...
export function buildStageDeliveryPublishPlan(
    options: StageDeliveryPublishOptions,
    manifest: StageDeliveryManifest,
    manifestBytes: number,
    manifestSha256: string,
    ...
    if (files.length !== expectedObjects.length) throw new Error("Stage delivery object count does not match the manifest");
...
async function main(): Promise<void> {
    ...
    const state: StageDeliveryPublishState = {
        schemaVersion: 2,
        status: "in-progress",
    ...
    await mapWithConcurrency(plan.changedFiles, UPLOAD_CONCURRENCY, async file => {
        await uploadObject(
            options.bucket,
            scopedObjectKey(options.objectPrefix, file.object.objectKey),
            file.absolutePath,
    ...
    if (plan.uploadManifest) {
        await uploadObject(
            options.bucket,
            scopedObjectKey(options.objectPrefix, MANIFEST_FILE_NAME),
...
async function readPlan(options: StageDeliveryPublishOptions): Promise<StageDeliveryPublishPlan> {
    if (!existsSync(options.manifestPath)) throw new Error(`Stage delivery manifest not found: ${options.manifestPath}`);
    const manifestBuffer = await readFile(options.manifestPath);
    const manifest = JSON.parse(manifestBuffer.toString("utf8")) as StageDeliveryManifest;
    validateManifestShape(manifest);
    ...
    return buildStageDeliveryPublishPlan(
        options,
        manifest,
...
async function validateObjectFile(
    releaseDir: string,
    object: StageDeliveryObject,
    manifest: StageDeliveryManifest,
): Promise<ValidatedStageDeliveryFile> {
    ...
    const absolutePath = resolve(releaseDir, ...object.objectKey.split("/"));
    const root = `${resolve(releaseDir)}${sep}`.toLowerCase();
    if (!absolutePath.toLowerCase().startsWith(root)) throw new Error(`Stage delivery object escapes the release directory: ${object.objectKey}`);
    ...
    if (payload.contract !== STAGE_DELIVERY_CONTRACT
        || payload.contractVersion !== STAGE_DELIVERY_CONTRACT_VERSION
    ...
    if (object.objectKey === manifest.catalog.objectKey) {
        ...
        validateShardPayload(payload as StageDetailShardPayload, shard, manifest);
    }
    return { object, absolutePath };
}

function validateManifestShape(manifest: StageDeliveryManifest): void {
    if (manifest.schemaVersion !== 2
        || manifest.contract !== STAGE_DELIVERY_CONTRACT
        || manifest.contractVersion !== STAGE_DELIVERY_CONTRACT_VERSION) {
    ...
    if (new Set(keys).size !== keys.length) throw new Error("Stage delivery manifest contains duplicate object keys");
...
function validateShardPayload(
    ...
    shard: StageDeliveryManifest["shards"][number],
    manifest: StageDeliveryManifest,
): void {
    if (payload.schemaVersion !== 1
        || payload.contract !== STAGE_DELIVERY_CONTRACT
        || payload.contractVersion !== STAGE_DELIVERY_CONTRACT_VERSION
        || payload.datasetVersion !== manifest.datasetVersion
        ...
        || !sameStringSet(
            [...new Set(payload.questLevels.map(entry => entry.areaId))],
            shard.areaIds,
        )) {
...
async function readState(
    path: string,
    options: StageDeliveryPublishOptions,
): Promise<StageDeliveryPublishState | undefined> {
    if (!existsSync(path)) return undefined;
    const parsed = JSON.parse(await readFile(path, "utf8")) as StageDeliveryPublishState | (
        Omit<StageDeliveryPublishState, "schemaVersion" | "destination"> & { schemaVersion: 1 }
    );
    ...
    const state = parsed as StageDeliveryPublishState;
...
function publishDestination(options: StageDeliveryPublishOptions): StageDeliveryPublishDestination {
    return {
...
function printPlan(plan: StageDeliveryPublishPlan): void {
...
function parsePositiveNumber(value: string | undefined, fallback: number): number {
    ...
    if (!Number.isSafeInteger(parsed) || parsed < 1) throw new Error(`Invalid positive number: ${value}`);
    return parsed;
}
