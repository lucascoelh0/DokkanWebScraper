...
import { TreasureSourceEntry } from "./game-db-treasure-catalog";

export const TREASURE_MODE_TABLES = ["budokais", "budokai_missions", "budokai_mission_rewards",
    "budokai_box_rankings", "budokai_box_ranking_reward_ranges", "budokai_box_ranking_rewards",
    "budokai_ranking_gift_sets", "budokai_ranking_gifts", "rmbattle_missions", "rmbattle_mission_rewards"] as const;
export type TreasureModeTables = Record<typeof TREASURE_MODE_TABLES[number], GameDbRow[]>;

const id = (v: string) => { if (!/^[1-9]\d*$/.test(v)) throw Error("Invalid mode ID"); return v; };
const text = (v: string) => { if (!v?.trim() || v.length > 8192) throw Error("Invalid mode text"); return v.replace(/\s*\n\s*/g, " ").trim(); };
const positive = (v: string) => { const n = Number(v); if (!/^\d+$/.test(v) || !Number.isSafeInteger(n) || n <= 0 || n > 2147483647) throw Error("Invalid reward value"); return n; };
function index(rows: GameDbRow[]) {
    const result = new Map<string, GameDbRow>();
    for (const row of rows) { if (result.has(id(row.id))) throw Error("Duplicate mode ID"); result.set(row.id, row); }
    return result;
}
function date(raw: string): string | null {
    if (!raw || raw === "2038-01-01 00:00:00") return null;
    if (!/^\d{4}-\d\d-\d\d \d\d:\d\d:\d\d$/.test(raw)) throw Error("Invalid mode date");
    const iso = raw.replace(" ", "T") + "Z", parsed = new Date(iso);
    if (!Number.isFinite(parsed.getTime()) || parsed.toISOString().slice(0,19) !== iso.slice(0,19)) throw Error("Invalid mode date");
    return parsed.toISOString();
}

/** Edition IDs are identities, never assumed to be the displayed edition number. */
export function buildTreasureModeSources(t: TreasureModeTables): TreasureSourceEntry[] {
    ...
    const result: TreasureSourceEntry[] = [];
    const get = (map: Map<string, GameDbRow>, key: string) => {
        const row = map.get(id(key)); if (!row) throw Error("Missing mode reward parent"); return row;
    };
    const add = (r: GameDbRow, prefix: string, editionId: string, title: string,
        rewardType: TreasureSourceEntry["rewardType"], missionId: string | null = null) => {
        ...
        if (startsAt && endsAt && startsAt >= endsAt) throw Error("Invalid mode interval");
    ...
    for (const r of t.budokai_box_ranking_rewards.filter(r => r.item_type === "TreasureItem")) {
        ...
        if (first > last) throw Error("Invalid local ranking interval");
