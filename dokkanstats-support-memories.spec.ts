...
import {
    buildDokkanStatsSupportMemoryDataset,
    DokkanStatsSupportMemoryEntry,
    parseDokkanStatsSupportMemoryCatalog,
    parseDokkanStatsSupportMemoryDetail,
...
function detailHtml(sources: string): string {
...
describe("DokkanStats Support Memory enrichment", () => {
    it("extracts roots and validates exact enhanced-level coverage", () => {
        const entries = parseDokkanStatsSupportMemoryCatalog(catalogHtml);
    ...
    it("extracts mission and stage-drop evidence with stable keys", () => {
        const entry = parseDokkanStatsSupportMemoryCatalog(catalogHtml).get("10001")!;
        const detail = parseDokkanStatsSupportMemoryDetail(detailHtml(JSON.stringify([
            {
        ...
        assert.equal(
            detail.acquisitionSources[0].imageUrl,
            "https://assets.dokkanstats.com/assets/global/en/images/en/mission/banner.png",
        );
    ...
    it("treats a null source payload as explicit no-evidence", () => {
        const entry = parseDokkanStatsSupportMemoryCatalog(catalogHtml).get("10001")!;
        const detail = parseDokkanStatsSupportMemoryDetail(detailHtml("null"), entry);
    ...
    it("fails closed on an unknown acquisition kind", () => {
        const entry = parseDokkanStatsSupportMemoryCatalog(catalogHtml).get("10001")!;
        assert.throws(
            () => parseDokkanStatsSupportMemoryDetail(detailHtml('[{"kind":"shop"}]'), entry),
            /unsupported acquisition source kind: shop/,
        );
    });
    ...
    it("builds deterministic counts", () => {
        ...
        const dataset = buildDokkanStatsSupportMemoryDataset([detail], "2026-09-02T00:00:00.000Z");
    ...
    it("rejects catalog/detail drift", () => {
        ...
        assert.throws(() => parseDokkanStatsSupportMemoryDetail(changed, entry), /does not match the catalog/);
    });
});
