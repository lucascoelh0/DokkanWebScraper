...
import { link, mkdtemp, readFile, rm, symlink, unlink, writeFile } from "fs/promises";
import * as ModuleApi from "module";
import { tmpdir } from "os";
import { basename, dirname, join, resolve } from "path";
import { ModuleKind, ScriptTarget, transpileModule } from "typescript";
import * as productionApi from "./taxonomy-projection-publisher-dry-run";
import {
...
import { parseTaxonomyProjectionPublisherDryRunCli } from "./taxonomy-projection-publisher-dry-run-run";
import {
...
async function loadInstrumentedApi(): Promise<InstrumentedApi> {
    ...
    const sourcePath = join(sourceRoot, "database-characters", "taxonomy-projection-publisher-dry-run.ts");
    const runtimePath = basename(parent).toLowerCase() === "lib"
        ? join(parent, "database-characters", ".taxonomy-projection-publisher-dry-run.test-runtime.js")
        : join(sourceRoot, "database-characters", ".taxonomy-projection-publisher-dry-run.test-runtime.js");
...
function makeFixture(statuses: Array<"matching" | "missing"> = ["matching", "missing", "matching", "missing"],
    manifestStatus: "matching" | "missing" | "different" = "different"): Fixture {
    ...
    const plan = {
        ...
        budget: {
            ...
            worstCaseNewBytes: immutableObjectBytes + TAXONOMY_PROJECTION_OBJECT_PLAN_MANIFEST_RESERVATION_BYTES,
...
async function runHarness(api: InstrumentedApi, temporary: string, options: HarnessOptions = {}) {
    ...
    const k37Calls: any[] = [];
    let sourceCalls = 0;
    (globalThis as any)[Symbol.for("dokkan.k39.k38-runner")] = (runOptions: any) => {
        sequence.push("k38");
        k38Calls.push(runOptions);
        return fixture.k38Result;
    };
    (globalThis as any)[Symbol.for("dokkan.k39.k37-reader")] = (readOptions: any) => {
...
describe("K39 taxonomy projection publisher dry-run-only checkpoint", () => {
    ...
    before(async () => {
        temporary = await mkdtemp(join(tmpdir(), "dokkan-k39-"));
        api = await loadInstrumentedApi();
    });
    after(async () => {
        delete (globalThis as any)[Symbol.for("dokkan.k39.k38-runner")];
        delete (globalThis as any)[Symbol.for("dokkan.k39.k37-reader")];
        await rm(temporary, { recursive: true, force: true });
    });

    it("requires every exact CLI switch and every K38 value argument with no defaults", async () => {
        const parsed = parseTaxonomyProjectionPublisherDryRunCli(cliArgs());
        equal(parsed.optInK39, true);
        equal(parsed.dryRunOnly, true);
        ...
        for (const removed of ["--opt-in-k39", "--dry-run-only", "--remote-read-only",
            ...
            await rejects(async () => parseTaxonomyProjectionPublisherDryRunCli(args), /requires exactly|missing/);
        }
        await rejects(async () => parseTaxonomyProjectionPublisherDryRunCli([...cliArgs(), "--unknown"]), /unsupported/);
        await rejects(async () => parseTaxonomyProjectionPublisherDryRunCli([...cliArgs(), "--dry-run-only"]), /exactly one/);
    });

    it("has no import side effects, injection surface, credentials, writer, or publication capability", async () => {
        deepStrictEqual(Object.keys(productionApi).sort(), ["runTaxonomyProjectionPublisherDryRun"]);
        const source = await readFile(join(repositorySourceRoot(), "database-characters",
            "taxonomy-projection-publisher-dry-run.ts"), "utf8");
        doesNotMatch(source, /export (?:interface|type|function|const).*?(?:Transport|Writer|SavedReport|BucketClient|ReportReader)/i);
        doesNotMatch(source, /(?:@aws-sdk|S3Client|PutObjectCommand|DeleteObjectCommand|UploadPartCommand|CopyObjectCommand)/);
        doesNotMatch(source, /(?:accessKeyId|secretAccessKey|credential|\bAuthorization\s*:)/i);
        doesNotMatch(source, /(?:httpsRequest|fetch\s*\(|execFile\s*\()/);
    ...
    it("productively reruns K38 first with every explicit input and requires GO", async () => {
        ...
        deepStrictEqual(run.sequence, ["k38", "k37-1", "k37-2"]);
        deepStrictEqual(run.k38Calls, [{
            optInK38: true,
            remoteReadOnly: true,
            ...roots,
        ...
        deepStrictEqual(run.k37Calls, Array(2).fill({
        ...
        const noGoRoot = await mkdtemp(join(temporary, "no-go-"));
        let sourceCalls = 0;
        (globalThis as any)[Symbol.for("dokkan.k39.k38-runner")] = () => noGo.k38Result;
        (globalThis as any)[Symbol.for("dokkan.k39.k37-reader")] = () => { sourceCalls++; return noGo.validated; };
        await rejects(() => api.runTaxonomyProjectionPublisherDryRun({
    ...
    it("emits four ordered immutable actions with exact cache, content types, and idempotency policy", async () => {
        ...
        deepStrictEqual(actions.map(action => action.kind), ["payload", "coverage", "validation", "manifest"]);
        deepStrictEqual(actions.map(action => action.order), [1, 2, 3, 4]);
        deepStrictEqual(actions.map(action => action.action), [
            "reuse_verified_remote_bytes", "create_if_absent", "reuse_verified_remote_bytes", "create_if_absent",
        ]);
        deepStrictEqual(actions.map(action => action.contentType), [
            "application/gzip", "application/json", "application/json", "application/json",
        ]);
        for (const action of actions) {
            equal(action.cacheControl, TAXONOMY_PROJECTION_IMMUTABLE_CACHE_CONTROL);
            equal(action.prospectiveCreateSemantics.ifNoneMatch, "*");
            equal(action.postCreateVerification.byteSha256AndSize, "REQUIRED");
            equal(action.postCreateVerification.contentTypeAndCacheControlMetadata, "REQUIRED");
            equal(action.overwrite, "FORBIDDEN");
            equal(action.delete, "FORBIDDEN");
        }
        equal(run.result.report.remote.mutationExecuted, false);
        equal(run.result.report.remote.overwriteCount, 0);
        equal(run.result.report.remote.deleteCount, 0);
    });
    ...
    it("puts the mutable manifest last and binds each K38 planning state to a future precondition", async () => {
        ...
        for (const status of ["matching", "missing", "different"] as const) {
            ...
            equal(manifest.action, expectations[status][0]);
            equal(manifest.futureWritePrecondition, expectations[status][1]);
            equal(manifest.contentType, "application/json");
            equal(manifest.cacheControl, "no-store");
            equal(manifest.freshDirectMetadataAndEtagVerificationRequired, true);
            equal(manifest.futureSeparatelyAuthorizedWriteGateRequired, true);
            equal(manifest.unconditionalOverwrite, "FORBIDDEN");
            equal(manifest.delete, "FORBIDDEN");
        }
    });

    it("copies exact K38 bucket/byte/projection budgets and closes all authority readiness", async () => {
        const run = await runHarness(api, temporary);
        deepStrictEqual(run.result.report.k38BucketUsage, run.fixture.k38Report.bucketUsage);
        deepStrictEqual(run.result.report.k38Budget, run.fixture.k38Report.budget);
        equal(run.result.report.source.k38.reportSha256, K38_REPORT_SHA256);
        equal(run.result.report.source.k38.productivelyRerunForThisDryRun, true);
        ...
        equal(run.result.report.checks.futurePublisherMustDirectlyVerifyMetadataAndEtag, true);
    });
    ...
    it("rejects K37 drift and K38 lineage mismatch before writing K39", async () => {
        ...
        await rejects(() => runHarness(api, temporary, { fixture, afterSource: drifted }), /source drifted/);

        const mismatch = makeFixture();
        mismatch.k38Report.source.k37PlanSha256 = "e".repeat(64);
        await rejects(() => runHarness(api, temporary, { fixture: mismatch }), /lineage does not match/);

        const incompleteK38 = makeFixture();
        incompleteK38.k38Report.source.sourceBoundAfterRemoteReads = "NO-GO";
        await rejects(() => runHarness(api, temporary, { fixture: incompleteK38 }), /freshly rerun K38 GO report/);
    });

    it("writes one bounded content-addressed create-only report", async () => {
        const run = await runHarness(api, temporary);
        const stored = await readFile(run.result.reportPath);
        ok(stored.length < TAXONOMY_PROJECTION_PUBLISHER_DRY_RUN_MAX_REPORT_BYTES);
        equal(sha256(stored), run.result.reportSha256);
        equal(basename(run.result.reportDirectory), run.result.reportSha256);
        equal(basename(run.result.reportPath), TAXONOMY_PROJECTION_PUBLISHER_DRY_RUN_REPORT_FILE);
    ...
    it("rejects hard-linked reports and linked output containment when the platform permits", async function () {
        ...
        const externalFile = join(temporary, "external-k39-report.json");
        ...
        try {
            await symlink(externalDirectory, join(junctionRoot, TAXONOMY_PROJECTION_PUBLISHER_DRY_RUN_NAMESPACE), "junction");
        } catch (error: any) {
            if (error?.code === "EPERM" || error?.code === "EACCES") { this.skip(); return; }
            throw error;
        }
        let k38Calls = 0;
        (globalThis as any)[Symbol.for("dokkan.k39.k38-runner")] = () => { k38Calls++; return makeFixture().k38Result; };
        await rejects(() => api.runTaxonomyProjectionPublisherDryRun({
            optInK39: true, dryRunOnly: true, remoteReadOnly: true, ...roots,
    ...
    it("rejects report file symlinks when the platform permits", async function () {
        const run = await runHarness(api, temporary);
        const externalFile = join(temporary, "external-k39-symlink.json");
        await writeFile(externalFile, "external");
        ...
        await rejects(() => runHarness(api, temporary, { fixture: run.fixture, outputRoot: run.outputRoot }),
            /existing report inventory rejected|existing report file identity rejected/);
    });
});
