...
export function parseTaxonomyProjectionRemotePreflightCli(args: string[]): TaxonomyProjectionRemotePreflightRunOptions {
    const switches = ["--opt-in-k38", "--remote-read-only"] as const;
    const allowed = new Set<string>([...switches, ...VALUE_ARGUMENTS]);
    for (let index = 0; index < args.length; index++) {
        const argument = args[index];
        if (!allowed.has(argument)) throw new Error(`K38 unsupported argument ${argument.slice(0, 128)}`);
    ...
    for (const flag of switches) {
        if (args.filter(value => value === flag).length !== 1) throw new Error(`K38 requires exactly one ${flag}`);
    }
    return {
        optInK38: true,
        remoteReadOnly: true,
...
async function main(): Promise<void> {
    const result = await runTaxonomyProjectionRemotePreflight(
        parseTaxonomyProjectionRemotePreflightCli(process.argv.slice(2)),
    );
    ...
    if (result.report.readiness.readOnlyRemotePreflight !== "GO") process.exitCode = 2;
}

if (require.main === module) main().catch(error => {
    const message = (error instanceof Error ? error.message : String(error))
        .slice(0, TAXONOMY_PROJECTION_REMOTE_PREFLIGHT_MAX_ERROR_LENGTH);
