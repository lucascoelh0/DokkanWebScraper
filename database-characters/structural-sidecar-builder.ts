import type { CharacterCardTaxonomy, DatabaseCharacterTaxonomyDataset, PresentationLabel } from "./taxonomy-contract";
import {
    CHARACTER_STRUCTURAL_SIDECAR_CONTRACT_VERSION,
    CharacterStructuralCollectionState,
    CharacterStructuralEvidenceStatus,
    CharacterStructuralIdentityRecord,
    CharacterStructuralIdentitySidecar,
    CharacterStructuralLabelEvidence,
...
function status(values: CharacterStructuralEvidenceStatus[], present: boolean): CharacterStructuralEvidenceStatus {
    if (!present || values.length === 0) return "unknown";
    if (values.every(value => value === "supported")) return "supported";
    if (values.every(value => value === "unknown")) return "unknown";
    return "partial";
}

function collectionState(values: unknown): CharacterStructuralCollectionState {
    if (!Array.isArray(values)) return "absent_unproved";
    return values.length === 0 ? "empty_with_container_provenance_absence_unproved" : "present_with_row_provenance";
}

function labelEvidence(label: PresentationLabel | undefined): CharacterStructuralLabelEvidence {
    if (!label) return { status: "unknown", reason: "dictionary_mapping_missing" };
    return {
        status: "supported",
        value: label.value,
        sourceLocale: label.sourceLocale,
        source: label.source as { table: "card_categories" | "link_skills"; rowId: string; column: "name" },
    };
}

function record(
    card: CharacterCardTaxonomy,
    productiveCardIds: Set<string>,
    categoryLabels: Map<string, PresentationLabel>,
    linkLabels: Map<string, PresentationLabel>,
): CharacterStructuralIdentityRecord {
    const rawCategories = (card as any).categoryAssignments;
    const rawLinks = (card as any).links;
    const categories = Array.isArray(rawCategories) ? rawCategories.map(item => ({
        categoryId: String(item.categoryId),
        relationRowId: String(item.relationRowId),
        status: item.status as CharacterStructuralEvidenceStatus,
...
function statusCounts(records: CharacterStructuralIdentityRecord[], field: "characterClass" | "categories" | "links") {
    const result: Record<CharacterStructuralEvidenceStatus, number> = { supported: 0, partial: 0, unknown: 0 };
    for (const item of records) result[item[field].status]++;
    return result;
}

export function buildCharacterStructuralSidecar(
...
): { sidecar: CharacterStructuralIdentitySidecar; coverage: CharacterStructuralSidecarCoverage } {
    ...
    if (duplicateCardIds.length) throw new Error(`duplicate K2 cardId: ${duplicateCardIds.join(",")}`);
    if (duplicateCategoryIds.length) throw new Error(`duplicate K2 category dictionary ID: ${duplicateCategoryIds.join(",")}`);
    if (duplicateLinkIds.length) throw new Error(`duplicate K2 link dictionary ID: ${duplicateLinkIds.join(",")}`);
