...
import {
    CHARACTER_LEADER_SCOPE_CONTRACT_VERSION,
    ...
    CHARACTER_LEADER_SCOPE_SAMPLE_LIMIT,
} from "./leader-scope-contract";
...
const limited = (values: Iterable<string>): string[] => [...values].sort(structuralOrder).slice(0, CHARACTER_LEADER_SCOPE_SAMPLE_LIMIT);

function validRef(ref: CharacterLeaderRowRef): boolean {
    return !!ref && exactKeys(ref, ["table", "rowId"]) && typeof ref.table === "string" && !!ref.table && typeof ref.rowId === "string" && !!ref.rowId;
}

function validateK43State(state: CharacterLeaderScopeK43State): void {
...
function validateK3State(state: CharacterLeaderScopeK3State): void {
...
export function evaluateCharacterLeaderStructuralScope(
    k43States: CharacterLeaderScopeK43State[],
    k3States: CharacterLeaderScopeK3State[],
): CharacterLeaderScopeEvaluation {
    if (!Array.isArray(k43States) || !Array.isArray(k3States)) throw new Error("K45 requires K43 and K3 state arrays");
    ...
    if (new Set(k43Ids).size !== k43Ids.length) throw new Error("K45 duplicate K43 stateId");
    if (new Set(k3Ids).size !== k3Ids.length) throw new Error("K45 duplicate K3 stateId");
    ...
    if (JSON.stringify(excluded.map(item => item.stateId)) !== JSON.stringify([...CHARACTER_LEADER_SCOPE_EXCLUDED_STATE_IDS])
        || excluded.some(item => item.releaseState !== "unknown")) throw new Error("K45 K3/K43 exclusion frontier changed");
    ...
    const multiEffectIds: string[] = [], multiTargetIds: string[] = [], multiPercentIds: string[] = [];
    for (const state of included) {
        ...
        effectReferences += effects; targetReferences += targets; structuredPercentValues += percents;
        if (effects > 1) { multiEffectStates++; multiEffectIds.push(state.stateId); }
        if (targets > 1) { multiTargetStates++; multiTargetIds.push(state.stateId); }
        if (percents > 1) { multiPercentStates++; multiPercentIds.push(state.stateId); }
        if (effects === 0) emptyEffectStates++;
        if (targets === 0) emptyTargetStates++;
        if (percents === 0) emptyPercentStates++;
    ...
    return {
        ...
        samples: {
            ...
            limitPerKind: CHARACTER_LEADER_SCOPE_SAMPLE_LIMIT,
        },
    };
}

export function assertPinnedCharacterLeaderScope(evaluation: CharacterLeaderScopeEvaluation): void {
    const pin = CHARACTER_LEADER_SCOPE_PIN;
    ...
    for (const [label, actual, expected] of pairs) if (actual !== expected) throw new Error(`K45 ${label} pin changed`);
    if (JSON.stringify(evaluation.excludedStateIds) !== JSON.stringify([...CHARACTER_LEADER_SCOPE_EXCLUDED_STATE_IDS])) throw new Error("K45 excluded state IDs changed");
    if (Object.values(evaluation.samples).some(value => Array.isArray(value) && value.length > CHARACTER_LEADER_SCOPE_SAMPLE_LIMIT)) throw new Error("K45 report sample bound exceeded");
}

export function buildCharacterLeaderScopeReport(
    ...
    const report: CharacterLeaderScopeReport = {
        schemaVersion: 1,
        contract: "dokkan-database-character-leader-structural-scope-audit",
        contractVersion: CHARACTER_LEADER_SCOPE_CONTRACT_VERSION,
        ...
        inputIntegrity: {
            ...
            reportTimestampIncluded: false, reportMaximumBytesExclusive: CHARACTER_LEADER_SCOPE_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: CHARACTER_LEADER_SCOPE_RSS_LIMIT_BYTES,
        },
        readiness: {
    ...
    if (Buffer.byteLength(`${JSON.stringify(report, null, 2)}\n`) >= CHARACTER_LEADER_SCOPE_REPORT_LIMIT_BYTES) throw new Error("K45 report byte limit reached");
    return report;
}
