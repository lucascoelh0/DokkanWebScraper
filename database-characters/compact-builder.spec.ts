import { deepStrictEqual, equal, ok, throws } from "assert";
import { CharacterFieldProjection } from "./shadow-contract";
import { CharacterCompactProjectionBuilder } from "./compact-builder";
import { pinnedCharacterCompactLineage } from "./compact-validator";
import * as compactBuilder from "./compact-builder";

const field = (cardId: string, name: "id" | "rarity" | "type", changes: Partial<CharacterFieldProjection> = {}): CharacterFieldProjection => ({
    cardId,
    ...
    growthRowId: null,
    productionJoin: { status: "joined", externalId: cardId, comparisonState: { stateKey: `${cardId}:initial` } as any },
    fyiJoin: { status: "unjoinable", externalId: null, comparisonState: { stateKey: `${cardId}:initial` } as any },
    field: name,
    characterField: name,
...
const addCard = (builder: CharacterCompactProjectionBuilder, cardId: string, changes: Partial<Record<"id" | "rarity" | "type", Partial<CharacterFieldProjection>>> = {}) => {
    for (const name of ["id", "rarity", "type"] as const) builder.accept(field(cardId, name, changes[name]));
};
...
describe("database character K15 compact builder", () => {
    it("allows agreements and a rarity representation gain while retaining only the minimum binding", () => {
        ...
        deepStrictEqual(Object.keys(result.projection.records[0]), ["cardId", "stateId", "rarity", "type"]);
    });
    ...
    it("excludes mismatch, conflict, unknown, partial and unjoinable cards instead of carrying audit data", () => {
        ...
        addCard(builder, "5", { id: { productionJoin: { status: "unjoinable", externalId: null, comparisonState: { stateKey: "5:initial" } as any }, comparison: "unjoinable", sourceComparisons: { production: "unjoinable", fyi: "unjoinable" }, authority: "unsupported" } });
        const result = builder.finish();
        equal(result.projection.records.length, 0);
        deepStrictEqual(result.coverage.exclusions, { unjoinable: 1, partial: 1, unknown: 1, mismatch: 1, conflict: 1, invalidEnum: 0, ambiguousBinding: 0, incomplete: 0 });
    });

    it("fails closed on duplicate source fields and pinned-count drift", () => {
        const builder = new CharacterCompactProjectionBuilder(pinnedCharacterCompactLineage(), "2026-08-05T00:00:00.000Z", "global-6.4.0-v338-2026-08-05-k15-v1");
        builder.accept(field("1", "id"));
        throws(() => builder.accept(field("1", "id")), /duplicate K11 compact source field/);
        throws(() => builder.finish(true), /pinned snapshot expectations changed/);
    });

    it("implements no Character apply, merge or fallback API", () => {
        equal((compactBuilder as any).applyCharacterCompactInMemory, undefined);
        equal((compactBuilder as any).mergeCharacters, undefined);
        const source = [{ id: "missing", rarity: "SSR", type: "PHY" }];
        const before = JSON.stringify(source);
        ok(!("fallback" in compactBuilder));
