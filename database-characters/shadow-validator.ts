...
export interface DatabaseCharacterShadowValidation {
    ...
    safety: {
        ...
        partialOrUnknownPatchCount: number;
        unjoinableDatabaseCandidateCount: number;
        conflictWinnerCount: number;
        ambiguousStateBindingCount: number;
        duplicateProjectionIdentityCount: number;
    };
}
...
function validateCharacterShadowProjectionUnsafe(projection: CharacterShadowProjection, coverage?: DatabaseCharacterShadowCoverage): DatabaseCharacterShadowValidation {
    ...
    if (duplicateProjectionIdentityCount) failures.push("duplicate projection identity");
    ...
    for (const item of projection.fields) {
        ...
        else bindingByCard.set(item.cardId, binding);
    }
    if (partialOrUnknownPatchCount) failures.push("partial or unknown database patch");
    if (unjoinableDatabaseCandidateCount) failures.push("unjoinable database patch");
    if (conflictWinnerCount) failures.push("conflict winner selected");
    if (ambiguousStateBindingCount) failures.push("ambiguous state binding");
    if (coverage) validateCoverage(projection, coverage, failures);
    return {
        schemaVersion: 1,
...
function validateCoverage(projection: CharacterShadowProjection, coverage: DatabaseCharacterShadowCoverage, failures: string[]): void {
    if (coverage.schemaVersion !== 1 || coverage.contract !== "dokkan-database-character-field-shadow-coverage" || coverage.contractVersion !== "1.0.0") failures.push("coverage schema");
    if (coverage.cardCount !== new Set(projection.fields.map(item => item.cardId)).size || coverage.fieldProjectionCount !== projection.fields.length) failures.push("coverage cardinality");
    if (coverage.cardCount === 5_759 && (coverage.productionUnjoinableCount !== 1_463 || coverage.productionJoinedCount !== 4_296 || coverage.fyiJoinedCount !== 1_625 || coverage.fyiUnjoinableCount !== 4_134)) failures.push("K7 join inventory");
    for (const field of coverage.fieldCoverage) {
        ...
        if (productionTotal !== coverage.cardCount || fyiTotal !== coverage.cardCount) failures.push(`non-exclusive classification:${field.field}`);
        if (field.patchableCharacterCount > coverage.productionJoinedCount) failures.push(`patchable cardinality:${field.field}`);
    }
    ...
    if (JSON.stringify(actualConflicts) !== JSON.stringify(expectedConflicts)) failures.push("K7 conflict inventory");
    if (coverage.catalogImpact.charactersCreatedByShadow !== 0 || coverage.catalogImpact.productionCatalogSizeChange !== 0) failures.push("catalog mutation");
    if (coverage.orderingAudits.some(item => item.policy !== "report_both_orders_no_silent_selection")) failures.push("ordering policy");
    const externalMatrix = projection.authorityMatrix.filter(item => item.owner === "external").map(item => item.field);
    if (JSON.stringify(coverage.fullyExternalFallbackFields) !== JSON.stringify(externalMatrix)) failures.push("external fallback inventory");
}
