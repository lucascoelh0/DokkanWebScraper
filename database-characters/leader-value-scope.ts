import {
    CHARACTER_LEADER_VALUE_SCOPE_CONTRACT_VERSION,
    ...
    CHARACTER_LEADER_VALUE_SCOPE_SAMPLE_LIMIT,
...
const limited = (values: Iterable<string>): string[] => [...new Set(values)].sort(structuralOrder).slice(0, CHARACTER_LEADER_VALUE_SCOPE_SAMPLE_LIMIT);
const numericDomain = (values: Iterable<number>): number[] => [...new Set(values)].sort((left, right) => left - right);
function baseK3Identity(identity: CharacterLeaderValueK3Identity) {
    const { valueInputFingerprintSha256: _valueFingerprint, ...base } = identity; return base;
}
function partial(row: CharacterLeaderValueRawEffect): boolean {
...
export function evaluateCharacterLeaderValueScope(
    k48: CharacterLeaderValueK48Source,
    k3: CharacterLeaderValueK3Source,
): CharacterLeaderValueScopeEvaluation {
    ...
    if (JSON.stringify(k48.identity.k3) !== JSON.stringify(baseK3Identity(k3.identity))) throw new Error("K49 K48/K3 identity mismatch");
    const effects = new Map(k3.effects.map(row => [row.rowId, row]));
    if (effects.size !== k3.effects.length) throw new Error("K49 duplicate raw effect identity");
    ...
    const typesBySet = new Map<string, Set<number>>();
    for (const row of k3.effects) {
        const types = typesBySet.get(row.leaderSkillSetId) ?? new Set<number>(); types.add(row.efficacyType); typesBySet.set(row.leaderSkillSetId, types);
    }
    ...
    for (const state of k48.states) for (const reference of state.effects) {
        includedEffectReferences++;
        const row = effects.get(reference.effectRowId);
        if (!row) { missingEffectRows++; mismatchIds.push(reference.effectRowId); continue; }
        if (row.leaderSkillSetId !== state.leaderSetRowId || row.subTargetTypeSetId !== reference.targetSetId) {
            structuralMismatchReferences++; mismatchIds.push(reference.effectRowId);
        }
        if (row.efficacyType === 82) includedType82References++;
        if (partial(row)) partialReferences++; else unknownReferences++;
    }
    ...
    for (const row of k3.effects) timing.set(row.execTimingType, (timing.get(row.execTimingType) ?? 0) + 1);
    ...
    return {
        ...
        samples: {
            ...
            mismatchEffectRowIds: limited(mismatchIds), limitPerKind: CHARACTER_LEADER_VALUE_SCOPE_SAMPLE_LIMIT,
        },
    };
}

export function assertPinnedCharacterLeaderValueScope(evaluation: CharacterLeaderValueScopeEvaluation): void {
    const pin = CHARACTER_LEADER_VALUE_SCOPE_PIN;
    ...
    ] as Array<[string, number, number]>) if (actual !== expected) throw new Error(`K49 ${label} hypothesis changed`);
    ...
    if (JSON.stringify(evaluation.calcOptionDomain) !== JSON.stringify([0, 2, 3])) throw new Error("K49 calc option domain hypothesis changed");
    if (JSON.stringify(evaluation.targetTypeDomain) !== JSON.stringify([0, 2, 4, 12, 13])) throw new Error("K49 target type domain hypothesis changed");
    if (JSON.stringify(evaluation.timingRows) !== JSON.stringify([{ id: 0, rows: pin.timing0Rows }, { id: 1, rows: pin.timing1Rows }])) throw new Error("K49 timing domain hypothesis changed");
    if (Object.values(evaluation.samples).some(value => Array.isArray(value) && value.length > CHARACTER_LEADER_VALUE_SCOPE_SAMPLE_LIMIT)) throw new Error("K49 sample bound exceeded");
}

export function buildCharacterLeaderValueScopeReport(
    k48: CharacterLeaderValueK48Identity,
    k3: CharacterLeaderValueK3Identity,
    scope: CharacterLeaderValueScopeEvaluation,
): CharacterLeaderValueScopeReport {
    assertPinnedCharacterLeaderValueScope(scope);
    const report: CharacterLeaderValueScopeReport = {
        schemaVersion: 1, contract: "dokkan-database-character-leader-value-scope-audit",
        contractVersion: CHARACTER_LEADER_VALUE_SCOPE_CONTRACT_VERSION, mode: "offline_local_explicit_opt_in_stdout_only",
        sources: { k48, k3 },
        policy: {
        ...
        inputIntegrity: {
            ...
            reportTimestampIncluded: false, reportMaximumBytesExclusive: CHARACTER_LEADER_VALUE_SCOPE_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: CHARACTER_LEADER_VALUE_SCOPE_RSS_LIMIT_BYTES,
        },
        readiness: {
    ...
    if (Buffer.byteLength(`${JSON.stringify(report, null, 2)}\n`) >= CHARACTER_LEADER_VALUE_SCOPE_REPORT_LIMIT_BYTES) throw new Error("K49 report byte limit reached");
    return report;
}
