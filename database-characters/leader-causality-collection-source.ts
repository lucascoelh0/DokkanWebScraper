...
function evidencePath(): string {
    const compiled = resolve(__dirname, "..", "database-experiment", "native-leader-causality-collection.json");
    return existsSync(compiled) ? compiled : resolve(__dirname, "..", "..", "database-experiment", "native-leader-causality-collection.json");
}

function branchTarget(callerVma: number, instruction: Buffer): number {
    if (instruction.length !== 4) throw new Error(`K53 malformed call instruction at ${callerVma}`);
    const word = instruction.readUInt32LE();
    if (((word & 0xfc000000) >>> 0) !== 0x94000000) throw new Error(`K53 native call opcode changed at ${callerVma}`);
...
function pltRelocationOffset(pltVma: number, bytes: Buffer): number {
    if (bytes.length !== 16) throw new Error(`K53 malformed PLT size at ${pltVma}`);
    const adrp = bytes.readUInt32LE(0), ldr = bytes.readUInt32LE(4);
    if (((adrp & 0x9f00001f) >>> 0) !== 0x90000010 || ((ldr & 0xffc003ff) >>> 0) !== 0xf9400211
        || bytes.subarray(12, 16).toString("hex") !== "20021fd6") throw new Error(`K53 malformed PLT at ${pltVma}`);
...
export async function loadCharacterLeaderCausalityCollectionNativeProof(
    nativeRuntimePath: string,
): Promise<CharacterLeaderCausalityCollectionNativeProof> {
    if (!nativeRuntimePath) throw new Error("K53 requires explicit native runtime path");
    const pin = CHARACTER_LEADER_CAUSALITY_COLLECTION_PIN;
    const [native, evidenceBytes] = await Promise.all([
        readPinnedLeaderScopeMember(resolve(nativeRuntimePath), { sha256: pin.nativeSha256, sizeBytes: pin.nativeSizeBytes }, "K53 native runtime"),
        readPinnedLeaderScopeMember(evidencePath(), { sha256: pin.nativeEvidenceSha256, sizeBytes: pin.nativeEvidenceSizeBytes }, "K53 native evidence"),
    ]);
    ...
    for (const vtable of evidence.vtables) {
        ...
        if (typeof vtable.role !== "string" || symbols.length !== 1 || symbols[0].value !== vtable.vma
            ...
            || !Array.isArray(vtable.bindings)) throw new Error(`K53 native vtable changed: ${vtable.role}`);
...
export function assertLeaderCausalityCollectionNativeStable(
    before: CharacterLeaderCausalityCollectionNativeProof,
    after: CharacterLeaderCausalityCollectionNativeProof,
): void {
    if (JSON.stringify(before) !== JSON.stringify(after)) throw new Error("K53 native collection evidence changed");
}
