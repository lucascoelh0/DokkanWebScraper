import { deepStrictEqual, equal, notStrictEqual, ok, rejects, throws } from "assert";
import type {
    CharacterStateProductProjectionCoverage,
    CharacterStateProductProjectionDataset,
} from "./state-product-projection-contract";
import { materializeCharacterStateProductProjection } from "./state-product-projection";
import {
...
import {
    ...
    buildCharacterStateProductShadowReport,
    createCharacterStateProductShadowConsumer,
    createCharacterStateProductShadowLookup,
    fingerprintCharacterStateProductShadowArtifact,
} from "./state-product-shadow";
import { parseCharacterStateProductShadowCli, runCharacterStateProductShadow } from "./state-product-shadow-run";

const count = (included = 0, excluded = 0) => ({ included, excluded });

function fixture(): { dataset: CharacterStateProductProjectionDataset; coverage: CharacterStateProductProjectionCoverage } {
...
describe("database character K44 state product shadow consumer", () => {
    it("accepts only the exact opt-in and four explicit roots", async () => {
        await rejects(runCharacterStateProductShadow({} as any), /explicit opt-in/);
        throws(() => parseCharacterStateProductShadowCli([]), /exactly one --opt-in-k44/);
        throws(() => parseCharacterStateProductShadowCli(["--opt-in-k44"]), /requires --sidecar-root/);
        throws(() => parseCharacterStateProductShadowCli(["--opt-in-k44", "--sidecar-root", "s", "--production-root", "p", "--fyi-root", "f", "--k43-root", "k", "--output", "x"]), /unsupported argument/);
        deepStrictEqual(parseCharacterStateProductShadowCli(["--opt-in-k44", "--sidecar-root", "s", "--production-root", "p", "--fyi-root", "f", "--k43-root", "k"]), {
    ...
    it("looks up cloned structural states and transitions by identity or card", () => {
        ...
        const lookup = createCharacterStateProductShadowLookup(dataset);
        equal(lookup.getStateByStateId("s2")?.growthRowId, "growth:1");
        deepStrictEqual(lookup.getStatesByCardId("1").map(item => item.stateId), ["s1", "s2"]);
        equal(lookup.getTransitionByTransitionId("a1")?.transitionType, "awakening");
        deepStrictEqual(lookup.getTransitionsByCardId("1").map(item => item.record.transitionId), ["a1", "f1", "r1"]);
        deepStrictEqual(lookup.getTransitionsByCardId("2").map(item => item.record.transitionId), ["f1"]);
        equal(lookup.getStateByStateId("missing"), null);
        deepStrictEqual(lookup.getTransitionsByCardId("missing"), []);
    });
    ...
    it("returns frozen clones and never mutates or exposes internal indexes", () => {
        ...
        throws(() => { (first as any).cardId = "mutated"; }, /read only|readonly|extensible|assign/i);
        equal(lookup.getStateByStateId("s1")!.cardId, "1");
        equal(JSON.stringify(dataset), before);
        equal(Object.isFrozen(lookup), true);
        equal((lookup as any).statesById, undefined);
    });

    it("refuses non-source-bound evidence, changed artifacts and non-pinned datasets", () => {
        throws(() => assertCharacterStateProductShadowSourceBound({ sourceBoundValidation: "NON_AUTHORITATIVE" }), /requires source-bound/);
        const { dataset, coverage } = fixture();
        const artifacts = materializeCharacterStateProductProjection(dataset, coverage);
        const fingerprint = fingerprintCharacterStateProductShadowArtifact(artifacts);
        assertCharacterStateProductShadowArtifactStable(fingerprint, fingerprint);
        throws(() => assertCharacterStateProductShadowArtifactStable(fingerprint, "0".repeat(64)), /artifact changed/);
        throws(() => assertPinnedCharacterStateProductShadowDataset(dataset), /projection pins changed/);
    });

    it("rejects presentation fields before indexing", () => {
        const { dataset } = fixture();
        (dataset.states[0] as any).name = "private-presentation";
        throws(() => createCharacterStateProductShadowLookup(dataset), /presentation or extra/);
    });
    ...
    it("builds a deterministic bounded timestamp-free report without Character data", () => {
        ...
        const artifacts = materializeCharacterStateProductProjection(dataset, coverage);
        const lookup = createCharacterStateProductShadowLookup(dataset);
        const first = buildCharacterStateProductShadowReport(artifacts, lookup);
        const second = buildCharacterStateProductShadowReport(artifacts, lookup);
        deepStrictEqual(first, second);
        const consumer = createCharacterStateProductShadowConsumer(artifacts);
        deepStrictEqual(consumer.report, first);
        const stdout = `${JSON.stringify(first, null, 2)}\n`;
        ok(Buffer.byteLength(stdout) < CHARACTER_STATE_PRODUCT_SHADOW_REPORT_LIMIT_BYTES);
        equal(stdout.includes("generatedAt"), false);
        equal(stdout.includes("private-presentation"), false);
        equal(stdout.includes("Character[]"), false);
        equal(first.samples.stateIds.length <= 5, true);
        equal(first.inputIntegrity.sourceBoundValidationBeforeLookup, "NOT_EXECUTED");
        equal(first.inputIntegrity.sourceBoundValidationAfterLookup, "NOT_EXECUTED");
        equal(first.readiness.consumerShadow, "NOT_EXECUTED");
        equal(first.readiness.persistedConsumer, "NO-GO");
    });
});
