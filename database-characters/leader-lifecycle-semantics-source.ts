...
function evidencePath(): string {
    const compiled = resolve(__dirname, "..", "database-experiment", "native-leader-lifecycle-semantics.json");
    return existsSync(compiled) ? compiled : resolve(__dirname, "..", "..", "database-experiment", "native-leader-lifecycle-semantics.json");
}

function branchTarget(callerVma: number, instruction: Buffer, link: boolean): number {
    if (instruction.length !== 4) throw new Error(`K55 malformed branch instruction at ${callerVma}`);
    const word = instruction.readUInt32LE();
    const expected = link ? 0x94000000 : 0x14000000;
    if (((word & 0xfc000000) >>> 0) !== expected) throw new Error(`K55 native branch opcode changed at ${callerVma}`);
...
function pltRelocationOffset(pltVma: number, bytes: Buffer): number {
    if (bytes.length !== 16) throw new Error(`K55 malformed PLT size at ${pltVma}`);
    const adrp = bytes.readUInt32LE(0), ldr = bytes.readUInt32LE(4);
    if (((adrp & 0x9f00001f) >>> 0) !== 0x90000010 || ((ldr & 0xffc003ff) >>> 0) !== 0xf9400211
        || bytes.subarray(12, 16).toString("hex") !== "20021fd6") throw new Error(`K55 malformed PLT at ${pltVma}`);
...
export async function loadCharacterLeaderLifecycleSemanticsNativeProof(
    nativeRuntimePath: string,
): Promise<CharacterLeaderLifecycleSemanticsNativeProof> {
    if (!nativeRuntimePath) throw new Error("K55 requires explicit native runtime path");
    const pin = CHARACTER_LEADER_LIFECYCLE_SEMANTICS_PIN;
    const [native, evidenceBytes] = await Promise.all([
        readPinnedLeaderScopeMember(resolve(nativeRuntimePath), { sha256: pin.nativeSha256, sizeBytes: pin.nativeSizeBytes }, "K55 native runtime"),
        readPinnedLeaderScopeMember(evidencePath(), { sha256: pin.nativeEvidenceSha256, sizeBytes: pin.nativeEvidenceSizeBytes }, "K55 native evidence"),
    ]);
...
export function assertLeaderLifecycleSemanticsNativeStable(
    before: CharacterLeaderLifecycleSemanticsNativeProof,
    after: CharacterLeaderLifecycleSemanticsNativeProof,
): void {
    if (JSON.stringify(before) !== JSON.stringify(after)) throw new Error("K55 native lifecycle evidence changed");
}
