import {
    CHARACTER_LEADER_CAUSALITY_DECK_INDEX_CONTRACT_VERSION,
...
const K53_GO = ["leaderCausalityCandidateCollection", "deckIndex0And1StructuralCollection"] as const;
const K53_NO_GO = [
...
export function assertConservativeK53ForDeckIndex(report: CharacterLeaderCausalityCollectionReport): void {
    ...
    for (const key of K53_GO) if (report.readiness[key] !== "GO") throw new Error(`K54 requires K53 ${key} GO`);
    for (const key of K53_NO_GO) if (report.readiness[key] !== "NO-GO") throw new Error(`K54 requires conservative K53 ${key} NO-GO`);
    if (report.upstreamK52.scope.type82NonNullRows !== CHARACTER_LEADER_CAUSALITY_DECK_INDEX_PIN.observedCausalityRows
...
export function assertPinnedCharacterLeaderCausalityDeckIndex(
    upstreamK53: CharacterLeaderCausalityCollectionReport,
    native: CharacterLeaderCausalityDeckIndexNativeProof,
): void {
...
export function buildCharacterLeaderCausalityDeckIndexReport(
    upstreamK53: CharacterLeaderCausalityCollectionReport,
    native: CharacterLeaderCausalityDeckIndexNativeProof,
): CharacterLeaderCausalityDeckIndexReport {
    const pin = CHARACTER_LEADER_CAUSALITY_DECK_INDEX_PIN;
    const report: CharacterLeaderCausalityDeckIndexReport = {
        schemaVersion: 1,
        contract: "dokkan-database-character-leader-causality-deck-index-audit",
        contractVersion: CHARACTER_LEADER_CAUSALITY_DECK_INDEX_CONTRACT_VERSION,
        ...
        inputIntegrity: {
            ...
            reportMaximumBytesExclusive: CHARACTER_LEADER_CAUSALITY_DECK_INDEX_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: CHARACTER_LEADER_CAUSALITY_DECK_INDEX_RSS_LIMIT_BYTES,
