...
import {
    ...
    TAXONOMY_PROJECTION_GZIP_LIMIT_BYTES,
    TAXONOMY_PROJECTION_MAX_EXAMPLES,
    TAXONOMY_PROJECTION_RAW_LIMIT_BYTES,
...
export interface TaxonomyProjectionArtifactSet {
    projection: TaxonomyProjectionDataset;
...
export interface TaxonomyProjectionSourceBoundValidationOptions {
    artifactRoot: string;
    k32Root: string;
    k2Root: string;
...
const exactKeys = (value: unknown, keys: string[]): boolean => Boolean(value) && typeof value === "object"
    && JSON.stringify(Object.keys(value as object)) === JSON.stringify(keys);

function countForbiddenRecordFields(value: unknown): number {
    if (!value || typeof value !== "object") return 0;
    let count = 0;
    for (const [key, nested] of Object.entries(value as Record<string, unknown>)) {
        if (/label|name|title|presentation/i.test(key)) count++;
        if (nested && typeof nested === "object") count += countForbiddenRecordFields(nested);
    }
    return count;
}

function validateTaxonomyProjectionShape(
    projection: TaxonomyProjectionDataset,
    coverage: TaxonomyProjectionCoverage,
    sizes: { rawSizeBytes: number; gzipSizeBytes: number },
    enforcePinnedSnapshot: boolean,
): TaxonomyProjectionValidation {
    ...
    try {
        if (!exactKeys(projection, ["schemaVersion", "contract", "contractVersion", "generatedAt", "datasetVersion", "mode", "source", "policy", "records"])
            ...
            || projection.contractVersion !== TAXONOMY_PROJECTION_CONTRACT_VERSION
            || projection.generatedAt !== TAXONOMY_PROJECTION_SOURCE_PIN.generatedAt
            || projection.datasetVersion !== TAXONOMY_PROJECTION_SOURCE_PIN.datasetVersion
        ...
        for (const record of projection.records) {
            const allowedKeys = ["cardId", "characterClass", "categories", "links"].filter(key => (record as any)[key] !== undefined);
            if (!exactKeys(record, allowedKeys) || allowedKeys[0] !== "cardId") extraRecordFieldCount++;
            if (typeof record.cardId !== "string" || !/^\d+$/.test(record.cardId)) failures.push("invalid cardId");
            if (seenCardIds.has(record.cardId)) duplicateCardIdCount++;
            seenCardIds.add(record.cardId);
            if (previousCardId !== undefined && numeric(previousCardId, record.cardId) >= 0) unstableCardOrderCount++;
            ...
            if (record.characterClass !== undefined) {
                ...
                if (!exactKeys(record.characterClass, ["raw", "normalized"]) || typeof record.characterClass.normalized !== "string"
                    || raw !== null && !["string", "number", "boolean"].includes(typeof raw)) unsupportedDimensionIncludedCount++;
            }
            if (record.categories !== undefined) {
                if (!Array.isArray(record.categories) || record.categories.length === 0) inventedEmptyDimensionCount++;
                ...
                for (let index = 0; index < (record.categories ?? []).length; index++) {
                    ...
                    if (!exactKeys(item, ["categoryId", "relationRowId"]) || !/^\d+$/.test(item.categoryId) || !/^\d+$/.test(item.relationRowId)
                        || relationRows.has(item.relationRowId)) invalidCategoryOrderCount++;
            ...
            if (record.links !== undefined) {
                if (!Array.isArray(record.links) || record.links.length === 0) inventedEmptyDimensionCount++;
                ...
                for (let index = 0; index < (record.links ?? []).length; index++) {
                    ...
                    if (!exactKeys(item, ["slot", "linkSkillId"]) || !Number.isInteger(item.slot) || item.slot < 1 || item.slot > 7
                        ...
                        || index > 0 && record.links![index - 1].slot >= item.slot) invalidLinkOrderOrSlotCount++;
                    slots.add(item.slot);
                }
            }
        }
        if (duplicateCardIdCount) failures.push("duplicate cardId");
        if (unstableCardOrderCount) failures.push("unstable card order");
        if (invalidCategoryOrderCount) failures.push("invalid category identity or order");
        if (invalidLinkOrderOrSlotCount) failures.push("invalid link identity, slot or order");
        if (unsupportedDimensionIncludedCount) failures.push("unsupported dimension representation included");
        if (inventedEmptyDimensionCount) failures.push("empty dimension invented without row evidence");
        if (labelOrPresentationFieldCount) failures.push("label or presentation field included");
        if (extraRecordFieldCount) failures.push("extra record field included");
        validateCoverage(projection, coverage, failures, enforcePinnedSnapshot);
        if (enforcePinnedSnapshot) validatePinnedLineage(projection, failures);
    } catch {
        failures.push("projection shape rejected");
    }
    if (sizes.rawSizeBytes > TAXONOMY_PROJECTION_RAW_LIMIT_BYTES) failures.push("raw byte budget exceeded");
    if (sizes.gzipSizeBytes > TAXONOMY_PROJECTION_GZIP_LIMIT_BYTES) failures.push("gzip byte budget exceeded");
    ...
    return {
        ...
        failures: uniqueFailures.slice(0, TAXONOMY_PROJECTION_MAX_EXAMPLES),
        failuresTruncated: uniqueFailures.length > TAXONOMY_PROJECTION_MAX_EXAMPLES,
        sizes: {
            rawMaximumBytes: TAXONOMY_PROJECTION_RAW_LIMIT_BYTES,
            gzipMaximumBytes: TAXONOMY_PROJECTION_GZIP_LIMIT_BYTES,
...
function validateCoverage(
    ...
    if (!exactKeys(coverage, ["schemaVersion", "contract", "contractVersion", "sourceCardCount", "projectedCardCount", "dimensions"])
        ...
        || coverage.contractVersion !== TAXONOMY_PROJECTION_CONTRACT_VERSION || coverage.projectedCardCount !== projection.records.length
        || coverage.sourceCardCount !== projection.records.length) failures.push("coverage contract or cardinality drift");
    const dimensions = ["characterClass", "categories", "links"] as const;
    for (const dimension of dimensions) {
        ...
        if (!exactKeys(item, ["includedCardCount", "includedFactCount", "excludedCardCount", "excludedFactCount", "exclusions", "examples"])
            ...
            || Object.values(item.examples).some(values => !Array.isArray(values) || values.length > TAXONOMY_PROJECTION_MAX_EXAMPLES)
        ...
        const factCount = dimension === "characterClass" ? includedCards.length
            : includedCards.reduce((sum, record) => sum + ((record[dimension] as unknown[])?.length ?? 0), 0);
        if (includedCards.length !== item.includedCardCount || factCount !== item.includedFactCount) failures.push(`${dimension} included coverage mismatch`);
    }
    if (enforcePinnedSnapshot) {
        ...
        if (coverage.sourceCardCount !== expected.cardCount
            ...
            || coverage.dimensions.links.includedFactCount !== expected.linkEntryCount) failures.push("pinned coverage changed");
    }
}

function validatePinnedLineage(projection: TaxonomyProjectionDataset, failures: string[]): void {
...
export function validateTaxonomyProjection(
    projection: TaxonomyProjectionDataset,
    coverage: TaxonomyProjectionCoverage,
    sizes: { rawSizeBytes: number; gzipSizeBytes: number },
): TaxonomyProjectionValidation {
...
export function materializeTaxonomyProjection(
    projection: TaxonomyProjectionDataset,
    coverage: TaxonomyProjectionCoverage,
): TaxonomyProjectionArtifactSet {
    const raw = Buffer.from(`${JSON.stringify(projection)}\n`, "utf8");
    if (raw.length > TAXONOMY_PROJECTION_RAW_LIMIT_BYTES) throw new Error(`K35 raw byte budget exceeded: ${raw.length}`);
    const gzip = gzipSync(raw, { level: 9 });
    if (gzip.length > TAXONOMY_PROJECTION_GZIP_LIMIT_BYTES) throw new Error(`K35 gzip byte budget exceeded: ${gzip.length}`);
    const validation = validateTaxonomyProjection(projection, coverage, { rawSizeBytes: raw.length, gzipSizeBytes: gzip.length });
    if (!validation.valid) throw new Error(`K35 projection validation failed: ${validation.failures.join("; ")}`);
    const coverageBytes = jsonBytes(coverage);
    const validationBytes = jsonBytes(validation);
    const payloadSha256 = hash(gzip);
    const manifest: TaxonomyProjectionManifest = {
        schemaVersion: 1,
    ...
    const release = TAXONOMY_PROJECTION_SOURCE_PIN.release;
...
async function regularRoot(value: string): Promise<string> {
    ...
    if (!metadata.isDirectory() || metadata.isSymbolicLink()) throw new Error("K35 artifact root must be an existing regular non-link directory");
    const canonical = await realpath(root);
    if (!samePath(root, canonical)) throw new Error("K35 artifact root symlink or junction rejected");
    return canonical;
}

async function readSnapshot(root: string, fileName: string): Promise<ArtifactSnapshot> {
    ...
    if (!samePath(path, resolve(root, fileName))) throw new Error("K35 artifact path escaped root");
    const before = await lstat(path);
    if (!before.isFile() || before.isSymbolicLink() || before.nlink !== 1) throw new Error(`K35 ${fileName} must be a single-link regular file`);
    const handle = await open(path, constants.O_RDONLY | (constants.O_NOFOLLOW ?? 0));
    try {
        const opened = await handle.stat();
        if (!sameFile(before, opened)) throw new Error(`K35 ${fileName} identity changed while opening`);
        const bytes = await handle.readFile();
        const after = await handle.stat();
        const pathAfter = await lstat(path);
        if (!sameFile(opened, after) || !sameFile(opened, pathAfter) || after.size !== bytes.length
            || after.mtimeMs !== opened.mtimeMs || after.ctimeMs !== opened.ctimeMs) throw new Error(`K35 ${fileName} changed while reading`);
...
async function gunzipBounded(bytes: Buffer): Promise<Buffer> {
    ...
    for await (const chunk of stream) {
        ...
        size += value.length;
        if (size > TAXONOMY_PROJECTION_RAW_LIMIT_BYTES) {
            stream.destroy();
...
function unchanged(before: ArtifactSnapshot, after: ArtifactSnapshot): void {
...
async function loadArtifact(rootValue: string): Promise<{ artifacts: TaxonomyProjectionArtifactSet; snapshots: ArtifactSnapshot[] }> {
    const root = await regularRoot(rootValue);
    const manifestSnapshot = await readSnapshot(root, TAXONOMY_PROJECTION_FILES.manifest);
    const manifest = JSON.parse(manifestSnapshot.bytes.toString("utf8")) as TaxonomyProjectionManifest;
    ...
    if (!exactKeys(manifest, ["schemaVersion", "contract", "contractVersion", "generatedAt", "datasetVersion", "fileName", "compression", "sha256", "sizeBytes", "uncompressedSha256", "uncompressedSizeBytes", "recordCount", "source", "coverageFile", "coverageSha256", "coverageSizeBytes", "validationFile", "validationSha256", "validationSizeBytes"])
        ...
        || manifest.contractVersion !== TAXONOMY_PROJECTION_CONTRACT_VERSION || manifest.generatedAt !== TAXONOMY_PROJECTION_SOURCE_PIN.generatedAt
        || manifest.datasetVersion !== TAXONOMY_PROJECTION_SOURCE_PIN.datasetVersion || manifest.compression !== "gzip"
        || !payloadMatch || payloadMatch[1] !== manifest.sha256 || manifest.coverageFile !== TAXONOMY_PROJECTION_FILES.coverage
        || manifest.validationFile !== TAXONOMY_PROJECTION_FILES.validation) throw new Error("K35 manifest contract rejected");
    ...
    if (payloadSnapshot.bytes.length !== manifest.sizeBytes || hash(payloadSnapshot.bytes) !== manifest.sha256
        || payloadSnapshot.bytes.length > TAXONOMY_PROJECTION_GZIP_LIMIT_BYTES) throw new Error("K35 payload identity rejected");
    ...
    if (raw.length !== manifest.uncompressedSizeBytes || hash(raw) !== manifest.uncompressedSha256) throw new Error("K35 raw payload identity rejected");
    const projection = JSON.parse(raw.toString("utf8")) as TaxonomyProjectionDataset;
    const canonicalRaw = Buffer.from(`${JSON.stringify(projection)}\n`, "utf8");
    if (!raw.equals(canonicalRaw) || !gzipSync(canonicalRaw, { level: 9 }).equals(payloadSnapshot.bytes)) {
        throw new Error("K35 canonical JSON or deterministic gzip rejected");
    }
    const coverage = JSON.parse(coverageSnapshot.bytes.toString("utf8")) as TaxonomyProjectionCoverage;
    const validation = JSON.parse(validationSnapshot.bytes.toString("utf8")) as TaxonomyProjectionValidation;
...
export async function validateTaxonomyProjectionArtifact(
    options: TaxonomyProjectionSourceBoundValidationOptions,
): Promise<{
    ...
    const k32 = await validateCharacterStructuralSidecarArtifact({
        artifactRoot: options.k32Root,
        k2Root: options.k2Root,
        productiveRoot: options.productiveRoot,
    });
    const k34Run = await runCardScopeAudit({
        optIn: true,
    ...
    const rebuilt = buildTaxonomyProjection(sources);
    ...
    for (const [label, actualBytes, expectedBytes] of [
        ...
        if (!actualBytes.equals(expectedBytes)) throw new Error(`K35 source-bound artifact mismatch: ${label}`);
    }
    const root = await regularRoot(options.artifactRoot);
    const after = await Promise.all([
        readSnapshot(root, TAXONOMY_PROJECTION_FILES.manifest),
        readSnapshot(root, actual.artifacts.manifest.fileName),
        readSnapshot(root, TAXONOMY_PROJECTION_FILES.coverage),
        readSnapshot(root, TAXONOMY_PROJECTION_FILES.validation),
    ]);
