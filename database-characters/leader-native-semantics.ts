...
const domain = (values: number[]): number[] => [...new Set(values)].sort((left, right) => left - right);

export function evaluateCharacterLeaderNativeSemantics(k48: CharacterLeaderValueK48Source, k3: CharacterLeaderValueK3Source): CharacterLeaderNativeEvaluation {
    ...
    if (mismatchedReferences.length) throw new Error("K50 K48/K3 type82 join changed");
    return {
...
export function assertPinnedCharacterLeaderNativeSemantics(scope: CharacterLeaderNativeEvaluation, native: CharacterLeaderNativeProof): void {
    ...
    for (const [label, actual, expected] of checks) if (actual !== expected) throw new Error(`K50 ${label} hypothesis changed`);
    if (JSON.stringify(scope.targetTypeDomain) !== JSON.stringify(pin.targetTypeDomain)) throw new Error("K50 target type domain hypothesis changed");
...
export function buildCharacterLeaderNativeSemanticsReport(
    k48: CharacterLeaderValueK48Source["identity"],
    k3: CharacterLeaderValueK3Source["identity"],
    native: CharacterLeaderNativeProof,
    scope: CharacterLeaderNativeEvaluation,
): CharacterLeaderNativeSemanticsReport {
    assertPinnedCharacterLeaderNativeSemantics(scope, native);
    const report: CharacterLeaderNativeSemanticsReport = {
        schemaVersion: 1, contract: "dokkan-database-character-leader-native-semantics-audit",
        contractVersion: CHARACTER_LEADER_NATIVE_SEMANTICS_CONTRACT_VERSION, mode: "offline_local_explicit_opt_in_stdout_only",
        ...
        inputIntegrity: {
            ...
            reportMaximumBytesExclusive: CHARACTER_LEADER_NATIVE_SEMANTICS_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: CHARACTER_LEADER_NATIVE_SEMANTICS_RSS_LIMIT_BYTES,
        },
        readiness: {
    ...
    if (Buffer.byteLength(`${JSON.stringify(report, null, 2)}\n`) >= CHARACTER_LEADER_NATIVE_SEMANTICS_REPORT_LIMIT_BYTES) throw new Error("K50 report byte limit reached");
    return report;
}
