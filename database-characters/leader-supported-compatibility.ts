...
import {
    ...
    CharacterLeaderCompatibilityClassification,
    CharacterLeaderSupportedCompatibilityArtifactSet,
    CharacterLeaderSupportedCompatibilityCoverage,
    CharacterLeaderSupportedCompatibilityManifest,
...
export interface CharacterLeaderCompatibilityPinnedReport {
    bytes: Buffer;
    value: any;
}

export interface CharacterLeaderCompatibilityProductiveBaseline {
    ids: Set<string>;
    textIds: Set<string>;
    structuredLeaderDetailIds: Set<string>;
}

export interface CharacterLeaderSupportedCompatibilityInputs {
    k56: CharacterLeaderSupportedProjectionArtifactSet;
    k58: any;
    k57: CharacterLeaderCompatibilityPinnedReport;
    k59: CharacterLeaderCompatibilityPinnedReport;
    k60: CharacterLeaderCompatibilityPinnedReport;
...
function assertLineage(inputs: CharacterLeaderSupportedCompatibilityInputs): void {
    const pin = CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN;
    const fingerprint = characterLeaderSupportedShadowArtifactFingerprint(inputs.k56);
    const lineage = characterLeaderSupportedShadowLineageFingerprint(inputs.k56);
    ...
    const inventory = createCharacterLeaderSupportedShadow(inputs.k56).inventory();
...
const classificationCounts = (): Record<CharacterLeaderCompatibilityClassification, number> => ({
...
export function buildCharacterLeaderSupportedCompatibilityReport(
    ...
    if (records.some(record => conditional.has(record.effectRowId))) throw new Error("K62 conditional effect leaked into supported audit");
    ...
    const effectReferences = new Map<string, number>();
    for (const record of records) effectReferences.set(record.effectRowId, (effectReferences.get(record.effectRowId) ?? 0) + 1);
    const effects = [...effectReferences.entries()].sort(([left], [right]) => order(left, right)).map(([effectRowId, referenceCount]) => ({
        effectRowId,
        referenceCount,
        classification: "additive_contract_required" as const,
        reason: "lossless_supported_shadow_requires_separate_additive_contract" as const,
    }));
    ...
    const productiveMetrics = {
        ...
        zeroConflictIsCompleteness: false as const,
        ...
        identityOnlyNoTextAuthority: true as const,
    };
    return {
        schemaVersion: 1,
        ...
        lineage: {
            k56: {
                ...
                fullArtifactFingerprintSha256: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.fullArtifactFingerprintSha256,
                lineageFingerprintSha256: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.lineageFingerprintSha256,
                sourceBoundViaK58: "GO",
            },
            k57: { reportSha256: k57Sha, localShadowIndexes: 4, lookupReadiness: "GO" },
            k58: { candidateManifestSha256: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.publicManifest.sha256, candidateOnly: true, sourceBoundValidation: "GO" },
            ...
            k61: {
                reportSha256: k61Sha,
                publicManifestSha256: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.publicManifest.sha256,
...
export function validateCharacterLeaderSupportedCompatibilityReport(
    ...
    if (report.dimensionMatrix.some(item => !item.classification || !item.reason || item.affectedReferences < 0)) failures.push("dimension classification incomplete");
    if (report.effectCompatibility.effects.length !== CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.effects
        || new Set(report.effectCompatibility.effects.map(effect => effect.effectRowId)).size !== CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.effects
        || report.effectCompatibility.effects.reduce((sum, effect) => sum + effect.referenceCount, 0) !== CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.references
        || report.effectCompatibility.effects.some(effect => effect.classification !== "additive_contract_required")) failures.push("effect classification changed");
    if (report.inventory.references !== CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.references
        || report.inventory.effects !== CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_PIN.effects
        || report.inventory.excludedEffects !== 17 || report.inventory.excludedReferences !== 45
        || report.inventory.excludedReason !== "runtime_deck_index_unresolved") failures.push("inventory boundary changed");
    if (report.comparison.productiveCharacterDataset.confirmedConflictReferences !== 0
        ...
        || report.comparison.productiveCharacterDataset.comparableValueReferences !== 0) failures.push("zero-conflict completeness boundary changed");
    const serialized = JSON.stringify(report);
    if (/"name"|"title"|"rawText"|"sourceText"/.test(serialized)) failures.push("text or presentation entered K62 identity report");
    ...
    if (report.boundaries.deckIndex !== "unknown" || report.boundaries.leaderFriendComposition !== "unknown"
        ...
        || report.boundaries.newTextParsing !== "NOT_EXECUTED") failures.push("runtime unknown boundary changed");
    if (report.k63Proposal.replacesEffectiveValues !== false || report.k63Proposal.existingFallback !== "byte_and_semantically_identical"
        || report.k63Proposal.modifiesUi !== false || report.k63Proposal.modifiesCharacterEntity !== false) failures.push("K63 proposal boundary changed");
    return failures;
}

export function materializeCharacterLeaderSupportedCompatibility(
    report: CharacterLeaderSupportedCompatibilityReport,
): CharacterLeaderSupportedCompatibilityArtifactSet {
    const raw = jsonBytes(report);
    const gzip = gzipSync(raw, { level: 9 });
    const dimensionClassificationCounts = classificationCounts();
    for (const item of report.dimensionMatrix) dimensionClassificationCounts[item.classification]++;
    const coverage: CharacterLeaderSupportedCompatibilityCoverage = {
        schemaVersion: 1,
    ...
    const validation: CharacterLeaderSupportedCompatibilityValidation = {
        schemaVersion: 1,
        ...
        sizes: {
            ...
            rawMaximumBytesExclusive: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_RAW_LIMIT_BYTES,
            gzipMaximumBytesExclusive: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_GZIP_LIMIT_BYTES,
            metadataMaximumBytesExclusive: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_METADATA_LIMIT_BYTES,
        },
        checks: {
    ...
    validation.sizes.metadataSizeBytes = coverageBytes.length + validationBytes.length;
    validationBytes = jsonBytes(validation);
    if (raw.length >= CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_RAW_LIMIT_BYTES) validation.failures.push("raw size limit reached");
    if (gzip.length >= CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_GZIP_LIMIT_BYTES) validation.failures.push("gzip size limit reached");
    if (validation.sizes.metadataSizeBytes >= CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_METADATA_LIMIT_BYTES) validation.failures.push("metadata size limit reached");
    ...
    const manifest: CharacterLeaderSupportedCompatibilityManifest = {
        schemaVersion: 1,
        ...
        coverageFile: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_FILES.coverage,
        coverageSha256: hash(coverageBytes),
        coverageSizeBytes: coverageBytes.length,
        validationFile: CHARACTER_LEADER_SUPPORTED_COMPATIBILITY_FILES.validation,
