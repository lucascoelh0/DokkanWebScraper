import {
    CHARACTER_LEADER_CAUSALITY_COLLECTION_CONTRACT_VERSION,
...
const K52_GO = ["leaderCausalityStructuralSemantics", "type35StructuralCondition"] as const;
const K52_NO_GO = [
...
export function assertConservativeK52ForCollection(report: CharacterLeaderCausalitySemanticsReport): void {
    ...
    for (const key of K52_GO) if (report.readiness[key] !== "GO") throw new Error(`K53 requires K52 ${key} GO`);
    for (const key of K52_NO_GO) if (report.readiness[key] !== "NO-GO") throw new Error(`K53 requires conservative K52 ${key} NO-GO`);
...
export function assertPinnedCharacterLeaderCausalityCollection(
    upstreamK52: CharacterLeaderCausalitySemanticsReport,
    native: CharacterLeaderCausalityCollectionNativeProof,
): void {
    ...
    if (native.elfSha256 !== pin.nativeSha256 || native.elfSizeBytes !== pin.nativeSizeBytes
        ...
        || native.productAuthorityBound !== false) throw new Error("K53 native collection proof does not match pin");
}

export function buildCharacterLeaderCausalityCollectionReport(
    upstreamK52: CharacterLeaderCausalitySemanticsReport,
    native: CharacterLeaderCausalityCollectionNativeProof,
): CharacterLeaderCausalityCollectionReport {
    const pin = CHARACTER_LEADER_CAUSALITY_COLLECTION_PIN;
    const report: CharacterLeaderCausalityCollectionReport = {
        schemaVersion: 1,
        contract: "dokkan-database-character-leader-causality-collection-audit",
        contractVersion: CHARACTER_LEADER_CAUSALITY_COLLECTION_CONTRACT_VERSION,
        ...
        inputIntegrity: {
            ...
            reportMaximumBytesExclusive: CHARACTER_LEADER_CAUSALITY_COLLECTION_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: CHARACTER_LEADER_CAUSALITY_COLLECTION_RSS_LIMIT_BYTES,
