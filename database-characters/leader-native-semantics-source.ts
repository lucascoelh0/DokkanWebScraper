...
import { CHARACTER_LEADER_NATIVE_PIN, CharacterLeaderNativeProof } from "./leader-native-semantics-contract";

const hash = (bytes: Buffer): string => createHash("sha256").update(bytes).digest("hex");
const evidencePath = (): string => {
    const compiledSibling = resolve(__dirname, "..", "database-experiment", "native-leader-skill-semantics.json");
    return existsSync(compiledSibling) ? compiledSibling : resolve(__dirname, "..", "..", "database-experiment", "native-leader-skill-semantics.json");
};
...
function exactObject(actual: unknown, expected: unknown, label: string): void {
    if (JSON.stringify(actual) !== JSON.stringify(expected)) throw new Error(`K50 ${label} evidence changed`);
}

export async function loadCharacterLeaderNativeProof(nativeRuntimePath: string): Promise<CharacterLeaderNativeProof> {
    if (!nativeRuntimePath) throw new Error("K50 requires explicit native runtime path");
    const pin = CHARACTER_LEADER_NATIVE_PIN;
    const [nativeBytes, evidenceBytes] = await Promise.all([
        readPinnedLeaderScopeMember(resolve(nativeRuntimePath), { sha256: pin.elfSha256, sizeBytes: pin.elfSizeBytes }, "K50 native runtime"),
        readPinnedLeaderScopeMember(evidencePath(), { sha256: pin.evidenceSha256, sizeBytes: pin.evidenceSizeBytes }, "K50 native evidence"),
    ]);
    ...
    if (evidence.schemaVersion !== 1 || evidence.contract !== "dokkan-native-leader-skill-semantics-evidence"
        || evidence.contractVersion !== "1.0.0") throw new Error("K50 native evidence contract changed");
    ...
    const inspection = parseNativeRuntimeElf(nativeBytes);
    if (inspection.machine !== 183) throw new Error("K50 native machine changed");
    if (!Array.isArray(evidence.codeRegions) || evidence.codeRegions.length !== 8) throw new Error("K50 code-region evidence changed");
    for (const region of evidence.codeRegions) {
        ...
        if (region.symbol !== null) {
            ...
            if (matches.length !== 1 || matches[0].value !== region.vma || matches[0].size !== region.sizeBytes) throw new Error(`K50 symbol changed for ${region.role}`);
        }
        if (hash(inspection.readVirtualBytes(region.vma, region.sizeBytes)) !== region.codeSha256) throw new Error(`K50 code changed for ${region.role}`);
    }
    ...
    exactObject(evidence.codeRegions.map((region: any) => region.role), expectedRoles, "code roles");
    ...
    if (!Array.isArray(evidence.constructorColumns) || evidence.constructorColumns.length !== expectedColumns.length) throw new Error("K50 constructor column evidence changed");
    for (let index = 0; index < expectedColumns.length; index++) {
        const [name, literalVma, objectOffset] = expectedColumns[index] as [string, number, number];
        ...
        if (literal.toString("utf8") !== `${name}\0`) throw new Error(`K50 constructor literal changed for ${name}`);
    }
    const mapSymbols = inspection.symbols.filter(symbol => symbol.name === evidence.dispatch.mapSymbol);
    if (mapSymbols.length !== 1 || mapSymbols[0].value !== evidence.dispatch.mapVma || mapSymbols[0].size !== evidence.dispatch.mapSizeBytes) throw new Error("K50 dispatch map symbol changed");
    ...
    if (initRelocations.length !== 1 || initRelocations[0].type !== initializer.type || initRelocations[0].addend !== initializer.addend) throw new Error("K50 dispatch initializer relocation changed");
    if (!Array.isArray(evidence.dispatch.entries) || evidence.dispatch.entries.length !== pin.dispatchEntries) throw new Error("K50 dispatch entry evidence changed");
    const efficacyDomain = evidence.dispatch.entries.map((entry: any) => entry.efficacyType);
    ...
    for (const entry of evidence.dispatch.entries) {
        ...
        if (relocations.length !== 1 || relocations[0].type !== 1025 || relocations[0].symbolName !== entry.symbol
            || relocations[0].symbolValue !== entry.symbolVma || relocations[0].addend !== 0) throw new Error(`K50 dispatch binding changed for ${entry.efficacyType}`);
    }
    const type82 = evidence.dispatch.entries.find((entry: any) => entry.efficacyType === 82);
    const type82Region = evidence.codeRegions.find((region: any) => region.role === "teaming_power_type82_handler");
    return {
...
export function assertCharacterLeaderNativeProofStable(before: CharacterLeaderNativeProof, after: CharacterLeaderNativeProof): void {
    if (JSON.stringify(before) !== JSON.stringify(after)) throw new Error("K50 native proof changed during audit");
}
