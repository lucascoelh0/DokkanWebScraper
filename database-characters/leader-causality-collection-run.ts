import {
    CHARACTER_LEADER_CAUSALITY_COLLECTION_REPORT_LIMIT_BYTES,
    CHARACTER_LEADER_CAUSALITY_COLLECTION_RSS_LIMIT_BYTES,
    CharacterLeaderCausalityCollectionReport,
} from "./leader-causality-collection-contract";
import {
    assertLeaderCausalityCollectionNativeStable,
    loadCharacterLeaderCausalityCollectionNativeProof,
} from "./leader-causality-collection-source";
import {
...
import { runCharacterLeaderCausalitySemanticsAudit } from "./leader-causality-semantics-run";

export interface CharacterLeaderCausalityCollectionRunOptions {
    optIn: true;
    sidecarRoot: string;
...
class RssGuard {
    ...
    private observe(): void {
        ...
        this.exceeded ||= this.peak >= CHARACTER_LEADER_CAUSALITY_COLLECTION_RSS_LIMIT_BYTES;
    }
    sample(): void {
        this.observe();
        if (this.exceeded) throw new Error(`K53 RSS limit reached: ${this.peak}`);
    }
    stop(): number { clearInterval(this.timer); this.sample(); return this.peak; }
    dispose(): void { clearInterval(this.timer); }
}

function authorize(report: CharacterLeaderCausalityCollectionReport): CharacterLeaderCausalityCollectionReport {
    return {
        ...report,
        inputIntegrity: {
...
export async function runCharacterLeaderCausalityCollectionAudit(
    options: CharacterLeaderCausalityCollectionRunOptions,
): Promise<CharacterLeaderCausalityCollectionReport> {
    if (options?.optIn !== true) throw new Error("K53 requires explicit opt-in");
    ...
    if (typeof (global as any).gc !== "function") throw new Error("K53 requires Node --expose-gc");
    const rss = new RssGuard();
    try {
        let beforeNative = await loadCharacterLeaderCausalityCollectionNativeProof(options.nativeRuntime);
        (global as any).gc();
        rss.sample();
        const upstreamK52 = await runCharacterLeaderCausalitySemanticsAudit({
            optIn: true,
        ...
        rss.sample();
        let afterNative = await loadCharacterLeaderCausalityCollectionNativeProof(options.nativeRuntime);
        (global as any).gc();
        rss.sample();
        assertLeaderCausalityCollectionNativeStable(beforeNative, afterNative);
        assertPinnedCharacterLeaderCausalityCollection(upstreamK52, beforeNative);
        const report = buildCharacterLeaderCausalityCollectionReport(upstreamK52, beforeNative);
...
function required(args: string[], name: string): string {
    ...
    if (!result) throw new Error(`K53 requires ${name}`);
    return result;
}

export function parseCharacterLeaderCausalityCollectionCli(args: string[]): CharacterLeaderCausalityCollectionRunOptions {
    const values = [
    ...
    for (let index = 0; index < args.length; index++) {
        ...
        if (!allowed.has(argument)) throw new Error(`K53 unsupported argument ${argument}`);
    ...
    if (args.filter(item => item === "--opt-in-k53").length !== 1) throw new Error("K53 requires exactly one --opt-in-k53");
    return {
        optIn: true,
...
async function run(): Promise<void> {
    ...
    if (Buffer.byteLength(stdout) >= CHARACTER_LEADER_CAUSALITY_COLLECTION_REPORT_LIMIT_BYTES) throw new Error("K53 stdout report byte limit reached");
