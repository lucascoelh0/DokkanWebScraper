import { TAXONOMY_PROJECTION_PUBLISHER_DRY_RUN_MAX_ERROR_LENGTH } from "./taxonomy-projection-publisher-dry-run-contract";
import {
...
function valueOf(args: string[], name: typeof VALUE_ARGUMENTS[number]): string {
...
export function parseTaxonomyProjectionPublisherDryRunCli(args: string[]): TaxonomyProjectionPublisherDryRunOptions {
    const switches = ["--opt-in-k39", "--dry-run-only", "--remote-read-only"] as const;
    const allowed = new Set<string>([...switches, ...VALUE_ARGUMENTS]);
    for (let index = 0; index < args.length; index++) {
        const argument = args[index];
        if (!allowed.has(argument)) throw new Error(`K39 unsupported argument ${argument.slice(0, 128)}`);
        if ((VALUE_ARGUMENTS as readonly string[]).includes(argument)) {
    ...
    for (const flag of switches) {
        if (args.filter(value => value === flag).length !== 1) throw new Error(`K39 requires exactly one ${flag}`);
    }
    return {
        optInK39: true,
        dryRunOnly: true,
...
async function main(): Promise<void> {
    const result = await runTaxonomyProjectionPublisherDryRun(
        parseTaxonomyProjectionPublisherDryRunCli(process.argv.slice(2)),
    );
...
if (require.main === module) main().catch(error => {
    const message = (error instanceof Error ? error.message : String(error))
        .slice(0, TAXONOMY_PROJECTION_PUBLISHER_DRY_RUN_MAX_ERROR_LENGTH);
