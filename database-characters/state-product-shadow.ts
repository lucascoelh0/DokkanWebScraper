...
import {
    CHARACTER_STATE_PRODUCT_SHADOW_CONTRACT_VERSION,
    CHARACTER_STATE_PRODUCT_SHADOW_PIN,
    CHARACTER_STATE_PRODUCT_SHADOW_REPORT_LIMIT_BYTES,
    CHARACTER_STATE_PRODUCT_SHADOW_RSS_LIMIT_BYTES,
    CHARACTER_STATE_PRODUCT_SHADOW_SAMPLE_LIMIT,
    CharacterStateProductShadowConsumer,
    CharacterStateProductShadowLookup,
    CharacterStateProductShadowLookupInventory,
    CharacterStateProductShadowReport,
    CharacterStateProductShadowTransition,
...
function frozenClone<T>(value: T): Readonly<T> { return deepFreeze(clone(value)); }

function assertDatasetBoundary(dataset: CharacterStateProductProjectionDataset): void {
    if (!dataset || dataset.contract !== "dokkan-database-character-state-product-projection" || dataset.contractVersion !== "1.0.0"
        ...
        || dataset.policy.characterArrayReturned || dataset.policy.characterArrayModified) throw new Error("K44 K43 structural dataset boundary rejected");
    ...
    if (dataset.states.some(item => !exactKeys(item, allowedState))
        ...
        || dataset.formTransitions.some(item => !exactKeys(item, allowedForm))) throw new Error("K44 presentation or extra projection field rejected");
}

interface InternalIndexes {
    statesById: Map<string, Readonly<CharacterStateProductProjectionState>>;
    stateIdsByCardId: Map<string, readonly string[]>;
    transitionsById: Map<string, CharacterStateProductShadowTransition>;
    transitionIdsByCardId: Map<string, readonly string[]>;
    inventory: Readonly<CharacterStateProductShadowLookupInventory>;
}

function indexDataset(dataset: CharacterStateProductProjectionDataset): InternalIndexes {
    assertDatasetBoundary(dataset);
    const statesById = new Map<string, Readonly<CharacterStateProductProjectionState>>();
    const stateIdsByCard = new Map<string, string[]>();
    for (const state of dataset.states) {
        if (statesById.has(state.stateId)) throw new Error(`K44 duplicate stateId ${state.stateId}`);
    ...
    const transitionsById = new Map<string, CharacterStateProductShadowTransition>();
    const transitionIdsByCard = new Map<string, string[]>();
    const acceptTransition = (transition: CharacterStateProductShadowTransition, cardIds: string[]): void => {
        const transitionId = transition.record.transitionId;
        if (transitionsById.has(transitionId)) throw new Error(`K44 duplicate transitionId ${transitionId}`);
        transitionsById.set(transitionId, deepFreeze(transition));
        for (const cardId of [...new Set(cardIds)]) transitionIdsByCard.set(cardId, [...(transitionIdsByCard.get(cardId) ?? []), transitionId]);
    };
...
export function createCharacterStateProductShadowLookup(dataset: CharacterStateProductProjectionDataset): Readonly<CharacterStateProductShadowLookup> {
    const index = indexDataset(dataset);
    const lookup: CharacterStateProductShadowLookup = {
        ...
        getTransitionByTransitionId: transitionId => {
            ...
            return value ? frozenClone(value) as CharacterStateProductShadowTransition : null;
        },
        getTransitionsByCardId: cardId => deepFreeze((index.transitionIdsByCardId.get(cardId) ?? []).map(transitionId => frozenClone(index.transitionsById.get(transitionId)!) as CharacterStateProductShadowTransition)),
        inventory: () => frozenClone(index.inventory),
    };
    return deepFreeze(lookup);
}

export function assertCharacterStateProductShadowSourceBound(value: unknown): asserts value is { sourceBoundValidation: "GO" } {
    if (!value || typeof value !== "object" || (value as any).sourceBoundValidation !== "GO") {
        throw new Error("K44 requires source-bound K43 validation GO");
    }
}

export function assertPinnedCharacterStateProductShadowDataset(dataset: CharacterStateProductProjectionDataset): void {
    const pin = CHARACTER_STATE_PRODUCT_SHADOW_PIN;
    if (dataset.states.length !== pin.states || dataset.releaseTransitions.length !== pin.releaseTransitions
        ...
        || dataset.source.k7ProductionCoverage.use !== "coverage_only") throw new Error("K44 K43 projection pins changed");
}

function limited(values: Iterable<string>): string[] { return [...values].sort(structuralOrder).slice(0, CHARACTER_STATE_PRODUCT_SHADOW_SAMPLE_LIMIT); }

export function buildCharacterStateProductShadowReport(
    ...
    const transitionCardIds = new Set<string>();
    ...
    return {
        ...
        contractVersion: CHARACTER_STATE_PRODUCT_SHADOW_CONTRACT_VERSION,
        mode: "offline_local_explicit_opt_in_stdout_only",
        source: {
            k43ManifestSha256: createHash("sha256").update(artifacts.manifestBytes).digest("hex"),
        ...
        samples: {
            ...
            transitionCardIds: limited(transitionCardIds), limitPerKind: CHARACTER_STATE_PRODUCT_SHADOW_SAMPLE_LIMIT,
        },
        inputIntegrity: {
            ...
            reportMaximumBytesExclusive: CHARACTER_STATE_PRODUCT_SHADOW_REPORT_LIMIT_BYTES,
            rssMaximumBytesExclusive: CHARACTER_STATE_PRODUCT_SHADOW_RSS_LIMIT_BYTES,
        },
        readiness: {
...
export function fingerprintCharacterStateProductShadowArtifact(artifacts: CharacterStateProductProjectionArtifactSet): string {
...
export function assertCharacterStateProductShadowArtifactStable(beforeFingerprint: string, afterFingerprint: string): void {
    if (beforeFingerprint !== afterFingerprint) throw new Error("K44 K43 artifact changed between source-bound validations");
}

export function createCharacterStateProductShadowConsumer(
    artifacts: CharacterStateProductProjectionArtifactSet,
): Readonly<CharacterStateProductShadowConsumer> {
    ...
    if (bytes.length >= CHARACTER_STATE_PRODUCT_SHADOW_REPORT_LIMIT_BYTES) throw new Error(`K44 stdout report byte limit reached: ${bytes.length}`);
