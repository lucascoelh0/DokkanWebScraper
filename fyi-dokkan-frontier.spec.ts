...
import {
    buildDokkanFrontierChaptersDataset,
    buildDokkanFrontierSeriesDataset,
    mapFrontierChapterFromFyi,
    mapFrontierMissionFromFyi,
    mapFrontierNodeFromFyi,
    mapFrontierRewardFromFyi,
    mapFrontierSeriesSummaryFromFyi,
} from "./fyi-dokkan-frontier";

describe("mapFrontierSeriesSummaryFromFyi", () => {
    it("maps chapter summaries under a frontier series", () => {
        const mapped = mapFrontierSeriesSummaryFromFyi({
            id: 2,
            name: "Dragon Ball Z",
...
describe("mapFrontierRewardFromFyi", () => {
    it("keeps frontier-specific reward fields like card skin scene links", () => {
        const mapped = mapFrontierRewardFromFyi({
            id: 55289,
            item_id: 1,
...
describe("mapFrontierMissionFromFyi", () => {
    it("normalizes mission copy and rewards", () => {
        const mapped = mapFrontierMissionFromFyi({
            id: 32176,
        ...
        equal(mapped.id, "32176");
        equal(mapped.name, "Clear Nodes 1 to 11 of\n\"Planet Namek Saga\".");
        equal(mapped.description, "Clear Nodes 1 to 11!\nRewards go to your Gift Box.");
        equal(mapped.categoryId, "200101");
...
describe("mapFrontierNodeFromFyi", () => {
    it("maps node restrictions, rounds and local missions", () => {
        const mapped = mapFrontierNodeFromFyi({
            id: 20010102,
            stamina: 12,
            user_exp: 9000,
            zeni: 50000,
        ...
        equal(mapped.id, "20010102");
        equal(mapped.unlockMissions[0].id, "32181");
        equal(mapped.requiredCharacters[0].name, "Krillin & Gohan (Kid)");
        equal(mapped.requiredCharacters[0].latestReleaseType, "eza");
        equal(mapped.intensityEffects[0].skill?.name, "Ki Disruption");
        equal(mapped.rounds[0].enemies[0].character?.name, "Frieza Soldier");
        equal(mapped.rounds[0].enemies[0].skills[0].name, "Damage Reduction");
        equal(mapped.missions[0].id, "32185");
    });
});

describe("mapFrontierChapterFromFyi", () => {
    it("keeps pages, group exchange steps and chapter missions together", () => {
        const mapped = mapFrontierChapterFromFyi({
        ...
        equal(mapped.seriesId, "2");
        equal(mapped.seriesName, "Dragon Ball Z");
        equal(mapped.pages[0].id, "200101");
        equal(mapped.groupExchange[0].charge, 1);
        equal(mapped.chapterMissions[0].id, "32176");
    });
});

describe("frontier dataset builders", () => {
    it("wraps series and chapter metadata with counts", () => {
        const seriesDataset = buildDokkanFrontierSeriesDataset([
            {
                id: "2",
                name: "Dragon Ball Z",
                bannerImagePath: "origin/series_banner_02.png",
        ...
        const chaptersDataset = buildDokkanFrontierChaptersDataset([
            {
                id: "2001",
                seriesId: "2",
        ...
        equal(seriesDataset.count, 1);
        equal(chaptersDataset.chapterCount, 1);
        equal(chaptersDataset.pageCount, 1);
        equal(chaptersDataset.nodeCount, 1);
        equal(chaptersDataset.missionCount, 2);
    });
});
