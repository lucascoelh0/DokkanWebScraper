import {
    ServerCollectionGate,
    ServerS0Catalog,
    ServerS0Coverage,
    ServerS0Endpoint,
    ServerS0Validation,
} from "./server-s0-contract";

function officialEndpoint(
    key: string,
    pathPattern: string,
    domains: ServerS0Endpoint["domains"],
    boundary: string,
    sourceLiteral = pathPattern,
): ServerS0Endpoint {
    return {
        key,
...
export function buildServerS0Catalog(): ServerS0Catalog {
    const endpoints: ServerS0Endpoint[] = [
    ...
    return {
        ...
        endpoints,
        headers: [
            ...["Authorization", "X-APIToken"].map(name => ({ name, classification: "secret" as const, logPolicy: "redact_value" as const, evidenceId: "global-elf-6.4.0-v338", evidenceLocator: `native_string_literal:${name}:`, requiredStatus: "unknown" as const })),
            ...["X-UserID", "X-UserCountry", "X-UserCurrency"].map(name => ({ name, classification: "pseudonymous" as const, logPolicy: "redact_value" as const, evidenceId: "global-elf-6.4.0-v338", evidenceLocator: `native_string_literal:${name}:`, requiredStatus: "unknown" as const })),
            ...["X-AssetVersion", "X-DatabaseVersion", "X-ClientVersion", "X-RequestVersion"].map(name => ({ name, classification: "version" as const, logPolicy: "allow_value" as const, evidenceId: "global-elf-6.4.0-v338", evidenceLocator: `native_string_literal:${name}:`, requiredStatus: "unknown" as const })),
...
export function buildServerS0Coverage(dataset: ServerS0Catalog): ServerS0Coverage {
    const gates: ServerCollectionGate[] = ["discover_only", "eligible_get_probe", "prohibited", "reference_only"];
    return {
        schemaVersion: 1,
        evidenceCount: dataset.evidence.length,
        hostCount: dataset.hosts.length,
        endpointCount: dataset.endpoints.length,
        endpointCountsByGate: Object.fromEntries(gates.map(gate => [gate, dataset.endpoints.filter(endpoint => endpoint.collectionGate === gate).length])) as Record<ServerCollectionGate, number>,
...
export function validateServerS0Catalog(dataset: ServerS0Catalog): ServerS0Validation {
    const failures: string[] = [];
    const unique = (values: string[]) => new Set(values).size === values.length;
    if (dataset.schemaVersion !== 1 || dataset.contract !== "dokkan-server-source-catalog" || dataset.contractVersion !== "0.1.0") failures.push("contract identity");
    if (dataset.collectionMode !== "static_inventory_no_network_capture") failures.push("network capture enabled");
    if (!unique(dataset.evidence.map(value => value.id)) || !unique(dataset.hosts.map(value => value.key)) || !unique(dataset.endpoints.map(value => value.key))) failures.push("duplicate structural key");
    ...
    const isActionShaped = (pathPattern: string) => pathPattern.toLowerCase().split(/[^a-z_]+/).some(segment => mutationSegments.has(segment));
    for (const endpoint of dataset.endpoints) {
        if (!hostKeys.has(endpoint.hostKey)) failures.push(`unknown host ${endpoint.key}`);
        if (endpoint.evidence.length === 0 || endpoint.evidence.some(claim => !evidenceIds.has(claim.evidenceId) || !claim.locator.trim() || !claim.observation.trim())) failures.push(`invalid evidence claim ${endpoint.key}`);
        if (/^https?:\/\//i.test(endpoint.pathPattern)) failures.push(`non-relative endpoint ${endpoint.key}`);
        if (endpoint.sourceClass === "official_client" && endpoint.collectionGate === "eligible_get_probe") failures.push(`premature official probe ${endpoint.key}`);
        if (endpoint.collectionGate === "eligible_get_probe" && endpoint.observedMethod !== "GET") failures.push(`non-GET eligible endpoint ${endpoint.key}`);
        if (isActionShaped(endpoint.pathPattern) && endpoint.collectionGate !== "prohibited") failures.push(`action-shaped endpoint not prohibited ${endpoint.key}`);
    }
    for (const header of dataset.headers) if (!evidenceIds.has(header.evidenceId) || !header.evidenceLocator.trim()) failures.push(`invalid header evidence ${header.name}`);
    ...
    if (!sensitiveHeadersRedacted) failures.push("sensitive header logging");
    ...
    if (!mutableEndpointsProhibited) failures.push("mutable endpoints not prohibited");
