import {
    ServerS3Assessment,
    ServerS3Classification,
    ServerS3Coverage,
    ServerS3Dataset,
    ServerS3ItemIdentity,
    ServerS3Observation,
    ServerS3Validation,
} from "./server-s3-contract";

function remoteRewardId(event: ServerS3Observation["remoteEvents"][number], reward: ServerS3Observation["remoteEvents"][number]["rewards"][number]): string | null {
    ...
    if (!reward.key.startsWith(prefix) || !reward.key.endsWith(suffix)) return null;
    const value = reward.key.slice(prefix.length, -suffix.length);
    return /^\d+$/.test(value) ? value : null;
}

function item(rawType: string, itemId: string, quantity: number | null): ServerS3ItemIdentity {
    return { itemType: rawType, itemId: String(itemId), quantity };
}

export function buildServerS3Dataset(observation: ServerS3Observation): ServerS3Dataset {
    ...
    const assessments: ServerS3Assessment[] = [];

    for (const event of observation.remoteEvents) for (const reward of event.rewards) {
...
export function buildServerS3Coverage(dataset: ServerS3Dataset): ServerS3Coverage {
    const classes: ServerS3Classification[] = ["agreement", "confirmed_conflict", "partial_candidate", "unjoinable"];
    const byClassification = Object.fromEntries(classes.map(classification => [classification, dataset.assessments.filter(value => value.classification === classification).length])) as Record<ServerS3Classification, number>;
    const byEventType: ServerS3Coverage["byEventType"] = {};
    for (const value of dataset.assessments) {
        ...
        row.total++;
        if (value.classification === "agreement") row.agreement++;
        else if (value.classification === "confirmed_conflict") row.confirmedConflict++;
        else if (value.classification === "partial_candidate") row.partialCandidate++;
        else row.unjoinable++;
...
export function validateServerS3Dataset(dataset: ServerS3Dataset, declaredRemoteRewardCount: number, expectedChannelCounts: ServerS3Observation["channelCounts"] & { preview: number }): ServerS3Validation {
    const failures: string[] = [];
    if (dataset.contract !== "dokkan-server-reward-identity" || dataset.contractVersion !== "0.4.0" || dataset.collectionMode !== "local_validated_artifacts_no_network") failures.push("contract identity");
    const expectedLineage = new Map<string, ServerS3Dataset["sourceLineage"][number]["authority"]>([["events_e5", "sqlite_first_party"], ["events_e7", "community_shadow"], ["dokkaninfo_event_rewards", "community_shadow"], ["dokkaninfo_reward_parser", "repository_implementation"]]);
    const lineageKeys = dataset.sourceLineage.map(value => value.key);
    if (lineageKeys.length !== expectedLineage.size || new Set(lineageKeys).size !== expectedLineage.size || [...expectedLineage.keys()].some(key => !lineageKeys.includes(key as any)) || dataset.sourceLineage.some(value => expectedLineage.get(value.key) !== value.authority || !/^[a-f0-9]{64}$/.test(value.sha256) || value.sizeBytes <= 0)) failures.push("source lineage");
    if (!dataset.authorityPolicy.structuralIdsOnly || dataset.authorityPolicy.titleJoinAllowed || dataset.authorityPolicy.remoteQuantityPromotedWhenFirstPartyMissing || dataset.authorityPolicy.chanceOrRepeatabilityInferred || dataset.authorityPolicy.legacyRewardIdentityOrigin !== "unknown_payload_id_or_synthetic_index" || dataset.authorityPolicy.normalizedRowCollisionStatus !== "unknown" || dataset.authorityPolicy.normalizedRowsLossless) failures.push("authority policy");
    if (new Set(dataset.assessments.map(value => value.remoteKey)).size !== dataset.assessments.length) failures.push("duplicate remote reward key");
    const exactNormalizedArtifactAccounting = dataset.assessments.length === declaredRemoteRewardCount;
    if (!exactNormalizedArtifactAccounting) failures.push("normalized artifact accounting");
    const allowedClassifications = new Set<ServerS3Classification>(["agreement", "confirmed_conflict", "partial_candidate", "unjoinable"]);
    const exclusiveClassifications = dataset.assessments.every(value => allowedClassifications.has(value.classification));
    if (!exclusiveClassifications) failures.push("classification domain");
    for (const value of dataset.assessments) {
        if (!/^\d+$/.test(value.eventId) || !/^\d+$/.test(value.remoteRewardId) || !/^\d+$/.test(value.remoteItem.itemId) || (value.stageId !== null && !/^\d+$/.test(value.stageId))) failures.push(`non-structural remote identity ${value.remoteKey}`);
        if (value.classification === "agreement" || value.classification === "confirmed_conflict") failures.push(`legacy identity origin cannot support ${value.classification} ${value.remoteKey}`);
        if (value.classification === "partial_candidate" && (value.target === null || (value.eventType === "zbattle"
            ...
            : value.target.family !== "quest_boss_drop" || value.rewardChannel !== "drop_candidate" || value.target.sourceRowId !== value.remoteRewardId || value.target.parentId !== value.stageId || value.target.quantityComparison !== "not_comparable" || value.target.itemComparison !== (value.target.firstPartyItem.itemType === value.remoteItem.itemType && value.target.firstPartyItem.itemId === value.remoteItem.itemId ? "agreement" : "conflict")))) failures.push(`invalid partial candidate ${value.remoteKey}`);
        if (value.classification === "unjoinable" && value.target !== null) failures.push(`unjoinable target ${value.remoteKey}`);
    }
    ...
    if (dataset.channels.length !== expectedChannels.length || channels.size !== expectedChannels.length || expectedChannels.some(value => !channels.has(value as any))) failures.push("reward channel set");
    ...
    if (!unknownSemanticsPreserved) failures.push("unknown reward semantics promoted");
    ...
    if (!staticAuthorityPreserved) failures.push("static authority");
