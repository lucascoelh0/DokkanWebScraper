...
import { ServerS5BuildResult, ServerS5Coverage, ServerS5InputGate, ServerS5Registry, ServerS5SidecarKind, ServerS5SidecarManifest, ServerS5SidecarPayload, ServerS5Validation } from "./server-s5-contract";

const kinds: ServerS5SidecarKind[] = ["schedule", "banners", "server_roots", "reward_joins", "asset_delivery"];
const dynamicKinds = new Set<ServerS5SidecarKind>(["schedule", "banners"]);
const ttl = new Map<ServerS5SidecarKind, number>([["schedule", 300], ["banners", 900]]);
const sourceGate = new Map<ServerS5SidecarKind, ServerS5InputGate["gate"]>([["schedule", "s1"], ["banners", "s1"], ["server_roots", "s2"], ["reward_joins", "s3"], ["asset_delivery", "s4"]]);
const sourceContractVersion = new Map<ServerS5InputGate["gate"], string>([["s1", "0.2.0"], ["s2", "0.3.0"], ["s3", "0.4.0"], ["s4", "0.5.0"]]);
const jsonText = (value: unknown) => `${JSON.stringify(value, null, 2)}\n`;
const sha256 = (value: string) => createHash("sha256").update(value).digest("hex");

function payload(kind: ServerS5SidecarKind, input: ServerS5InputGate, data: unknown): ServerS5SidecarPayload {
    return {
...
export function buildServerS5(inputs: { s1: ServerS5InputGate; s2: ServerS5InputGate; s3: ServerS5InputGate; s4: ServerS5InputGate }): ServerS5BuildResult {
    const { s1, s2, s3, s4 } = inputs;
    const payloads: ServerS5SidecarPayload[] = [
    ...
    const payloadTexts = new Map<ServerS5SidecarKind, string>(), manifests: ServerS5SidecarManifest[] = [], manifestTexts = new Map<ServerS5SidecarKind, string>();
    for (const value of payloads) {
        ...
        const manifest: ServerS5SidecarManifest = {
        ...
        manifests.push(manifest); manifestTexts.set(value.kind, jsonText(manifest));
    }
    const registry: ServerS5Registry = {
...
export function buildServerS5Coverage(result: ServerS5BuildResult): ServerS5Coverage {
...
export function validateServerS5(result: ServerS5BuildResult): ServerS5Validation {
    const failures: string[] = [], keys = result.payloads.map(value => value.kind), manifestKeys = result.manifests.map(value => value.kind), registryKeys = result.registry.sidecars.map(value => value.kind);
    const exactKinds = (values: string[]) => values.length === kinds.length && new Set(values).size === kinds.length && kinds.every(value => values.includes(value));
    const independentPayloads = exactKinds(keys) && exactKinds(manifestKeys) && exactKinds(registryKeys);
    if (!independentPayloads) failures.push("sidecar set");
    ...
    for (const kind of kinds) {
        ...
        if (!value || !manifest || !entry || !valueText || !manifestText) { contentAddressed = false; continue; }
        ...
        if (manifest.dynamic !== dynamic || entry.dynamic !== dynamic || manifest.ttlSeconds !== expectedTtl || entry.ttlSeconds !== expectedTtl || manifest.cachePolicy !== (dynamic ? "revalidate_after_ttl" : "content_addressed_immutable")) ttlOnlyDynamic = false;
        const compatibility = manifest.compatibility;
        if (compatibility.missing !== "ignore_sidecar_continue_database_first" || compatibility.unknownSchemaOrContract !== "reject_sidecar_continue_database_first" || compatibility.lineageMismatch !== "reject_sidecar_continue_database_first" || compatibility.stale !== (dynamic ? "ignore_sidecar_continue_database_first" : "not_applicable_content_addressed")) failClosedCompatibility = false;
        if (!value.optional || value.defaultEnabled || !entry.optional || entry.defaultEnabled) absentCompatible = false;
        const expectedGate = sourceGate.get(kind)!, expectedVersion = sourceContractVersion.get(expectedGate)!;
        if (value.schemaVersion !== 1 || value.contract !== `dokkan-server-${kind.replace(/_/g, "-")}-sidecar` || value.contractVersion !== "1.0.0" || value.kind !== kind || manifest.schemaVersion !== 1 || manifest.contractVersion !== "1.0.0" || manifest.kind !== kind || value.source.gate !== expectedGate || value.source.contractVersion !== expectedVersion || !/^[a-f0-9]{64}$/.test(value.source.sha256) || !Number.isSafeInteger(value.source.sizeBytes) || value.source.sizeBytes <= 0 || JSON.stringify(manifest.source) !== JSON.stringify(value.source) || manifest.generatedAt !== value.generatedAt || manifest.fetchedAt !== value.fetchedAt) failures.push(`contract or lineage ${kind}`);
        if (dynamic ? value.fetchedAt === null || value.authority !== "community_dynamic_shadow" : value.fetchedAt !== null || value.authority !== "derived_static_shadow") failures.push(`time or authority boundary ${kind}`);
    }
    const latestGeneratedAt = result.payloads.map(value => value.generatedAt).sort().at(-1);
    if (result.registry.generatedAt !== latestGeneratedAt) failures.push("registry generation time");
    if (!contentAddressed) failures.push("content addressing"); if (!ttlOnlyDynamic) failures.push("TTL policy"); if (!failClosedCompatibility) failures.push("compatibility policy"); if (!absentCompatible) failures.push("absent compatibility");
