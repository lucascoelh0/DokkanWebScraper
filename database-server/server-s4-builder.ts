import { ServerS4Coverage, ServerS4Dataset, ServerS4Observation, ServerS4Validation } from "./server-s4-contract";

export function buildServerS4Dataset(observation: ServerS4Observation): ServerS4Dataset {
    const containers = [...new Set(observation.nativeContainerKeys)].sort().map(containerKey => ({ containerKey, containsFormatPlaceholder: /%\d*d/.test(containerKey), status: "partial" as const, boundary: "native_path_literal_without_manifest_entry_or_delivery_bytes" as const }));
...
export function buildServerS4Coverage(dataset: ServerS4Dataset): ServerS4Coverage {
    return {
...
export function validateServerS4Dataset(dataset: ServerS4Dataset): ServerS4Validation {
    const failures: string[] = [];
    if (dataset.contract !== "dokkan-server-asset-delivery" || dataset.contractVersion !== "0.5.0" || dataset.collectionMode !== "local_static_inventory_no_network") failures.push("contract identity");
    const expectedLineage = new Map<string, ServerS4Dataset["sourceLineage"][number]["authority"]>([["server_s0", "official_static"], ["events_e6", "sqlite_first_party"], ["native_elf", "official_static"], ["historical_acquisition_metadata", "historical_first_party_export"]]);
    ...
    if (new Set(dataset.nativeContainerCandidates.map(value => value.containerKey)).size !== dataset.nativeContainerCandidates.length || dataset.nativeContainerCandidates.some(value => value.status !== "partial" || !/^[A-Za-z0-9_./%-]+\.cpk$/.test(value.containerKey) || value.containerKey.startsWith("/") || value.containerKey.includes(".."))) failures.push("container candidate domain");
    const expectedSources = new Map<string, { host: string; sourceClass: string; collectionGate: string; status: string }>([
    ...
    const sourceKeys = dataset.deliverySources.map(value => value.key), deliverySourcesValid = sourceKeys.length === expectedSources.size && new Set(sourceKeys).size === expectedSources.size && [...expectedSources.keys()].every(key => sourceKeys.includes(key as any)) && dataset.deliverySources.every(value => { const expected = expectedSources.get(value.key); return expected?.host === value.host && expected.sourceClass === value.sourceClass && expected.collectionGate === value.collectionGate && expected.status === value.status && value.hashSemantics === "unknown" && !!value.pathTemplate && !/^https?:/i.test(value.pathTemplate); });
    if (!deliverySourcesValid) failures.push("delivery source gate");
    const expectedManifests = new Map<string, { status: string; schemaStatus: string }>([["official_client_assets_split", { status: "partial", schemaStatus: "unknown" }], ["official_database_download", { status: "partial", schemaStatus: "partial" }]]);
    const manifestKeys = dataset.manifestCandidates.map(value => value.key), manifestCandidatesValid = manifestKeys.length === expectedManifests.size && new Set(manifestKeys).size === expectedManifests.size && [...expectedManifests.keys()].every(key => manifestKeys.includes(key as any)) && dataset.manifestCandidates.every(value => { const expected = expectedManifests.get(value.key); return expected?.status === value.status && expected.schemaStatus === value.schemaStatus && value.capturedBytes === false && value.entryHashStatus === "unknown" && value.entrySizeStatus === "unknown" && !!value.locator && !!value.boundary; });
    if (!manifestCandidatesValid) failures.push("manifest evidence promotion");
    ...
    if (!sampleHashesValid || dataset.baseApkInventory.representativeSampleBytes !== dataset.baseApkInventory.representativeSamples.reduce((sum, value) => sum + value.sizeBytes, 0)) failures.push("APK sample identity");
    ...
    if (!noUngatedNetworkCollection) failures.push("ungated network collection");
    ...
    const versionScopesPreserved = versionKeys.length === expectedVersionKeys.length && new Set(versionKeys).size === expectedVersionKeys.length && expectedVersionKeys.every(key => versionKeys.includes(key as any)) && currentApk?.scope === "current_s4_checkpoint" && currentApk.status === "supported" && currentApk.value.length > 0 && currentApk.observedAt === null && historicalVersions.length === 3 && historicalVersions.every(value => value.scope === "historical_first_party_export_2026_06_28" && value.status === "partial" && value.value.length > 0 && value.observedAt !== null);
    if (!versionScopesPreserved) failures.push("version scope promotion");
    ...
    if (!referenceDeliverySeparationPreserved) failures.push("reference delivery promotion");
