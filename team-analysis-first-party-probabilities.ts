...
import { ChanceSemantic, QualitativeChanceTerm } from "./team-analysis-chance-lexicon";

export interface FirstPartyProbabilityEvidence {
    stateKey: string,
...
export const FIRST_PARTY_PROBABILITY_EVIDENCE: readonly FirstPartyProbabilityEvidence[] = [
...
function evidence(
    ...
    passiveSkillIds: string[],
    efficacyType: number,
    valueField: FirstPartyProbabilityEvidence["valueField"],
): FirstPartyProbabilityEvidence {
    return {
        stateKey,
        passiveTextSha256,
        ruleLineIndex,
...
export function resolveFirstPartyProbability(
    stateKey: string,
    rawText: string,
    ruleLineIndex: number,
    qualitativeChanceTerm: QualitativeChanceTerm,
    semantic: ChanceSemantic,
): FirstPartyProbabilityEvidence | undefined {
    const passiveTextSha256 = createHash("sha256").update(rawText).digest("hex");
    return FIRST_PARTY_PROBABILITY_EVIDENCE.find(item =>
