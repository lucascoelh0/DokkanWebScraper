...
describe("DokkanInfo DB Stories dataset", () => {
    it("maps unique DB Story links in numeric order", () => {
        ...
        deepEqual(mapDbStoryIndex(dom.window.document), [
    ...
    it("maps stage metadata from labelled cells instead of concatenated text", () => {
        ...
        deepEqual(mapDbStoryStages(dom.window.document, {
            id: "901",
    ...
    it("maps quest enemy presentation and leaves unavailable sections optional", () => {
        ...
        const mapped = mapDbStoryStageDetail(dom.window.document, stage);

        equal(mapped.enemyDataStatus, "available");
        equal(mapped.enemies.length, 1);
        deepEqual(mapped.enemies[0], {
            source: "dokkaninfo",
            sequence: 1,
            group: 1,
            name: "Raditz",
    ...
    it("treats a missing enemy block as supported optional absence", () => {
        ...
        const mapped = mapDbStoryStageDetail(new JSDOM("<main>No enemy information</main>").window.document, stage);
        equal(mapped.enemyDataStatus, "not-provided");
        deepEqual(mapped.enemies, []);
    });

    it("does not turn an HTTP-200 server error page into missing enemy data", () => {
        const dom = new JSDOM("<title>General server error</title><main></main>");
        throws(() => mapDbStoryStageDetail(dom.window.document, {
            id: "1750020",
            title: "Level 2",
    ...
    it("reads enemy identity from the server-rendered card payload", () => {
        ...
        `);
        const mapped = mapDbStoryStageDetail(dom.window.document, {
            id: "9010012",
            title: "Level 1",
            sourcePath: "https://dokkaninfo.com/events/dbstories/901/9010012",
            enemyDataStatus: "not-provided",
        ...
        equal(mapped.enemies[0].name, "Raditz");
        equal(mapped.enemies[0].cardReferenceId, "1001131");
        equal(mapped.enemies[0].typeCode, "21");
        equal(mapped.enemies[0].portraitPath, "https://dokkaninfo.com/assets/global/en/character/thumb/card_1001130_thumb/card_1001130_thumb.png");
    });

    it("reports failures separately from legitimate missing enemy data", () => {
        const dataset = buildDokkanInfoDbStoryDataset([{
            id: "901",
            type: "dbstories",
            name: "Story",
            sourcePath: "https://dokkaninfo.com/events/dbstories/901",
        ...
        equal(dataset.storyCount, 1);
        equal(dataset.stageCount, 1);
        equal(dataset.stagesWithoutEnemyData, 1);
        deepEqual(dataset.failedStoryIds, ["933"]);
        deepEqual(dataset.failedStageIds, ["9010020"]);
    });
});
