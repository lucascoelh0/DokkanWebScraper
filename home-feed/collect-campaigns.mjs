...
export async function collectCampaigns({ enabled = false, config, definitionBytes,
  ...
  try {
    ...
    session = createSession(config, { fetchImpl, apiScope: 'campaigns' });
    ...
    collected.set(result, { indexBytes: Buffer.from(delivery.indexBytes),
      details: delivery.details.map(detail => Buffer.from(detail.bytes)) });
    return result;
  } catch {
...
export function prepareCollectedCampaignCandidate(collection, { now = Date.now } = {}) {
  try {
    ...
    const campaigns = stored.details.map(bytes => {
      ...
      return { campaign: detail.campaign, missions: detail.missions };
    });
    const publicContext = { schemaVersion: 1, region: 'global', source: 'global-game-campaigns',
    ...
    return { target: 'staging/v2/campaigns/', publicationAllowed: false,
      manifestBytes, indexBytes, indexSha256, revisionSha256, details, totalBytes,
