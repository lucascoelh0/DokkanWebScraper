...
const expectedPaths: Record<string, string> = { h8: "data/database-server-captures/h8/capture-h8-inventory.json", h9: "data/database-server-captures/h9/capture-h9-observed-schemas.json", h10: "data/database-server-captures/h10/capture-h10-domains.json", h11: "data/database-server-captures/h11/capture-h11-shadow-parity.json" };
if (lock.value.schemaVersion !== 1 || lock.value.contract !== "dokkan-official-capture-h12-source-lock" || lock.value.contractVersion !== "0.13.0" || JSON.stringify(lock.value.artifacts.map((value: any) => value.key).sort()) !== JSON.stringify(Object.keys(expectedPaths).sort())) throw new Error("H12 source lock contract mismatch");
...
const h8 = files.get("h8")!.value as CaptureH8Dataset, h9 = files.get("h9")!.value as CaptureH9Dataset, h10 = files.get("h10")!.value as CaptureH10Dataset, h11 = files.get("h11")!.value as CaptureH11Dataset;
if (!validateCaptureH11(h11).valid) throw new Error("H12 requires valid H11");
const sourceLockSha256 = createHash("sha256").update(lock.text).digest("hex"), h11File = files.get("h11")!;
const dataset = buildCaptureH12(h8, h9, h10, h11, sourceLockSha256, h11File.sha256, h11File.sizeBytes), validation = validateCaptureH12(dataset), text = `${JSON.stringify(dataset, null, 2)}\n`;
