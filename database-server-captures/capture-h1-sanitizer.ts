import { auditSanitizedCaptureEntries, loadSanitizedCaptureEntries, SanitizedHarEntryStructure } from "./capture-h0-audit";
import { CaptureH0Dataset, CaptureInputManifest } from "./capture-h0-contract";
import { CaptureBodyDisposition, CaptureH1Dataset, CaptureH1SchemaObservation } from "./capture-h1-contract";

function disposition(entry: SanitizedHarEntryStructure, side: "request" | "response"): CaptureBodyDisposition {
    ...
    if (side === "request" && ["auth", "mutation", "user_state"].includes(entry.classification)) return "omitted_by_classification";
    return schema.length > 0 ? "schema_only" : "non_json_or_unavailable_omitted";
}

function observation(entry: SanitizedHarEntryStructure): CaptureH1SchemaObservation {
    return {
...
function aggregate(entries: SanitizedHarEntryStructure[]): CaptureH1SchemaObservation[] {
    const groups = new Map<string, CaptureH1SchemaObservation>();
    for (const entry of entries) {
        ...
        if (!current) {
            ...
            current.count += 1;
            const timestamps = [current.capturedAtStart, current.capturedAtEnd, next.capturedAtStart, next.capturedAtEnd].filter((value): value is string => value !== null).sort((a, b) => a.localeCompare(b));
