...
import { CaptureH4Dataset, CaptureH4Validation } from "./capture-h4-contract";
...
type Kind = "identity" | "positiveInt" | "nonNegativeInt" | "timestamp" | "boolean" | "positiveIntArray" | "nonNegativeIntArray" | "rateArray" | "rarityArray";
type Rule = { kind: Kind; endpoints: string[]; paths: string[] };
const rule = (kind: Kind, endpoints: string | string[], paths: string | string[]): Rule => ({ kind, endpoints: Array.isArray(endpoints) ? endpoints : [endpoints], paths: Array.isArray(paths) ? paths : [paths] });
...
function normalize(kind: Kind, raw: unknown): CaptureProductValue | undefined {
    ...
    if ((kind === "positiveIntArray" || kind === "nonNegativeIntArray") && Array.isArray(raw) && raw.length <= 100000 && raw.every(value => Number.isSafeInteger(value) && (kind === "positiveIntArray" ? value > 0 : value >= 0) && value <= 10 ** 12)) return [...new Set(raw as number[])].sort((a, b) => a - b);
    if (kind === "rateArray" && Array.isArray(raw) && raw.length <= 1000 && raw.every(value => typeof value === "number" && Number.isFinite(value) && value >= 0 && value <= 100)) return [...new Set(raw as number[])].sort((a, b) => a - b);
    if (kind === "rarityArray" && Array.isArray(raw) && raw.length <= 16 && raw.every(value => typeof value === "string" && /^(?:N|R|SR|SSR|UR|LR)$/i.test(value))) return [...new Set((raw as string[]).map(value => value.toUpperCase()))].sort((a, b) => a.localeCompare(b));
    return undefined;
}

function values(items: any[], key: string): unknown[] { return items.flatMap(item => item?.[key] === undefined || item?.[key] === null ? [] : [item[key]]); }
function numericPathId(pathname: string, suffix: string): number | null { const match = pathname.match(new RegExp(`^/gashas/(\\d+)/${suffix}$`)); const value = match ? Number(match[1]) : NaN; return Number.isSafeInteger(value) && value > 0 ? value : null; }
function objects(value: unknown): any[] { return Array.isArray(value) ? value.filter(item => item && typeof item === "object") : []; }

function add(candidates: Candidate[], context: CaptureProductResponseContext, entityType: string, entityId: number, field: string, raw: unknown, path: string): void {
    const fieldRule = rules[entityType]?.[field];
    if (!fieldRule) return;
    const value = normalize(fieldRule.kind, raw);
    if (value !== undefined) candidates.push({ entityType, entityId, fact: makeProductFact(entityType, entityId, field, value, context, path) });
}

export function buildCaptureH4(manifest: CaptureInputManifest, roots: Record<string, string>, h0: CaptureH0Dataset): CaptureH4Dataset {
...
export function validateCaptureH4(dataset: CaptureH4Dataset, h0: CaptureH0Dataset): CaptureH4Validation {
    ...
    for (const entity of dataset.entities) {
        ...
        for (const fact of entity.facts) {
            ...
            if (!fieldRule?.endpoints.includes(p.normalizedEndpoint) || !fieldRule?.paths.includes(p.jsonPath)) failures.push("coordinate allowlist");
