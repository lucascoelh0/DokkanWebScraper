...
if (lock.value.schemaVersion !== 1 || lock.value.contract !== "dokkan-official-capture-h11-source-lock" || lock.value.contractVersion !== "0.12.1" || JSON.stringify(lock.value.artifacts.map((value: any) => value.key).sort()) !== JSON.stringify(Object.keys(expectedLockPaths).sort())) throw new Error("invalid H11 source lock contract");
...
const h13 = read<CaptureH13Dataset>("data/database-server-captures/h13/capture-h13-gasha-audit.json").value;
...
if (!validateCaptureH13(h13).valid) throw new Error("H11 requires a valid H13 corrective audit");
...
const dataset = buildCaptureH11(lock.text, h7, h10, h3, h4, h5, h6, h8.captures.map(value => value.captureId), h13);
