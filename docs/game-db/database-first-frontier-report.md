...
Status: experimental, non-production. Updated through DB41 (`0.40.0`) against snapshot `global-6.4.0-v338-2026-08-05`.
...
Confirmed condition families include the earlier SQLite projections for causality types `1, 2, 5, 15, 16, 19, 24, 25, 30, 38, 42`, selector masks from type `46`, and native-backed runtime types `3, 4, 34, 43, 51, 55`. Types `3/4` compare a truncated float32 battle-gauge/cap100 percentage with `cau_val1` using `>=` / strict `<`; their raw status-category gate and current-character deck-index selection are explicit, while gauge reset/evaluation timing remains partial. Type `34` uses raw group mode 1 to enumerate eligible puzzle enemies, filters their structured `MasterCard` category IDs by `cau_val2`, and tests matching partner count unsigned `>=` sign-extended `cau_val3`; modes 0/2, the enemy eligibility-field name and scheduling remain unknown. Type `49` has a supported Super Attack category selector backed by `special_categories.raw_attribute` and the native bitmask consumer. Types `17/18/33` have supported target-HP metrics, parameters and inclusive comparators, with runtime scope/gates kept explicit. Types `47/54` have supported revival-activation counter predicates: ability-owner pure-current record for 47, and deck indices 0–6 across pure/back current records for 54, whose `cau_val1` zero/nonzero value selects any/none. These families remain partial at activation/history level. Types `40` and `56` have partial native predicates over `AdditionalParam` byte 1, but their attack kind, direction and scope are unknown. Type `41` preserves 602 name tokens but lacks a first-party dictionary. Remaining numeric causalities stay raw/unknown even when the native dispatch slot has been identified.
