...
Status: experimental, non-production. Updated through DB32 (`0.31.0`) against snapshot `global-6.4.0-v338-2026-08-05`.
...
| Execution timing | SQLite-to-runtime field chain, equality filter and turn-sequence call sites | timing value 1 = native turn start; 9,061 rules / 11,809 effects / 1,449 states supported, other values unknown |
...
Confirmed passive execution timing: `exec_timing_type = 1` selects the native turn-start execution event. The supported sequence is after character appearance and reversible-result fixation and before support-memory and potential-skill execution. Observed values `3, 4, 5, 6, 7, 9, 11, 12, 14, 15` remain unknown. Timing 1 does not imply a calculation bucket, and does not settle `turn`, `is_once`, duration, recurrence or stacking.
...
DB17–DB22 produced material value: three scoped native promotions, 195 sound AST simplifications, 60 additional exact pairs, and a reduction of 188 residual occurrences. DB20–DB21 also prevented a high-correlation but false universal interpretation of `passive_skills.turn`. DB23 itself is diagnostic and establishes a clean parity frontier. DB24 then moved to combat semantics and promoted one high-impact efficacy family, resolving all 150 counter payload fields in the 50 in-scope rules while preserving every activation uncertainty. DB25 delivered negative but material value: it removed false precision from 132 rules and established a reproducible dynamic boundary rather than entrenching a parser-shaped label. DB26 resolved 83/83 structured Super Attack category selectors through a direct SQLite-to-runtime field path, DB27 resolved another 59 target-HP conditions with exact comparator and HP-rate calculation boundaries, DB28 resolves 70 revival-history predicates through a concrete increment/read/reset chain rather than symbol naming alone, DB29 resolves all 36 attack-break effects with native enemy scoping and multiplicity, DB30 resolves 367 native removal operations while keeping the attractive but unproved SkillType-2 name only as a partial ID candidate, DB31 promotes the shared operation enum across all 14,301 projected passive rules without conflating it with unit or scheduling, and DB32 schedules 9,061 of those rules at a precisely bounded native turn-start event.

Recommendation: continue database-first mapping. DB32 makes raw timing 4 the next timing target by volume (1,830 projected rules), followed by the calculation-bucket consumers shared across the now-scheduled timing-1 rules. Do not label timing 4 from parser text; require the same SQLite-to-runtime equality path plus concrete call-site ordering used by DB32. If that path is ambiguous, move to target/sub-target dispatch or efficacy 61. Integration into production remains **NO-GO** until calculation buckets/recurrence and the highest-impact unknown efficacy/target families are either proved or explicitly isolated behind optional unknown-safe enrichment.
