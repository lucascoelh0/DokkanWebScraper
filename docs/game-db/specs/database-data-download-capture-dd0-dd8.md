...
As 14 linhas exclusivas usam unidades de evidência, não entidades somáveis: sete agreements, três representation gains, dois representation mismatches, um unknown, dois unjoinable e zero confirmed conflicts. Zero conflito nunca implica completude. A identidade canônica de cada fonte rastreada usa os bytes exatos do Git blob `HEAD:<path>` (`git_blob_bytes_v1`), não bytes sujeitos a conversão CRLF/LF do worktree. O contrato fixa também o mapeamento exato de cada uma das 19 chaves para seu path; unicidade isolada não autoriza remapeamento semântico. O runner exige a raiz canônica do checkout, lê cada blob por argumento separado do Git e falha fechado para path, tamanho ou SHA divergente. DD2 é validado antes da construção, recebe identidade por JSON recursivamente ordenado (`recursive_lexicographic_json_v1`) e essa identidade integra `inputLineage`; timestamp, contagens e razões DD6 precisam corresponder ao mesmo DD2. Assim, o mesmo commit e o mesmo DD2 produzem o mesmo resultado em checkout limpo independentemente de `core.autocrlf`. A atualização de hardening AQ/C4 altera somente contrato/source lineage de DD6; nenhuma classificação ou total muda.
