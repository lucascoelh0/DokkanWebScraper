...
The DokkanInfo event artifact accounts for 898 event roots and its output keys exactly match all 898 cache basenames. All area-backed families join by root ID: Bonus `87/87`, Challenge `106/106`, Dragon Ball Stories `27/27`, Growth `203/203`, Limited `16/16`, Quest `42/42` and Story `187/187`. Z-Battle roots join `203/203`; Dokkan Frontier series roots join `2/2`. Super Battle Road's 25 roots have no proved first-party root and remain `unjoinable`. Across traditional event stages, all 5,299 unique map IDs join. Restricting the first-party comparison universe to quests under those structurally joined event roots adds 8 map IDs; maps elsewhere in the database are not counted as event representation gain. The 40,345 scraped reward rows are also `unjoinable`: their only row-like identifier is embedded in a derived textual key, which E7 deliberately does not parse or treat as a first-party ID.
