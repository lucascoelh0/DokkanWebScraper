...
Readiness is GO only for reviewed disabled infrastructure and GET-only synthetic fixtures. Active Pettan, Burst gameplay/scoring, opaque start configuration, source/cache replacement, Android, R2/publisher, production and request/replay/mutation automation remain NO-GO. M6 supplies seven ordered passive-capture flows with preconditions and stop boundaries. The complete report and exact future checklist are in [`specs/database-special-modes-capture-m0-m6.md`](specs/database-special-modes-capture-m0-m6.md).
