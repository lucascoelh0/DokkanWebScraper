...
## Offline Frontier capture follow-up

F0–F6 audit the exact 2,181,883-byte `frontier.har` capture (`2bd584040f46025fa5f9cd7275846bf68dd27bb9c52a65ef3429d872a74f61cb`) without requests or replay. Twenty battle responses are valid Zstandard frames and all declare dictionary ID `315060143`, but no exact dictionary header/ID candidate exists in the pinned APK, ELF, SQLite or local package artifacts. Discovery can never promote a candidate to proof; an explicitly pinned dictionary, approved provider and green real-body decode receipt are separate requirements. The responses therefore remain `compressed_unknown`.

The capture closes the previously missing server topology for the two detailed episodes: 11 pages, 65 spots and 47 of the 52 E2 battles join structurally. The five battles and two pages in the undetailed episode remain coverage gaps. Parity has 69 agreements, 80 representation gains, 11 temporal/account observations, 43 representation mismatches, zero confirmed conflicts, nine coverage gaps, eight unknowns and 12 unjoinable facts. The 43 mismatches compare display enemy-card references with E3 encounter arrays and are not contradictions.

Readiness is GO only for reviewed disabled infrastructure and synthetic fixtures. Real-body decoding, E0–E9 replacement, Android, R2, automated refresh, battle-protocol consumption and any replay or automation remain NO-GO. The full contract and evidence are in [`specs/database-frontier-capture-f0-f6.md`](specs/database-frontier-capture-f0-f6.md).

## Offline special modes capture follow-up

M0–M6 audit the exact Burst and inactive-Pettan captures (118 + 165 entries) offline, without requests, replay or payload retention. Pettan exposes two empty capture-time pack indexes, five shape-only mutation observations, three announcements, four assets and zero battle routes. It does not prove an active catalog or any battle. Burst exposes an explicit genkai briefing root `48`, schedule `52`, route quest `1511002`, 38 gimmick IDs and 40 condition IDs; raw modifier semantics and scoring remain partial or unknown. The observed 200 start route shares the quest and follows the briefing, but both bodies contain only an opaque signature with zero visible structural IDs, so route and order do not prove a Burst association or configuration.

Database-first parity joins quest `1511002` to E2 area `1511` and levels/maps `15110022`/`15110023`. It does not use that quest or the filename as Burst gameplay identity. All 13 captured gimmick-subcategory IDs join exact rows in the pinned SQLite, supporting four raw category types and ten invalidation edges but no scoring semantics. The 131 SD packs and 48 → 231 → 1,169 SD topology remain capture gaps; S2 community series and SD map IDs are separate namespaces. The 747 Burst candidate rows have no genkai-root foreign key and do not prove a formula.

M4 reconstructs 349 sanitized facts with 359 source spans: 135 supported, 206 partial and eight unknown. M5 closes E0–E9, S0–S7 and H0–H13 lineage and reports 48 agreement, one representation gain, one representation mismatch, 1,604 coverage-gap, 208 unknown, 772 unjoinable and zero confirmed-conflict units. Counts use non-equivalent units and are not an entity total. Quest/map, 200/304, cache absence and namespace differences are representation or coverage evidence, never conflicts.

The user-observed predicted-score change with no additional API call is preserved as a strong `partial` hypothesis of local recomputation. The briefing's 38 `point` values are a structural candidate; SQLite supplies taxonomy only, three `/events` bodies are absent from the HAR, and four captured assets are presentation binaries. Aggregation, interactions, caps, precedence and rounding remain unknown.

Neither input contains a Zstandard body, so no decode occurs. Dictionary ID `315060143` alone would never prove identity; a future decode requires independently sourced exact bytes pinned by size, SHA-256 and ID plus a bounded green decode receipt.

Readiness is GO only for reviewed disabled infrastructure and GET-only synthetic fixtures. Active Pettan, Burst gameplay/scoring, opaque start configuration, source/cache replacement, Android, R2/publisher, production and request/replay/mutation automation remain NO-GO. M0–M6 is complete: no additional Pettan or start/finish capture is required, and the interception crash at stage boundaries is accepted as the capture boundary. Five passive flows are retained only as optional dependencies of a future event window. The complete report and checklist are in [`specs/database-special-modes-capture-m0-m6.md`](specs/database-special-modes-capture-m0-m6.md).
