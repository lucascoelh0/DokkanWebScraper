...
export async function runEventsRefresh(options: { databasePath?: string; elfPath?: string; apkPath?: string; outputDir?: string; refreshProfilePath?: string } = {}) {
    ...
    for (const baseline of profile.baselineFiles) { const bytes = await readFile(resolve(profileDirectory, baseline.fileName)); if (sha(bytes) !== baseline.sha256) throw Error(`Refresh baseline identity ${baseline.role}`); }
