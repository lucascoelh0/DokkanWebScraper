...
import {
    ...
    EventsE0Dataset,
    EventsE0Observation,
    EventsE0TableInventory,
    EventsEvidenceStatus,
} from "./events-e0-contract";

const SUPPORTED_TABLES = new Map<string, { families: string[]; role: string }>([
...
const PARTIAL_EXACT = new Map<string, { families: string[]; role: string }>([
...
function partialPrefix(name: string): { families: string[]; role: string } | undefined {
    if (name.startsWith("z_battle_")) return { families: ["z_battle"], role: "candidate_z_battle_structure" };
    if (name.startsWith("budokai_" ) || name === "budokais") return { families: ["budokai"], role: "candidate_budokai_structure" };
    if (name.startsWith("rmbattle_")) return { families: ["rmbattle"], role: "candidate_rmbattle_structure" };
    if (name.startsWith("sd_")) return { families: ["sd_mode"], role: "candidate_sd_structure" };
    if (name.startsWith("mission_" ) || name === "missions") return { families: ["mission_reference"], role: "candidate_mission_join" };
    if (name.startsWith("bgm_") || name.startsWith("jukebox_") || name === "sound_effect_offsets") return { families: ["asset_reference"], role: "candidate_audio_reference" };
    if (name.endsWith("_items") || name === "support_memories" || name === "support_films") return { families: ["item_reference"], role: "candidate_item_catalog_target" };
    return undefined;
}

export function classifyEventsE0Table(name: string): EventsE0TableInventory["domain"] {
    const supported = SUPPORTED_TABLES.get(name);
    if (supported) return { status: "supported", ...supported, basis: "validated_structural_join" };
    const partial = PARTIAL_EXACT.get(name) ?? partialPrefix(name);
    if (partial) return { status: "partial", ...partial, basis: "bounded_schema_candidate" };
    return { status: "unknown", families: [], role: "not_yet_classified_for_events", basis: "not_yet_classified" };
}

export function eventsE0SchemaSha256(observation: EventsE0Observation): string {
...
export function buildEventsE0Dataset(options: {
    observation: EventsE0Observation;
    generatedAt: string;
...
export function buildEventsE0Coverage(dataset: EventsE0Dataset): EventsE0Coverage {
    const tableStatusCounts: Record<EventsEvidenceStatus, number> = { supported: 0, partial: 0, unknown: 0 };
    for (const table of dataset.tables) tableStatusCounts[table.domain.status]++;
    return {
        schemaVersion: 1,
