...
export async function runEventsE6(options: { databasePath?: string; apkPath?: string; outputDir?: string; e5Dir?: string; baselinePath?: string; apkBaselinePath?: string } = {}) {
    ...
    const e5Manifest = JSON.parse(await readFile(await resolveEventsInputFile(e5Dir, "events-e5-manifest.json", "events-e5-manifest.json"), "utf8")) as EventsE5Manifest;
    const [e5Path, e5CoveragePath, e5ValidationPath] = await Promise.all([resolveEventsInputFile(e5Dir, e5Manifest.fileName, "events-e5-rewards.json"), resolveEventsInputFile(e5Dir, e5Manifest.coverage?.fileName, "events-e5-coverage.json"), resolveEventsInputFile(e5Dir, e5Manifest.validation?.fileName, "events-e5-validation.json")]);
    const [e5Fingerprint, e5CoverageBytes, e5ValidationBytes] = await Promise.all([fingerprint(e5Path), readFile(e5CoveragePath), readFile(e5ValidationPath)]); memory();
    ...
    await mkdir(outputDir, { recursive: true }); const outputs = await resolveEventsOutputFiles(outputDir, [manifest.fileName, manifest.coverage.fileName, manifest.validation.fileName, "events-e6-manifest.json"]); await Promise.all([writeFile(outputs.get(manifest.fileName)!, payloadText), writeFile(outputs.get(manifest.coverage.fileName)!, coverageText), writeFile(outputs.get(manifest.validation.fileName)!, validationText), writeFile(outputs.get("events-e6-manifest.json")!, `${JSON.stringify(manifest, null, 2)}\n`)]); memory();
