...
import { EventsE7Classification, EventsE7Comparison, EventsE7Coverage, EventsE7Dataset, EventsE7Observation, EventsE7SourceLineage } from "./events-e7-contract";

const canonicalIdHash = (values: string[]) => createHash("sha256").update(JSON.stringify([...values].sort((a, b) => a.length - b.length || a.localeCompare(b)))).digest("hex");
const evidence = (values: string[]) => ({ count: values.length, sha256: canonicalIdHash(values), samples: values.slice(0, 20) });

function comparison(key: string, classification: EventsE7Classification, values: Partial<EventsE7Comparison>): EventsE7Comparison {
...
export function buildEventsE7Dataset(options: { observation: EventsE7Observation; legacySources: EventsE7SourceLineage[]; implementationEvidence: Array<{ fileName: string; sha256: string; sizeBytes: number }>; implementationBaselineSha256: string; generatedAt: string; sourceSnapshotVersion: string; sourceDatabaseSha256: string; sourceE6Sha256: string }): EventsE7Dataset {
    const { observation } = options, comparisons: EventsE7Comparison[] = [];
    for (const facet of observation.facets) {
        const classification: EventsE7Classification = !facet.structuralJoin ? "unjoinable" : !facet.comparable ? "unknown" : facet.conflictCount > 0 ? "confirmed_conflict" : facet.legacyOnlyIds.length > 0 ? "unknown" : "agreement";
    ...
    if (observation.firstPartyLinkedMissionOnlyIds.length) comparisons.push(comparison("event_missions.structurally_linked_subset.first_party_only", "representation_gain", { legacyCount: observation.linkedLegacyMissionCount, firstPartyCount: observation.firstPartyLinkedMissionCount, joinedCount: observation.linkedLegacyMissionCount, agreementCount: observation.linkedLegacyMissionCount, conflictCount: 0, idEvidence: evidence(observation.firstPartyLinkedMissionOnlyIds), boundary: "first_party_structural_event_targets_absent_from_pinned_event_missions" }));
    for (const value of observation.unknownSurfaces) comparisons.push(comparison(value.key, "unknown", { legacyCount: value.legacyCount, boundary: value.reason }));
    for (const value of observation.unjoinableSurfaces) comparisons.push(comparison(value.key, "unjoinable", { legacyCount: value.legacyCount, idEvidence: value.key === "event_missions.event_relation" ? evidence(observation.unlinkedLegacyMissionIds) : null, boundary: value.reason }));

    const eventFamilies = observation.eventFamilies.map(value => ({ ...value, rootClassification: (value.rootTarget === null ? "unjoinable" : value.joinedRootCount === value.eventCount ? "agreement" : "unknown") as EventsE7Classification, stageClassification: (value.joinedStageCount === value.stageCount ? "agreement" : "unknown") as EventsE7Classification }));
    const paginationAudits: EventsE7Dataset["paginationAudits"] = [
...
export function buildEventsE7Coverage(dataset: EventsE7Dataset, observation: EventsE7Observation): EventsE7Coverage {
    const byClassification = { agreement: 0, representation_gain: 0, confirmed_conflict: 0, unknown: 0, unjoinable: 0 };
    for (const value of dataset.comparisons) byClassification[value.classification] += 1;
    const paginationByStatus = { supported: 0, partial: 0, unknown: 0 };
    for (const value of dataset.paginationAudits) paginationByStatus[value.status] += 1;
